...
class FusedConv3DTest(test_util.TensorFlowTestCase):
    ...
    def testConv3dBiasAdd(self):
        ...
        # output = array_ops.identity(nn_ops.relu(output))
        
        with self.session(use_gpu=True) as sess:
            start_time = time.time()
            ret_gpu = sess.run(output, feed_dict={x: x_arr, w: w_arr},options=run_options, run_metadata=metadata)
        ...
        with self.session(use_gpu=False) as sess:
            ret_cpu = sess.run(output, feed_dict={x: x_arr, w: w_arr})
