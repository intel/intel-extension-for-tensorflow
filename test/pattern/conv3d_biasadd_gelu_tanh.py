...
class FusedConv3DTest(test_util.TensorFlowTestCase):
    ...
    def testFuseBiasAddGelu_tanh(self):
        tf.compat.v1.disable_eager_execution()
        ...
        conv_bias_gelu_tanh = load_ops_library.itex_gelu(conv_bias,approximate=True)
        fused = array_ops.identity(conv_bias_gelu_tanh)
        ...
        with self.session(use_gpu=True) as sess:
            ...
            for node in graph.node:
                if "FusedConv3D" in node.op:
                    ...
                    found_fused_op = len(fused_ops) == 2 and fused_ops[0] == b'BiasAdd' and fused_ops[1] == b'GeluApproximate'
                    break
