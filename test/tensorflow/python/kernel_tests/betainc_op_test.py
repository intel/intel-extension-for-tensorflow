...
class BetaincTest(test.TestCase):
  ...
  def testBetaIncGrads(self):
    ...
    with self.cached_session(use_gpu=True):
      ...
      tf_ga_s = constant_op.constant(ga_s, dtype=dtypes.float32)
      tf_gb_s = constant_op.constant(gb_s, dtype=dtypes.float32)
      tf_gx_s = constant_op.constant(gx_s, dtype=dtypes.float32)
