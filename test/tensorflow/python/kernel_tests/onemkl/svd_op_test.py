...
def _GetSvdOpTest(dtype_, shape_, use_static_shape_, compute_uv_,
  ...
  def CheckUnitary(self, x, tol):
    ...
    # TODO(itex): it seems we have a bug in generating identity matrix using itex
  ...
  def Test(self):
    ...
    if test.is_gpu_available():
      ...
      tol *= 100
    np.random.seed(42)
    # TODO(itex): OneMKL bug walk around, hardcode dummy input for now.
    # x_np = np.random.uniform(
    #     low=-1.0, high=1.0, size=np.prod(shape_)).reshape(shape_).astype(dtype_)
    shape_ = (5, 5)
    x_np = np.arange(25).reshape(shape_).astype(dtype_)
    ...
    if compute_uv_:
      # TODO(itex): hardcode the result for now.
      # u_np, s_np, v_np = np.linalg.svd(
      #     x_np, compute_uv=compute_uv_, full_matrices=full_matrices_)
      u_np = np.array([[-0.06767881, -0.77163436,  0.59765843, -0.19880069,  0.05729476],
       [-0.22699517, -0.49847085, -0.59036184,  0.3706095 ,  0.46273264],
       [-0.38631153, -0.22530735, -0.1751204 ,  0.2361091 , -0.8447398 ],
       [-0.54562789,  0.04785615, -0.26930738, -0.78884393,  0.07210266],
       [-0.70494424,  0.32101965,  0.4371312 ,  0.38092602,  0.25260975]], dtype=np.float32)
      s_np = np.array([6.99085940e+01, 3.57609824e+00, 4.50566172e-15, 1.37011875e-15, 1.74614773e-16], dtype=np.float32)
      v_np = np.array([[-0.39024378, -0.41787354, -0.4455033 , -0.47313306, -0.50076282],
       [ 0.66911119,  0.35409279,  0.03907438, -0.27594403, -0.59096243],
       [ 0.35400242, -0.05819232, -0.82800352,  0.41457432,  0.1176191 ],
       [-0.26182654,  0.72151088, -0.33809441, -0.44103767,  0.31944773],
       [-0.45401448,  0.41956609, -0.01011676,  0.5775932 , -0.53302804]], dtype=np.float32)
    else:
      # TODO(itex): hardcode the result for now.
      # s_np = np.linalg.svd(
      #     x_np, compute_uv=compute_uv_, full_matrices=full_matrices_)
    ...
    if compute_uv_:
      ...
      CompareSingularVectors(self, u_np, u_tf_val, 2, tol)
      ...
      CheckApproximation(self, x_np, u_tf_val, s_tf_val, v_tf_val,
                         full_matrices_, tol)
      #TODO(itex): there is an accu issue in u_tf_val
      ...
      for dtype in dtypes_to_test:
        for rows in [0, 1, 2, 5, 10, 32, 100]:
          for cols in [0, 1, 2, 5, 10, 32, 100]:
            for batch_dims in [()] * (max(rows, cols) < 10):
              full_shape = batch_dims + (rows, cols)
              for use_static_shape in set([True]):
