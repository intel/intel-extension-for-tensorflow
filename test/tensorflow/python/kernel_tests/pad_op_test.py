...
class PadOpTest(test.TestCase):
  ...
  def testPaddingsDim4(self):
    ...
    #below is the shape in RN50
    DIMS = 4
    shapes = [[256, 56, 56, 128], [256, 14,14,512], [256, 224, 224,3], [256, 28,28,256]]
    type_list = [np.float32, np.float16, dtypes.bfloat16.as_numpy_dtype, np.double]
    for shape in shapes:
      for dtype in type_list:
        np_inputs = np.random.uniform(0, 3, size = shape).astype(dtype)
        paddings = np.array([[0,0],[1,1],[1,1],[0,0]])
        self._testPad(np_inputs, paddings, "CONSTANT", -1)
  ...
  def testZeroInput(self):
    inp = np.zeros((0, 2), dtype=np.float32)
    paddings = np.ones((2,2), dtype=np.int32)
    res = np.ones((2,4), dtype=np.float32)
    with test_util.use_gpu():
      tf_val = array_ops.pad_v2(inp, paddings, "CONSTANT", 1)
      out = self.evaluate(tf_val)
      print(out)
    self.assertAllEqual(res, out)
