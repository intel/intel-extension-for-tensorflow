    ...
    for t in [np.float16, np.float32, np.float64]:
      with self.subTest(t=t):
        ...
        if t == np.float16:
          ...
          self._compareGradientX(fn, c, xt, yt, np.float32)
          self._compareGradientY(fn, c, xt, yt, np.float32)
        else:
...
class RoundingTest(test.TestCase):
  ...
  def _compare_values(self, x, y=None, dtype=dtypes_lib.float32):
    with self.cached_session(use_gpu=True) as sess:
      y = np.rint(x) if y is None else np.asarray(y)

      tx = constant_op.constant(x, dtype=dtype)
      px = array_ops.zeros(tx.get_shape().as_list(), dtype)
      x = math_ops.add_n([tx, px])
      tf_rint = math_ops.rint(x)
      tf_rint = array_ops.identity(tf_rint)
  ...
  def _testDtype(self, dtype):
    ...
    if dtype in (np.float16, dtypes_lib.bfloat16.as_numpy_dtype):
      return
    dtype_mapper = {
          np.float16: dtypes_lib.float16,
          np.float32: dtypes_lib.float32,
          np.float64: dtypes_lib.float64,
          dtypes_lib.bfloat16.as_numpy_dtype: dtypes_lib.bfloat16
    }
    self._compare_values(data, dtype=dtype_mapper[dtype])
    x = [0.5, 0.5000001]
    y = [0.0, 1.0]
    self._compare_values(x, y=y, dtype=dtype_mapper[dtype])
  ...
  @test_util.run_in_native_and_block_format
  def testTypes(self):
    for dtype in [np.float16, np.float32, np.float64,
...
class ComplexMakeRealImagTest(test.TestCase):
  ...
  def _compareGradient(self, x):
    ...
    with self.cached_session():
      ...
      epsilon = 1e-3
      jacob_t, jacob_n = gradient_checker.compute_gradient(
          inx, list(x.shape), loss, list(loss.shape), x_init_value=x, delta=epsilon)
...
class InvGradTest(test.TestCase):
  ...
  def testBasic(self):
    ...
    dy = np.random.rand(2, 2).astype(np.float32)
    np_val = -dy * y * y
    with self.cached_session():
      tf_val = tf.raw_ops.InvGrad(y=y,dy=dy)
      self.assertAllClose(np_val, self.evaluate(tf_val))
