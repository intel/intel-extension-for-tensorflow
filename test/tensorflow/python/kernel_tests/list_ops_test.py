...
class ListOpsTest(test_util.TensorFlowTestCase, parameterized.TestCase):
  ...
  # TODO(itex): Enable this UT when the test environment supports testing
  # with GRPC
  ...
  # TODO(itex): Enable this UT when DT_RESOURCE datatype is supported
  ...
  #   updated_v_elems = tf.unstack(updated_v)
  #   updated_v_stacked = [
  ...
  def testConcat(self):
    ...
    l_batch_0 = tf.stack([l0, l1])
    l_batch_1 = tf.stack([l1, l0])
  ...
  def testPushBackBatch(self):
    ...
    with ops.control_dependencies([l_push]):
      l_unstack_orig = tf.unstack(l_batch)
