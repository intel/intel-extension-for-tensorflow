...
class InplaceOpsTest(test_util.TensorFlowTestCase): 
  def testBasicUpdate(self):
    for dtype in [dtypes.float32, dtypes.int32, dtypes.int64, dtypes.bfloat16, dtypes.float64]:
  ...
  def testDeepCopy(self):
    for dtype in [dtypes.float16, dtypes.bfloat16, dtypes.float32, dtypes.float64, dtypes.int32, dtypes.int64]:
      with self.cached_session() as sess:
        shape = [7, 3]
        z = constant_op.constant(np.zeros(shape), dtype=dtype)
        x = array_ops.ones(shape, dtype)
        x = math_ops.add_n([z, x])
        y = array_ops.deep_copy(x)
        x = array_ops.identity(x)
        y = array_ops.identity(y)
        x_val, y_val = sess.run([x, y])
        self.assertAllCloseAccordingToType(x_val, y_val)

        x1 = inplace_ops.alias_inplace_add(x_val, [0], [[1, 2, 3]])
        y1 = inplace_ops.alias_inplace_add(y_val, [0], [[1, 2, 3]])
        x1 = array_ops.identity(x1)
        y1 = array_ops.identity(y1)
        x1_val, y1_val = sess.run([x1, y1])
        self.assertAllCloseAccordingToType(x1_val, y1_val)
