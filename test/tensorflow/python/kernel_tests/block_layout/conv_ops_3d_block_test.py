...
def GetTestConfigs():
  ...
  return [("NDHWC", test.is_gpu_available())]
...
class Conv3DTest(test.TestCase):
  ...
  def testFusedConv3D(self):
    ...
    conv1 = nn_ops.bias_add(conv1, bias)
    conv1 = relu = nn_ops.leaky_relu(conv1)
    conv1 = array_ops.identity(conv1)
    conv2 = nn_ops.conv3d_v2(
        x2, filter_in, strides=[1, 1, 1, 1, 1], padding="VALID")
    conv2 = conv2 + bias
    conv2 = nn_ops.leaky_relu(conv2)
  ...
  @test_util.run_in_graph_and_eager_modes
  def testFusedConv3DWithPad(self):
    tensor_in_sizes_batch = [2, 5, 5, 5, 2]
    filter_in_sizes = [3, 3, 3, 2, 4]
    filter_in = self._CreateNumpyTensor(filter_in_sizes)
    paddings = [[0, 0], [2, 2], [2, 2], [2, 2], [0, 0]]
    tensor_out_shape = [2, 7, 7, 7, 4]

    x1 = self._CreateNumpyTensor(tensor_in_sizes_batch)
    bias = self._CreateNumpyTensor([4])
    pad1 = array_ops.pad(x1, paddings)
    conv1 = nn_ops.conv3d_v2(
        pad1, filter_in, strides=[1, 1, 1, 1, 1], padding="VALID")
    conv1 = array_ops.identity(conv1)

    pad2 = array_ops.pad(x1, paddings)
    conv2 = nn_ops.conv3d_v2(
        pad2, filter_in, strides=[1, 1, 1, 1, 1], padding="VALID")

    self.assertEqual(conv1.shape, tensor_out_shape)
    self.assertEqual(conv2.shape, tensor_out_shape)
    self.assertAllEqual(conv1, self.evaluate(conv2).reshape(conv1.shape))
  ...
  def testFusedConv3DBackpropInputWithSlice(self):
    ...
    # self.evaluate(conv_backprop_input2)
  ...
  def _TestConv3DEmptyTensorOutputShape(self):
    ...
    input_data = 1
    filter_data = 1
    for data_type in self._DtypesToTest(False, forward=True):
  ...
  def _ConstructAndTestGradientForConfig(
    ...
    for data_type in self._DtypesToTest(use_gpu=use_gpu, forward=False):
      ...
      with self.cached_session(use_gpu=use_gpu):
        ...
        input_tensor = array_ops.identity(input_tensor)
        filter_tensor = array_ops.identity(filter_tensor)
        conv = nn_ops.conv3d(
            input_tensor,
            filter_tensor,
            new_strides,
  ...
  def _RunAndVerifyBackprop(self, input_sizes, filter_sizes, output_sizes,
    ...
    if default_dilations or use_gpu:
      with self.cached_session(use_gpu=use_gpu) as sess:
        ...
        t1 = array_ops.identity(t1)
        t2 = array_ops.identity(t2)
        actual = nn_ops.conv3d(
            ...
            padding=padding,
            data_format=data_format)
        actual = array_ops.identity(actual)
        expected = nn_ops.convolution(
            t1,
            t2,
            padding=padding,
        ...
        actual_grad = array_ops.identity(actual_grad)
