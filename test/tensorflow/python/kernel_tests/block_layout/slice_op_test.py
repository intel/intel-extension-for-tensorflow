...
class SliceTest(test.TestCase):
  ...
  def testEmpty(self):
    ...
    for k in xrange(4):
      with self.cached_session(use_gpu=True):
        ...
        slice_t = a[2, k:k]
        slice_t = array_ops.identity(slice_t)
  ...
  def testSelectAll(self):
    for _ in range(10):
      with self.cached_session(use_gpu=True):
        ...
        slice_implicit_t = a[:, :, :, :]
        slice_explicit_t = array_ops.identity(slice_explicit_t)
  ...
  def testSingleDimension(self):
    for _ in range(10):
      with self.cached_session(use_gpu=True):
        ...
        scalar_t = a[hi]
        scalar_t = array_ops.identity(scalar_t)
  ...
  def testScalarInput(self):
    ...
    with self.assertRaisesWithPredicateMatch(
        ...
        "out of range|Attempting to slice scalar input."):
  ...
  def _testSliceMatrixDim0(self, x, begin, size):
    ...
    np_ans = array_ops.identity(np_ans)
  ...
  def testSimple(self):
    with self.session(use_gpu=True):
      for dtype in [
        ...
        slice_t = array_ops.identity(slice_t)
        slice2_t = a[:2, :2]
        slice2_t = array_ops.identity(slice2_t)
