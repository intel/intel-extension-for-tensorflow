...
class ConcatOpTest(test.TestCase):
  ...
  # TODO(itex): should enable after variables are enabled.
  ...
  def testShapeWithUnknownConcatDim(self):
    ...
    concat = array_ops.identity(concat)
    ...
    concat2 = array_ops.identity(concat2)
  ...
  def testConcatWithAllBlockFormatInput(self):
    ...
    with self.session(use_gpu=True):
      ...
      c = array_ops.concat([conv1, conv2, conv3], -1)
      c = array_ops.identity(c)
  ...
  def testConcatWithBlockandPlainFormatInput(self):
    ...
    with self.session(use_gpu=True):
      ...
      c = array_ops.concat([conv1, conv2, x3], -1)
      c = array_ops.identity(c)
  ...
  @test_util.run_deprecated_v1
  def testConcatZeroSizeThenSplit(self):
    """Test Concat with block and plain input format
    """
    a1 = np.random.rand(0, 1).astype("f")
    a2 = np.random.rand(0, 1).astype("f")
    a3 = np.random.rand(0, 1).astype("f")
    with self.session(use_gpu=True):
      x1 = constant_op.constant(a1, dtype=dtypes.float32)
      x2 = constant_op.constant(a2, dtype=dtypes.float32)
      x3 = constant_op.constant(a3, dtype=dtypes.float32)

      c = array_ops.concat([x1, x2, x3], -1)
      split = array_ops.split(c, 3, axis=1)

      split_res = self.evaluate(split)

    self.assertAllEqual(a1, split_res[0])
    self.assertAllEqual(a2, split_res[1])
    self.assertAllEqual(a3, split_res[2])
...
class RawConcatOpTest(test.TestCase):
  ...
  def testHStack(self):
    with self.session(use_gpu=True):
      p1 = array_ops.placeholder(dtypes.float32, shape=[4, 4])
      p2 = array_ops.placeholder(dtypes.float32, shape=[4, 4])
      c = gen_array_ops.concat(0, [p1, p2])
  ...
  def testVStack(self):
    with self.session(use_gpu=True):
      p1 = array_ops.placeholder(dtypes.float32, shape=[4, 4])
      p2 = array_ops.placeholder(dtypes.float32, shape=[4, 4])
      c = gen_array_ops.concat(1, [p1, p2])
  ...
  def test4DStack(self):
    with self.session(use_gpu=True):
      p1 = array_ops.placeholder(dtypes.float32, shape=[2, 3, 1, 1])
      p2 = array_ops.placeholder(dtypes.float32, shape=[2, 3, 4, 1])
      c = gen_array_ops.concat(2, [p1, p2])
  ...
  def testInt32GPU(self):
    with self.session(use_gpu=True):
      ...
      x1 = constant_op.constant(p1)
      x2 = constant_op.constant(p2)
      c = gen_array_ops.concat(0, [x1, x2])
