...
class BatchToSpaceDepthToSpace(test.TestCase, PythonOpImpl):
  ...
  def testDepthToSpaceTranspose(self):
    for data_type in [np.float32, np.float64]:
      x = np.arange(20 * 5 * 8 * 7, dtype=data_type).reshape([20, 5, 8, 7])
      block_size = 2
  ...
  def testUnknownShape(self):
    for data_type in [dtypes.float32, dtypes.float64]:
      t = self.batch_to_space(
          array_ops.placeholder(data_type),
...
class BatchToSpaceNDErrorHandlingTest(test.TestCase):
  ...
  def _testStaticShape(self, input_shape, block_shape, paddings, error):
    ...
    with self.assertRaises(error):
      ...
      _ = array_ops.batch_to_space_nd(
          np.zeros(input_shape, np.float64), block_shape, paddings)
  ...
  def _testDynamicShape(self, input_shape, block_shape, paddings):
    ...
    for data_type in [dtypes.float32, dtypes.float64]:
      input_placeholder = array_ops.placeholder(data_type)
      ...
      t = array_ops.batch_to_space_nd(input_placeholder,
                                      block_shape_placeholder,
  ...
  def testUnknownShape(self):
    for data_type in [dtypes.float32, dtypes.float64]:
      ...
      t = array_ops.batch_to_space_nd(
          array_ops.placeholder(
              data_type, shape=(None, None, None, None)),
          array_ops.placeholder(
              dtypes.int32, shape=(2,)),
          array_ops.placeholder(dtypes.int32))
      self.assertEqual(4, t.get_shape().ndims)
      ...
      t = array_ops.batch_to_space_nd(
          array_ops.placeholder(
              data_type, shape=(None, None, None, 2)),
          array_ops.placeholder(
              dtypes.int32, shape=(2,)),
          array_ops.placeholder(dtypes.int32))
      ...
      t = array_ops.batch_to_space_nd(
          array_ops.placeholder(
              data_type, shape=(3 * 2 * 3, None, None, 2)), [2, 3],
          array_ops.placeholder(dtypes.int32))
      ...
      t = array_ops.batch_to_space_nd(
          array_ops.placeholder(
              data_type, shape=(3 * 2 * 3, None, 2, 2)), [2, 3],
          [[1, 1], [0, 1]])
      ...
      t = array_ops.batch_to_space_nd(
          array_ops.placeholder(
              data_type, shape=(3 * 2 * 3, 2, 1, 2)), [2, 3],
          [[1, 1], [0, 0]])
  ...
  def _compare(self, b, h, w, d, block_size, crop_beg, crop_end):
    ...
    for data_type in [np.float32, np.float64]:
      x = np.random.normal(0, 1, b * h * w * d *
                          block_size_sq).astype(data_type).reshape(
...
class BatchToSpaceNDGradientTest(test.TestCase):
  ...
  def _compare(self, input_shape, block_shape, crops, crops_dtype):
    for data_type in [np.float32, np.float64]:
      ...
      x = np.random.normal(
          0, 1, np.prod(input_shape)).astype(data_type).reshape(input_shape)
