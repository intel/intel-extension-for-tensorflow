...
class NumericsTest(test.TestCase):
  ...
  def testInf(self):
    with self.session(graph=ops.Graph()):
      ...
      a = tf.raw_ops.CheckNumerics(tensor=a, message="Inf")
  ...
  def testNaN(self):
    with self.session(graph=ops.Graph()):
      ...
      t2 = constant_op.constant(0.0)
      a = math_ops.div(t1, t2)
      a = tf.raw_ops.CheckNumerics(tensor=a, message="NaN")
  ...
  def testBoth(self):
    with self.session(graph=ops.Graph()):
      ...
      a = math_ops.div(t1, t2)
      a = tf.raw_ops.CheckNumerics(tensor=a, message="Inf and NaN")
  ...
  def testPassThrough(self):
    with self.session(graph=ops.Graph()):
      ...
      checked = tf.raw_ops.CheckNumerics(tensor=t1, message="pass through test")
