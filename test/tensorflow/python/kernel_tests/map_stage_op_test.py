...
class MapStageTest(test.TestCase):
  ...
  def testColocation(self):
    ...
    with ops.Graph().as_default() as G:
      ...
      with ops.device(gpu_dev):
        stager = data_flow_ops.MapStagingArea([dtypes.float32])
        y = stager.put(1, [v], [0])
        expected_name = gpu_dev if 'gpu' not in gpu_dev else '/device:XPU:0'
  ...
  def testPartialDictGetsAndPeeks(self):
    ...
    with self.session(use_gpu=True, graph=G) as sess:
      ...
      # TODO(itex): Whether we can enable such ASSERT. The output on ATS is
      # --- "Tensor at index '' for key '' " 'has already been removed.' ---
