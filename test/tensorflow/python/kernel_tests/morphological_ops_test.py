...
class DilationTest(test.TestCase):
  ...
  def _ConstructAndTestGradient(self, image_shape, kernel_shape, strides, rates,
    ...
    for dtype in [np.float64, np.float32, np.half]:
      image = np.random.random_sample(image_shape).astype(dtype)
      kernel = np.random.random_sample(kernel_shape).astype(dtype)
