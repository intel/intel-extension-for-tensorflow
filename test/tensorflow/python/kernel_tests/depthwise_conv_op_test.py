    ...
    with self.session(graph=graph, use_gpu=use_gpu) as sess:
      ...
      if dilations is None:
        with sess.graph._kernel_label_map({
          ...
          conv_native = array_ops.identity(conv_native)
      ...
      conv_interface = array_ops.identity(conv_interface)
  ...
  def _ConstructAndTestGradient(self,
    ...
    with self.session(graph=graph, use_gpu=use_gpu) as sess:
      ...
      with sess.graph._kernel_label_map({
        ...
        depthwise_conv2d = array_ops.identity(depthwise_conv2d)
  ...
  def _CompareBackpropInput(self, input_sizes, filter_sizes, output_sizes,
    ...
    def _GetVal(use_gpu):
      with self.cached_session(use_gpu=use_gpu):
        ...
        backprop = array_ops.identity(backprop)
