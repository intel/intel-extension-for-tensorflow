...
# TO avoid legacy UT problem, only test newly registered dt
...
class ParameterizedTruncatedNormalTest(test.TestCase):
      ...
      with self.cached_session():
        if use_stateless:
          ...
          samples = random_ops.parameterized_truncated_normal(
              shape, mean, stddev, minval, maxval, dtype).eval()
  ...
  def testDefaults(self):
    for dtype in DTYPES:
      self.validateMoments([int(1e5)], 0.0, 1.0, -2.0, 2.0, dtype)
      self.validateMoments([int(1e5)], 0.0, 1.0, -2.0, 2.0, dtype, use_stateless=True)

  @test_util.run_deprecated_v1
  def testShifted(self):
    for dtype in DTYPES:
      self.validateMoments([int(1e5)], -1.0, 1.0, -2.0, 2.0, dtype)
      self.validateMoments([int(1e5)], -1.0, 1.0, -2.0, 2.0, dtype, use_stateless=True)

  @test_util.run_deprecated_v1
  def testRightTail(self):
    for dtype in DTYPES:
      self.validateMoments([int(1e5)], 0.0, 1.0, 4.0, np.infty, dtype)
  ...
  def testLeftTail(self):
    for dtype in DTYPES:
      self.validateMoments([int(1e5)], 0.0, 1.0, -np.infty, -4.0, dtype)
      self.validateMoments([int(1e5)],
                          0.0,
                          1.0,
                          -np.infty,
  ...
  def testLeftTailTwoSidedBounds(self):
    for dtype in DTYPES:
      self.validateMoments([int(1e5)], 0.0, 1.0, -6.0, -3.0, dtype)
      self.validateMoments([int(1e5)], 0.0, 1.0, -6.0, -3.0, dtype, use_stateless=True)

  @test_util.run_deprecated_v1
  @test_util.disable_xla("Low probability region")
  def testTwoSidedLeftTailShifted(self):
    for dtype in DTYPES:
      self.validateKolmogorovSmirnov([int(1e5)], 6.0, 1.0, -1.0, 1.0, dtype)
  ...
  def testRightTailShifted(self):
    for dtype in DTYPES:
      self.validateMoments([int(1e5)], -5.0, 1.0, 2.0, np.infty, dtype)
      self.validateMoments([int(1e5)],
                          -5.0,
                          1.0,
                          2.0,
  ...
  def testTruncateOnLeft_entireTailOnRight(self):
    for dtype in DTYPES:
      self.validateKolmogorovSmirnov([int(1e5)], 10.0, 1.0, 4.0, np.infty, dtype)
      self.validateKolmogorovSmirnov([int(1e5)],
                                    10.0,
                                    1.0,
                                    4.0,
  ...
  def testTruncateOnRight_entireTailOnLeft(self):
    for dtype in DTYPES:
      self.validateKolmogorovSmirnov([int(1e5)], -8, 1.0, -np.infty, -4.0, dtype)
      self.validateKolmogorovSmirnov([int(1e5)],
                                    -8.,
                                    1.0,
                                    -np.infty,
  ...
  def testSmallStddev(self):
    for dtype in DTYPES:
      self.validateKolmogorovSmirnov([int(1e5)], 0.0, 0.1, 0.05, 0.10, dtype)
      self.validateKolmogorovSmirnov([int(1e5)],
                                    0.0,
                                    0.1,
                                    0.05,
...
def randn_sampler_switchover(shape, num_iters, use_gpu=False):
  ...
  with session.Session(config=config) as sess:
    with ops.device("/cpu:0" if not use_gpu else "/xpu:0"):
