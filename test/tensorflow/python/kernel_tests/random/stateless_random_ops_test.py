...
def get_device():
  ...
  for device_type in ('XPU', 'GPU', 'CPU'):
...
def float_cases(shape_dtypes=(None,)):
  ...
  def wrap(op, dtype, shape, shape_dtype, seed, **kwargs):
    ...
    if dtype == dtypes.bfloat16 and device_type == 'GPU':
...
def multinomial_cases():
  ...
  for logits_dtype in np.float16, np.float32:
...
def poisson_cases():
  def wrap(op, lam, lam_dtype, out_dtype, shape, seed):
    return op(seed=seed, shape=shape,
              ...
              dtype=out_dtype)
  for lam_dtype in np.float16, np.float32, np.int32, np.int64:
    for out_dtype in np.float16, np.float32, np.int32, np.int64:
