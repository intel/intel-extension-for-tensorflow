...
class AdamOptimizerTest(test.TestCase):
  ...
  def doTestSparse(self, use_resource=False):
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64, dtypes.complex64, dtypes.complex128]:
  ...
  def testTwoSessions(self):
    ...
    with context.eager_mode():
      var0 = variables.Variable(np.array([1.0, 2.0]), name="v0", dtype=dtypes.float32)
      grads0 = constant_op.constant(np.array([0.1, 0.1]), dtype=dtypes.float32)
