...
class GradientDescentOptimizerTest(test.TestCase):
  ...
  def testBasic(self):
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64, dtypes.bfloat16]:
  ...
  def testMinimizeResourceVariable(self):
    for dtype in [dtypes.half, dtypes.float32, dtypes.float64, dtypes.bfloat16]:
      '''wenjie: _ITEXFusedMatMul op does not register dtypes.half on CPU
        test case for dtypes.half needs to be enabled when registration is done
      '''
      if not test.is_gpu_available() and dtype == dtypes.half:
        continue
      # train.GradientDescentOptimizer is V1 only API.
      with ops.Graph().as_default(), self.cached_session():
        var0 = resource_variable_ops.ResourceVariable([[1.0, 2.0]], dtype=dtype)
