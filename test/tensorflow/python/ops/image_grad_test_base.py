...
class CropAndResizeOpTestBase(test.TestCase):
  ...
  def testShapeIsCorrectAfterOp(self):
    ...
    for dtype in [np.float32, np.float16, np.double]:
      image = np.arange(0, batch * image_height * image_width *
                        depth).reshape(image_shape).astype(dtype)
...
@test_util.run_all_in_graph_and_eager_modes
class HSVToRGBOpTestBase(test.TestCase):
  ...
  def testShapeIsCorrectAfterOp(self):
    ...
    for nptype in self.TYPES:
      x = np.random.randint(0, high=255, size=[2, 20, 30, 3]).astype(nptype)
      hsv_input_tensor = constant_op.constant(x, shape=in_shape)
      rgb_out = gen_image_ops.hsv_to_rgb(hsv_input_tensor)
      with self.cached_session():
        self.assertEqual(out_shape, list(rgb_out.get_shape()))
      rgb_out = self.evaluate(rgb_out)
      self.assertEqual(out_shape, list(rgb_out.shape))
