...
class BatchMatMulWithMulAndAddTest(test_lib.TestCase):
  ...
  def testWithBroadcastAdd(self):
    ...
    c2 = math_ops.matmul(a, b)
    c2 = math_ops.mul(c2, scale_value)
    d2 = math_ops.add(c2, badd_value)

    output1 = self.evaluate(d1)
    output2 = self.evaluate(d2)
    self.assertAllClose(output1, output2)

  def testWithNormalAdd(self):
    a = constant_op.constant(np.arange(1, 25, dtype=np.float32), shape=[2, 2, 2, 3])
    b = constant_op.constant(np.arange(25, 49, dtype=np.float32), shape=[2, 2, 3, 2])
    scale_value = constant_op.constant(np.array(3, dtype=np.float32), shape=[1])
    badd_value = constant_op.constant(np.arange(1, 17, dtype=np.float32), shape=[2, 2, 2, 2])
