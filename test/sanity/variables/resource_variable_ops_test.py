...
try:
  from tensorflow.python.ops.variables import VariableV1, is_variable_initialized
except ImportError:
  from tensorflow.python.ops.variable_v1 import VariableV1, is_variable_initialized
...
class ResourceVariableOpsTest(test_util.TensorFlowTestCase,
  ...
  def testStridedSliceAssign(self):
    for dtype in [dtypes.bfloat16, dtypes.float16, dtypes.float32, dtypes.float64, dtypes.complex64, dtypes.complex128]:
      v = resource_variable_ops.ResourceVariable([1.0, 2.0], dtype=dtype)
      vnp = np.array([2.0, 2.0], dtype=dtype.as_numpy_dtype)
      self.evaluate(variables.global_variables_initializer())
      self.evaluate(v[0].assign(2.0))
      self.assertAllEqual(self.evaluate(v), vnp)
  ...
  def testUseResource(self):
    v = VariableV1(1.0, use_resource=True)
  ...
  # TODO(itex): Enable this UT, once we enable MakeWeakResourceHandleOp op
...
# TODO(itex): variant data type support
...
# TODO(itex): enable it after variant supported
#   @def_function.function
