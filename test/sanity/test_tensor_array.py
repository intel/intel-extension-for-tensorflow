...
# ==============================================================================
"""Functional tests for TF1 TensorArray Ops."""
...
# Enable below env option to test TF1 TensorArray ops not TF2 TensorList ops
import os
os.environ["TF_ENABLE_CONTROL_FLOW_V2"]="0"
os.environ["TF_ENABLE_COND_V2"]="0"
os.environ["TF_ENABLE_WHILE_V2"]="0"
os.environ["TF_ENABLE_TENSOR_ARRAY_V2"]="0"
...
class TensorArrayTest(test_util.TensorFlowTestCase):
  numeric_tf_types = [dtypes.float32, dtypes.float16, dtypes.bfloat16]
  ...
  def testTensorArrayIdentity(self):
    with self.session() as session:
      ...
      read0_v, read1_v, size0_v, size1_v, v0, v1 = session.run(fn())
      ...
      self.assertEqual(0.0, v0)
      self.assertEqual(0.0, v1)
  ...
  def testSimpleTensorArray(self):
    with self.session() as session:
  ...
  def testEmptyTensorArrayPack(self):
    with self.session() as session:
      ...
      self.assertAllEqual([3, 0, 1], res.shape)
  ...
  def testTensorGradArrayWriteRead(self):
    with self.session() as session:
      ...
      d0, d1, d2, g_d0, g_d1, g_d2 = session.run(fn())
  ...
  def testTensorGradArrayDynamicWriteRead(self):
    with self.session() as session:
      ...
      d0, d1, d2, g_d0, g_d1, g_d2, vs, g_vs = session.run(fn())
      self.assertAllEqual([[4.0]], d0)
      self.assertAllEqual([[1.0]], d1)
      self.assertAllEqual([[-3.0]], d2)
      self.assertAllEqual([[5.0]], g_d0)
  ...
  def testTensorArrayWriteWrongIndexOrDataTypeFails(self):
    with self.session() as session:
      ...
      with self.assertRaisesRegex(
          (ValueError, errors.InvalidArgumentError), r"TensorArray dtype"):
        session.run(fn())
  ...
  def testTensorArraySplitIncompatibleShapesFails(self):
    with self.session() as session:
      ...
      with self.assertRaisesOpError(
          r"Expected sum of lengths to be equal to"):
        session.run(fn())
      ...
      with self.assertRaisesOpError(
          r"Expected value to be at least a vector"):
        session.run(fn())
  ...
  def _testTensorArrayWriteGradientAddMultipleAdds(self, dtype):
    with self.session() as session:
      ...
      def fn():
        ...
        w2_grad = ta_grad.write(2, c(3.0))

        return w2_grad.read(2)

      # Assert that aggregation works correctly
      self.assertAllEqual(c(3.00), session.run(fn()))
      ...
      def fn():
        ...
        wb1_grad = ta_grad.write(1, c([1.0]))

        return wb1_grad.flow
      self.assertAllEqual(0.0, session.run(fn()))
  ...
  def testMultiTensorArray(self):
    with self.session() as session:
      ...
      self.assertAllClose(9.0, session.run(fn()))

  def _testTensorArrayGradientWriteReadType(self, dtype):
    print("debug before session", flush=True)
    with self.session() as session:
      c = lambda x: np.array(x, dtype=dtype)
  ...
  def _testTensorArrayGradientWritePackConcatAndRead(self):
      with self.session() as session:
        ...
        grad_vals = session.run(fn())  # 2 + 2 entries

        # self.assertAllClose([2.0 - 0.5 + 20.0, 3.0 + 1.5 + 30.0], grad_vals[0])
        # self.assertAllEqual([4.0 + 40.0, 5.0 + 50.0], grad_vals[1])
# TODO(zw): cannot run on XPU with gradients_impl.gradients part
  @test_util.disable_control_flow_v2("b/122315751 (concat)")
  ...
  def testTensorArrayReadTwice(self):
    with self.session() as session:
      ...
      self.assertAllEqual([1.0, -1.0], session.run(fn())[0])
      self.assertAllEqual([1.0, -1.0], session.run(fn())[1])
  ...
  def testSizeTensorArray(self):
    with self.session() as session:
      ...
      self.assertAllEqual(3, session.run(fn()))
  ...
  def testSumOfTwoReadVariablesWithoutRepeatGrad(self):
    with self.session() as session:
      ...
      joint_grad_a_t, joint_grad_b_t = [grad_a, grad_b]
  ...
  def testWriteShape(self):
    with self.session() as session:
      ...
      c0, r0 = session.run(fn())

      self.assertAllEqual(c0.shape, r0.shape)
      ...
      [r0, c1, r1] = session.run(fn())

      self.assertAllEqual(c0.shape, r0.shape)
      self.assertAllEqual(c1.shape, r1.shape)
  ...
  def _testTensorArrayEvalEmpty(self):
    with self.session() as session:
      ...
      with self.assertRaisesWithPredicateMatch(
          errors.UnimplementedError, "TensorArray has size zero"):
        session.run(fn())
...
# Remove this as TensorArrayConcat without Rewrite is not supported currently
  ...
  #       self.assertAllEqual([0, 5], session.run(fn()).shape)
  ...
  def _testTensorArrayScatterRead(self, tf_dtype):
    with self.session() as session:
      ...
      read_vals = session.run(fn(), feed_dict={id0: 1, id1: 8})
