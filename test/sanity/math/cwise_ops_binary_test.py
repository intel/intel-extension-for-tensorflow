...
class BinaryOpTest(test.TestCase):
  ...
  def _compareGpu(self, x, y, np_func, tf_func):
    ...
    self.assertAllCloseAccordingToType(np_ans, tf_gpu)
    self.assertShapeEqual(np_ans, out)
  ...
  def testDoubleBasic(self):
    ...
    try:
      from scipy import special  # pylint: disable=g-import-not-at-top
      a_pos_small = np.linspace(0.1, 2, 15).reshape(1, 3, 5).astype(np.float64)
      x_pos_small = np.linspace(0.1, 10, 15).reshape(1, 3, 5).astype(np.float64)
      self._compareBoth(a_pos_small, x_pos_small, special.gammainc,
                        math_ops.igamma)
      self._compareBoth(a_pos_small, x_pos_small, special.gammaincc,
                        math_ops.igammac)
      ...
      self._compareBoth(x_pos_small + 1, a_pos_small, special.zeta,
                        math_ops.zeta)
      n_small = np.arange(0, 15).reshape(1, 3, 5).astype(np.float64)
      self._compareBoth(n_small, x_pos_small, special.polygamma,
                        math_ops.polygamma)
    except ImportError as e:
      tf_logging.warn("Cannot test special functions: %s" % str(e))
  ...
  def testComplex64Basic(self):
    x = (1 + 1j) * np.linspace(-10, 10, 6).reshape(1, 3, 2).astype(
        np.complex64)
    y = (1 + 1j) * np.linspace(20, -20, 6).reshape(1, 3, 2).astype(
        np.complex64)
    self._compareBoth(x, y, np.add, math_ops.add)
    self._compareBoth(x, y, np.subtract, math_ops.subtract)
    self._compareBoth(x, y, np.multiply, math_ops.multiply)
    ...
    if x.dtype in (np.float16, np.float32, np.float64):
      ...
      if tf_func not in (_FLOORDIV, math_ops.floordiv):
        if x.dtype == np.float16:
          ...
          self._compareGradientX(x, y, np_func, tf_func, np.float_)
          self._compareGradientY(x, y, np_func, tf_func, np.float_)
        else:
          self._compareGradientX(x, y, np_func, tf_func)
          self._compareGradientY(x, y, np_func, tf_func)
      self._compareGpu(x, y, np_func, tf_func)
  ...
  def _testBCastB(self, xs, ys):
    funcs = [
        ...
        (np.subtract, _SUB),
        # TODO(itex): need to fix pow accuracy issue.
  ...
  def _SetupConv2DBinary(self, in0_size, in1_size, data_format, use_gpu, expected):
    x1 = self._CreateNumpyTensor(in0_size)
    x2 = self._CreateNumpyTensor(in1_size)
    w = self._CreateNumpyTensor([1, 1, 3, 3])
    with self.cached_session(use_gpu=use_gpu):
      x1 = constant_op.constant(x1, shape=in0_size)
      w = constant_op.constant(w, shape=[1, 1, 3, 3])
      conv = nn_ops.conv2d(
          x1,
          w,
          dilations=(1, 1),
      ...
      output = array_ops.identity(math_ops.add(conv, x2))
      output = self.evaluate(output)
    self.assertAllClose(expected, np.ravel(output))

  @test_util.run_deprecated_v1
  def testBlockedBinary(self):
    expected = [31, 38, 45, 67, 83, 99, 103, 128, 153, 139, 173, 207]
    self._SetupConv2DBinary((1,2,2,3), (1,3), data_format="NHWC", use_gpu="True", expected=expected)
