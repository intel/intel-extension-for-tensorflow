...
class ReluTest(test.TestCase):
  ...
  def testGradientFloat16(self):
    ...
    def grad(x):
      with backprop.GradientTape() as tape:
        tape.watch(x)
        y = nn_ops.l2_loss(array_ops.identity(nn_ops.relu(x)))
  ...
  def testGradGradFloat32(self):
    with self.cached_session():
      ...
      def f(x):
        ...
        with backprop.GradientTape() as tape:
          tape.watch(x)
          y = array_ops.identity(nn_ops.relu(x))
        return tape.gradient(y, x)
  ...
  # TODO(itex): will enable after ApplyGrdientDescent
  # @test_util.run_deprecated_v1
...
class LeakyReluTest(test.TestCase):
  ...
  @test_util.run_deprecated_v1
  def testNumbersGPU(self):
    # TODO(itex): LeakyReluTest has accuracy issue caused by updating oneDNN. [TFDO-4283]
    self.skipTest(
      "LeakyReluTest has accuracy issue caused by updating oneDNN.")
    if not test.is_gpu_available():
      self.skipTest("No GPU available")
    type_candidates = [np.float16, np.float32]  # TODO(itex): np.float64]
    ...
    with compat.forward_compatibility_horizon(2018, 11, 2):
      with self.cached_session():
        ...
        def f(x):
          ...
          with backprop.GradientTape() as tape:
            tape.watch(x)
            y = array_ops.identity(nn_ops.leaky_relu(x))
          return tape.gradient(y, x)
  ...
  #   def loss():
  #     return array_ops.identity(nn_ops.leaky_relu(x, 0.05))**2
...
class Relu6Test(test.TestCase):
  ...
  # TODO(itex): reenable it when intel-tf fix the issue
  # @test_util.run_deprecated_v1
  # def testGradientFloat32(self):
  #   with self.cached_session():
  #     x = np.asarray(
