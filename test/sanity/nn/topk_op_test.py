...
class TopKTest(test.TestCase):
  ...
  def testDataType(self):
    for dtype in [tf.int32, tf.int64, tf.float32, tf.float16, tf.bfloat16, tf.float64]:
        in_array = np.random.uniform(0, 1, [10, 50000])
        with test_util.force_cpu():
            array = tf.constant(in_array, dtype=dtype)
            out_cpu = tf.nn.top_k(
                array, 10000, sorted=True)    

        with test_util.force_gpu():
            array = tf.constant(in_array, dtype=dtype)
            out_gpu = tf.nn.top_k(
                array, 10000, sorted=True) 
        self.assertAllClose(out_cpu, out_gpu)
  ...
  def testTop3(self):
    for k in range(3, 11, 2):
      for dim in range(512, 51150, 512):
  ...
  def _testLargeSort(self, dtype):
    ...
    n = 5000
    inputs = np.random.permutation(
        np.linspace(-50, 50, b * n, dtype=dtype)).reshape(b, n)
