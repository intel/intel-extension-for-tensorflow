...
class UnaryOpTest(test.TestCase):
  ...
  def _compareCpu(self, x, np_func, tf_func, grad_rtol=None, grad_atol=None):
    ...
    with self.cached_session(use_gpu=False):
      ...
      y = tf_func(x=inx) if tf_func == tf.raw_ops.Sign else tf_func(inx)
      ...
      if x.dtype in (np.complex64, np.complex128) and tf_func in (tf.raw_ops.Sign, math_ops.sign):
        return  # Return early
  ...
  def _compareGpu(self, x, np_func, tf_func):
    ...
    with self.session(use_gpu=True):
      result = tf_func(x=ops.convert_to_tensor(x)) if tf_func == tf.raw_ops.Sign \
        else tf_func(ops.convert_to_tensor(x))
    ...
    # test bfloat16
    with self.session(use_gpu=True):
      supported_bf16_ops = (math_ops.erf, math_ops.erfc, math_ops.exp, math.expm1, math.floor, math_ops.is_finite,
                            math_ops.logical_not, math_ops.neg, math_ops.reciprocal, math_ops.rsqrt,
                            math_ops.sqrt, math_ops.square, math_ops.tanh, math_ops.abs,
                            math_ops.cos, math_ops.sin, math_ops.sigmoid, math_ops.cosh,
                            math_ops.sign, math_ops.log1p)
      if tf_func in supported_bf16_ops and x.dtype == np.float32:
        result_bf16 = tf_func(ops.convert_to_tensor(x, dtype=tf.bfloat16))
        result_bf16 = tf.cast(result_bf16, tf.float32)
        tf_gpu_bf16 = self.evaluate(result_bf16)    
        self.assertAllClose(np_ans, tf_gpu_bf16, rtol=1e-2, atol=1e-2)
    # TODO(itex): make gradient checker work on GPU.
  ...
  def testFloatBasic(self):
    ...
    if test_util.is_gpu_available() or version(tf.__version__) >= version("2.9.0"):
  ...
  def testBFloat16Basic(self):
    ...
    if test_util.is_gpu_available():
      self._compareGpu(
          y, compute_f32(np.vectorize(self._replace_domain_error_with_inf(math.lgamma))),
          math_ops.lgamma)
  ...
  def testComplex64Basic(self):
    ...
    self._compareBoth(y, complex_sign, tf.raw_ops.Sign)
