...
class ApplyPowerSign_ResourceApplyPowerSignTest(test.TestCase):
    def _test_impl(self, size, dtype):        
        ...
        flush_cache()
        op_out = training_ops.resource_apply_power_sign(var=x.handle, m=y.handle, lr=0.001, logbase=0.9, sign_decay=0.999, 
                                                        beta=1e-7, grad=grad)

    @add_profiling
    @multi_run(ITERATION)
    def testApplyPowerSign_ResourceApplyPowerSign(self):
