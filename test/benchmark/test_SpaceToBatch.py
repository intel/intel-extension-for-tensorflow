...
class SpaceToBatchTest(test.TestCase):
    def _test_impl(self, size, dtype):
        in_array = np.random.normal(size=size)
        in_array = constant_op.constant(in_array, dtype=dtype)
        in_paddings = np.random.choice([3, 5, 7, 9], size=(2, 2))
        paddings = constant_op.constant(in_paddings, dtype=tf.int32)
        block_size = 2
      
        flush_cache()
        out_gpu = array_ops.space_to_batch(in_array, paddings, block_size=block_size)
    @add_profiling
    @multi_run(ITERATION)
