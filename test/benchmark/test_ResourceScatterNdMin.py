...
class ResourceScatterNdMinTest(test.TestCase):
    def _test1d(self,  m, n, dtype):
        ...
        shape = resource_variable_ops.ResourceVariable(x, dtype=dtype) 
        flush_cache()
        op = gen_state_ops.resource_scatter_nd_min(ref=shape.handle, indices=indice, updates=update)
            
    def _test2d(self,  m, n, dtype):
        np.random.seed(0)
        indice = constant_op.constant(np.random.choice(m, n, replace=False), dtype=tf.int64) 
