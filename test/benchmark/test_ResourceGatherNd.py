...
class ResourceGatherNdTest(test.TestCase):
  def _test_impl(self, input_size, indices_size, dtype):
    ...
    input_tensor = resource_variable_ops.ResourceVariable(input_array, dtype=dtype) 
  ...
  @add_profiling
  @multi_run(ITERATION)
  def testResourceGatherNd(self):
    for dtype in FLOAT_COMPUTE_TYPE:
      self._test_impl([4,128,91,28,28], [4,128,3], dtype)
