...
class StatelessTruncatedNormalTestV2(test.TestCase):
    def _test_impl(self, size, dtype):
        seed = [1, 2]
        key, counter, alg = _get_key_counter_alg(seed)
        flush_cache()
        out_gpu = gen_stateless_random_ops_v2.stateless_truncated_normal_v2(shape=size, key=key, counter=counter, alg=alg, dtype=dtype, name=None )

    @add_profiling
    @multi_run(ITERATION)
    def testStatelessTruncatedNormalV2(self):
