...
class ResizeBilinearGradTest(test.TestCase):

    def _test_impl(self, size, target_size, dtype):
        np.random.seed(4)
        in_array_1 = np.random.normal(size=size)
        in_array_2 = np.random.normal(size=target_size)
        in_array_1 = constant_op.constant(in_array_1, dtype=dtype)
        in_array_2 = constant_op.constant(in_array_2, dtype=dtypes.float32) # Grad must be a float tensor
        flush_cache()
        result = gen_image_ops.resize_bilinear_grad(in_array_2, in_array_1)
