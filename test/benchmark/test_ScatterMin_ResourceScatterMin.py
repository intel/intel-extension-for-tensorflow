...
class ScatterMin_ResourceScatterMin_Test(test.TestCase):
    def _test_impl(self, ref_size, dtype):
        ...
        flush_cache()
        out_gpu = resource_variable_ops.resource_scatter_min(handle, indices, updates)

    @add_profiling
    @multi_run(ITERATION)
    def test_ScatterMin_ResourceScatterMin(self):
