...
class ApplyGradientDescent_ResourceApplyGradientDescentTest(test.TestCase): 
    def _test_impl(self, size, dtype):
        ...
        alpha = constant_op.constant(np.random.normal(), dtype = dtype)
        flush_cache()
        op_out = gen_training_ops.resource_apply_gradient_descent(var=x.handle, alpha=alpha, delta=y)

    @add_profiling
    @multi_run(ITERATION)
    def testApplyGradientDescent_ResourceApplyGradientDescent(self):
