...
class SparseApplyFtrlTest(test.TestCase):
    def _test_impl(self, ref_size, dtype, index_dtype, slice_1d):
        ...
        flush_cache()
        out_gpu = training_ops.resource_sparse_apply_ftrl(x.handle, y.handle, z.handle, updates_con, indices_con, lr, 0.0, 0.0, -0.5)

    @add_profiling
    @multi_run(ITERATION)
    def testSparseApplyFtrl(self):
