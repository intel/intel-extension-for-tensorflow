...
class ApplyAdagradV2_ResourceApplyAdagradV2Test(test.TestCase): 
    def _test_impl(self, size, dtype):
        ...
        flush_cache()
        op_out=gen_training_ops.resource_apply_adagrad_v2(var=x.handle, accum=y.handle, lr=0.001, epsilon=1e-7, grad=grad)

    @add_profiling
    @multi_run(ITERATION)
    def testApplyAdagrad_ResourceApplyAdagradV2(self):
