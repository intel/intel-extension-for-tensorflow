...
class ResourceSparseApplyKerasMomentumTest(test.TestCase):
    def _test_impl(self, ref_size, dtype, index_dtype, slice_1d):
        ...
        momentum = tf.constant(0.99, dtype=dtype)
        ...
        flush_cache()
        out_gpu = training_ops.resource_sparse_apply_keras_momentum(x.handle, y.handle, lr, updates_con, indices_con, momentum)

    @add_profiling
    @multi_run(ITERATION)
    def testResourceSparseApplyKerasMomentum(self):
