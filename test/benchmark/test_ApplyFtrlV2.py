...
class ApplyFtrlV2_ResourceApplyFtrlV2Test(test.TestCase): 
    def _test_impl(self, size, dtype):
        ...
        flush_cache()
        op_out=training_ops.resource_apply_ftrl_v2(var=x.handle, accum=y.handle, linear=z.handle, grad=grad, lr=0.001, l1=0.1, l2=0.2, l2_shrinkage=0.1, lr_power=0.00001)

    @add_profiling
    @multi_run(ITERATION)
    def testApplyFtrlV2_ResourceApplyFtrlV2Test(self):
