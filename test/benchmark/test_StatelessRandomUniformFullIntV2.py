...
class StatelessRandomUniformFullIntTestV2(test.TestCase):
    def _test_impl(self, size, dtype):
        seed = [1, 2]
        key, counter, alg = _get_key_counter_alg(seed)
        flush_cache()
        out_gpu = gen_stateless_random_ops_v2.stateless_random_uniform_full_int_v2(shape=size, key=key, counter=counter, alg=alg, dtype=tf.dtypes.uint64, name=None)

    @add_profiling
    @multi_run(ITERATION)
    def testStatelessRandomUniformFullIntV2(self):
