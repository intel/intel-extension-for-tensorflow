...
class ApplyAdadelta_ResourceApplyAdadeltaTest(test.TestCase): 
    def _test_impl(self, size, dtype):
        ...
        flush_cache()
        op_out=training_ops.resource_apply_adadelta(var=x.handle, accum=y.handle, accum_update=z.handle, lr=0.001, rho=0.95, epsilon=1e-7, grad=grad)

    @add_profiling
    @multi_run(ITERATION)
    def testApplyAdadelta_ResourceApplyAdadelta(self):
