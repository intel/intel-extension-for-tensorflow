...
class SparseApplyFtrlV2Test(test.TestCase):
    def _test_impl(self, ref_size, dtype, index_dtype, slice_1d):
        ...
        z = resource_variable_ops.ResourceVariable(np.random.normal(size=ref_size), dtype=dtype)  
        ...
        flush_cache()
        out_gpu = training_ops.resource_sparse_apply_ftrl_v2(x.handle, y.handle, z.handle, updates_con, indices_con, lr, 0.0, 0.0, 0.9, -0.5)

    @add_profiling
    @multi_run(ITERATION)
    def testSparseApplyFtrlV2(self):
