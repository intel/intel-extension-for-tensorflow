...
class ApplyProximalAdagrad_ResourceApplyProximalAdagradTest(test.TestCase): 
    def _test_impl(self, size, dtype):
        ...
        flush_cache()
        op_out=training_ops.resource_apply_proximal_adagrad(var=x.handle, accum=y.handle, lr=0.001, l1=0.1, l2=0.1, grad=grad)

    @add_profiling
    @multi_run(ITERATION)
    def testApplyProximalAdagrad_ResourceApplyProximalAdagrad(self):
