...
class ApplyMomentum_ResourceApplyMomentumTest(test.TestCase): 
    def _test_impl(self, size, dtype):
        ...
        flush_cache()
        op_out=training_ops.resource_apply_momentum(var=x.handle, accum=y.handle, lr=0.001, grad=grad, momentum=0.1)

    @add_profiling
    @multi_run(ITERATION)
    def testApplyMomentum_ResourceApplyMomentum(self):
