...
class AssignVariableOpTest(test.TestCase):
    def _test_impl(self, size, dtype):
        x = resource_variable_ops.ResourceVariable(np.zeros(size), dtype=dtype) 
        np.random.seed(4)
        y = constant_op.constant(np.random.normal(size=size), dtype=dtype)
        flush_cache()
        out_gpu = gen_resource_variable_ops.assign_variable_op(x.handle, y)

    @add_profiling
    @multi_run(ITERATION)
    def testAssignVariableOp(self):
