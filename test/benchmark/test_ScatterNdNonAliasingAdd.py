...
class ScatterNdNonAliasingAddTest(test.TestCase):
    def _test1d(self,  m, n, dtype):
        ...
        shape = tf.compat.v1.Variable(x, use_resource=True, dtype=dtype) 
    
        flush_cache()
        op = array_ops.scatter_nd_non_aliasing_add(shape, indice, update)
