...
class StatelessRandomUniformIntTestV2(test.TestCase):
    def _test_impl(self, size, dtype):
        ...
        flush_cache()
        out_gpu = gen_stateless_random_ops_v2.stateless_random_uniform_int_v2(shape=size, key=key, counter=counter, alg=alg, minval=minval2, maxval=maxval2)

    @add_profiling
    @multi_run(ITERATION)
    def testStatelessRandomUniformIntV2(self):
