...
class UnsortedSegmentSumTest(test.TestCase):
    def _test_impl(self, outer_dim, ratio, inner_dim, dtype):
        ...
        seg_ids = np.random.randint(output_outer_dim, size=outer_dim)
        ids = constant_op.constant(seg_ids.astype(np.int32))
        data = constant_op.constant(const, dtype=dtype)
        flush_cache()
        out_gpu = math_ops.unsorted_segment_sum(data, ids, output_outer_dim)
