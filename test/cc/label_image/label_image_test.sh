...
function test_case_build {
    ...
    if [ $? -eq 0 ]; then
        echo "Label image C++ example is built successfully!"
    else
        echo "Label image C++ example is built failed! please check build.log!"
        exit 1
    fi
}
...
function download_pb {
    ...
    if [ -f "data/inception_v3_2016_08_28_frozen.pb" ]; then
        echo "The pb file (data/inception_v3_2016_08_28_frozen.pb) is ready."
    else
        echo "Starting to download pb file (inception_v3_2016_08_28_frozen.pb)."
...
function run_test_case {
    ...
    if [ "$test_target" == "CPU" ] && [ -f "${itex_cc_dir}/libitex_cpu_cc.so" ]; then
        echo "Starting to run label image C++ example with intel_extension_for_tensorflow CPU."
    elif [ "$test_target" == "GPU" ] && [ -f "${itex_cc_dir}/libitex_gpu_cc.so" ]; then
        echo "Starting to run label image C++ example with intel_extension_for_tensorflow GPU."
    else
        echo "Could not find the intel_extension_for_tensorflow CC library (Type: $test_target) from $itex_cc_dir, please check!"
        exit 1
    fi

    rm label_image.log.${test_target}
    export LD_LIBRARY_PATH=$itex_cc_dir:$LD_LIBRARY_PATH
    export ITEX_VERBOSE=3
    ./label_image >& label_image.log.${test_target}
}

function check_result {
    if [ $(grep "military uniform" label_image.log.${test_target} >& /dev/null; echo $?) -eq 0 ]; then
        echo "Label image test [Passed]"
    else
        echo "Label image test [Failed]"
        exit 1
    fi
}
