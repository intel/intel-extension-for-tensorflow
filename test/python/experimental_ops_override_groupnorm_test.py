...
class GroupNormTest(test_lib.TestCase):

  def _compute_group_norm(self, input_shape, dtype=dtypes.float32, axis=-1, groups=2):
    ...
    layer = keras.layers.GroupNormalization(dtype=dtype, axis=axis, groups=groups,
  ...
  def testGroupNorm(self):
    ...
    for dtype in test_types:
      for input_shape, axis, groups in input_params:
        outputs, dx, dweights = self._compute_group_norm(input_shape, dtype, axis, groups)
    ...
    # Enable ITEX GroupNorm call function.
