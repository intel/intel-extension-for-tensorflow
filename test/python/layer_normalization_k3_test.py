        ...
        for epsilon in 1e-12, 1e-3:
            for dtype in 'float32', 'float16':
                ...
                if not test.is_gpu_available() and dtype == 'float16':
                    continue
                input_layer = tf.keras.layers.Input(batch_shape=batch_input_shape)
                tf_layer = tf.keras.layers.LayerNormalization(
                    axis=axis, dtype=dtype,
                    epsilon=epsilon, beta_initializer=tf.constant_initializer(beta),
                    gamma_initializer=tf.constant_initializer(gamma))(input_layer)
                tf_model = tf.keras.Model(input_layer,tf_layer)
                tf_result = tf_model(tf.keras.ops.cast(x, dtype))
                itex_layer = itex.ops.LayerNormalization(
                    axis=axis, dtype=dtype, 
                    epsilon=epsilon, beta_initializer=tf.constant_initializer(beta),
                    gamma_initializer=tf.constant_initializer(gamma))(input_layer)
                itex_model = tf.keras.Model(input_layer,itex_layer)
                itex_result = itex_model(tf.cast(x, dtype))
