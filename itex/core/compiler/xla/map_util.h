...
#ifndef ITEX_CORE_COMPILER_XLA_MAP_UTIL_H_
#define ITEX_CORE_COMPILER_XLA_MAP_UTIL_H_
...
const typename Collection::value_type::second_type& FindOrDie(
  ...
  ITEX_CHECK(it != collection.end()) << "Map key not found: " << key;
  return it->second;
}
...
void InsertOrDie(Collection* const collection, Key&& key, Value&& value) {
  ...
  ITEX_CHECK(p.second) << "duplicate key: " << key;
}
...
void InsertOrDie(Set* const set, Value&& value) {
  ITEX_CHECK(set->insert(std::forward<Value>(value)).second)
      << "duplicate value: " << value;
}

}  // namespace itex_xla

#endif  // ITEX_CORE_COMPILER_XLA_MAP_UTIL_H_
