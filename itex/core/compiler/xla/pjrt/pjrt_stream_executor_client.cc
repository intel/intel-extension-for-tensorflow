...
// that are unlikely to fail; as a rule error ITEX_CHECKing must be done eagerly
...
#include "itex/core/compiler/xla/cpu_function_runtime.h"
...
#include "itex/core/compiler/xla/pjrt/utils.h"
#include "itex/core/compiler/xla/service/computation_layout.h"
#include "itex/core/compiler/xla/service/executable.h"
#include "itex/core/compiler/xla/service/hlo_cost_analysis.h"
#include "itex/core/compiler/xla/service/hlo_input_output_alias_config.h"
...
#include "itex/core/compiler/xla/service/transfer_manager.h"
...
PjRtStreamExecutorClient::PjRtStreamExecutorClient(
    ...
    : platform_id_(itex::Fingerprint64(platform_name)),
      ...
      thread_pool_(
          itex::Env::Default(), "pjrt_thread_pool",
  ...
  for (const std::unique_ptr<PjRtStreamExecutorDevice>& device :
    ...
    ITEX_CHECK(id_to_device_.insert({device->id(), device.get()}).second)
...
void StallStreamOnError(LocalDeviceState* local_device, se::Stream* stream) {
  switch (local_device->allocation_model()) {
    ...
    case LocalDeviceState::kSynchronous:
      ...
      // way to synchronize.
      ITEX_CHECK_OK(stream->BlockHostUntilDone());
      break;
  }
}
...
void RecordUsage(PjRtStreamExecutorBuffer::ScopedHold device_buffer,
  ...
  // itex::profiler::TraceMe traceme("RecordUsage");
...
StatusOr<std::unique_ptr<PjRtStreamExecutorBuffer>> AllocateDestinationBuffer(
  ...
  auto* se_client = itex::down_cast<PjRtStreamExecutorClient*>(client);
  ...
  if (local_device->allocation_model() ==
    ...
    if (copy_stream == nullptr) {
      ITEX_CHECK(is_uninitialized_create);
    } else {
      copy_stream->ThenWaitFor(local_device->compute_stream());
    }
  } else {
    ITEX_DCHECK(transfer_manager->CanShapedBufferBeAccessedNow(
...
void PjRtStreamExecutorBuffer::ScopedHold::Acquire(
  ...
  // ITEX_CHECK the invariant holds.
  ITEX_CHECK(!ok() || buffer_ != nullptr);
}
...
class ScopedHoldAsExternalReference : public PjRtBuffer::ExternalReference {
 public:
  explicit ScopedHoldAsExternalReference(
    ...
    ITEX_CHECK(external_reference_.type() ==
...
PjRtStreamExecutorClient::BufferFromHostBuffer(
  ...
  // itex::profiler::TraceMe traceme(
  //     "PjRtStreamExecutorClient::BufferFromHostBuffer");
  Shape shape = ShapeUtil::MakeShape(type, dims);
  ITEX_VLOG(1) << "PjRtStreamExecutorClient::BufferFromHostBuffer: shape: "
               << shape.ToString() << " device: " << device->DebugString();
  TF_ASSIGN_OR_RETURN(LocalDeviceState * local_device,
                      itex::down_cast<PjRtStreamExecutorDevice*>(device)
                          ->GetLocalDeviceState());
  ...
  if (host_buffer_semantics == HostBufferSemantics::kImmutableOnlyDuringCall ||
    ...
    void* ptr = host_memory_allocator()->AllocateRaw(64, size);
  ...
  auto transfer_h2d =
      [local_client = client(), transfer_manager, local_device, data, size,
       ...
       host_buffer_semantics, transpose{std::move(transpose)}]() {
        ...
        // This function uses ITEX_CHECK_OK and value() since we have no way
        ...
        if (staging_buffer) {
          ...
          ITEX_CHECK_OK(transfer_manager->TransferLiteralToDeviceAsync(
        ...
        ITEX_CHECK_OK(AddDestinationBufferSynchronization(
...
PjRtStreamExecutorClient::CreateUninitializedBuffer(
  ...
  // itex::profiler::TraceMe traceme(
  //     "PjRtStreamExecutorClient::CreateUninitializedBuffer");
  ITEX_VLOG(1) << "PjRtStreamExecutorClient::CreateUninitializedBuffer: shape: "
               << shape.ToString() << " device: " << device->DebugString();
  TF_ASSIGN_OR_RETURN(LocalDeviceState * local_device,
                      itex::down_cast<PjRtStreamExecutorDevice*>(device)
                          ->GetLocalDeviceState());
...
PjRtStreamExecutorClient::BufferFromHostLiteral(const LiteralSlice& literal,
  ...
  ITEX_VLOG(1) << "PjRtStreamExecutorClient::BufferFromHostLiteral: shape: "
...
PjRtStreamExecutorClient::MakeCrossHostReceiveBuffersForGather(
  ...
  ITEX_VLOG(2) << "Making " << gather_details.size()
...
Status PjRtStreamExecutorDevice::TransferFromOutfeed(
  ...
  ITEX_VLOG(1) << "PjRtStreamExecutorDevice::TransferFromOutfeed";
  TF_ASSIGN_OR_RETURN(LocalDeviceState * local_device, GetLocalDeviceState());
...
PjRtStreamExecutorBuffer::PjRtStreamExecutorBuffer(
    ...
    : client_(itex::down_cast<PjRtStreamExecutorClient*>(client)),
      on_device_shape_(std::move(on_device_shape)),
      device_(itex::down_cast<PjRtStreamExecutorDevice*>(device)),
...
PjRtStreamExecutorBuffer::Release(bool wait_for_operations_to_complete) {
  // itex::profiler::TraceMe trace_me("PjRtStreamExecutorBuffer::Release");
...
void PjRtStreamExecutorBuffer::Delete() {
  ITEX_VLOG(1) << "PjRtStreamExecutorBuffer::Delete";
  // When wait_for_reads_to_complete is false, Release should never fail.
  ITEX_CHECK_OK(Release(/*wait_for_operations_to_complete=*/false).status());
}
...
PjRtStreamExecutorBuffer::GetBufferForHoldLocked(ScopedHold::Type type) {
  ...
  ITEX_CHECK_EQ(holds_[ScopedHold::kDonation], 0);
  if (type == ScopedHold::kDonation) {
    ...
    // we were waiting.
    ITEX_CHECK(device_buffer_ != nullptr);
  } else {
    if (device_buffer_ == nullptr) {
...
void PjRtStreamExecutorBuffer::ConvertUsageHold(
  ...
  absl::MutexLock lock(&mu_);
  ITEX_CHECK(device_buffer_.get() == buffer || device_buffer_ == nullptr);
  buffer->AddUsageEvent(usage_stream, std::move(event), reference_held);
  ITEX_CHECK_GT(holds_[ScopedHold::kUsage], 0);
...
void PjRtStreamExecutorBuffer::ConfirmDonation(
  ...
  {
    absl::MutexLock lock(&mu_);
    ITEX_CHECK_EQ(holds_[ScopedHold::kUsage], 0);
    ITEX_CHECK_EQ(holds_[ScopedHold::kExternalReference], 0);
    ITEX_CHECK_EQ(holds_[ScopedHold::kDonation], 1);
    holds_[ScopedHold::kDonation] = 0;
    ITEX_CHECK(device_buffer_.get() == device_buffer);
    // As a sanity ITEX_CHECK ensure no more usage events can be added to the
    // buffer.
...
PjRtFuture<Status> PjRtStreamExecutorBuffer::ToLiteral(
  ...
  ITEX_VLOG(1) << "PjRtStreamExecutorBuffer::ToLiteral";
  if (IsEmptyTuple()) {
...
PjRtStreamExecutorBuffer::CopyToDeviceHelper(
  ...
  ITEX_CHECK(dst_device_buffer.ok());
...
StatusOr<std::unique_ptr<PjRtBuffer>> PjRtStreamExecutorBuffer::CopyToDevice(
  ...
  // itex::profiler::TraceMe traceme("PjRtStreamExecutorBuffer::CopyToDevice");
  ITEX_VLOG(1) << "PjRtStreamExecutorBuffer::CopyToDevice";
  ...
  TF_ASSIGN_OR_RETURN(LocalDeviceState * dst_local_device,
                      itex::down_cast<PjRtStreamExecutorDevice*>(dst_device)
                          ->GetLocalDeviceState());
  LocalDeviceState* transfer_local_device =
      client_->EnqueueD2DTransfersOnSrcStream() ? device_->local_device_state()
                                                : dst_local_device;
  ITEX_CHECK_EQ(dst_local_device->allocation_model(),
...
void PjRtStreamExecutorBuffer::CopyToRemoteDevice(
  ...
  ITEX_VLOG(1) << "PjRtStreamExecutorBuffer::CopyToRemoteDevice";
...
void PjRtStreamExecutorBuffer::CopyToRemoteDeviceScattered(
  ...
  ITEX_VLOG(1) << "PjRtStreamExecutorBuffer::CopyToRemoteDeviceScattered";
...
PjRtFuture<Status> PjRtStreamExecutorBuffer::GetReadyFuture() {
  ...
  return PjRtFuture<Status>(std::move(definition_promise));
}
...
StatusOr<TupleHandle> MakeTupleHelper(
  ...
  if (local_device->allocation_model() ==
    ...
    ITEX_DCHECK(transfer_manager->CanBufferBeAccessedNow(
  ...
  ITEX_CHECK(input_iterator == iterator_end);
...
PjRtStreamExecutorExecutable::PjRtStreamExecutorExecutable(
  ...
  if (device_assignment_ == nullptr) {
    ...
    ITEX_VLOG(3) << "PjRtStreamExecutorExecutable portable single-core";
    num_partitions = 1;
    ITEX_CHECK(addressable_devices_.empty());
  } else {
    // This must go after `executables_` is initialized.
    ITEX_VLOG(3) << "PjRtStreamExecutorExecutable device_assignment:\n"
                 << device_assignment_->ToString();
    ITEX_CHECK_GE(addressable_devices_.size(), 1)
        << device_assignment_->ToString();
    ITEX_CHECK_LE(addressable_devices_.size(),
  ...
  if (executables_.size() > 1) {
    ITEX_CHECK_EQ(num_partitions, executables_.size())
...
StatusOr<ScopedShapedBuffer> PjRtStreamExecutorExecutable::EnqueueExecution(
  ...
  int device_ordinal = itex::down_cast<PjRtStreamExecutorDevice*>(device)
                           ->local_device_state()
                           ->device_ordinal();
  LocalDeviceState* device_state = &(client_->device_state(device_ordinal));
  // itex::profiler::TraceMeConsumer activity(
  //     "PjRtStreamExecutorExecutable::EnqueueExecution",
  //     itex::profiler::ContextType::kPjRt, run_id.ToInt());
  ITEX_VLOG(3) << "Replica " << replica << ", partition " << partition
  ...
  for (int i = 0; i < argument_handles.size(); ++i) {
    auto* handle =
        itex::down_cast<PjRtStreamExecutorBuffer*>(argument_handles[i]);
  ...
  if (run_options.launch_id() != 0) {
    ITEX_VLOG(3) << "launch id for " << name() << ": "
  ...
  {
    //   itex::profiler::TraceMe traceme("ComputeSemaphoreAcquire");
  ...
  ITEX_VLOG(1) << "Replica " << replica << " partition " << partition
        ...
        [references{std::make_tuple(executables_[executable_idx],
         ...
         device_ordinal]() {
          for (const auto& ptr : donated_ptrs) {
            ITEX_CHECK_OK(allocator->Deallocate(device_ordinal, ptr));
          }
        });
  } else {
...
PjRtStreamExecutorExecutable::MakeOutputBuffers(
    ...
    std::vector<std::shared_ptr<TrackedDeviceBuffer>>& buffers_to_release)
    const {
  // itex::profiler::TraceMe traceme("MakeOutputBuffers");
    ...
    if (device_state->allocation_model() == LocalDeviceState::kSynchronous) {
      ...
      compute_callbacks.push_back(
          [root_buffer, allocator{client_->allocator()}, device_ordinal]() {
            ITEX_CHECK_OK(allocator->Deallocate(device_ordinal, root_buffer));
          });
    }
  } else {
...
PjRtStreamExecutorExecutable::ExecuteHelper(
  ...
  const uint64_t start_time_usecs = itex::Env::Default()->NowMicros();
  std::shared_ptr<DeviceAssignment> device_assignment;
  if (device == nullptr) {
    ITEX_CHECK(device_assignment_ != nullptr);
    const int device_id = (*device_assignment_)(replica, partition);
    TF_ASSIGN_OR_RETURN(device, client_->LookupDevice(device_id));
    device_assignment = device_assignment_;
  } else {
    ITEX_CHECK(device_assignment_ == nullptr);
    ITEX_CHECK_EQ(replica, 0);
    ITEX_CHECK_EQ(partition, 0);
    ITEX_CHECK(addressable_devices_.empty());
    device_assignment = std::make_shared<DeviceAssignment>(1, 1);
    (*device_assignment)(0, 0) = device->id();
  }

  ITEX_CHECK_EQ(device->process_index(), client_->process_index());
  int device_ordinal = itex::down_cast<PjRtStreamExecutorDevice*>(device)
                           ->local_device_state()
                           ->device_ordinal();
  // itex::profiler::TraceMe
  // traceme("PjRtStreamExecutorExecutable::ExecuteHelper");
  ITEX_VLOG(1) << "Replica " << replica << ", partition " << partition
               << " mapped to device ordinal for execution: " << device_ordinal;
  ...
  if (!result_buffer_or_status.ok()) {
    ITEX_LOG(ERROR) << "Execution of replica " << replica
  ...
  for (PjRtStreamExecutorBuffer::ScopedHold& b : device_buffers) {
    ...
    if (b.type() == PjRtStreamExecutorBuffer::ScopedHold::kUsage) {
      ...
      ITEX_CHECK(b.type() == PjRtStreamExecutorBuffer::ScopedHold::kDonation);
      b.ConfirmDonation();
    }
  }
  ...
  // ReportExecutableEnqueueTime(itex::Env::Default()->NowMicros() -
  //                            start_time_usecs);
...
PjRtStreamExecutorExecutable::Execute(
  ...
  RunId run_id;
  // itex::profiler::TraceMeProducer activity(
  //     "PjRtStreamExecutorExecutable::Execute",
  //     itex::profiler::ContextType::kPjRt, run_id.ToInt());
  ...
  ITEX_VLOG(1) << "Executing computation " << name()
  ...
  ITEX_VLOG(1) << "Replicated execution complete.";
...
PjRtStreamExecutorExecutable::ExecuteSharded(
  ...
  for (int i = 0; i < addressable_devices_.size(); ++i) {
    if (addressable_devices_[i] == device) {
      ITEX_VLOG(1) << "ExecuteShard executes computation " << name()
...
PjRtStreamExecutorExecutable::ExecutePortable(
  ...
  ITEX_VLOG(1) << "ExecutePortable executes single-core portable executable "
               << name();
    ...
    for (int replica = 0; replica < num_replicas; ++replica) {
      for (int partition = 0; partition < num_partitions; ++partition) {
        ...
        if (device->process_index() != process_index()) {
          ITEX_VLOG(3) << "Non-local device: " << device_id;
          continue;
        }
...
PjRtStreamExecutorClient::Compile(const XlaComputation& computation,
  ...
  // itex::profiler::TraceMe traceme("PjRtStreamExecutorClient::Compile");
  ITEX_VLOG(1) << "PjRtStreamExecutorClient::Compile";
...
StatusOr<std::string> PjRtStreamExecutorClient::SerializeExecutable(
  ...
  const PjRtStreamExecutorExecutable* se_executable =
      itex::down_cast<const PjRtStreamExecutorExecutable*>(&executable);
...
PjRtStreamExecutorClient::DeserializeExecutable(
  ...
  ITEX_VLOG(1) << "PjRtStreamExecutorClient::DeserializeExecutable";
