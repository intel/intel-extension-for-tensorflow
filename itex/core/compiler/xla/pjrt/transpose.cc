...
#include "itex/core/compiler/xla/pjrt/transpose_kernels.h"
...
void MacroKernel(const char* __restrict a, int64_t lda, int outer_bs_a,
  ...
  ITEX_DVLOG(10) << "MacroKernel lda=" << lda << " ldb=" << ldb
  ...
  if (transformation == TransposePlan::Transformation::kF64ToEf57) {
    ITEX_DCHECK_EQ(outer_bs_a * inner_bs % 2, 0);
...
void Transpose(const char* __restrict a, int outer_bs_a, char* __restrict b,
               ...
               void* __restrict scratch) {
  ITEX_DVLOG(10) << "Transpose " << outer_bs_a << " " << outer_bs_b;
  ITEX_DCHECK_GT(outer_bs_a, 0);
  ITEX_DCHECK_GT(outer_bs_b, 0);
  ...
  if (next_node->inc < 0) {
    ...
    if (i < end) {
      ITEX_DCHECK_EQ(node->trailing_tile_next_node_inc, 0);
      ITEX_DCHECK(node->is_inner_dim_in_a || node->is_inner_dim_in_b);
...
void TransposePlan::ExecuteTyped(const char* a, char* b,
  ...
  if (inner_kernel_is_memcpy_) {
    ITEX_DCHECK(transformation_ == Transformation::kNone);
    ...
    switch (inner_block_elems_) {
      ...
      case 16:
        ...
        break;
      default:
        ITEX_LOG(FATAL) << "Invalid inner_block_size " << inner_block_elems_;
    }
  }
}
    ...
    switch (elem_size_in_bytes_) {
      ...
      case 4:
        if (transformation_ == Transformation::kNone) {
          ...
          ITEX_DCHECK(transformation_ == Transformation::kF64ToEf57);
      ...
      case 16:
        ...
        break;
      default:
        ITEX_LOG(FATAL) << "Unimplemented element size " << elem_size_in_bytes_;
    }
  };
...
void TransposePlan::RemoveTrivialDimensions(
  ...
  ITEX_DCHECK(IsPermutation(updated_permutation));
...
void TransposePlan::BuildPlanNodes(
  ...
  ITEX_VLOG(8) << "Before plan build: " << ToString();
  const int ndim = a_dims_.size();
  ITEX_DCHECK_GT(ndim, 0);
  ...
  while (!agenda.empty()) {
    ...
    if (agendum.loop_id == loop_order_.size()) {
      ...
      ITEX_DCHECK_EQ(agendum.num_tasks_at_loop, 1);
      ...
      ITEX_DCHECK(!(inner_kernel_is_memcpy_ && agendum.parent_node_id >= 0));
      continue;
    }
    ...
    ITEX_DCHECK(a_tiling_[a_dim] == 1 || b_tiling_[b_dim] == 1 ||
...
void TransposePlan::Initialize() {
  ...
  if (inner_kernel_is_memcpy_) {
    ...
    switch (elem_size_in_bytes_) {
      ...
      case 16:
        ...
        break;
      default:
        ITEX_LOG(FATAL) << "Unreachable: element size " << elem_size_in_bytes_;
    }
  ...
  ITEX_DCHECK(!inner_kernel_is_memcpy_ ||
              loop_order_.back().dim_in_a == ndim - 1)
      << ToString();
...
std::vector<int> TransposePlan::ChooseParallelizationStrategy(
  ...
  ITEX_VLOG(7) << "Per-loop iteration work in bytes: "
  ...
  for (size_t i = 0; i < loop_order_.size(); ++i) {
    ...
    ITEX_CHECK_GE(available_parallelism, 1);
    ...
    ITEX_VLOG(8) << "iterations=" << iterations
