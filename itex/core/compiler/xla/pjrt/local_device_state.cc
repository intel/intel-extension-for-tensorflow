...
LocalDeviceState::LocalDeviceState(se::StreamExecutor* executor,
  ...
  execute_thread_ =
      std::make_unique<WorkerThread>(itex::Env::Default(), "py_xla_execute");
  callback_thread_ =
      std::make_unique<WorkerThread>(itex::Env::Default(), "py_xla_callback");
}

LocalDeviceState::~LocalDeviceState() {
  Status status = SynchronizeAllActivity();
  if (!status.ok()) {
    ITEX_LOG(ERROR) << "Error when closing device: " << status;
  }
}
...
void LocalDeviceState::ThenExecuteCallback(se::Stream* stream,
  ...
  // itex::profiler::TraceMe traceme("ThenExecuteCallback");
  if (callback_stream_map_.has_value()) {
...
std::unique_ptr<se::Stream> LocalDeviceState::BorrowStreamFromPool() {
  ...
  if (usage_stream_pool_.empty()) {
    ...
    if (status.code() != itex::error::ABORTED) {
      ITEX_CHECK(stream->ok()) << status;
    }
    return stream;
  }
}
