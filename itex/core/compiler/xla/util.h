...
#ifndef ITEX_CORE_COMPILER_XLA_UTIL_H_
#define ITEX_CORE_COMPILER_XLA_UTIL_H_
...
#include "itex/core/utils/errors.h"  // IWYU pragma: keep
...
//
// By default, the timing traces are only printed at ITEX_VLOG(1) and above:
//
...
#define XLA_SCOPED_LOGGING_TIMER_HELPER2(label, level, counter)           \
  static ::itex_xla::TimerStats XLA_TimerStats##counter;                  \
  ::itex_xla::ScopedLoggingTimer XLA_ScopedLoggingTimerInstance##counter( \
      label, /*enabled=*/ITEX_VLOG_IS_ON(level), __FILE__, __LINE__,      \
...
// the timer a name or worry about calling ITEX_VLOG_IS_ON yourself.
...
absl::Span<const T> CastByteSlice(absl::Span<const uint8_t> slice) {
  ITEX_CHECK_EQ(0, slice.size() % sizeof(T));
...
std::string RoundTripFpToString(Eigen::bfloat16 value);
...
T FloorOfRatio(T dividend, T divisor) {
  return itex::MathUtil::FloorOfRatio<T>(dividend, divisor);
}
...
T CeilOfRatio(T dividend, T divisor) {
  return itex::MathUtil::CeilOfRatio<T>(dividend, divisor);
}
...
#define XLA_LOG_LINES(SEV, STRING) \
  ::itex_xla::LogLines(SEV, STRING, __FILE__, __LINE__)

#define ITEX_XLA_VLOG_LINES(LEVEL, STRING)                           \
  do {                                                               \
    if (ITEX_VLOG_IS_ON(LEVEL)) XLA_LOG_LINES(::itex::INFO, STRING); \
  } while (false);

// Utility macro that performs the equivalent of what one would expect
// ITEX_LOG_LINES(FATAL, X) to do but can be used at the end of a function that
// returns a value without getting a compiler warning that no value is returned.
#define XLA_FATAL_LOG(X)                            \
  XLA_LOG_LINES(::itex::ERROR, X);                  \
  ITEX_LOG(FATAL) << "Aborting in " << __FUNCTION__ \
                  << " due to previous errors.";

#endif  // ITEX_CORE_COMPILER_XLA_UTIL_H_
