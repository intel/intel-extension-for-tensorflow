...
#ifndef ITEX_CORE_COMPILER_XLA_LITERAL_H_
#define ITEX_CORE_COMPILER_XLA_LITERAL_H_
...
#include "itex/core/compiler/xla/array3d.h"
#include "itex/core/compiler/xla/array4d.h"
  ...
  static H Hash(H state, const LiteralBase& literal) {
    ...
    ShapeUtil::ForEachSubshape(
        literal.shape(), [&](const Shape& subshape, const ShapeIndex& index) {
          ...
          ITEX_CHECK(LayoutUtil::IsDenseArray(subshape));
  ...
  class Piece {
   public:
    ...
    void set_buffer(char* buffer) {
      ITEX_CHECK(subshape_->IsArray());
      auto* array_rep = std::holds_alternative<Uninitialized>(rep_)
                            ? &rep_.emplace<ArrayRep>()
                            : GetArrayRep();
      ITEX_DCHECK(array_rep);
      array_rep->data = buffer;
    }
    void MoveDataFrom(Piece& from) {
      ITEX_DCHECK(!std::holds_alternative<ArrayRep>(rep_));
      ITEX_DCHECK(!std::holds_alternative<TupleRep>(rep_));
    ...
    void ForEachMutableSubpiece(const Fn& func) {
      ...
      return ForEachMutableHelper(
                 ...
                 const_cast<itex_xla::LiteralBase::Piece*>(this), &index)
          .IgnoreError();
    }
    // Same as above, but the function has the type:
    ...
    Status ForEachMutableSubpieceWithStatus(const Fn& func) {
      ShapeIndex index;
      return ForEachMutableHelper(
          func, const_cast<itex_xla::LiteralBase::Piece*>(this), &index);
    }
...
class MutableLiteralBase : public LiteralBase {
 public:
  ...
  void PopulateR1(const itex::core::Bitmap& values);
  template <typename NativeT>
 ...
 protected:
  ...
  class MaybeOwningShapePtr {
   public:
    ...
    MaybeOwningShapePtr(std::unique_ptr<Shape> unique)  // NOLINT
        : ptr_and_owning_bit_(TakeUnique(std::move(unique))) {}

    MaybeOwningShapePtr(const Shape* borrowed)  // NOLINT
   ...
   private:
    ...
    static intptr_t TakeUnique(std::unique_ptr<Shape> unique) {
      ...
      ITEX_DCHECK_EQ(reinterpret_cast<intptr_t>(released) & kOwningBitMask, 0);
      return reinterpret_cast<intptr_t>(released) | kOwningBitMask;
    }

    static intptr_t Borrow(const Shape* borrowed) {
      ITEX_DCHECK_EQ(reinterpret_cast<intptr_t>(borrowed) & kOwningBitMask, 0);
...
class Literal : public MutableLiteralBase {
 public:
  ...
  Literal(const Shape& shape);  // NOLINT
  ~Literal() override;
...
class MutableBorrowingLiteral : public MutableLiteralBase {
 public:
  ...
  MutableBorrowingLiteral(MutableLiteralBase* literal);  // NOLINT
...
class LiteralSlice : public LiteralBase {
 public:
  ...
  // Implicit conversion constructors.
  LiteralSlice(const LiteralBase& literal);  // NOLINT
...
absl::Span<const NativeT> LiteralBase::Piece::data() const {
  ITEX_DCHECK(subshape().IsArray()) << ShapeUtil::HumanString(subshape());
  ITEX_DCHECK_EQ(subshape().element_type(),
...
NativeT LiteralBase::Piece::Get(absl::Span<const int64_t> multi_index) const {
  ITEX_CHECK(LayoutUtil::IsDenseArray(subshape())) << subshape();
...
template <typename NativeT>
void MutableLiteralBase::PopulateR1(absl::Span<const NativeT> values) {
  ITEX_CHECK(shape().IsArray());
  ITEX_CHECK_EQ(shape().rank(), 1);
  if (shape().is_static()) {
    ITEX_CHECK_EQ(ShapeUtil::ElementsIn(shape()), values.size());
  } else {
    ITEX_CHECK_EQ(GetDynamicSize(0), values.size());
  }
  ITEX_CHECK_EQ(shape().element_type(),
                primitive_util::NativeToPrimitiveType<NativeT>());
...
void MutableLiteralBase::PopulateR2(
  ...
  ITEX_CHECK_EQ(values_dim0_size, literal_dim0_size);
  ITEX_CHECK_EQ(values_dim1_size, literal_dim1_size);
  ...
  for (auto inner_list : values) {
    ...
    ITEX_CHECK_EQ(values_dim1_size, dim1);
    ++dim0;
  }
}
...
template <typename NativeT>
Status MutableLiteralBase::PopulateInternal(
    const std::function<NativeT(absl::Span<const int64_t>, int)>& generator,
    bool parallel) {
...
template <typename NativeT>
Status MutableLiteralBase::Populate(
    const std::function<NativeT(absl::Span<const int64_t>)>& generator) {
...
template <typename NativeT>
Status MutableLiteralBase::PopulateParallel(
    const std::function<NativeT(absl::Span<const int64_t>, int)>& generator) {
  return PopulateInternal<NativeT>(
      [&](absl::Span<const int64_t> indexes, int thread_id) {
...
#endif  // ITEX_CORE_COMPILER_XLA_LITERAL_H_
