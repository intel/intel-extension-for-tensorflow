...
/* static */ int64_t IndexUtil::MultidimensionalIndexToLinearIndex(
  ...
  ITEX_DCHECK_EQ(shape.dimensions_size(), multi_index.size());

  for (size_t i = 0; i < multi_index.size(); ++i) {
    ITEX_DCHECK_GE(multi_index[i], 0);
    ITEX_DCHECK_LT(multi_index[i], shape.dimensions(i))
...
/* static */ std::vector<int64_t> IndexUtil::LinearIndexToMultidimensionalIndex(
  ...
  ITEX_DCHECK_GE(linear_index, 0);
  ITEX_DCHECK_LT(linear_index, ShapeUtil::ElementsIn(shape));
