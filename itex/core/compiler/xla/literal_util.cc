      ...
      [&](const Shape& subshape, const ShapeIndex& shape_index) {
        if (subshape.IsArray()) {
          if (subshape.element_type() ==
            ...
            ITEX_CHECK_OK(result.CopyFrom(literal,
...
Literal CreateScalar(PrimitiveType primitive_type, Args... args) {
  switch (primitive_type) {
    ...
    case TUPLE:
      ITEX_LOG(FATAL) << "tuple element type cannot be a scalar type.";
    case OPAQUE_TYPE:
      ITEX_LOG(FATAL) << "opaque element type cannot be a scalar type.";
    default:
...
std::enable_if_t<!IsReal<NativeT>::value, NativeT> GetMaxImpl() {
  ITEX_LOG(FATAL) << "No max value for given type.";
}

template <typename NativeT>
std::enable_if_t<!IsReal<NativeT>::value, NativeT> GetMinImpl() {
  ITEX_LOG(FATAL) << "No min value for given type.";
}
...
std::enable_if_t<!IsReal<NativeT>::value, NativeT> GetMaxElementImpl(
    const LiteralBase& literal) {
  ITEX_LOG(FATAL) << "Unsupported type.";
}
...
GetElementAtIndexImpl(const LiteralBase* literal,
                      absl::Span<const int64_t> multi_index) {
  ITEX_LOG(FATAL) << "Not a valid scalar element type.";
}
...
struct GetElementAtIndexProvider {
  NativeT<kType> operator()(const LiteralBase* literal,
    ...
    ITEX_DCHECK_EQ(literal->shape().element_type(), kType);
...
/* static */ Literal LiteralUtil::CreateR1(const itex::core::Bitmap& values) {
  Literal literal(
...
/* static */ Literal LiteralUtil::ReshapeSlice(
  ...
  ITEX_CHECK_EQ(ShapeUtil::ElementsIn(literal.shape()), new_num_elements);
  ITEX_CHECK_EQ(new_dimensions.size(), minor_to_major.size());
  ...
  for (int64_t i = 0; i < ShapeUtil::ElementsIn(literal.shape()); ++i) {
    ...
    switch (literal.shape().element_type()) {
      ...
      case C128:
        ...
        break;
      default:
        ITEX_LOG(FATAL) << "Unhandled primitive element type: "
...
/* static */ Literal LiteralUtil::GetFirstScalarLiteral(
  ...
  ITEX_CHECK(literal.shape().IsArray());
  ITEX_CHECK_GT(ShapeUtil::ElementsIn(literal.shape()), 0);
...
/*static*/ void LiteralUtil::SetScalarLiteral(
  ...
  switch (literal.shape().element_type()) {
    ...
    case C128:
      ...
      break;
    default:
      ITEX_LOG(FATAL) << "Unsupported element type: "
...
/* static */ Literal LiteralUtil::MakeTuple(
  ...
  for (int i = 0, end = elements.size(); i < end; ++i) {
    ITEX_CHECK_OK(literal.CopyFrom(*elements[i], /*dest_shape_index=*/{i}));
  }
  return literal;
}
