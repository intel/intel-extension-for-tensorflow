...
#include "itex/core/compiler/xla/client/sharding_builder.h"
...
std::string GetBaseName(const std::string& name, char separator) {
  ...
  ITEX_CHECK_NE(pos, std::string::npos) << name;
...
XlaOp operator>>(XlaOp x, XlaOp y) {
  ...
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    TF_ASSIGN_OR_RETURN(const itex_xla::Shape* shape, builder->GetShapePtr(x));
...
static std::string ShapeToString(const itex_xla::ShapeProto& shape) {
  if (shape.tuple_shapes_size() > 1) {
    return absl::StrCat(
        "(",
        absl::StrJoin(
            shape.tuple_shapes(), ", ",
            [&](std::string* s, const itex_xla::ShapeProto& subshape) {
...
XlaOp XlaBuilder::ReportError(const Status& error) {
  ...
  if (die_immediately_on_error_) {
    ITEX_LOG(FATAL) << "error building computation: " << error;
  }
...
void XlaBuilder::IsConstantVisitor(const int64_t op_handle, int depth,
  ...
  if (ITEX_VLOG_IS_ON(1) && !*is_constant) {
    ITEX_VLOG(1) << indent << "Non-constant: ";
    for (const auto& l : absl::StrSplit(to_print.DebugString(), '\n')) {
      ITEX_VLOG(1) << indent << l;
    }
  }
...
XlaComputation XlaBuilder::BuildAndNoteError() {
  ITEX_DCHECK(parent_builder_ != nullptr);
...
Status XlaBuilder::GetCurrentStatus() const {
  if (!first_error_.ok()) {
    ...
    first_error_backtrace_.Dump(itex::DebugWriteToString, &backtrace);
...
StatusOr<XlaOp> XlaBuilder::AddBroadcastSequence(const Shape& output_shape,
  ...
  ITEX_CHECK(ShapeUtil::IsScalar(*operand_shape) ||
...
XlaOp XlaBuilder::Collapse(XlaOp operand,
  ...
  return ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    ITEX_VLOG(3) << "original shape: "
                 << ShapeUtil::HumanString(*original_shape);
    ITEX_VLOG(3) << "dims to collapse: " << absl::StrJoin(dimensions, ",");
    ...
    ITEX_VLOG(3) << "new sizes: [" << absl::StrJoin(new_sizes, ",") << "]";
...
StatusOr<XlaOp> XlaBuilder::CholeskyInternal(const Shape& shape, XlaOp a,
                                             bool lower) {
  HloInstructionProto instr;
  itex_xla::CholeskyOptions& options = *instr.mutable_cholesky_options();
  ...
  return ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    switch (distribution) {
      ...
      case RandomDistribution::RNG_UNIFORM:
        ...
        break;
      default:
        ITEX_LOG(FATAL) << "unhandled distribution " << distribution;
    }
...
XlaOp XlaBuilder::Conditional(XlaOp predicate, XlaOp true_operand,
  ...
  return ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    TF_ASSIGN_OR_RETURN(const itex_xla::Shape* shape, GetShapePtr(predicate));
...
XlaOp XlaBuilder::Conditional(
  ...
  return ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    TF_ASSIGN_OR_RETURN(const itex_xla::Shape* shape,
...
XlaOp XlaBuilder::ReduceWindow(absl::Span<const XlaOp> operands,
  ...
  return ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    ITEX_CHECK(operand_shape != nullptr);
...
StatusOr<XlaComputation> XlaBuilder::BuildConstantSubGraph(
  ...
  if (ITEX_VLOG_IS_ON(4)) {
    ITEX_VLOG(4) << "Build constant subgraph for:\n" << OpToString(root_op);
  }
  ...
  *module->add_computations() = std::move(entry);
  if (ITEX_VLOG_IS_ON(4)) {
    ITEX_VLOG(4) << "Constant computation:\n" << module->DebugString();
  }
  return std::move(computation);
}
...
XlaOp TriangularSolve(XlaOp a, XlaOp b, bool left_side, bool lower,
  ...
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    itex_xla::TriangularSolveOptions options;
