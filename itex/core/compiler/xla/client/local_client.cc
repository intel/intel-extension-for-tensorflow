...
LocalExecutable::LocalExecutable(std::unique_ptr<Executable> executable,
  ...
  ITEX_CHECK_GE(build_options_.device_ordinal(), 0)
...
StatusOr<ScopedShapedBuffer> LocalExecutable::Run(
  ...
  return AsyncCallAndBlockHostUntilDone<itex_xla::ScopedShapedBuffer>(
  ...
  for (const ShapedBuffer* arg : arguments) {
    ...
    backend->transfer_manager()->TransferLiteralFromDevice(
        stream, *arg, literal.get(), [snapshot, literal](Status status) {
          if (!status.ok()) {
            ITEX_LOG(ERROR)
                << "TransferLiteralFromDevice for HLO snapshot inputs "
                   "failed: "
                << status;
            return;
          }
...
static StatusOr<ExecutableBuildOptions> UpdateBuildOptions(
  ...
  if (options.device_ordinal() == -1) {
    ...
    ITEX_VLOG(3) << "Set device ordinal to default value of: "
...
StatusOr<std::unique_ptr<LocalExecutable>> LocalClient::Load(
  ...
  TF_ASSIGN_OR_RETURN(
      std::unique_ptr<itex_xla::AotCompilationResult> aot_result,
...
StatusOr<TransferToServerResponse> LocalClient::TransferToLocalServer(
    const ::itex_xla::BorrowingLiteral& literal, int device_ordinal) {
  const ::itex_xla::Shape& shape = literal.shape();

  TF_ASSIGN_OR_RETURN(::itex_xla::ScopedShapedBuffer shaped_buffer,
                      backend().transfer_manager()->AllocateScopedShapedBuffer(
  ...
  std::vector<::itex_xla::ScopedShapedBuffer> replicated_buffer;
  replicated_buffer.emplace_back(std::move(shaped_buffer));
  ::itex_xla::TransferToServerResponse result;
  TF_ASSIGN_OR_RETURN(
      *result.mutable_data(),
