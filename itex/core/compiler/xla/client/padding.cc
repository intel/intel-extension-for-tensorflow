...
std::vector<std::pair<int64_t, int64_t>> MakePadding(
  ...
  ITEX_CHECK_OK(ValidatePaddingValues(input_dimensions, window_dimensions,
                                      window_strides));
    ...
    case Padding::kSame:
      for (size_t i = 0; i < input_dimensions.size(); ++i) {
        ...
        int64_t output_dimension =
            itex::MathUtil::CeilOfRatio(input_dimension, window_stride);
        int64_t padding_size =
            std::max<int64_t>((output_dimension - 1) * window_stride +
                                  window_dimension - input_dimension,
                              0);
        low_high_padding.emplace_back(
            itex::MathUtil::FloorOfRatio(padding_size, int64_t{2}),
            itex::MathUtil::CeilOfRatio(padding_size, int64_t{2}));
      }
      break;
  }
