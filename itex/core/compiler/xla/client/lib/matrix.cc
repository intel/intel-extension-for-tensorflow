...
#include "itex/core/compiler/xla/client/lib/arithmetic.h"
...
XlaOp GetMatrixDiagonalViaGather(XlaOp x, int k) {
  ...
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    itex_xla::GatherDimensionNumbers dim_numbers;
...
// reduction.
itex_xla::XlaOp EinsumDiagonalMask(XlaOp x, absl::Span<const int64_t> config) {
  XlaBuilder* builder = x.builder();
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
...
itex_xla::XlaOp EinsumDiagonal(XlaOp x, absl::Span<const int64_t> config) {
  XlaBuilder* builder = x.builder();
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
...
itex_xla::XlaOp EinsumInverseDiagonal(XlaOp x,
                                      absl::Span<const int64_t> config) {
  XlaBuilder* builder = x.builder();
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    auto labels = EinsumDiagonalLabels(config);
...
itex_xla::XlaOp Einsum(itex_xla::XlaOp x, absl::Span<const int64_t> x_config,
                       itex_xla::XlaOp y, absl::Span<const int64_t> y_config,
