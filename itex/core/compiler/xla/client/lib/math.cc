...
#include "itex/core/compiler/xla/client/lib/loops.h"
    ...
    switch (shape.element_type()) {
      ...
      case BF16:
        ...
        return Eq(BitcastConvertType(ConvertElementType(operand, F32), U32),
                  ConstantR0WithType(&b, U32, uint32_t{1} << 31));
      default:
        ITEX_LOG(FATAL) << "Expected real fp type.";
    }
  });
}
...
static XlaOp I0eImpl32(XlaOp x) {
  ...
  x = Abs(x);
  auto half = itex_xla::ScalarLike(x, 0.5);
  auto two = itex_xla::ScalarLike(x, 2.0);
  auto thirty_two = itex_xla::ScalarLike(x, 32.0);
  auto result_le_8 =
      EvaluateChebyshevPolynomial<float>(half * x - two, kI0eCoeffsA);
  auto result_gt_8 =
      EvaluateChebyshevPolynomial<float>(thirty_two / x - two, kI0eCoeffsB) /
      Sqrt(x);
  return Select(Le(x, itex_xla::ScalarLike(x, 8.0)), result_le_8, result_gt_8);
}
...
static XlaOp I1eImpl32(XlaOp x) {
  ...
  auto result_gt_8 =
      ...
      Sqrt(z);
  return Sign(x) *
         Select(Le(z, itex_xla::ScalarLike(x, 8.0)), result_le_8, result_gt_8);
}
