...
#ifndef ITEX_CORE_COMPILER_XLA_CLIENT_LIB_QUANTIZE_H_
#define ITEX_CORE_COMPILER_XLA_CLIENT_LIB_QUANTIZE_H_
...
#include "itex/core/compiler/xla/client/lib/constants.h"
...
struct QuantizedRange {
  ...
  Eigen::bfloat16 min = Eigen::bfloat16(0.0f);
  Eigen::bfloat16 max = Eigen::bfloat16(0.0f);
};
  ...
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    if (element_type != U32) {
      return InvalidArgument(
          "Only U32 is supported for input type of itex_xla::Dequantize Op.");
    }
    ...
    // significant bytes.
    XlaOp shift_bytes =
        itex_xla::ConstantR0<uint32_t>(builder, unpack_size - 1) - iota_r1;

    const int bytes_of_type = sizeof(T) / sizeof(uint8_t);
    std::vector<uint32_t> shift_vec(unpack_size, CHAR_BIT * bytes_of_type);
    XlaOp shift_bits =
        shift_bytes * itex_xla::ConstantR1<uint32_t>(builder, shift_vec);
    ...
    XlaOp unpack_input =
        And(shifted_input, itex_xla::ConstantR0<uint32_t>(builder, bit_mask));

    XlaOp result;

    if (mode_string == "MIN_COMBINED") {
      const Eigen::bfloat16 scale_factor =
          (range.max - range.min) /
          (static_cast<Eigen::bfloat16>(std::numeric_limits<T>::max() -
                                        std::numeric_limits<T>::min()));
      // result = bfloat16(input + half_range) * scale_factor + range.min
      XlaOp unpack_input_bf16 = ConvertElementType(unpack_input, BF16);
      XlaOp half_range_bf16 = itex_xla::ConstantR0<Eigen::bfloat16>(
          builder, static_cast<bfloat16>(half_range));
      XlaOp sum = unpack_input_bf16 + half_range_bf16;

      result =
          sum * itex_xla::ConstantR0<Eigen::bfloat16>(builder, scale_factor) +
          itex_xla::ConstantR0<Eigen::bfloat16>(builder, range.min);
    } else {
      // TODO(wangtao): support other modes.
      return InvalidArgument(
          "Only MIN_COMBINED mode is supported in itex_xla::Dequantize Op.");
    }
...
#endif  // ITEX_CORE_COMPILER_XLA_CLIENT_LIB_QUANTIZE_H_
