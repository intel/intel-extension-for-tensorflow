...
#include "itex/core/compiler/xla/client/lib/pooling.h"
...
#include "itex/core/compiler/xla/client/lib/conv_grad_size_util.h"
...
XlaOp AvgPoolDivideByCountWithGeneralPadding(
  ...
  ITEX_CHECK_EQ(data_format.num_spatial_dims(), num_spatial_dims)
...
XlaOp AvgPoolGrad(XlaOp out_backprop, absl::Span<const int64_t> gradients_size,
  ...
  return b->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    if (num_gradients != num_dims) {
      return itex::errors::InvalidArgument("gradients must be ", num_dims,
                                           "-dimensional");
    }
    ...
    if (backprop_xla_num_dims != num_dims) {
      return itex::errors::InvalidArgument("out_backprop must be ", num_dims,
                                           "-dimensional");
    }
