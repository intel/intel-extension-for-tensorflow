...
StatusOr<SpatialDimensionOutputSizeAndPadding> GetWindowedOutputSize(
  ...
  if (stride <= 0) {
    return itex::errors::InvalidArgument("Stride must be > 0, but got ",
                                         stride);
  }
  if (dilation_rate < 1) {
    return itex::errors::InvalidArgument("Dilation rate must be >= 1, but got ",
                                         dilation_rate);
  }
