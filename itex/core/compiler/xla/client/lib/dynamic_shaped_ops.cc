...
#include "itex/core/compiler/xla/client/lib/dynamic_shaped_ops.h"
...
Shape FindMaxShape(absl::Span<const Shape*> shapes) {
  ...
  for (const Shape* shape : shapes) {
    ITEX_CHECK(result.rank() == shape->rank());
...
XlaOp ReconsileBranchDifference(const Shape& left_branch_shape,
  ...
  if (left_branch_shape.IsTuple()) {
    ...
    // compatible shapes.
    ITEX_CHECK(right_branch_shape.IsTuple() &&
  ...
  // compatible shapes.
  ITEX_CHECK(!right_branch_shape.IsTuple());
  ITEX_CHECK(left_branch_shape.rank() == right_branch_shape.rank());
...
XlaOp DynamicConditional(XlaBuilder* builder, XlaOp predicate,
  ...
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    if (ShapeUtil::Compatible(true_shape, false_shape)) {
      return itex_xla::Conditional(predicate, true_operand, true_computation,
    ...
    auto reconsile_branch = [](const Shape& root_shape,
      ...
      itex_xla::XlaBuilder builder("dynamic_builder");
      auto param = itex_xla::Parameter(&builder, 0, operand_shape, "param");
    ...
    return itex_xla::Conditional(predicate, true_operand,
                                 true_computation_rewritten, false_operand,
                                 false_computation_rewritten);
  });
}
...
XlaOp DynamicConditional(
  ...
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    if (all_shapes_compatible) {
      ...
      return itex_xla::Conditional(branch_index, branch_computations,
                                   branch_operands);
    }
    ...
    return itex_xla::Conditional(branch_index, rewritten_computation_ptrs,
                                 branch_operands);
  });
}
...
StatusOr<XlaOp> SetDimensionSizeWithRebound(ValueInference* value_inference,
  ...
  auto inferred_bound_status_or = value_inference->AnalyzeConstant(
      dimension_size, itex_xla::ValueInferenceMode::kUpperBound);
  ...
  if (inferred_bound_status_or->AllValid()) {
    ...
    if (shape_ptr->dimensions(dimension) > inferred_bound) {
      operand = itex_xla::SliceInDim(operand, 0, inferred_bound, 1, dimension);
    }
  }
  if (dynamism_status_or->Get<bool>({})) {
    // dimension size is dynamic, make output dynamic by calling set dimension
    // size.
    operand = itex_xla::SetDimensionSize(operand, dimension_size, dimension);
  }
  return operand;
}
...
StatusOr<XlaOp> SetAllDimensionSizes(ValueInference* value_inference,
  ...
  for (int64_t i = 0; i < shape_ptr->rank(); ++i) {
    ...
    auto dim_size = itex_xla::Slice(size_vector, {i}, {i + 1}, {1});
    dim_size = itex_xla::Reshape(dim_size, {});
    dim_size = itex_xla::ConvertElementType(dim_size, itex_xla::S32);
    TF_ASSIGN_OR_RETURN(auto dynamism,
                        value_inference->AnalyzeIsDynamic(dim_size));
    if (dynamism.Get<bool>({})) {
      operand = itex_xla::SetDimensionSize(operand, dim_size, i);
    }
  }
  return operand;
}
