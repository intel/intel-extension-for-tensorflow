...
#include "itex/core/compiler/xla/client/lib/prng.h"
...
#include "itex/core/compiler/xla/client/lib/math.h"
#include "itex/core/compiler/xla/client/xla_builder.h"
#include "itex/core/compiler/xla/util.h"

namespace itex_xla {

itex_xla::XlaOp ConcatScalars(itex_xla::XlaBuilder* builder,
                              absl::Span<const itex_xla::XlaOp> scalars) {
  std::vector<itex_xla::XlaOp> vectors;
  absl::c_transform(
      scalars, std::back_inserter(vectors),
      [](itex_xla::XlaOp x) { return itex_xla::Reshape(x, {1}); });
...
SplitShapePair SplitShapeIntoHalves(const Shape& shape) {
  ...
  ITEX_CHECK_GE(pair.split_dim, 0);
...
std::array<XlaOp, 2> Uint128FromOp(XlaOp op) {
  auto u128_low = itex_xla::Reshape(itex_xla::Slice(op, {0}, {1}, {1}), {});
  auto u128_high = itex_xla::Reshape(itex_xla::Slice(op, {1}, {2}, {1}), {});
...
XlaOp ConvertRandomBitsToUniformFloatingPoint(XlaOp bits, XlaOp minval,
  ...
  return builder->ReportErrorOrReturn([&]() -> StatusOr<XlaOp> {
    ...
    ITEX_CHECK((value_type == F32 && bit_type == U32) ||
...
RngOutput NormalFloatingPointDistribution(XlaOp key, XlaOp initial_state,
  ...
  ITEX_DCHECK(primitive_type == F32 || primitive_type == F64);

  XlaBuilder* builder = key.builder();
  auto shape_pair = SplitShapeIntoHalves(shape);
  RngOutput bits_state = UniformFloatingPointDistribution(
      key, initial_state, bit_generator,
      itex_xla::ConstantR0WithType(builder, primitive_type, 0.0),
      itex_xla::ConstantR0WithType(builder, primitive_type, 1.0),
