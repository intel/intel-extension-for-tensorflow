...
#ifndef ITEX_CORE_COMPILER_XLA_PRIMITIVE_UTIL_H_
#define ITEX_CORE_COMPILER_XLA_PRIMITIVE_UTIL_H_
...
// For non-float datatypes, results in a ITEX_LOG(FATAL).
...
// ITEX_LOG(FATAL)'s if complex_type is not complex.
...
inline bool CastPreservesValues(PrimitiveType from_type,
  ...
  // Additional VOID type
  if (from_type == VOID || to_type == VOID) return false;
  // * -> *
  ...
  ITEX_CHECK(primitive_util::IsIntegralType(to_type));
...
//  IsCanonicalRepresentation<itex_xla::bfloat16>(BF16) // true
...
#endif  // ITEX_CORE_COMPILER_XLA_PRIMITIVE_UTIL_H_
