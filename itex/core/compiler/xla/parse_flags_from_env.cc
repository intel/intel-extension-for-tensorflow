...
bool ParseFlagsFromEnvAndDieIfUnknown(
    absl::string_view envvar, const std::vector<itex::Flag>& flag_list) {
  ...
  if (ITEX_VLOG_IS_ON(1)) {
    ITEX_VLOG(1) << "For env var " << envvar << " found arguments:";
    for (int i = 0; i < env_argv->argc; i++) {
      ITEX_VLOG(1) << "  argv[" << i << "] = " << env_argv->argv[i];
    }
  }

  bool result =
      itex::Flags::Parse(&env_argv->argc, &env_argv->argv[0], flag_list);
  ...
  if (result && env_argv->argc != 1) {
    ...
    ITEX_LOG(FATAL) << "Unknown flag" << (unknown_flags.size() > 1 ? "s" : "")
