...
#ifndef ITEX_CORE_COMPILER_XLA_REFERENCE_UTIL_H_
#define ITEX_CORE_COMPILER_XLA_REFERENCE_UTIL_H_
...
#include "itex/core/compiler/xla/client/padding.h"
...
class ReferenceUtil {
 public:
  ...
  static std::unique_ptr<Array2D<T>> Concat2D(const Array2D<T>& lhs,
    ...
    ITEX_CHECK(0 <= concatenate_dimension && concatenate_dimension < 2);
  ...
  static std::unique_ptr<Array3D<T>> Concat3D(const Array3D<T>& lhs,
                                              const Array3D<T>& rhs,
                                              int concatenate_dimension) {
    ITEX_CHECK(0 <= concatenate_dimension && concatenate_dimension < 3);
    ...
    for (int i = 0; i < 3; ++i) {
      if (i != concatenate_dimension) {
        ...
        ITEX_CHECK_EQ(lhs_dims[i], rhs_dims[i]);
      } else {
  ...
  static std::unique_ptr<Array4D<T>> Concat4D(const Array4D<T>& lhs,
                                              const Array4D<T>& rhs,
                                              int concatenate_dimension) {
    ITEX_CHECK(0 <= concatenate_dimension && concatenate_dimension < 4);
  ...
  static std::unique_ptr<Array2D<T>> Slice2D(const Array2D<T>& input,
    ...
    ITEX_CHECK_LE(starts[0], input.n1());
    ITEX_CHECK_LE(starts[1], input.n2());
    ITEX_CHECK_LE(limits[0], input.n1());
    ITEX_CHECK_LE(limits[1], input.n2());
    ITEX_CHECK_GE(strides[0], 1);
    ITEX_CHECK_GE(strides[1], 1);
    auto result = absl::make_unique<Array2D<T>>(
  ...
  static std::unique_ptr<Array3D<T>> Slice3D(const Array3D<T>& input,
    ...
    ITEX_CHECK_LE(starts[0], input.n1());
    ITEX_CHECK_LE(starts[1], input.n2());
    ITEX_CHECK_LE(starts[2], input.n3());
    ITEX_CHECK_LE(limits[0], input.n1());
    ITEX_CHECK_LE(limits[1], input.n2());
    ITEX_CHECK_LE(limits[2], input.n3());
    ITEX_CHECK_GE(strides[0], 1);
    ITEX_CHECK_GE(strides[1], 1);
    ITEX_CHECK_GE(strides[2], 1);
  ...
  static std::unique_ptr<Array4D<T>> Slice4D(const Array4D<T>& input,
    ...
    ITEX_CHECK_LE(starts[1], input.n2());
    ITEX_CHECK_LE(starts[2], input.n3());
    ITEX_CHECK_LE(starts[3], input.n4());
    ITEX_CHECK_LE(limits[0], input.n1());
    ITEX_CHECK_LE(limits[1], input.n2());
    ITEX_CHECK_LE(limits[2], input.n3());
    ITEX_CHECK_LE(limits[3], input.n4());
    ITEX_CHECK_GE(strides[0], 1);
    ITEX_CHECK_GE(strides[1], 1);
    ITEX_CHECK_GE(strides[2], 1);
    ITEX_CHECK_GE(strides[3], 1);
  ...
  static Array3D<NativeT> PadArray3D(const Array3D<NativeT>& operand,
                                     ...
                                     const NativeT pad) {
    ITEX_CHECK_EQ(padding.dimensions_size(), 3);
    ...
    for (int64_t i = 0; i < 3; ++i) {
      ...
      ITEX_CHECK_LE(0, pad_low[i]);
      ITEX_CHECK_LE(0, pad_high[i]);
      ITEX_CHECK_LE(0, padding.dimensions(i).interior_padding())
          << "not implemented";
  ...
  static Array4D<NativeT> PadArray4D(const Array4D<NativeT>& operand,
                                     const PaddingConfig& padding,
                                     const NativeT pad) {
    ITEX_CHECK_EQ(padding.dimensions_size(), 4);
 ...
 private:
  ...
  static void AssertSameSize2D(const Array2D<T1>& array1,
    ...
    ITEX_CHECK_EQ(array1.n1(), array2.n1());
    ITEX_CHECK_EQ(array1.n2(), array2.n2());
...
#endif  // ITEX_CORE_COMPILER_XLA_REFERENCE_UTIL_H_
