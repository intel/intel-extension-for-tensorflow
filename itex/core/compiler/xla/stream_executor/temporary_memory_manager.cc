...
void TemporaryMemoryManager::ForceDeallocateAll() {
  ...
  ITEX_VLOG(1) << "force-deallocating " << records_.size()
               << " remaining records";
...
void TemporaryMemoryManager::MarkFinalized(
  ...
  if (it == records_.end()) {
    if (must_exist) {
      ITEX_LOG(FATAL) << "attempted to mark finalization for temporary "
...
void TemporaryMemoryManager::DeallocateFinalizedTemporaries() {
  ...
  ITEX_VLOG(1) << "deallocated " << deallocated_count
...
TemporaryMemoryManager::AllocateArrayBase(uint64_t element_count,
  ...
  if (device_memory == nullptr) {
    return port::Status(itex::error::RESOURCE_EXHAUSTED,
  ...
  // check the allocation invariant at TemporaryDeviceMemory construction time.
  {
    ...
    ITEX_DCHECK(records_.find(device_memory) == records_.end());
