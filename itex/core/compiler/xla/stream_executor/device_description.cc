...
#include "itex/core/compiler/xla/stream_executor/lib/human_readable.h"
...
bool ThreadDimOk(const DeviceDescription& device_description,
  ...
  if (total_threads > threads_per_block_limit) {
    ITEX_VLOG(2) << "exceeded total-thread-per-block limit: " << total_threads
  ...
  if (!ok) {
    ITEX_VLOG(2) << "thread dim " << thread_dim.ToString()
...
void CalculateDimensionality(const DeviceDescription& device_description,
  ...
  *block_count = port::MathUtil::CeilOfRatio(element_count, *threads_per_block);
  if (*block_count == 1) {
    ITEX_CHECK_LE(element_count, *threads_per_block);
