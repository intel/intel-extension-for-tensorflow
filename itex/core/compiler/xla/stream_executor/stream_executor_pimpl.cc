...
static int64_t GetMemoryLimitBytes() {
  int64_t value;
  SE_CHECK_OK(
      itex::ReadInt64FromEnvVar("TF_PER_DEVICE_MEMORY_LIMIT_MB", 0, &value));
  return value * (1ll << 20);
}
...
StreamExecutor::~StreamExecutor() {
  ...
  if (live_stream_count_.load() != 0) {
    ITEX_LOG(WARNING)
        << "Not all streams were deallocated at executor destruction "
  ...
  if (FLAGS_check_device_leaks) {
    for (const auto& it : mem_allocs_) {
      ITEX_LOG(INFO) << "Memory alloced at executor exit: addr: "
...
void StreamExecutor::Deallocate(DeviceMemoryBase* mem) {
  ITEX_VLOG(1) << "Called StreamExecutor::Deallocate(mem=" << mem->opaque()
...
DeviceMemoryBase StreamExecutor::Allocate(uint64_t size, int64_t memory_space) {
  if (memory_limit_bytes_ > 0 &&
    ...
    ITEX_LOG(WARNING) << "Not enough memory to allocate " << size
                      << " on device " << device_ordinal_
  ...
  ITEX_VLOG(1) << "Called StreamExecutor::Allocate(size=" << size
               << ", memory_space=" << memory_space << ") returns "
...
void* StreamExecutor::UnifiedMemoryAllocate(uint64_t bytes) {
  ...
  ITEX_VLOG(1) << "Called StreamExecutor::UnifiedMemoryAllocate(size=" << bytes
               << ") returns " << buffer << StackTraceIfVLOG10();
  return buffer;
}

void StreamExecutor::UnifiedMemoryDeallocate(void* location) {
  ITEX_VLOG(1) << "Called StreamExecutor::UnifiedMemoryDeallocate(location="
...
void* StreamExecutor::HostMemoryAllocate(uint64_t size) {
  ...
  ITEX_VLOG(1) << "Called StreamExecutor::HostMemoryAllocate(size=" << size
               << ") returns " << buffer << StackTraceIfVLOG10();
  return buffer;
}

void StreamExecutor::HostMemoryDeallocate(void* location) {
  ITEX_VLOG(1) << "Called StreamExecutor::HostMemoryDeallocate(location="
               << location << ")" << StackTraceIfVLOG10();
...
bool StreamExecutor::HostMemoryRegister(void* location, uint64_t size) {
  ITEX_VLOG(1) << "Called StreamExecutor::HostMemoryRegister(location="
               << location << ", size=" << size << ")" << StackTraceIfVLOG10();
  if (location == nullptr || size == 0) {
    ITEX_LOG(WARNING) << "attempting to register null or zero-sized memory: "
                      << location << "; size " << size;
  }
...
bool StreamExecutor::HostMemoryUnregister(void* location) {
  ITEX_VLOG(1) << "Called StreamExecutor::HostMemoryUnregister(location="
               << location << ")" << StackTraceIfVLOG10();
  return implementation_->HostMemoryUnregister(location);
}

bool StreamExecutor::SynchronizeAllActivity() {
  ITEX_VLOG(1) << "Called StreamExecutor::SynchronizeAllActivity()"
               << StackTraceIfVLOG10();
...
port::Status StreamExecutor::SynchronousMemZero(DeviceMemoryBase* location,
                                                uint64_t size) {
  ITEX_VLOG(1) << "Called StreamExecutor::SynchronousMemZero(location="
               << location << ", size=" << size << ")" << StackTraceIfVLOG10();
...
port::Status StreamExecutor::SynchronousMemSet(DeviceMemoryBase* location,
  ...
  ITEX_VLOG(1) << "Called StreamExecutor::SynchronousMemSet(location="
...
bool StreamExecutor::SynchronousMemcpy(DeviceMemoryBase* device_dst,
  ...
  ITEX_VLOG(1) << "Called StreamExecutor::SynchronousMemcpy(device_dst="
  ...
  if (!status.ok()) {
    ITEX_LOG(ERROR) << "synchronous memcpy: " << status;
  }
  return status.ok();
}
...
bool StreamExecutor::SynchronousMemcpy(void* host_dst,
                                       ...
                                       uint64_t size) {
  ITEX_VLOG(1) << "Called StreamExecutor::SynchronousMemcpy(host_dst="
               << host_dst << ", device_src=" << device_src.opaque()
...
port::Status StreamExecutor::SynchronousMemcpyD2H(
  ...
  ITEX_VLOG(1) << "Called StreamExecutor::SynchronousMemcpyD2H(device_src="
...
port::Status StreamExecutor::SynchronousMemcpyH2D(
  ...
  ITEX_VLOG(1) << "Called StreamExecutor::SynchronousMemcpyH2D(host_src="
               << host_src << ", size=" << size
               << ", device_dst=" << device_dst->opaque() << ")"
               << StackTraceIfVLOG10();
...
bool StreamExecutor::AllocateStream(Stream* stream) {
  ...
  if (!implementation_->AllocateStream(stream)) {
    ...
    ITEX_CHECK_GE(count, 0) << "live stream count should not dip below zero";
    ITEX_LOG(INFO) << "failed to allocate stream; live stream count: " << count;
    return false;
  }
...
void StreamExecutor::DeallocateStream(Stream* stream) {
  ...
  ITEX_CHECK_GE(live_stream_count_.fetch_sub(1), 0)
...
void StreamExecutor::EraseAllocRecord(void* opaque) {
  if (FLAGS_check_device_leaks && opaque != nullptr) {
    ...
    if (mem_allocs_.find(opaque) == mem_allocs_.end()) {
      ITEX_LOG(ERROR) << "Deallocating unknown pointer: " << opaque;
    } else {
...
void StreamExecutor::RegisterTraceListener(TraceListener* listener) {
  {
    ...
    if (listeners_.find(listener) != listeners_.end()) {
      ITEX_LOG(INFO) << "Attempt to register already-registered listener, "
                     << listener;
    } else {
...
bool StreamExecutor::UnregisterTraceListener(TraceListener* listener) {
  {
    absl::MutexLock lock(&mu_);
    if (listeners_.find(listener) == listeners_.end()) {
      ITEX_LOG(INFO) << "Attempt to unregister unknown listener, " << listener;
      return false;
    }
...
port::StatusOr<OwningDeviceMemory> StreamExecutorMemoryAllocator::Allocate(
  ...
  if (size > 0 && result == nullptr) {
    return itex::errors::ResourceExhausted(absl::StrFormat(
        "Failed to allocate request for %s (%uB) on device ordinal %d",
        itex::strings::HumanReadableNumBytes(size), size, device_ordinal));
  }
...
port::Status StreamExecutorMemoryAllocator::Deallocate(int device_ordinal,
  ...
  if (!mem.is_null()) {
    TF_ASSIGN_OR_RETURN(StreamExecutor * executor,
                        GetStreamExecutor(device_ordinal));
    ITEX_VLOG(3) << absl::StreamFormat("Freeing %p on device ordinal %d",
...
port::StatusOr<Stream*> StreamExecutorMemoryAllocator::GetStream(
    int device_ordinal) {
  ITEX_CHECK(!AllowsAsynchronousDeallocation())
