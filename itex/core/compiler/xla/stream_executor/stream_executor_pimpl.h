...
#ifndef ITEX_CORE_COMPILER_XLA_STREAM_EXECUTOR_STREAM_EXECUTOR_PIMPL_H_
#define ITEX_CORE_COMPILER_XLA_STREAM_EXECUTOR_STREAM_EXECUTOR_PIMPL_H_
...
class StreamExecutor {
 public:
  ...
  port::Status SynchronousMemcpyH2D(
    ...
    ITEX_CHECK(device_dst->size() == 0 || device_dst->size() >= host_size);
  ...
  port::Status SynchronousMemcpyD2H(const DeviceMemory<T>& device_src,
    ...
    ITEX_CHECK(device_src.size() == 0 || host_size >= device_src.size());
 ...
 public:
  ...
  ~ScopedModuleHandle() {
    if (static_cast<bool>(module_handle_)) {
      ITEX_CHECK(executor_->UnloadModule(module_handle_));
    }
  }
...
#endif  // ITEX_CORE_COMPILER_XLA_STREAM_EXECUTOR_STREAM_EXECUTOR_PIMPL_H_
