...
#include "itex/core/compiler/xla/stream_executor/sycl/sycl_kernel.h"
#include "itex/core/compiler/xla/stream_executor/sycl/sycl_types.h"
...
class GpuExecutor : public internal::StreamExecutorInterface {
 ...
 private:
  ...
  // (supported on CUDA only)
  // port::Status LoadModuleFromCuBin(const char* cubin, GpuModuleHandle*
  // module)
  //     TF_EXCLUSIVE_LOCKS_REQUIRED(in_memory_modules_mu_);

  port::Status LoadModuleFromSpir(const char* spirv, const size_t size,
  ...
  // (supported on ROCm only)
  // port::Status LoadModuleFromHsaco(const char* hsaco, GpuModuleHandle*
  // module)
  //     TF_EXCLUSIVE_LOCKS_REQUIRED(in_memory_modules_mu_);
