...
#ifndef ITEX_CORE_COMPILER_XLA_SHAPE_H_
#define ITEX_CORE_COMPILER_XLA_SHAPE_H_
...
class Shape {
 public:
  ...
  // an array.
  int64_t rank() const {
    ITEX_DCHECK(IsArray()) << "Non-arrays do not have a rank, shape: "
                           << ToString();
  ...
  const Layout& layout() const {
    ITEX_CHECK(has_layout()) << ShortDebugString();
    return *layout_;
  }
  Layout* mutable_layout() {
    ITEX_CHECK(IsArray()) << ShortDebugString();
...
#endif  // ITEX_CORE_COMPILER_XLA_SHAPE_H_
