...
Status WithLogBacktrace(const Status& status) {
  ITEX_CHECK(!status.ok());
  ITEX_VLOG(1) << status.ToString();
  ITEX_VLOG(2) << itex::CurrentStackTrace();
  return status;
}
...
void ScopedLoggingTimer::StopAndLog() {
  if (enabled_) {
    ...
    ITEX_LOG(INFO).AtLocation(file_, line_)
        << label_ << " time: " << itex::strings::HumanReadableElapsedTime(secs)
        << " (cumulative: "
        << itex::strings::HumanReadableElapsedTime(stats.cumulative_secs)
        << ", max: " << itex::strings::HumanReadableElapsedTime(stats.max_secs)
...
std::string HumanReadableNumOps(double flops, double nanoseconds,
  ...
  std::string throughput =
      itex::strings::HumanReadableNum(static_cast<int64_t>(nano_flops * 1e9));
...
void LogLines(int sev, absl::string_view text, const char* fname, int lineno) {
  ...
  while (cur < text.size()) {
    ...
    itex::internal::LogString(fname, lineno, sev,
                              std::string(msg.data(), msg.size()));
    cur = eol + 1;
  }

  if (orig_sev == itex::FATAL) {
    itex::internal::LogString(fname, lineno, orig_sev,
...
absl::InlinedVector<std::pair<int64_t, int64_t>, 8> CommonFactors(
  ...
  ITEX_CHECK_EQ(Product(a), Product(b));
...
std::pair<float, float> SplitF64ToF32(double x) {
  ...
  if (!std::isfinite(x_f32)) {
    ...
    if (std::isfinite(x)) {
      ITEX_LOG(WARNING) << "Out of range F64 constant detected: " << x;
    }
