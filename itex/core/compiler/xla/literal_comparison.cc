...
#include "itex/core/compiler/xla/bit_cast.h"
...
Status Equal(LiteralSlice expected, LiteralSlice actual,
  ...
  if (expected.shape().is_dynamic_dimension(dimension)) {
    // If the dimension is dynamic, we only want to ITEX_CHECK up until the
 ...
 private:
  ...
  Status Run() {
    ...
    if (num_mismatches_ == 0) {
      return OkStatus();
    } else if (!ITEX_VLOG_IS_ON(1) && miscompare_callback_ != nullptr) {
  ...
  void UpdateErrorBucket(double error, absl::Span<int64_t> error_buckets) {
    ITEX_CHECK_EQ(error_buckets.size(), kErrorBucketBounds.size());
  ...
  void CompareValues(T expected, T actual, int64_t linear_index) {
    ...
    if (CompareEqual<T>(expected, actual, {linear_index})) {
      ...
      ITEX_CHECK(!CompareEqual(expected, actual, {linear_index}));
      abs_error = std::numeric_limits<float>::infinity();
      rel_error = std::numeric_limits<float>::infinity();
    } else {
  ...
  std::string ErrorMessage() {
    ...
    ITEX_CHECK_EQ(abs_value_buckets_.size() + 1, kAbsValueBucketBounds.size());
    for (int i = 0; i < abs_value_buckets_.size(); ++i) {
    ...
    auto print_accum_buckets = [&](const std::string& header, int64_t total,
      ...
      ITEX_CHECK_EQ(buckets.size(), kErrorBucketBounds.size());
  ...
  if (expected.shape().IsTuple()) {
    ...
    switch (expected.shape().element_type()) {
      ...
      case TOKEN:
        ...
        return OkStatus();
      default:
        ITEX_LOG(FATAL) << "Unsupported primitive type: "
  ...
  if (ShapeUtil::ElementIsFloating(expected.shape()) ||
    ...
    switch (expected.shape().element_type()) {
      ...
      case C128:
        ...
        break;
      default:
        ITEX_LOG(FATAL) << "Unsupported primitive type in near comparator: "
                        << PrimitiveType_Name(expected.shape().element_type())
...
Status Equal(const LiteralSlice& expected, const LiteralSlice& actual) {
  ITEX_VLOG(1) << "expected:";
  ITEX_XLA_VLOG_LINES(1, expected.ToString());
  ITEX_VLOG(1) << "actual:";
  ITEX_XLA_VLOG_LINES(1, actual.ToString());
...
Status Near(const LiteralSlice& expected, const LiteralSlice& actual,
            ...
            const MiscompareCallback& miscompare_callback) {
  ITEX_VLOG(1) << "Expected literal:";
  ITEX_XLA_VLOG_LINES(1, expected.ToString());
  ITEX_VLOG(1) << "Actual literal:";
  ITEX_XLA_VLOG_LINES(1, actual.ToString());
