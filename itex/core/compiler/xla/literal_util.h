...
#ifndef ITEX_CORE_COMPILER_XLA_LITERAL_UTIL_H_
#define ITEX_CORE_COMPILER_XLA_LITERAL_UTIL_H_
...
class LiteralUtil {
 public:
  ...
  static Literal CreateR1(const itex::core::Bitmap& values);
  template <typename NativeT>
...
/* static */ Literal LiteralUtil::CreateR3Projected(
  ...
  for (int64_t dim0 = 0; dim0 < dim0_size; ++dim0) {
    ...
    for (auto inner_list : values) {
      ...
      ITEX_CHECK_EQ(dim2_size, dim2);
      ++dim1;
    }
    ITEX_CHECK_EQ(dim1_size, dim1);
  }
...
#endif  // ITEX_CORE_COMPILER_XLA_LITERAL_UTIL_H_
