...
/* static */ int64_t LayoutUtil::Major(const Layout& layout,
  ...
  ITEX_CHECK_LE(0, physical_dimension_number);
  ITEX_CHECK_LT(physical_dimension_number, layout.minor_to_major_size());
  return Minor(layout,
...
/*static*/ int64_t LayoutUtil::LinearIndex(const Shape& shape,
  ...
  ITEX_CHECK(shape.IsArray());
  ITEX_CHECK(shape.has_layout());
  const int rank = shape.rank();
  ITEX_CHECK_EQ(rank, indices.size());
...
/*static*/ DimLevelType LayoutUtil::GetDimLevelType(const Layout& layout,
  ...
  ITEX_CHECK_LT(dim, layout.dim_level_types_size());
...
/*static*/ bool LayoutUtil::DimUnique(const Layout& layout, int64_t dim) {
  ...
  ITEX_CHECK_LT(dim, layout.dim_unique_size());
...
/*static*/ bool LayoutUtil::DimOrdered(const Layout& layout, int64_t dim) {
  ...
  ITEX_CHECK_LT(dim, layout.dim_ordered_size());
