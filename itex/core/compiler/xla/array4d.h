...
#ifndef ITEX_CORE_COMPILER_XLA_ARRAY4D_H_
#define ITEX_CORE_COMPILER_XLA_ARRAY4D_H_
...
class Array4D : public Array<T> {
 public:
  ...
  void FillWithYX(const Array2D<T>& value) {
    ITEX_CHECK_EQ(value.height(), height());
    ITEX_CHECK_EQ(value.width(), width());
  ...
  void FillWithZY(const Array2D<T>& value) {
    ITEX_CHECK_EQ(value.height(), depth());
    ITEX_CHECK_EQ(value.width(), height());
    for (int64_t plane = 0; plane < planes(); ++plane) {
      for (int64_t depth = 0; depth < this->depth(); ++depth) {
        for (int64_t height = 0; height < this->height(); ++height) {
          for (int64_t width = 0; width < this->width(); ++width) {
  ...
  void FillWithPZ(const Array2D<T>& value) {
    ITEX_CHECK_EQ(value.height(), planes());
    ITEX_CHECK_EQ(value.width(), depth());
    for (int64_t height = 0; height < this->height(); ++height) {
      for (int64_t width = 0; width < this->width(); ++width) {
        for (int64_t plane = 0; plane < planes(); ++plane) {
          for (int64_t depth = 0; depth < this->depth(); ++depth) {
  ...
  void FillWithMinorDimNum() {
    ITEX_LOG(INFO) << "width: " << this->width();
    ITEX_LOG(INFO) << "height: " << this->height();
    ITEX_LOG(INFO) << "depth: " << this->depth();
    ITEX_LOG(INFO) << "planes: " << this->planes();
    for (int64_t height = 0; height < this->height(); ++height) {
      for (int64_t width = 0; width < this->width(); ++width) {
        for (int64_t plane = 0; plane < planes(); ++plane) {
          for (int64_t depth = 0; depth < this->depth(); ++depth) {
...
#endif  // ITEX_CORE_COMPILER_XLA_ARRAY4D_H_
