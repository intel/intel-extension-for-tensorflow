...
#ifndef ITEX_CORE_COMPILER_XLA_STATUS_MACROS_H_
#define ITEX_CORE_COMPILER_XLA_STATUS_MACROS_H_
...
class MakeErrorStream {
 ...
 private:
  class Impl {
   public:
    Impl(const char* file, int line, itex::error::Code code,
...
#define TF_RET_CHECK(condition)                                            \
  ...
  return itex_xla::status_macros::MakeErrorStream(__FILE__, __LINE__,      \
                                                  ::itex::error::INTERNAL) \
      .with_log_stack_trace()                                              \
      .add_ret_check_failure(#condition)

#endif  // ITEX_CORE_COMPILER_XLA_STATUS_MACROS_H_
