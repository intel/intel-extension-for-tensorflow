...
#ifndef ITEX_CORE_COMPILER_XLA_COMPARISON_UTIL_H_
#define ITEX_CORE_COMPILER_XLA_COMPARISON_UTIL_H_
...
//     itex_xla::PrimitiveType::BF16,
//     Comparison::Order::kTotal
//   );
//   a.ToString(); /* ".LT.BF16.TOTALORDER" */
//
//   Comparison b(Comparison::Direction::kEq, itex_xla::PrimitiveType::U32);
//   b.IsTotalOrder(); /* true */
class Comparison {
 public:
  ...
  // (DEPRECATED) Represents the type of comparison. Prefer
  // itex_xla::PrimitiveType and Comparison::Order, since there are multiple
  // floating point representations that support total ordering.
  ...
  bool Compare(const T a, const T b) const {
    ITEX_CHECK(primitive_util::IsCanonicalRepresentation<T>(primitive_type_));
  ...
  template <typename T,
            absl::enable_if_t<std::is_floating_point<T>::value ||
                                  std::is_same<T, itex_xla::bfloat16>::value,
                              int> = 0>
  bool Compare(const T a, const T b) const {
    ITEX_CHECK(primitive_util::IsCanonicalRepresentation<T>(primitive_type_));
    if (IsTotalOrder()) {
...
#endif  // ITEX_CORE_COMPILER_XLA_COMPARISON_UTIL_H_
