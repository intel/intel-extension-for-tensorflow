...
int SignificandWidth(PrimitiveType type) {
  switch (type) {
    ...
    default:
      ITEX_LOG(FATAL) << "Not a floating data type " << type;
  }
}
...
int BitWidth(PrimitiveType type) {
  switch (type) {
    ...
    case TUPLE:
      ITEX_LOG(FATAL) << "TUPLE is an invalid type for BitWidth";

    case OPAQUE_TYPE:
      ITEX_LOG(FATAL) << "OPAQUE_TYPE is an invalid type for BitWidth";

    default:
      ITEX_LOG(FATAL) << "Unhandled primitive type " << type;
  }
}

int ByteWidth(PrimitiveType type) { return CeilOfRatio(BitWidth(type), 8); }

itex_xla::PrimitiveType UnsignedIntegralTypeForBitWidth(int64_t src_bitwidth) {
  switch (src_bitwidth) {
    case 8:
      return itex_xla::U8;
    case 16:
    ...
    case 64:
      return itex_xla::U64;
    default:
      return itex_xla::PRIMITIVE_TYPE_INVALID;
  }
}

itex_xla::PrimitiveType SignedIntegralTypeForBitWidth(int64_t src_bitwidth) {
  switch (src_bitwidth) {
    case 8:
      return itex_xla::S8;
    case 16:
...
PrimitiveType ComplexComponentType(PrimitiveType complex_type) {
  switch (complex_type) {
    ...
    case C128:
      return F64;
    default:
      ITEX_LOG(FATAL) << "Primitive type is not complex: "
...
//
// itex_xla::OPAQUE_TYPE canonically maps to the string "opaque" -- the only
// reason it's called OPAQUE_TYPE is to avoid clashing with a windows.h macro.
...
// the itex_xla::OPAQUE_TYPE enumerator.
