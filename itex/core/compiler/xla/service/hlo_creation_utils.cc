...
#include "itex/core/compiler/xla/client/lib/comparators.h"
...
StatusOr<HloInstruction*> MakeBinaryHlo(HloOpcode opcode, HloInstruction* lhs,
  ...
  ITEX_CHECK_EQ(computation, rhs->parent());
...
StatusOr<HloInstruction*> MakePadHlo(HloInstruction* operand,
                                     ...
                                     const OpMetadata* metadata) {
  HloComputation* computation = operand->parent();
  ITEX_CHECK_EQ(computation, padding_value->parent());
  TF_ASSIGN_OR_RETURN(
      Shape pad_shape,
...
StatusOr<HloInstruction*> MakeDynamicSliceHlo(
    ...
    absl::Span<const int64_t> slice_sizes, const OpMetadata* metadata) {
  HloComputation* computation = operand->parent();
  ITEX_CHECK_EQ(computation, start_indices->parent());
...
StatusOr<HloInstruction*> MakeDynamicUpdateSliceHlo(
  ...
  HloComputation* computation = operand->parent();
  ITEX_CHECK_EQ(computation, update->parent());
  ITEX_CHECK_EQ(computation, start_indices->parent());
  int64_t rank = start_indices->shape().dimensions(0);
  std::vector<HloInstruction*> scalar_start_indices;
  for (int i = 0; i < rank; ++i) {
...
StatusOr<HloInstruction*> MakeConcatHlo(
    ...
    const OpMetadata* metadata) {
  ITEX_CHECK_GT(operands.size(), 0);

  HloComputation* computation = operands[0]->parent();
  ITEX_CHECK(absl::c_all_of(operands, [&](HloInstruction* instr) {
...
HloInstruction* MakeConvertToHlo(HloInstruction* hlo, PrimitiveType type,
  ...
  hlo = hlo->parent()->AddInstruction(HloInstruction::CreateConvert(shape, hlo),
                                      metadata);
  ITEX_CHECK_EQ(hlo->shape().element_type(), type);
  return hlo;
}
...
StatusOr<HloInstruction*> MakeMapHlo(absl::Span<HloInstruction* const> operands,
                                     ...
                                     const OpMetadata* metadata) {
  ITEX_CHECK(!operands.empty()) << "Map Hlo requires at least one operand.";
  HloComputation* computation = operands.front()->parent();
  std::vector<const Shape*> operand_shapes;
  int64_t max_operand_rank = 0;
  for (const HloInstruction* operand : operands) {
    ITEX_CHECK_EQ(computation, operand->parent());
...
StatusOr<HloInstruction*> MakeReduceHlo(
  ...
  ITEX_CHECK_EQ(operands.size(), init_values.size());
  auto root = reduce_computation->root_instruction();
  if (root->shape().IsTuple()) {
    ITEX_CHECK_EQ(root->shape().tuple_shapes_size(), operands.size());
  } else {
    ITEX_CHECK_EQ(operands.size(), 1);
  }
...
StatusOr<HloInstruction*> MakeSelectHlo(HloInstruction* pred,
  ...
  ITEX_DCHECK_EQ(computation, on_true->parent());
  ITEX_DCHECK_EQ(computation, on_false->parent());
...
StatusOr<HloInstruction*> MakeSortHlo(
  ...
  ITEX_CHECK(!operands.empty()) << "Sort Hlo requires at least one operand.";
...
StatusOr<HloInstruction*> CollapseFirstNDims(HloInstruction* operand,
  ...
  ITEX_CHECK_GE(operand_shape.dimensions_size(), n);
...
StatusOr<HloInstruction*> ExpandFirstDimIntoNDims(
  ...
  ITEX_CHECK_GT(operand->shape().dimensions_size(), 0);
  ITEX_CHECK_EQ(operand->shape().dimensions(0), Product(expanded_dims));
...
StatusOr<HloInstruction*> PadVectorWithZeros(HloInstruction* operand,
  ...
  HloComputation* computation = operand->parent();
  ITEX_CHECK_EQ(operand->shape().dimensions_size(), 1);
