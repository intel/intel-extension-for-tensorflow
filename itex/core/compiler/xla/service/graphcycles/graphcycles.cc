...
#include "itex/core/compiler/xla/service/graphcycles/ordered_set.h"
...
bool GraphCycles::CheckInvariants() const {
  ...
  for (Vec<Node*>::size_type x = 0; x < r->nodes_.size(); x++) {
    Node* nx = r->nodes_[x];
    if (nx->visited) {
      ITEX_LOG(FATAL) << "Did not clear visited marker on node " << x;
    }
    if (!ranks.insert(nx->rank).second) {
      ITEX_LOG(FATAL) << "Duplicate occurrence of rank " << nx->rank;
    }
    for (int32_t y : nx->out.GetSequence()) {
      Node* ny = r->nodes_[y];
      if (nx->rank >= ny->rank) {
        ITEX_LOG(FATAL) << "Edge " << x << "->" << y
                        << " has bad rank assignment " << nx->rank << "->"
                        << ny->rank;
      }
    }
  }
...
bool GraphCycles::CanContractEdge(int32_t a, int32_t b) {
  ITEX_CHECK(HasEdge(a, b)) << "No edge exists from " << a << " to " << b;
  RemoveEdge(a, b);
...
void SortInPostOrder(absl::Span<Node* const> nodes,
  ...
  absl::c_sort(*to_sort, [&](int32_t a, int32_t b) {
    ITEX_DCHECK(a == b || nodes[a]->rank != nodes[b]->rank);
    return nodes[a]->rank > nodes[b]->rank;
  });
}
}  // namespace
