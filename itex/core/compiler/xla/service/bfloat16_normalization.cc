...
Status BFloat16NormalizationVisitor::ChangeOutputTypeThenInsertConvertBack(
  ...
  ShapeUtil::ForEachMutableSubshape(
      hlo->mutable_shape(),
      [&](Shape* subshape, const itex_xla::ShapeIndex& index) {
...
Status BFloat16NormalizationVisitor::HandleMultipleOutputs(
  ...
  for (int64_t i = 0; i < hlo->operand_count(); ++i) {
    ITEX_CHECK(hlo->operand(i)->shape().IsArray());
    ITEX_CHECK(ShapeUtil::GetSubshape(hlo->shape(), {i}).IsArray());
...
StatusOr<bool> BFloat16Normalization::Run(HloModule* module) {
  ...
  ITEX_XLA_VLOG_LINES(
      2, "BFloat16Normalization::Run(), after:\n" + module->ToString());
  if (visitor.changed()) {
