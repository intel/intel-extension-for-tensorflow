...
int64_t GetNumberOf32bitUnits(const Shape& shape) {
  ...
  ITEX_CHECK(bit_width == 32 || bit_width == 64);
...
StatusOr<HloInstruction*> ConvertSmallFpRngToF32Rng(HloInstruction* rng) {
  ITEX_CHECK_EQ(rng->opcode(), HloOpcode::kRng);
  PrimitiveType primitive_type = rng->shape().element_type();
  ITEX_CHECK(primitive_type == F16 || primitive_type == BF16);

  std::vector<HloInstruction*> new_operands;
  absl::c_transform(rng->operands(), std::back_inserter(new_operands),
                    [&](HloInstruction* operand) {
                      ITEX_CHECK_EQ(operand->shape().element_type(),
                                    primitive_type);
...
StatusOr<HloComputation*> GetComputationForRng(HloInstruction* rng) {
  ...
  const Shape u64_shape = ShapeUtil::MakeShape(itex_xla::U64, {});
  const Shape u128_shape = ShapeUtil::MakeShape(itex_xla::U64, {2});
  ...
  auto generator = [](itex_xla::XlaOp key, itex_xla::XlaOp state,
  ...
  if (rng->random_distribution() == RNG_NORMAL) {
    ...
    ITEX_CHECK_EQ(rng->random_distribution(), RNG_UNIFORM);
...
StatusOr<HloInstruction*> RngExpander::ExpandInstruction(HloInstruction* rng) {
  ITEX_VLOG(2) << "Expand rng instruction " << rng->ToString();
  ...
  ITEX_VLOG(2) << "Rng key " << key->ToString();
  ITEX_VLOG(2) << "Rng state " << state->ToString();
