...
#include "itex/core/utils/proto_serialization.h"
...
struct CanonicalDebugOptions {
  explicit CanonicalDebugOptions(const DebugOptions& opts)
    ...
    if (dump_to_lower == "sponge" ||
      ...
      if (!itex::io::GetTestUndeclaredOutputsDir(&dump_to)) {
        ITEX_LOG(ERROR)
            << "--xla_dump_to=" << opts.xla_dump_to()
...
static Status WriteStringToFile(itex::Env* env, const std::string& fname,
                                DataProducer& data_producer, bool compressed) {
  std::unique_ptr<itex::WritableFile> file;
  TF_RETURN_IF_ERROR(env->NewWritableFile(fname, &file));
  if (compressed) {
    return Status(TF_UNIMPLEMENTED, "compressed file is not supported");
    // auto gz_opts = itex::io::ZlibCompressionOptions::GZIP();
    // itex::io::ZlibOutputBuffer gz_file(
...
static Status WriteStringToFile(itex::Env* env, const std::string& fname,
  ...
  if (!compressed) {
    return itex::WriteStringToFile(env, fname, data);
  } else {
    return Status(TF_UNIMPLEMENTED, "compressed file is not supported");
    // std::unique_ptr<itex::WritableFile> file;
    // TF_RETURN_IF_ERROR(env->NewWritableFile(fname, &file));
    // auto gz_opts = itex::io::ZlibCompressionOptions::GZIP();
    // itex::io::ZlibOutputBuffer gz_file(file.get(),
    //                                          gz_opts.input_buffer_size,
    //                                          gz_opts.output_buffer_size,
    //                                          gz_opts);
    // TF_RETURN_IF_ERROR(gz_file.Init());
...
static absl::optional<std::string> GetDumpFilePath(
  ...
  ITEX_VLOG(1) << "Dumping " << filename << " to " << dir;
  ...
  if (!env->IsDirectory(dir).ok()) {
    ...
    if (!status.ok() && !env->IsDirectory(dir).ok()) {
      ITEX_LOG(ERROR) << "Could not create directory " << dir
  ...
  if (opts.dump_max_hlo_modules > 0) {
    ...
    auto pattern = itex::io::JoinPath(dir, "*module_*.*");
    auto status = env->GetMatchingPaths(pattern, &matches);
    if (!status.ok()) {
      ITEX_LOG(ERROR) << "Could not get matching paths for pattern " << pattern
                      << ": " << status;
    }
    ...
    if (dumped_module_ids.size() >= opts.dump_max_hlo_modules) {
      ...
      if (RE2::FullMatch(filename, *module_id_regex, &module_id) &&
        ...
        ITEX_LOG(ERROR) << "Have already dumped " << dumped_module_ids.size()
  ...
  return itex::io::JoinPath(dir, SanitizeFileName(std::string(filename)));
}
...
static absl::optional<std::string> DumpToFileInDirImpl(
  ...
  auto status =
      WriteStringToFile(itex::Env::Default(), *file_path, contents, compress);
  if (!status.ok()) {
    ITEX_LOG(ERROR) << "Could not write XLA debug data to " << *file_path
                    << ": " << status;
    return absl::nullopt;
  }
...
static absl::optional<std::string> DumpToFileInDirImpl(
  ...
  auto status = WriteStringToFile(itex::Env::Default(), *file_path,
...
static std::vector<std::string> DumpHloModuleImpl(
  ...
  if (opts.dump_as_proto) {
    ...
    std::string pb;
    if (!itex::SerializeToStringDeterministic(module_proto, &pb)) {
  ...
  if (opts.dump_fusion_visualization) {
    for (const HloComputation* computation :
      ...
      if (IsTrivial(*computation)) {
        ITEX_VLOG(1) << "Skipping computation " << computation->name()
                     << " as trivial";
        continue;
      }
      ...
      if (!rendered_graph.ok()) {
        ITEX_VLOG(1) << "Skipping fusion visualization"
...
static void DumpHloModuleMetadata(
  ...
  std::string content;
  if (itex::protobuf::TextFormat::PrintToString(metadata, &content)) {
    DumpToFileInDirImpl(filename, content, opts);
  } else {
    ITEX_LOG(ERROR) << "Failed to convert HloModuleMetadataProto to text.";
  }
}
...
void DumpStringToFileInDirOrStdout(string_view filename, string_view contents,
                                   const DebugOptions& opts) {
  // Dump to stdout if that's called for.
  CanonicalDebugOptions copts = CanonicalDebugOptions(opts);
  if (copts.dumping_to_stdout()) {
    std::cout << "*** Begin " << filename << " ***\n"
              << contents << "\n*** End " << filename << " ***" << std::endl;
    return;
  }
  // Otherwise, dump to a file.
  DumpToFileInDirImpl(filename, contents, copts);
}
...
std::string TimestampFor(const HloModule& module) {
  ...
  auto timestamp_emplace = module_id_to_timestamp.try_emplace(
      module.unique_id(), itex::Env::Default()->NowMicros());
...
void DumpProtobufToFile(const itex::protobuf::Message& proto,
  ...
  if (env->IsDirectory(dir).ok()) {
    const std::string path = itex::io::JoinPath(dir, filename);
    Status status;
    if (opts.dump_as_text) {
      status = itex::WriteTextProto(env, absl::StrCat(path, ".txt"), proto);
    } else {
      status = itex::WriteBinaryProto(env, absl::StrCat(path, ".pb"), proto);
    }
    if (!status.ok()) {
      ITEX_LOG(ERROR) << "Could not write XLA debug data to " << filename
                      << ": " << status;
    }
  }
}
...
void DumpHloSnapshotIfEnabled(const HloModule& module,
  ...
  if (opts.dumping_to_stdout()) {
    ITEX_LOG(ERROR)
        << "Refusing to write HLO snapshot proto for " << filename
        << " to stdout.  Pass --xla_dump_to=<path> to write to a file.";
    return;
  }
  std::string pb;
  if (!itex::SerializeToStringDeterministic(snapshot, &pb)) {
    ITEX_LOG(ERROR) << "Failed to serialize HLO snapshot proto " << filename;
  }
