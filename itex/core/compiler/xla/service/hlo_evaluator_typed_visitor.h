...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_HLO_EVALUATOR_TYPED_VISITOR_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_HLO_EVALUATOR_TYPED_VISITOR_H_
...
// HloEvaluatorTypedVisitor so they don't pollute namespace itex_xla, but that
...
detail::unsigned_promoted_type_t<T> ToArithSafeType(T t) {
  return static_cast<detail::unsigned_promoted_type_t<T>>(t);
}
template <typename T,
          typename std::enable_if<!std::is_integral<T>::value>::type* = nullptr>
T ToArithSafeType(T t) {
  return std::move(t);
}
...
class HloEvaluatorTypedVisitor : public DfsHloVisitorWithDefault {
 private:
  ...
  // static_cast value to a double so just ITEX_CHECK fail. This method is not
  // used at run-time, but must be available at compile-time to keep the
  // compiler happy.
  template <
      typename NativeT,
      typename std::enable_if<is_complex_t<NativeT>::value>::type* = nullptr>
  double GetAsDouble(const Literal& literal,
                     absl::Span<const int64_t> input_index) {
    ITEX_LOG(FATAL) << "Trying to get complex literal as double: "
                    << literal.ToString();
  }
 ...
 public:
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        ElementWiseBinaryOp(multiply,
                            [](ElementwiseT lhs_elem, ElementwiseT rhs_elem) {
                              return ElementwiseT(ToArithSafeType(lhs_elem) *
                                                  ToArithSafeType(rhs_elem));
                            }));
    return Status::OK();
  }
  ...
  Status HandleMinimum(HloInstruction* minimum) {
    ITEX_VLOG(2) << "Evaluating minimum\n";
  ...
  Status HandleSelect(HloInstruction* select) override {
    ITEX_CHECK(!ShapeUtil::IsScalar(select->operand(0)->shape()));
    ITEX_CHECK(select->shape().IsArray());
  ...
  Status HandleConvolutionWithLiterals(HloInstruction* conv,
    ...
    ITEX_CHECK_OK(ShapeUtil::ValidateShape(lhs_shape));
    ITEX_CHECK_OK(ShapeUtil::ValidateShape(rhs_shape));
    ITEX_CHECK(lhs_shape.IsArray());
    ITEX_CHECK(rhs_shape.IsArray());
    ITEX_CHECK(ShapeUtil::SameElementType(lhs_shape, rhs_shape));
    ITEX_CHECK(ShapeUtil::SameElementType(lhs_shape, result_shape));

    const auto& dnums = conv->convolution_dimension_numbers();
    const int64_t num_spatial_dims = dnums.output_spatial_dimensions_size();
    ITEX_CHECK_EQ(num_spatial_dims, dnums.input_spatial_dimensions_size());
    ITEX_CHECK_EQ(num_spatial_dims, dnums.kernel_spatial_dimensions_size());
    ITEX_CHECK_GE(num_spatial_dims, 0);
    ITEX_CHECK_EQ(window.dimensions_size(), num_spatial_dims);
  ...
  Status HandleConvolution(HloInstruction* conv) override {
    ...
    ITEX_CHECK_EQ(num_spatial_dims + 2, lhs_rank);
    ITEX_CHECK_EQ(num_spatial_dims + 2, rhs_rank);
    ...
    ITEX_CHECK(ShapeUtil::Compatible(result_shape, inferred_return_shape))
        << "return shape set to: " << ShapeUtil::HumanString(result_shape)
        << " but is inferred to be: "
        << ShapeUtil::HumanString(inferred_return_shape);
  ...
  Status HandleDot(HloInstruction* dot) {
    ...
    ITEX_CHECK(dot->shape().IsArray());
    ITEX_CHECK(lhs->shape().IsArray());
    ITEX_CHECK(rhs->shape().IsArray());
    ...
    ITEX_CHECK(ShapeUtil::SameElementType(lhs->shape(), rhs->shape()));
    ITEX_CHECK(ShapeUtil::SameElementType(lhs->shape(), dot->shape()));
    ...
    ITEX_CHECK_EQ(lhs->shape().dimensions(lhs_contracting_dimension),
  ...
  Status HandleDotSlowPathWithLiterals(HloInstruction* dot,
    ...
    ITEX_CHECK(
        ShapeUtil::SameElementType(lhs_literal.shape(), rhs_literal.shape()));
    ITEX_CHECK(ShapeUtil::SameElementType(lhs_literal.shape(), dot->shape()));

    ITEX_CHECK_EQ(dnums.lhs_batch_dimensions_size(),
    ...
    TF_RETURN_IF_ERROR(result.PopulateParallel<ReturnT>(
        [&](absl::Span<const int64_t> result_index, int) {
          ...
          for (int64_t k = 0; k < total_contraction_size; k++) {
            ...
            result_val += ToArithSafeType(lhs_val) * ToArithSafeType(rhs_val);
  ...
  Status HandlePad(HloInstruction* pad) override {
    ITEX_CHECK(pad->operand(0)->shape().IsArray());
    // Padding value must be scalar.
    ITEX_CHECK(ShapeUtil::IsScalar(pad->operand(1)->shape()));
    ITEX_CHECK_EQ(pad->operand(0)->shape().rank(),
    ...
    ITEX_CHECK(ShapeUtil::Compatible(pad->shape(), inferred_return_shape))
  ...
  Status HandleDynamicSlice(HloInstruction* dynamic_slice) override {
    ...
    switch (start_indices->shape().element_type()) {
      ...
      default:
        ITEX_LOG(FATAL) << "HandleDynamicSlice: unhandled primitive type for "
                           "start_indices: "
                        << PrimitiveType_Name(
                               start_indices->shape().element_type());
    }
  ...
  Status HandleDynamicUpdateSlice(
    ...
    switch (start_indices->shape().element_type()) {
      ...
      default:
        ITEX_LOG(FATAL)
            << "HandleDynamicUpdateSlice: unhandled primitive type for "
               "start_indices: "
  ...
  Status HandleMap(HloInstruction* map) override {
    switch (map->operand(0)->shape().element_type()) {
      ...
      default:
        ITEX_LOG(FATAL) << "HandleMap: unhandled primitive type for "
                           "input operand: "
                        << PrimitiveType_Name(
  ...
  Status HandleReduceWindow(HloInstruction* reduce_window) override {
    ...
    const int num_threads = itex::port::MaxParallelism() + 1;
    ...
    auto evaluate_impl = [&init_literal_vec, &window_shape, &window,
      ...
      ITEX_CHECK_GE(embedded_evaluator_index, 0);
      ITEX_CHECK_LT(embedded_evaluator_index, embedded_evaluators.size());
      ...
      IterateThroughWindow(
          ...
          [&](absl::Span<const int64_t> operand_index) -> void {
            ...
            for (auto& curr_result_val : computed_result) {
              ITEX_VLOG(2) << "Pushing:" << curr_result_val.ToString() << "\n";
            ...
            for (const auto* input_literal : input_literal_vec) {
              ...
              ITEX_CHECK_OK(curr_val_literal_vec.back().CopyElementFrom(
            ...
            computed_result[0] = embedded_evaluator.Evaluate(*function, args)
                                     .ConsumeValueOrDie();
            ITEX_VLOG(2) << "Computed result:" << computed_result[0].ToString()
                         << "\n";
      ...
      ITEX_VLOG(2) << "Final result size:" << computed_result.size() << "\n";
      for (const auto& res : computed_result) {
        ITEX_VLOG(2) << res.ToString() << "\n";
      }
      ...
      ShapeUtil::ForEachIndexParallel(
          ...
          [&results, &evaluate_impl](absl::Span<const int64_t> output_index,
            ...
            for (int i = 0; i < computed_result_vec.size(); ++i) {
              ...
              //    `output_index`.
              ITEX_CHECK_OK(results[i].CopyElementFrom(computed_result_vec[i],
                                                       {}, output_index));
            }
            return true;
          });
      result = Literal::MoveIntoTuple(absl::MakeSpan(results));
      ITEX_VLOG(2) << "Final result is:" << result.ToString() << "\n";
    } else {
      TF_RETURN_IF_ERROR(result.PopulateParallel<ReturnT>(
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        ElementWiseUnaryOp(reduce_precision, [&](ElementwiseT elem) {
          ...
          if (dest_exponent_bits < src_exponent_bits) {
            ...
            const Uint exp_bits_mask =
                (Uint{1 << src_exponent_bits} - 1)  // NOLINT
                << src_mantissa_bits;
    ...
    TF_RETURN_IF_ERROR(
        result.Populate<ReturnT>([&](absl::Span<const int64_t> multi_index) {
          for (int64_t i = 0; i < operand_indices.size(); ++i) {
            ITEX_CHECK_GE(multi_index[i] + start[i], 0);
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_HLO_EVALUATOR_TYPED_VISITOR_H_
