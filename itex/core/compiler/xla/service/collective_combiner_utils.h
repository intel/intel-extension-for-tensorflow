...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_COLLECTIVE_COMBINER_UTILS_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_COLLECTIVE_COMBINER_UTILS_H_
...
StatusOr<bool> CombineInstructionsByKey(
  ...
  while (!keys.empty()) {
    ...
    for (HloInstruction* instruction :
      ...
      ITEX_VLOG(1) << "Considering HLO " << instruction->ToString()
      ...
      if (instruction->operands().size() != 1) {
        ITEX_VLOG(1) << "Skipping due to " << instruction->operands().size()
                     << " operands";
        keys.erase(it);
        continue;
      }
      ...
      if (instruction_bytes > combine_threshold_bytes) {
        ITEX_VLOG(1) << "Size " << instruction_bytes << " above threshold.";
        keys.erase(it);
        continue;
      }

      if (to_combine_bytes + instruction_bytes > combine_threshold_bytes) {
        ITEX_VLOG(1) << "Combined size threshold exceeded.";
        break;
      }
      ...
      if (is_reachable) {
        ITEX_VLOG(1) << "Instruction is reachable.";
        break;
      }

      ITEX_VLOG(1) << "Adding instruction to set.";
      to_combine.push_back(instruction);
      to_combine_bytes += instruction_bytes;
      keys.erase(it);

      if (to_combine.size() >= combine_threshold_count) {
        ITEX_VLOG(1) << "Combined count threshold reached.";
        break;
      }
    }
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_COLLECTIVE_COMBINER_UTILS_H_
