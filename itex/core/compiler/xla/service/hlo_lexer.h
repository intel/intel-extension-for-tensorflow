...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_HLO_LEXER_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_HLO_LEXER_H_
 ...
 public:
  ...
  std::string GetStrVal() const {
    switch (GetKind()) {
      ...
      default:
        ITEX_LOG(FATAL) << "This token does not have string value";
    }
  }
  int64_t GetInt64Val() const {
    ITEX_CHECK(GetKind() == TokKind::kInt) << TokKindToString(GetKind());
    return token_state_.int64_val;
  }
  double GetDecimalVal() const {
    ITEX_CHECK(GetKind() == TokKind::kDecimal);
    return token_state_.decimal_val;
  }
  PrimitiveType GetPrimitiveTypeVal() const {
    ITEX_CHECK(GetKind() == TokKind::kPrimitiveType);
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_HLO_LEXER_H_
