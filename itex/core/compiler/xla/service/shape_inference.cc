...
/* static */ StatusOr<Shape> ShapeInference::InferUnaryOpShape(
  ...
  ITEX_DCHECK_OK(ShapeUtil::ValidateShapeWithOptionalLayout(shape));
  switch (opcode) {
...
/* static */ StatusOr<Shape> ShapeInference::InferDotOpShape(
  ...
  ITEX_VLOG(2) << "inferred dot shape: " << ShapeUtil::HumanString(result);
  return result;
}
...
/* static */ StatusOr<Shape> ShapeInference::InferBinaryOpShape(
  ...
  ITEX_DCHECK_OK(ShapeUtil::ValidateShapeWithOptionalLayout(lhs));
  ITEX_DCHECK_OK(ShapeUtil::ValidateShapeWithOptionalLayout(rhs));
...
/* static */ StatusOr<Shape> ShapeInference::InferTernaryOpShape(
  ...
  ITEX_DCHECK_OK(ShapeUtil::ValidateShapeWithOptionalLayout(lhs));
  ITEX_DCHECK_OK(ShapeUtil::ValidateShapeWithOptionalLayout(rhs));
  ITEX_DCHECK_OK(ShapeUtil::ValidateShapeWithOptionalLayout(ehs));
  switch (opcode) {
...
#define RET_CHECK_RANK(x)                            \
  ...
  switch (fft_type) {
    ...
    default:
      ITEX_LOG(FATAL) << "Unexpected fft_type: " << fft_type;
  }
...
/* static */ StatusOr<Shape> ShapeInference::InferReduceWindowShape(
  ...
  if (ShapeUtil::IsScalar(to_apply_shape.result())) {
    ITEX_CHECK_EQ(output_shape_vec.size(), 1);
...
/* static */ StatusOr<Shape> ShapeInference::InferSliceShape(
  ...
  ITEX_VLOG(2) << StrFormat("slicing shape %s starts={%s} limits={%s}",
  ...
  for (int64_t dimension = 0; dimension < starts_size; ++dimension) {
    ...
    ITEX_VLOG(2) << StrFormat("starts[%d] = %d", dimension, start_index);
    ITEX_VLOG(2) << StrFormat("limits[%d] = %d", dimension, limit_index);
...
/* static */ StatusOr<Shape> ShapeInference::InferDynamicSliceShape(
  ...
  if (!allow_scalar_indices ||
    ...
    ITEX_VLOG(2) << StrFormat("slicing shape %s a with slice_sizes={%s}",
                              ShapeUtil::HumanString(operand_shape),
                              StrJoin(slice_sizes, ", "));
  ...
  for (int64_t dim = 0; dim < slice_sizes.size(); ++dim) {
    ...
    ITEX_VLOG(2) << StrFormat("slice_sizes[%d] = %d", dim, slice_dim_size);
  }
...
/* static */ StatusOr<Shape> ShapeInference::InferDynamicUpdateSliceShape(
  ...
  if (!allow_scalar_indices ||
    ...
    ITEX_VLOG(2) << StrFormat("updating slice of shape %s with update shape %s",
                              ShapeUtil::HumanString(operand_shape),
                              ShapeUtil::HumanString(update_shape));
  ...
  for (int64_t dim = 0; dim < operand_shape.rank(); ++dim) {
    ...
    ITEX_VLOG(2) << StrFormat("update_sizes[%d] = %d", dim, update_dim_size);
  }
...
/* static */ StatusOr<Shape> ShapeInference::InferReshapeShape(
  ...
  ITEX_VLOG(3) << "Reshape inferred shape: "
               << ShapeUtil::HumanString(inferred_shape);
  ...
  for (int64_t input_dim = 0; input_dim < operand.rank(); ++input_dim) {
    ...
    if (output_dynamic_dimension != -1) {
      // TODO(yunxing): Turn this into a ITEX_CHECK.
