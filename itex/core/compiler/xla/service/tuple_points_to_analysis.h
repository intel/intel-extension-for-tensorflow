...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_TUPLE_POINTS_TO_ANALYSIS_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_TUPLE_POINTS_TO_ANALYSIS_H_
...
#include "itex/core/compiler/xla/service/logical_buffer_analysis.h"
...
class PointsToSet {
 public:
  ...
  // PointsToSet.
  using BufferSet = itex::gtl::CompactPointerSet<const LogicalBuffer*>;
  ...
  using SourceSet = itex::gtl::CompactPointerSet<HloInstruction*>;
 ...
 private:
  ...
  const PerInstruction* PerInst(const HloInstruction* inst) const {
    ...
    if (iter == per_instruction_.end()) {
      ITEX_LOG(FATAL)
          << "Expected per-instruction information to already exist";
    } else {
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_TUPLE_POINTS_TO_ANALYSIS_H_
