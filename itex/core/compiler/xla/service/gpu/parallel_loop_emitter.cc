...
#include "itex/core/compiler/xla/service/llvm_ir/kernel_support_library.h"
...
ParallelLoopEmitter::EmitLinearBaseAndThreadIdx(llvm::Type* index_type,
  ...
  if (launch_dimensions_.thread_counts_per_block().y > 1) {
    ITEX_CHECK(!launch_config_.logical_order);
...
ParallelLoopEmitter::EmitIndexAndSetExitBasicBlock(absl::string_view loop_name,
  ...
  if (launch_dimensions_.thread_counts_per_block().y > 1) {
    ...
    // efficient.
    ITEX_CHECK(launch_config_.row_vectorized);
    ITEX_CHECK_EQ(shape_.dimensions().back(),
                  launch_dimensions_.thread_counts_per_block().x *
                      launch_config_.unroll_factor);
  }
  ITEX_CHECK_EQ(launch_dimensions_.thread_counts_per_block().z, 1);
  ITEX_CHECK_EQ(launch_dimensions_.block_counts().y, 1);
  ITEX_CHECK_EQ(launch_dimensions_.block_counts().z, 1);
  ITEX_VLOG(3) << "EmitIndexAndSetExitBasicBlock unroll_factor "
               << launch_config_.unroll_factor;
  ITEX_CHECK_NE(index_type, nullptr);
  ...
  ITEX_CHECK_NE(nullptr, exit_bb_);
...
Status ParallelLoopEmitter::EmitLoop(absl::string_view loop_name,
  ...
  if (total_threads * launch_config_.unroll_factor >= num_elements) {
    ITEX_VLOG(1) << "No loops inside the kernel";
