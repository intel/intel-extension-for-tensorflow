...
MatchBackwardFilter(HloInstruction* conv) {
  ITEX_VLOG(2) << "Trying to match convolution backward filter.";
  ...
  //              Convolution
  //                 conv
  ITEX_CHECK_EQ(HloOpcode::kConvolution, conv->opcode());
  ...
  for (const WindowDimension& window_dim : conv->window().dimensions()) {
    if (window_dim.stride() != 1) {
      ITEX_VLOG(1) << "Forward convolution's window "
    ...
    if (window_dim.padding_low() < 0) {
      ITEX_VLOG(1) << "Padding low should be non-negative.";
      return no_match_result;
    }
    if (window_dim.window_reversal()) {
      ITEX_VLOG(1) << "Window reversal field not supported";
      return no_match_result;
    }
  ...
  for (int i = 0; i < input_spatial_dims.size(); ++i) {
    ...
    ITEX_CHECK_GE(dim->padding_low(), 0);
...
MatchBackwardInput(HloInstruction* conv) {
  ITEX_VLOG(2) << "Trying to match convolution backward input.";
  const auto no_match_result =
      std::make_tuple(false, Window(), ConvolutionDimensionNumbers(), nullptr);
  ...
  if (!is_reversed_filter &&
    ...
    ITEX_VLOG(1)
        << "Can't match to backwards convolution. Either filter is not "
  ...
  ITEX_CHECK_EQ(conv->window().dimensions().size(), input_spatial_dims.size());
  ITEX_CHECK_EQ(output_spatial_dims.size(), input_spatial_dims.size());
  ...
  for (size_t i = 0; i < input_spatial_dims.size(); ++i) {
    ...
    if (dim->padding_high() < 0) {
      ITEX_LOG(WARNING)
          << "Fusing this pattern to backward convolution would cause "
             "negative padding ("
          << dim->padding_high()
  ...
  if (reverse_filter->opcode() != HloOpcode::kReverse &&
    ...
    reverse_filter = c->AddInstruction(
        HloInstruction::CreateReverse(reverse_filter->shape(), reverse_filter,
                                      dnums.kernel_spatial_dimensions()));
    ITEX_CHECK_OK(conv->ReplaceOperandWith(/*operand_num=*/1, reverse_filter));
  }
  ...
  ITEX_CHECK_EQ(input_features % num_groups, 0)
...
HloInstruction* ConvertBatchGroupedToFeatureGroupedConvolution(
    HloInstruction* conv) {
  ITEX_CHECK_EQ(conv->feature_group_count(), 1);
...
StatusOr<bool> RunOnInstruction(HloInstruction* conv) {
  ITEX_CHECK_EQ(conv->opcode(), HloOpcode::kConvolution);
  ...
  ITEX_VLOG(1) << "Replacing convolution " << conv->ToString() << " with "
