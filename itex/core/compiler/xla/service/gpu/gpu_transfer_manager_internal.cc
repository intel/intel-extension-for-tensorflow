...
namespace {
int64_t GetSizeOfShape(const itex_xla::Shape& shape, int pointer_size) {
  if (shape.is_static() || shape.IsTuple()) {
    return itex_xla::ShapeUtil::ByteSizeOf(shape, pointer_size);
  }
  // Each dynamic dimension size is represented as a S32.
  int64_t metadata_size = sizeof(int32_t) * shape.dimensions_size();
  return itex_xla::ShapeUtil::ByteSizeOf(shape, pointer_size) + metadata_size;
}
...
Status GpuTransferManager::TransferLiteralToInfeed(
    stream_executor::StreamExecutor* executor, const LiteralSlice& literal) {
...
Status GpuTransferManager::ReadDynamicShapes(se::Stream* stream,
                                             ...
                                             Shape* device_shape) {
  ITEX_DCHECK(device_shape->is_dynamic());
  Shape original_device_shape = *device_shape;
  std::string target_triple(spir::TargetTriple());
  std::string data_layout(spir::DataLayout());

  int pointer_size = llvm::DataLayout(data_layout)
  ...
  TF_RETURN_IF_ERROR(device_buffer->buffers().ForEachMutableElementWithStatus(
      [&](const ShapeIndex& index, se::DeviceMemoryBase* buffer) {
        ...
        const int64_t offset =
            GetSizeOfShape(buffer_shape_static, pointer_size);
        int64_t metadata_size =
            GetSizeOfShape(buffer_shape, pointer_size) - offset;
        if (metadata_size == 0) {
  ...
  auto cleanup = itex::gtl::MakeCleanup([&] {
  ...
  TF_RET_CHECK(itex_xla::ShapeUtil::DynamicShapeIsCompatible(
      *device_shape, original_device_shape));
  return Status::OK();
}
