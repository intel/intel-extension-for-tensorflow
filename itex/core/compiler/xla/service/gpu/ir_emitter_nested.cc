    ...
    if (ShapeUtil::IsScalar(return_shape)) {
      llvm::Value* ret_value =
          Load(llvm_ir::ShapeToIrType(return_shape, module_), root_value,
               "load_ret_value");
      Store(ret_value,
            PointerBitCastOrAddrSpaceCast(out_parameter, root_value->getType(),
                                          "bitcast_ret_value"));
    } else {
      ...
      llvm::Value* tuple_ptr =
          PointerBitCastOrAddrSpaceCast(out_parameter, tuple_type_ptr);
...
Status IrEmitterNested::EmitConstants(const HloComputation& computation) {
  for (HloInstruction* instr : computation.instructions()) {
    ...
    if (should_emit_initializer) {
      ITEX_VLOG(3) << "Emitted initializer for constant with shape "
