...
#include "itex/core/compiler/xla/service/gpu/kernel_mapping_scheme.h"
#include "itex/core/compiler/xla/service/gpu/kernel_thunk.h"
#include "itex/core/compiler/xla/service/gpu/launch_dimensions.h"
#include "itex/core/compiler/xla/service/gpu/memset_thunk.h"
...
#include "itex/core/compiler/xla/service/gpu/nccl_collective_permute_thunk.h"
#include "itex/core/compiler/xla/service/gpu/outfeed_thunk.h"
#include "itex/core/compiler/xla/service/gpu/parallel_loop_emitter.h"
#include "itex/core/compiler/xla/service/gpu/replica_id_thunk.h"
...
#include "itex/core/compiler/xla/service/gpu/while_thunk.h"
...
#include "itex/core/compiler/xla/service/llvm_ir/dynamic_update_slice_util.h"
#include "itex/core/compiler/xla/service/llvm_ir/fused_ir_emitter.h"
#include "itex/core/compiler/xla/service/llvm_ir/ir_array.h"
...
#include "itex/core/compiler/xla/service/llvm_ir/sort_util.h"
#include "itex/core/compiler/xla/service/llvm_ir/tuple_ops.h"
...
#include "itex/core/compiler/xla/union_find.h"
#include "itex/core/compiler/xla/util.h"
#include "itex/core/compiler/xla/window_util.h"
#include "itex/core/utils/errors.h"
#include "itex/core/utils/human_readable_json.h"
...
bool MhloOpIsElementwise(mlir::Operation* op) {
  ...
  if (opcode == HloOpcode::kMap) {
    ...
    for (const llvm::APInt& i :
         mlir::cast<mlir::mhlo::MapOp>(op).getDimensions()) {
...
StatusOr<Shape> GetConsistentInputShapeForRootSlices(
  ...
  ITEX_CHECK_EQ(root.opcode(), HloOpcode::kTuple);
...
StatusOr<BufferAllocation::Slice> IrEmitterUnnested::GetAllocationSlice(
  ...
  return itex_xla::gpu::GetAllocationSlice(
...
Status IrEmitterUnnested::EmitConstant(mlir::Operation* op) {
  ...
  auto global = mlir::cast<mlir::memref::GlobalOp>(
      module.lookupSymbol(get_global.getNameAttr()));
  ...
  if (should_emit_initializer) {
    ...
    // Initialize info.content, so device memory will be allocated by xla.
    // zeModuleGetGlobalPointer doesn't work.
    TF_RETURN_IF_ERROR(
...
void IrEmitterUnnested::CreateStore(llvm::Value* data, llvm::Value* address,
  ...
  int data_bytes = data->getType()->getPrimitiveSizeInBits() /
                   primitive_util::BitWidth(PrimitiveType::U8);
  ITEX_CHECK_GE(data_bytes, alignment_bytes);
  if (alignment_bytes == 0) {
    b_.CreateStore(data,
...
Status IrEmitterUnnested::EmitConvolutionThunk(mlir::Operation* op) {
  ...
  if (IsBefThunkEnabled(hlo_module_config_)) {
    ITEX_LOG(FATAL) << "BEF thunk is not supported";
  }
  ...
  auto set_activation_mode = [&](auto op) -> Status {
    descriptor.activation = op.getActivationMode();
    return Status::OK();
  };
...
Status IrEmitterUnnested::EmitGemmThunk(mlir::Operation* op) {
  ...
  auto make_gemm_thunk =
      [&](auto op, absl::optional<double> gemm_bias_beta =
                       absl::nullopt) -> StatusOr<std::unique_ptr<Thunk>> {
    TF_ASSIGN_OR_RETURN(auto lhs, GetAllocationSlice(op.getA()));
    TF_ASSIGN_OR_RETURN(auto rhs, GetAllocationSlice(op.getB()));
    TF_ASSIGN_OR_RETURN(auto output, GetAllocationSlice(op.getC()));
  ...
  TF_ASSIGN_OR_RETURN(auto thunk, [&]() -> StatusOr<std::unique_ptr<Thunk>> {
    ...
    //   TF_ASSIGN_OR_RETURN(auto bias, GetAllocationSlice(gemm.getC()));
    //   // c = alpha * (a x b) + beta * c
    //   TF_ASSIGN_OR_RETURN(auto output, GetAllocationSlice(gemm.getC()));
    ...
    //       /*mem_size=*/
    //       ShapeUtil::ByteSizeOf(GetShape(gemm.getC()))));
    ...
    return itex::errors::Internal("Unexpected op.");
  }());
...
std::pair<bool, int> RowVectorizationEnabled(mlir::lmhlo::FusionOp fusion) {
  ...
  for (mlir::Operation& op : fusion.getRegion().front()) {
    ...
    ITEX_VLOG(2) << "Row vectorization not enabled due to this op: "
                 << MlirToString(&op);
...
Status IrEmitterUnnested::EmitCustomCallThunk(mlir::Operation* op) {
  ...
  if (custom_call.getTargetArgMapping()) {
    ...
    TF_ASSIGN_OR_RETURN(results,
                        values_to_slices_with_token_holes(
  ...
  if (IsBefThunkEnabled(hlo_module_config_)) {
    ...
    */
    ...
    switch (custom_call.getApiVersion()) {
      ...
      //     LOG(FATAL) << "Typed FFI custom call must be called by XLA
      //     runtime";
      //   };
      //   break;
      default:
...
Status IrEmitterUnnested::EmitTriangularSolveCustomCall(mlir::Operation* op) {
  ...
  if (auto str = custom_call.getBackendConfig()
                     ...
                     .dyn_cast_or_null<mlir::StringAttr>())
    TF_RETURN_IF_ERROR(
        itex::HumanReadableJsonToProto(str.str(), &backend_config));
...
Status IrEmitterUnnested::EmitLoopFusion(mlir::Operation* op) {
  ...
  bool few_waves = [fusion, row_vectorized, num_big_inputs]() mutable {
    for (mlir::Operation& op : fusion.getRegion().front()) {
      ...
      ITEX_VLOG(2) << "few_waves not enabled due to: " << MlirToString(&op);
      return false;
    }
    return true;
  }();
  ...
  if (launch_config.row_vectorized &&
      ThreadsPerBlockRowVectorized(element_shape,
                                   ...
                                   launch_config) <= 0) {
    ITEX_VLOG(2)
        << "Cancelling row_vectorization as the shape isn't supported.";
...
Status IrEmitterUnnested::EmitFusion(mlir::Operation* op) {
  ...
  if (ir_emitter_context_->mlir_pass_success() && EnableGpuMlirLowering()) {
    std::vector<llvm_ir::IrArray> ir_arrays;
    TF_ASSIGN_OR_RETURN(auto kernel_thunk,
                        BuildKernelThunk(fusion_op, Thunk::ThunkInfo(),
                                         &ir_arrays, LaunchDimensions()));
    KernelThunk* kernel = dynamic_cast<KernelThunk*>(kernel_thunk.get());
    kernel->set_kernel_name(std::move(std::string("spv_dummy_kernel")));
  ...
  if (fusion_results.size() > 1) {
    ...
    if (IsInputFusibleSlices(op, /*verify_no_strides=*/true)) {
      ...
      if (status.code() == TF_FAILED_PRECONDITION) {
  ...
  if (!is_single_instruction &&
    ...
    ITEX_CHECK_EQ(1, GetHloOutputs(op).size());
  ...
  if (auto copy = mlir::dyn_cast<mlir::mhlo::CopyOp>(fusion_root)) {
    if (IsSingleInstructionFusion(fusion_op)) {
      ...
      ITEX_CHECK(ShapeUtil::Compatible(operand_shape, output_shape));
...
Status IrEmitterUnnested::EmitExtraOutputsForReduce(
  ...
  for (const auto& p : extra_output_ir_values) {
    ...
    ITEX_CHECK_EQ(result_ir.size(), 1);
...
Status IrEmitterUnnested::EmitSelectAndScatter(mlir::Operation* op) {
  ...
  ITEX_CHECK_EQ(rank, source_shape.rank());
  if (select_and_scatter_op.getWindowDimensions()) {
    ITEX_CHECK_EQ(rank, select_and_scatter_op.getWindowDimensions()->size());
  }
  ...
  ITEX_CHECK_EQ(ir_arrays.size(), 3);
  ...
  auto loop_body_emitter = [&](const IrArray::Index& source_index) -> Status {
    ...
    for (const auto& dim : window_dimensions) {
      ...
      ITEX_CHECK_GT(dim.getSExtValue(), 0);
    }
...
Status IrEmitterUnnested::EmitLaunchFunc(mlir::Operation* op) {
  ...
  std::string kernel_name = launch_func.getKernelName().str();

  std::vector<BufferSlice> slices;
  std::vector<const BufferAllocation*> buffers_needed;
  for (auto operand : launch_func.getKernelOperands()) {
    BufferSlice slice;
    TF_ASSIGN_OR_RETURN(auto buffer_slice,
                        GetAllocationSlice(operand, &slice.constant_name));
    buffers_needed.emplace_back(buffer_slice.allocation());
  }
  absl::Span<const BufferAllocation* const> buffer_span(
      const_cast<const BufferAllocation* const*>(buffers_needed.data()),
      buffers_needed.size());

  kernel_thunk = std::make_unique<KernelThunk>(GetThunkInfo(op), buffer_span,
...
Status IrEmitterUnnested::EmitScatter(
  ...
  ITEX_CHECK(ShapeUtil::Equal(GetShape(scatter.getOutput()), operand_shape));
...
Status IrEmitterUnnested::EmitScatter(
  ...
  auto loop_body_emitter = [&](const IrArray::Index& index) -> Status {
    ...
    ITEX_DCHECK_EQ(raw_window_multidim.size(),
    ...
    ITEX_DCHECK_EQ(input_window_multidim.size(), desc.operand_shape.rank());
...
IrEmitterUnnested::GetOrCreateSubComputationFromRegion(mlir::Region* region,
  ...
  if (module == nullptr) {
    ...
    itex_xla::XlaComputation xla_computation;
    ...
    if (is_fusion) {
      ...
      ITEX_CHECK_EQ(operand_shapes.size(), fused_computation->num_parameters());
      for (int i = 0; i < fused_computation->num_parameters(); i++) {
        *fused_computation->parameter_instruction(i)
             ->mutable_shape()
      ...
      if (root->opcode() == HloOpcode::kTuple) {
        [&] {
          ...
          for (auto instr : to_be_removed) {
            ITEX_CHECK_OK(fused_computation->RemoveInstruction(instr));
          }
      ...
      if (output_shapes.size() > 1) {
        ITEX_CHECK(root->shape().IsTuple());
        ITEX_CHECK_EQ(root->shape().tuple_shapes_size(), output_shapes.size());

        for (int i = 0; i < output_shapes.size(); i++) {
          *root->mutable_shape()->mutable_tuple_shapes(i) = output_shapes.at(i);
        }
      } else {
        ITEX_CHECK_EQ(1, output_shapes.size());
...
Status IrEmitterUnnested::EmitSort(mlir::Operation* op) {
  ...
  for (int64_t i = 0; i < operands.size(); ++i) {
    ...
    if (destination_buffer != source_address) {
      ...
      // key/value sort.
      ITEX_VLOG(2) << op_name << " requires initial D2D copy for operand " << i;
      thunks.push_back(absl::make_unique<DeviceToDeviceCopyThunk>(
          Thunk::ThunkInfo(),
  ...
  ITEX_VLOG(2) << op_name << " requires " << num_stages << " stages.";
  ITEX_CHECK_GE(1ULL << num_stages, dimension_to_sort_bound);
  ITEX_CHECK_LT(1ULL << (num_stages - 1), dimension_to_sort_bound);
  ...
  ITEX_VLOG(2) << absl::StreamFormat(
      "%s launch dims: %d blocks, %d threads/block", op_name, num_blocks,
      kThreadsPerBlock);
...
Status IrEmitterUnnested::EmitReplicaOrPartitionId(mlir::Operation* op) {
  ...
  if (IsBefThunkEnabled(hlo_module_config_)) {
    ...
    TF_ASSIGN_OR_RETURN(thunk,
                        CreateBefThunk(GetThunkInfo(op), op,
                        {result_slice}));
    */
  } else {
  ...
  if (NcclCollectivePermuteThunk::IsDegenerate(
    ...
    if (IsBefThunkEnabled(hlo_module_config_)) {
      ...
      TF_ASSIGN_OR_RETURN(thunk, CreateBefCollectiveThunk(
                                     GetThunkInfo(op), op,
                                     std::move(buffers), replica_count,
                                     partition_count));
      */
    } else {
...
Status IrEmitterUnnested::EmitNcclThunk(mlir::Operation* untyped_op) {
  ...
  ITEX_VLOG(2) << NcclThunkType::GetName()
  ...
  if (should_use_nccl_thunk) {
    ...
    if (IsBefThunkEnabled(hlo_module_config_) &&
      ...
      TF_ASSIGN_OR_RETURN(
          thunk,
          CreateBefCollectiveThunk(GetThunkInfo(op), op,
          std::move(arg_buffers),
  ...
  ITEX_VLOG(1) << "Collective call is degenerate, not doing NCCL call";
...
Status IrEmitterUnnested::EmitOutfeed(mlir::Operation* op) {
  mlir::Operation::operand_range operands =
      mlir::cast<mlir::lmhlo::OutfeedOp>(op).getOperands();
  std::unique_ptr<Thunk> thunk;
  if (IsBefThunkEnabled(hlo_module_config_)) {
    // TF_ASSIGN_OR_RETURN(auto slices, GetSlices(operands));
    // TF_ASSIGN_OR_RETURN(
  ...
  for (const BufferSlice& slice : slices) {
    ...
    if (!slice.constant_name.empty()) {
      loc = ir_emitter_context_->llvm_module()->getGlobalVariable(
          slice.constant_name);
      ITEX_CHECK_NE(loc, nullptr)
          << "Could not find variable '" << slice.constant_name << "'";
    } else {
      ITEX_CHECK(!buffer_slice.allocation()->is_constant());
      loc =
...
static IrArray::Index GetUnnormalizedIndex(
  ...
  ITEX_DCHECK_EQ(normalized_shape_index.size(), 3);
  ...
  if (unnormalized_shape.rank() == 2 && unnormalized_shape.has_layout() &&
    ...
    ITEX_CHECK_EQ(normalized_shape_index.dims()[0], 1);
...
void IrEmitterUnnested::EmitTileElementForFusion(
  ...
  if (output_arrays.size() > 1) {
    ITEX_DCHECK(output_value->getType()->isStructTy());
    ITEX_DCHECK_EQ(output_value->getType()->getStructNumElements(),
                   output_arrays.size());
...
ReductionCodegenState IrEmitterUnnested::GenerateReductionCodegenState(
  ...
  ITEX_VLOG(10) << "Emit prologue for reduction: " << MlirToString(fusion);
    ...
    for (int op_result_idx = 0;
      ...
      llvm::GlobalVariable* shared_cache = [&] {
        if (reduction_codegen_state.IsRowReduction()) {
          ...
          ITEX_CHECK_EQ(tiling_scheme.GetNumThreadsPerBlock() % WarpSize(), 0);
          ...
          ITEX_CHECK_EQ(num_threads_x, tiling_scheme.GetNumThreadsFor(kDimY));
...
void IrEmitterUnnested::EmitFullWarpShuffleDownLoopForReduce(
  ...
  ITEX_CHECK_EQ(threads_per_block % 32, 0);
  ...
  for (int distance = 16; distance >= 1; distance /= 2) {
    ...
    ITEX_CHECK_OK(returned_scalars.status());
...
void IrEmitterUnnested::EmitPrintfWithThreadId(
  ...
  KernelSupportLibrary ksl(&b_, llvm_ir::UnrollMode::kDefaultUnroll);
  ksl.If(constraint, [&] {
    itex_xla::gpu::EmitPrintf(absl::StrCat("[TID=%d,BID=%d] ", fmt, "\n"),
    ...
    ksl.If("reduction_write_output", is_zero(thread_id_info.thread_id_x), [&] {
      for (int oidx = 0; oidx < num_outputs; oidx++) {
        ...
        if (reduction_codegen_state.IsRaceFree()) {
          ...
          ITEX_CHECK_EQ(num_outputs, 1);
          ITEX_CHECK_OK(EmitAtomicOperationForNestedComputation(
...
void IrEmitterUnnested::EmitHlo021Tile(
  ...
  for (int64_t id = 0; id < fusion.getInputBuffers().size(); id++) {
    ...
    if (absl::c_linear_search(tiled_param_ids, id)) {
      ...
      ITEX_VLOG(3) << "Added shmem buffer for parameter " << id << ": "
...
StatusOr<bool> IrEmitterUnnested::CheckAndEmitHloWithTile021(
  ...
  optional<Vector3> reduced_dims_021;
  ...
  ITEX_CHECK_EQ(WarpSize() % kNumRows, 0);
  ...
  return absl::c_all_of(value.getUsers(), [&](mlir::OpOperand use) {
    ...
    return use_chain_endings.count(user) ||
           (HloInstruction::IsOpElementwise(*MhloToHloOpcode(user)) &&
            absl::c_all_of(
                user->getResults(), [&](const mlir::OpResult result) {
                  return AreUsersElementwise(result, use_chain_endings);
                }));
  });
}
...
llvm::GlobalVariable* IrEmitterUnnested::AllocateShared(
    ...
    absl::string_view buffer_name) {
  ITEX_CHECK(!dimensions_major_to_minor.empty());
...
StatusOr<ReductionCodegenInfo> IrEmitterUnnested::ComputeReductionCodegenInfo(
  ...
  ITEX_VLOG(10) << "is_row_reduction " << reduction_dimensions.is_row_reduction
  ...
  TilingScheme::IndexingOrder indexing_order =
      reduction_dimensions.is_row_reduction ? kStridedIndexingX
                                            : kLinearIndexingX;
  bool vectorize =
      CanVectorizeReduction(fusion, reduction_dimensions, num_threads_x,
  ...
  ITEX_VLOG(3) << "Each threads will produce " << num_partial_results
               << " output(s)";
  ...
  ITEX_VLOG(2) << "Using virtual thread scaling: "
...
void IrEmitterUnnested::GenerateElementForReducer(
  ...
  ITEX_CHECK_EQ(reducer->num_parameters() % 2, 0);
...
Status IrEmitterUnnested::EmitIRForReduction(
  ...
  EmitElementFunction emit_reduction_element =
      [&](const ThreadIdInfo& thread_id_info,
        ...
        ITEX_CHECK_OK(EmitExtraOutputsForReduce(
...
static void GetFusionRootsRec(HloInstruction* root,
  ...
  if (root->opcode() == HloOpcode::kGetTupleElement) {
    ...
    ITEX_CHECK(!absl::c_linear_search(out, root))
...
// with the same root. ITEX_CHECKs that the fusion never outputs the same
// instruction twice, as well as that there are no explicitly created tuples or
// nested gtes in fusion output.
//
...
std::vector<std::vector<HloInstruction*>> GroupDisjointReductions(
  ...
  int num_fusion_outputs =
      fused_computation->root_instruction()->opcode() == HloOpcode::kTuple
          ...
          : 1;
  ITEX_CHECK_NE(0, num_fusion_outputs);
  ...
  HloInstructionMap<itex::UnionFind<HloInstruction*>> disjoint_sets;
...
Status IrEmitterUnnested::EmitUnnestedReduction(mlir::lmhlo::FusionOp fusion) {
  ...
  ITEX_CHECK(first_reduce) << MlirToString(fusion);
  ...
  ITEX_CHECK(input_shape.has_layout())
      << "LayoutAssignment or InstructionFusion "
  ...
  ITEX_VLOG(2) << StrCat("Generate in ", instr_index_groups.size(),
                         " groups for ", MlirToString(fusion));
  ...
  ITEX_VLOG(3) << "Launch dimensions of "
               << mlir::GetNameFromLoc(fusion.getLoc())
               << ": number of blocks: "
               << tiling_scheme.GetNumberOfBlocksPhysical()
               << " - threads per block: "
               << tiling_scheme.GetNumThreadsPerBlockPhysical();
  ...
  GpuElementalIrEmitter elemental_emitter(hlo_module_config_,
                                          ...
                                          &b_, GetNestedComputer());
  FusedIrEmitter fused_emitter(elemental_emitter);
  ITEX_CHECK_LT(fused_computation->num_parameters(), ir_arrays.size());
  for (int i = 0; i < fused_computation->num_parameters(); i++) {
...
Status IrEmitterUnnested::EmitElementForInputFusibleSlices(
    ...
    const llvm_ir::IrArray::Index& index) {
  ITEX_VLOG(10) << "Emitting slice input fusion for "
  ...
  auto slice_instructions = [&]() -> absl::Span<HloInstruction* const> {
    ...
    ITEX_CHECK_EQ(slice_or_tuple->opcode(), HloOpcode::kTuple);
  ...
  for (int64_t i = 0; i < slice_instructions.size(); ++i) {
    ...
    for (size_t dim = 0; dim < slice->slice_starts().size(); ++dim) {
      ITEX_CHECK_EQ(slice->slice_strides(dim), 1);
...
Status IrEmitterUnnested::EmitOp(mlir::Operation* op) {
  if (mlir::isa<mlir::func::ConstantOp, mlir::arith::ConstantOp,
                ...
                mlir::bufferization::ToMemrefOp, mlir::tensor::PadOp,
                mlir::memref::AllocOp, mlir::memref::CollapseShapeOp,
                mlir::memref::CopyOp, mlir::memref::ViewOp,
                mlir::memref::ReinterpretCastOp, mlir::func::ReturnOp,
                mlir::memref::SubViewOp, mlir::lmhlo::TerminatorOp>(op)) {
    return Status::OK();
  }
...
#else
    ITEX_LOG(FATAL) << "MKL is not enabled for Cholesky";
#endif  // ITEX_USE_MKL
  }
...
#else
    ITEX_LOG(FATAL) << "MKL is not enabled for FFT";
#endif  // ITEX_USE_MKL
  }
