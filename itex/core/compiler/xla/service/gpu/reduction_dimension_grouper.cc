...
class ReduceDimensionGroupVisitor : public DfsHloRewriteVisitor {
 public:
  Status HandleReduce(HloInstruction* hlo) override {
    ...
    ITEX_VLOG(4) << "Input: " << reduce->ToString();
    ...
    for (HloInstruction* operand : reduce->inputs()) {
      ...
      ITEX_CHECK(shape == LayoutUtil::GetWithDefaultLayout(shape))
      ...
      for (int logical_dim = 0; logical_dim < shape.rank(); logical_dim++) {
        ITEX_VLOG(5) << "Processing dimension " << logical_dim << " of size "
                     << shape.dimensions(logical_dim);
        if (is_reduced(logical_dim) && logical_dim < shape.rank() - 1 &&
            is_reduced(logical_dim + 1)) {
          ITEX_VLOG(5) << "This and consecutive dimension are reduced, merging";
          changed = true;
      ...
      ITEX_VLOG(5) << "Adding bitcast: "
                   << reduce_inputs_grouped.back()->ToString();
    }
