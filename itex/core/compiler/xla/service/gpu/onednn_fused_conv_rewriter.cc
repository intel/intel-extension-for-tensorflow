...
void VlogIfFailureToMatch(HloInstruction* instr, const LogPattern& log_pattern,
  ...
  if (!ITEX_VLOG_IS_ON(3) || !Match(instr, filter_pattern)) {
    return;
  }
  std::stringstream os;
  if (!Match(instr, log_pattern, {/*capture=*/false, /*explain_os=*/&os})) {
    ITEX_VLOG(3) << "Failed to match " << desc << ":\n" << os.str();
  }
}
...
StatusOr<HloInstruction*> EnsureIsConvBiasActivation(HloInstruction* conv) {
  ITEX_CHECK_EQ(conv->opcode(), HloOpcode::kCustomCall);
...
StatusOr<bool> FuseBiasOrSideInput(HloComputation* comp) {
  ...
  for (auto instr : comp->MakeInstructionPostOrder()) {
    ...
    TF_ASSIGN_OR_RETURN(auto config,
                        conv->backend_config<CudnnConvBackendConfig>());
    if (config.activation_mode() != ActivationMode::kNone) {
      continue;
    }
  ...
  for (HloInstruction* instr : comp->MakeInstructionPostOrder()) {
    ...
    std::function<HloInstruction*(const HloInstruction*)> clone =
        [&](const HloInstruction* instr) {
          ...
          ITEX_CHECK(instr->opcode() == HloOpcode::kReshape ||
                     instr->opcode() == HloOpcode::kTranspose)
...
StatusOr<bool> FuseRelu(HloComputation* comp) {
  ...
  for (HloInstruction* instr : comp->MakeInstructionPostOrder()) {
    ...
    TF_ASSIGN_OR_RETURN(conv, EnsureIsConvBiasActivation(conv));
    config.set_activation_mode(ActivationMode::kRelu);
    TF_RETURN_IF_ERROR(conv->set_backend_config(config));
...
StatusOr<bool> FuseConvertToF16(HloComputation* comp) {
  ...
  for (HloInstruction* instr : comp->MakeInstructionPostOrder()) {
    ...
    ITEX_VLOG(2) << "Matched fp16 conv: " << conv->ToString();
...
void VlogStats(HloModule* module) {
  ...
  ITEX_VLOG(1) << "Results of CudnnFusedConvRewriter for " << module->name();
    ...
    for (HloInstruction* instr : comp->instructions()) {
      ...
      if (conv_in_ty == F32) {
        ...
        if (conv_out_ty == S8) {
          ...
          ITEX_LOG(ERROR) << "Unexpected conv: " << instr->ToString();
        }
      }
      ...
      if (!config.ok()) {
        ITEX_LOG(ERROR) << "Couldn't parse backend config for "
                        << instr->ToString();
        continue;
      }
      ...
      stats[absl::StrCat("32 convs with activation mode ",
  ...
  for (const auto& kv : stats_sorted) {
    ITEX_VLOG(1) << absl::StreamFormat("%4d %s", kv.second,
