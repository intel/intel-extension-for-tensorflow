 ...
 public:
  ...
  Status HandleReduce(HloInstruction* hlo) override {
    if (IsMinMaxReduction(hlo)) {
      ...
      ITEX_VLOG(1) << "Not performing tree expansion on min/max-reduction: "
                   << hlo->ToString()
 ...
 private:
  ...
  Status RewriteReduction(HloInstruction* hlo) {
    ...
    if (ReductionIsRaceFree(reduction_dimensions, reduction_tiling)) {
      ITEX_VLOG(3) << "Base case: dimensions fit";
      return Status::OK();
    }

    ITEX_VLOG(1) << "Input: " << hlo->ToString();
    int64_t reduced_dim_size = input_shape_dims[reduced_input_dimension];
    ITEX_VLOG(3) << "reduced_dim_size = " << reduced_dim_size;
    ...
    auto padded = [&]() -> InstructionVector {
      ...
      for (int i = 0; i < reduce->input_count(); i++) {
        ...
        ITEX_VLOG(3) << "Generated padded shape: " << padded_shape.ToString();
    ...
    ITEX_VLOG(2) << "Generated padding: " << padded[0]->ToString();
    ...
    int64_t inner_reduced_dimension = is_row_reduction
                                          ...
                                          : reduced_input_dimension;
    ITEX_VLOG(2) << "inner_reduced_dimension = " << inner_reduced_dimension;
    ...
    for (int i = 0; i < padded.size(); i++) {
      ...
      ITEX_VLOG(2) << "Generated reshape: "
                   << reshaped_padded_input->ToString();
    ...
    HloInstruction* inner_reduce =
        hlo->parent()->AddInstruction(HloInstruction::CreateReduce(
            ...
            hlo->to_apply()));
    ITEX_VLOG(1) << "Generated inner reduction: " << inner_reduce->ToString();
    absl::InlinedVector<int64_t, 3> outer_reduce_dimensions =
        inner_reduce_dimensions;
    ITEX_VLOG(3) << "outer_reduce_dimensions = "
    ...
    ITEX_VLOG(1) << "Generated outer reduction: " << outer_reduce->ToString();
  ...
  Status RewriteBatchDimensionLargerThanTile(
    ...
    // undoing each other.
    ITEX_CHECK(reduction_dimensions.is_row_reduction);
