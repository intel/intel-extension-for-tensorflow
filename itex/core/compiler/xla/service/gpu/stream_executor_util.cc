...
#include "itex/core/compiler/xla/stream_executor/kernel_spec.h"
...
StreamExecutorConvLayoutsToXlaLayouts(const ConvolutionDimensionNumbers& dnums,
  ...
  switch (input) {
    ...
    case DataLayout::kBatchYXDepth:  // NHWC
      ...
      break;
    default:
      return InternalError("Invalid input layout for conv with dnums %s",
  ...
  switch (filter) {
    ...
    case FilterLayout::kOutputYXInput:  // OHWI
      ...
      break;
    default:
      return InternalError("Invalid filter layout for conv with dnums %s",
                           ConvolutionDimensionNumbersToString(dnums));
  }
  ...
  switch (output) {
    ...
    case DataLayout::kBatchYXDepth:  // NHWC
      ...
      break;
    default:
      return InternalError("Invalid output layout for conv with dnums %s",
                           ConvolutionDimensionNumbersToString(dnums));
  }
...
XlaConvShapesToStreamExecutorLayouts(const ConvolutionDimensionNumbers& dnums,
                                     ...
                                     const Shape& output) {
  ITEX_CHECK(input.has_layout());
  ITEX_CHECK(filter.has_layout());
  ITEX_CHECK(output.has_layout());
...
static void InitializeTypedBuffer(se::Stream* stream,
  ...
  ITEX_CHECK_EQ(0, buffer.size() % sizeof(T));
  int64_t elements_left = buffer.size() / sizeof(T);
  while (elements_left > 0) {
    ITEX_CHECK_LE(host_index, host_buffer->size());
...
void InitializeBuffer(se::Stream* stream, PrimitiveType buffer_type,
  ...
  switch (buffer_type) {
    ...
    default:
      ITEX_LOG(FATAL) << "Unexpected type: "
...
bool RequireDeterminism(const HloModuleConfig& config) {
  static bool require_cudnn_determinism = [] {
    ...
    ITEX_CHECK_OK(itex::ReadBoolFromEnvVar("TF_CUDNN_DETERMINISTIC",
                                               false,
                                               &cudnn_deterministic));
    return cudnn_deterministic;
  }();
  return itex::OpDeterminismRequired() || require_cudnn_determinism ||
  ...
  if (!RequireDeterminism(instr.parent()->parent()->config())) {
    selected_result = absl::c_min_element(
        ...
        [](const AutotuneResult& lhs, const AutotuneResult& rhs) {
          return itex::proto_utils::FromDurationProto(lhs.run_time()) <
                 itex::proto_utils::FromDurationProto(rhs.run_time());
        });
  }
