...
class EnforceMinorToMajorReduceOpVisitor : public DfsHloRewriteVisitor {
  Status HandleReduce(HloInstruction* hlo) override {
    ...
    ITEX_VLOG(5) << "Input: " << reduce->ToString();
    ...
    for (HloInstruction* operand : reduce->inputs()) {
      ...
      if (operand_idx != 0 &&
        ...
        operand = copy;
        ITEX_VLOG(3) << "Copying to establish consistent inputs layout: "
                     << copy->ToString();
      }
      ...
      for (int i = 0; i < operand_shape.rank(); i++) {
        ...
        ITEX_VLOG(5) << "Processing logical dimension " << logical_dim
                     << " of size " << dim_size;
        ...
        if (absl::c_linear_search(reduce->dimensions(), logical_dim)) {
          ...
          ITEX_VLOG(5) << "logical_reduce_dim = " << logical_reduce_dim << ", "
      ...
      ITEX_VLOG(5) << "Reduction input: " << canonical_reduce_input->ToString();
    ...
    ITEX_VLOG(5) << "Generated new reduction: " << new_reduce->ToString();
    ...
    ITEX_VLOG(5) << "Generated output: " << new_reduce->ToString();
