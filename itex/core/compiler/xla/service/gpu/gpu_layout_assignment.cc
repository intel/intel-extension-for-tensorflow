...
HeuristicLayoutAssignment(const HloInstruction* instr) {
  ...
  if (primitive_util::IsIntegralType(input_ty)) {
    if (input_ty == S8 && dnums.input_spatial_dimensions_size() == 2 &&
      ...
      ITEX_VLOG(2) << "Using NCHW_VECT_C for int8_t conv " << instr->ToString();
      return kAllNCHW_VECT_C;
    }
    ITEX_VLOG(2) << "Using NHWC for int8_t conv " << instr->ToString();
    return kAllNHWC;
  }
  ...
  if (debug_options.xla_gpu_force_conv_nchw()) {
    ITEX_VLOG(2) << "Overriding layout to NCHW for " << instr->ToString();
    return kAllNCHW;
  }

  if (debug_options.xla_gpu_force_conv_nhwc()) {
    ITEX_VLOG(2) << "Overriding layout to NHWC for " << instr->ToString();
    return kAllNHWC;
  }
  ...
  ITEX_VLOG(2) << "Using heuristic to figure out layouts for "
               << instr->ToString();
...
Status GpuLayoutAssignment::AddBackendConstraintsToDnnConvCustomCall(
...

  {
    ...
    FilterLayout filter;
    DataLayout output;
    std::tie(input, filter, output) = HeuristicLayoutAssignment(instr);
...
Status GpuLayoutAssignment::AddBackendConstraints(
  ...
  for (auto iterator = post_order.rbegin(); iterator != post_order.rend();
    ...
    if (IsMatrixMultiplication(*instruction) &&
      ...
      ITEX_CHECK_EQ(dim_nums.lhs_batch_dimensions_size(),
                    dim_nums.rhs_batch_dimensions_size());
      ITEX_CHECK_EQ(dim_nums.lhs_batch_dimensions_size() + 2,
                    instruction->shape().rank());
      for (int64_t batch_dim : dim_nums.lhs_batch_dimensions()) {
        ITEX_CHECK_LT(batch_dim, instruction->shape().rank() - 2);
      }
