...
double CalculateBytesReadByFusionParameter(HloInstruction* param) {
  ITEX_CHECK_EQ(HloOpcode::kParameter, param->opcode());
...
double GetCurrentBytesTransferred(HloInstruction* fusion) {
  ITEX_CHECK_EQ(HloOpcode::kFusion, fusion->opcode());
...
Status FusionInstructionMerger::FuseIntoAllUsers(HloInstruction* instruction) {
  ...
  for (HloInstruction* user : users) {
    ...
    if (consumer->opcode() != HloOpcode::kFusion) {
      ...
      ITEX_CHECK_OK(computation_->ReplaceInstruction(user, consumer));
    }
  ...
  ITEX_CHECK_EQ(0, instruction->user_count()) << instruction->ToString();
  TF_RETURN_IF_ERROR(computation_->RemoveInstruction(instruction));
  ITEX_VLOG(2) << "Merged fusion instruction: " << instruction->name()
               << " into users { "
               << absl::StrJoin(users, ", ",
...
Status FusionInstructionMerger::Run() {
  for (HloInstruction* instruction : computation_->MakeInstructionPostOrder()) {
    if (instruction->opcode() == HloOpcode::kFusion) {
      ...
      if (!was_fused) {
        ITEX_VLOG(2) << "Not fusing fusion |" << instruction->name()
                     << "| with all of it's users due to: "
                     << was_fused.Explain();
  ...
  ITEX_VLOG(1) << "FusionInstructionMerger EXIT"
...
StatusOr<bool> FusionMerger::Run(HloModule* module) {
  bool changed = false;
  ITEX_VLOG(2) << "FusionMerger for module: " << module->name();
  for (auto* computation : module->MakeNonfusionComputations()) {
    ITEX_VLOG(1) << "Before running FusionInstructionMerger for computation: "
                 << computation->name();
    ITEX_XLA_VLOG_LINES(3, computation->ToString());

    FusionInstructionMerger fusion_merger(computation);
    TF_RETURN_IF_ERROR(fusion_merger.Run());
    changed |= fusion_merger.changed();

    ITEX_VLOG(1) << "After running FusionInstructionMerger for computation: "
