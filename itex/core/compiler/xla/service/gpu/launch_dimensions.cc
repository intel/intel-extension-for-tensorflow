...
static int64_t ThreadsPerBlockLimit(GpuDeviceInfo gpu_device_info) {
  ...
  if (threads_per_block <= 0) {
    ...
    if (log_count.fetch_add(1) < 8) {
      ITEX_LOG(WARNING)
          << "Attempting to calculate launch dimensions for GPU "
...
StatusOr<LaunchDimensions> CalculateLaunchDimensions(
  ...
  ITEX_CHECK_EQ(num_elements % dim_config.unroll_factor, 0);
  ...
  int64_t threads_per_block_y =
      threads_per_block_row_vectorized > 0 &&
          ...
          ? CeilOfRatio(static_cast<int64_t>(128),
                        threads_per_block_row_vectorized)
          : 1;
  ITEX_VLOG(2) << "Set # of threads per block to (.x=" << threads_per_block_x
  ...
  if (dim_config.few_waves && !dim_config.row_vectorized) {
    int64_t xla_threads_per_block_capped = 128;
    itex::ReadInt64FromEnvVar("XLA_THREADS_PER_BLOCK_CAPPED", 128,
                              &xla_threads_per_block_capped);
    int64_t capped_threads_per_block_x =
        std::min<int64_t>(threads_per_block_x, xla_threads_per_block_capped);
    ...
    if (capped_block_count < block_count) {
      ...
      ITEX_VLOG(2) << "Update the # of blocks to " << block_count
    ...
    if (capped_block_count < block_count) {
      ITEX_VLOG(2)
          << "Update # of blocks to block_count as few_waves is enabled.";
      block_count = capped_block_count;
    }
  }
  if (gpu_device_info.block_dim_limit_x > 0 &&
      block_count >= gpu_device_info.block_dim_limit_x) {
    return itex::errors::Unimplemented("Kernel launch needs more blocks (",
                                       block_count,
                                       ") than allowed by hardware (",
                                       gpu_device_info.block_dim_limit_x, ").");
  }

  ITEX_VLOG(2) << absl::StrFormat(
