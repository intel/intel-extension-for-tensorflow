...
bool IsForwardConvolutionCanonical(const HloInstruction& conv) {
  ITEX_CHECK(conv.custom_call_target() == kCudnnConvForwardCallTarget ||
...
bool GpuConvPaddingLegalization::CanonicalizeForwardConvolution(
  ...
  ITEX_VLOG(1) << "Canonicalizing forward conv";
  ...
  ITEX_VLOG(1) << "Replacing:\n  " << conv->ToString() << "\nwith:\n  "
               << new_conv->ToString();
  ITEX_CHECK_OK(conv->parent()->ReplaceInstruction(conv, new_conv));
  return true;
}
...
bool GpuConvPaddingLegalization::CanonicalizeBackwardFilterConvolution(
  ...
  ITEX_CHECK_EQ(backward_conv->custom_call_target(),
  ...
  ITEX_VLOG(1) << "Canonicalizing backward filter conv";
...
bool GpuConvPaddingLegalization::CanonicalizeBackwardInputConvolution(
  ...
  Shape slice_shape =
      ShapeInference::InferSliceShape(new_backward_conv->shape(), start_indices,
          ...
          .ConsumeValueOrDie();
  ITEX_CHECK(ShapeUtil::Compatible(slice_shape, backward_conv_shape))
  ...
  ITEX_VLOG(1) << "Canonicalizing backward input conv";
  ITEX_VLOG(1) << "Replacing:\n  " << backward_conv->ToString() << "\nwith:\n  "
               << new_tuple->ToString();

  ITEX_CHECK_OK(computation->ReplaceInstruction(backward_conv, new_tuple));
  return true;
}
