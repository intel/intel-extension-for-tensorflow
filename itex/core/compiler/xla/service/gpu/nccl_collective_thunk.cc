...
Status NcclCollectiveThunk::ExecuteOnStream(const ExecuteParams& params) {
#if ITEX_USE_CCL
  ITEX_VLOG(1) << absl::StreamFormat("Starting %s.",
  ...
  if (first_call_to_execute_) {
    TF_RETURN_IF_ERROR(params.stream->BlockHostUntilDone(params));
