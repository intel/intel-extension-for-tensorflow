...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_GPU_KERNEL_MAPPING_SCHEME_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_GPU_KERNEL_MAPPING_SCHEME_H_
#include <string>
...
class TilingScheme {
 public:
  ...
  TilingScheme(Vector3 dims_in_elems, Vector3 tile_sizes, Vector3 num_threads,
    ...
    ITEX_CHECK_EQ(tile_sizes[2] % vector_size_, 0);
  }
 ...
 public:
  explicit ReductionCodegenInfo(TilingScheme mapping_scheme,
    ...
    if (num_partial_results > 1) {
      ITEX_CHECK_EQ(num_partial_results,
...
class ReductionCodegenState {
 public:
  ...
  const ReductionCalculationState& GetCalculationStateFor(
    ...
    ITEX_CHECK_LT(operand_idx, op_state.size());
  ...
  void SetCalculationStateFor(
    ...
    ITEX_CHECK_EQ(operand_idx, op_state.size());
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_GPU_KERNEL_MAPPING_SCHEME_H_
