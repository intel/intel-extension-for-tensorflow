...
std::array<int64_t, 3> PartitionShapeByMiddleDimensions(
  ...
  ITEX_CHECK(LayoutUtil::AreDimensionsConsecutive(shape.layout(), dims_middle));
...
Shape GetShapeFromTensorType(mlir::Value value) {
  ...
  ITEX_CHECK(op);
  ITEX_CHECK(value.getType().isa<mlir::TensorType>());
  Shape shape;
  if (auto attr = op->getAttrOfType<mlir::StringAttr>(kDefaultLayoutAttrName)) {
    shape = *itex_xla::ParseShape(
...
bool IsReductionFromOrToContiguousDimensions(mlir::Operation* op) {
  ...
  mlir::Value first_operand = reduce.getOperands()[0];
  Shape operand_shape = GetShape(first_operand);

  llvm::SmallVector<int64_t> dimensions_to_reduce;
  for (const llvm::APInt& d : reduce.getDimensions()) {
...
ReductionDimensions GetReductionKindAndContiguousComponents(
  ...
  for (const llvm::APInt& d :
       mlir::cast<mlir::mhlo::ReduceOp>(reduce).getDimensions()) {
    dimensions_to_reduce.push_back(d.getZExtValue());
  }
...
llvm::Value* EmitAMDGPUShflDown(llvm::Value* value, llvm::Value* offset,
  ...
  ITEX_CHECK_EQ(value->getType()->getPrimitiveSizeInBits(), 32);
...
llvm::Value* EmitFullWarpShuffleDown(llvm::Value* value, llvm::Value* offset,
  ...
  if (target_triple.isSPIR() && bit_width < 32) {
    // A special case for shuffle function if `bit_width < 32`, since
    // single-element vector is not allowed for llvm-spirv-translator tool. For
    // the other cases, use a vector.
    llvm::Value* x = builder->CreateZExt(
        builder->CreateBitCast(value, builder->getIntNTy(bit_width)),
        builder->getIntNTy(32));
    x = EmitSPIRShflDown(x, offset, builder);
    return builder->CreateBitCast(
        builder->CreateTrunc(x, builder->getIntNTy(bit_width)),
        value->getType());
  }
...
int PartitionLmhloOperandsAndOutputs(mlir::Operation* op) {
  ITEX_CHECK(op->getDialect() == op->getContext()->getLoadedDialect("lmhlo"));
...
std::vector<mlir::Value> GetHloOperands(mlir::Operation* op) {
  ...
  ITEX_LOG(FATAL) << "Unexpected op: " << MlirToString(op);
}
...
StatusOr<BufferAllocation::Slice> GetAllocationSlice(
  ...
  //  root := base | MemRefSubView(base) | MemRefCollapseShapeOp(subview) |
  //          MemRefReinterpretCastOp(base | collapse_shape)

  if (auto alloc =
          mlir::dyn_cast_or_null<mlir::memref::AllocOp>(v.getDefiningOp())) {
    for (auto& use : alloc.getMemref().getUses()) {
      mlir::Operation* user = use.getOwner();
      if (auto copy = mlir::dyn_cast_or_null<mlir::memref::CopyOp>(user)) {
        v = copy.getSource();
      }
    }
  }
  ...
  if (auto collapse = mlir::dyn_cast_or_null<mlir::memref::CollapseShapeOp>(
          v.getDefiningOp())) {
    v = collapse.getViewSource();
  }
  if (auto subview =
          mlir::dyn_cast_or_null<mlir::memref::SubViewOp>(v.getDefiningOp())) {
    v = subview.getSource();
  }
  if (auto view =
...
bool CanEmitFusedDynamicUpdateSliceInPlaceForGpu(
  ...
  ITEX_CHECK_EQ(1, output_buffers.size());
...
Shape GetShape(mlir::Value value) {
  ...
  ITEX_LOG(FATAL) << "Unexpected value type to get shape for";
  return {};
}
