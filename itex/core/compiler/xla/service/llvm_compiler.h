...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_LLVM_COMPILER_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_LLVM_COMPILER_H_
...
class LLVMCompiler : public Compiler {
 public:
  ...
  void SetPreOptimizationHook(ModuleHook hook) {
    ITEX_CHECK(!user_pre_optimization_hook_)
        << "Pre-optimization hook is already set";
    ITEX_CHECK(hook) << "hook cannot be null";
  ...
  void SetPostOptimizationHook(ModuleHook hook) {
    ITEX_CHECK(!user_post_optimization_hook_)
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_LLVM_COMPILER_H_
