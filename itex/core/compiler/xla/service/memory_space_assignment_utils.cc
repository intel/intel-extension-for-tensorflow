...
bool MemorySpaceAssignmentUtils::IsValueAllowedInAlternateMemory(
  ...
  if (value->shape().IsTuple()) {
    ITEX_VLOG(4) << "Keeping value " << value->ToShortString()
  ...
  for (const HloPosition& position : value->positions()) {
    if ((position.instruction->opcode() == HloOpcode::kSend ||
      ...
      ITEX_VLOG(4)
          << "Keeping value " << value->ToShortString()
...
/*static*/ void MemorySpaceAssignmentUtils::HoistEntryParameters(
  ...
  ITEX_CHECK(schedule.is_computation_scheduled(entry));
  ...
  ITEX_CHECK_EQ(new_sequence.size(), sequence.size());
...
/*static*/ void MemorySpaceAssignmentUtils::HoistConstantOperations(
  ...
  for (const HloComputation* computation : module.MakeNonfusionComputations()) {
    ITEX_CHECK(schedule.is_computation_scheduled(computation));
