...
#include "itex/core/compiler/xla/service/hlo_memory_scheduler.h"
 ...
 private:
  ...
  ReadyListEntry MakeReadyListEntry(HloInstruction* instruction) {
    ...
    for (auto* buffer : buffer_uses_.at(instruction)) {
      ...
      ITEX_CHECK(unscheduled_use_count_it != unscheduled_use_count_.end());
  ...
  HloInstructionSequence CreateSchedule() {
    ...
    while (!ready_queue.empty()) {
      ...
      ITEX_VLOG(2) << "Schedule instruction: " << best->ToShortString()
    ...
    ITEX_CHECK_EQ(schedule.size(), computation_->instruction_count());
    ITEX_CHECK_EQ(scheduled_instructions_.size(),
                  computation_->instruction_count());
...
StatusOr<HloInstructionSequence> DFSMemoryScheduler(
  ...
  ITEX_CHECK_EQ(stats_map.size(), computation->instruction_count());
  ...
  ITEX_CHECK_EQ(sequence.size(), computation->instruction_count());
  if (peak_memory) {
    TF_ASSIGN_OR_RETURN(
...
StatusOr<HloInstructionSequence> DefaultMemoryScheduler(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      ListMemoryScheduler(computation, points_to_analysis, alias_analysis,
                          size_function, memory_by_computation, postprocessor,
                          &list_memory));
  ITEX_VLOG(2) << "Min-memory list sequence: "
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      DFSMemoryScheduler(computation, points_to_analysis, alias_analysis,
                         size_function, memory_by_computation, postprocessor,
                         &dfs_memory));
  ITEX_VLOG(2) << "Min-memory dfs sequence: "
  ...
  ITEX_VLOG(2) << "Min-memory post order sequence: "
  ...
  if (min_memory == list_memory) {
    ITEX_VLOG(2) << "Chose min-memory list sequence: "
                 << HumanReadableNumBytes(list_memory);
    return list_sequence;
  } else if (min_memory == dfs_memory) {
    ITEX_VLOG(2) << "Chose min-memory dfs sequence: "
                 << HumanReadableNumBytes(dfs_memory);
    return dfs_sequence;
  } else {
    ITEX_VLOG(2) << "Chose min-memory post_order sequence: "
                 << HumanReadableNumBytes(post_order_memory);
...
StatusOr<HloSchedule> ScheduleModule(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      (algorithm ? algorithm
                 : DefaultModuleScheduler)(module, *points_to_analysis,
                                           *alias_analysis, size_function,
...
StatusOr<HloInstructionSequence> ScheduleComputation(
    ...
    const MemorySchedulerPostprocessor& postprocessor) {
  ITEX_CHECK(!computation->IsFusionComputation());
  TF_ASSIGN_OR_RETURN(std::unique_ptr<TuplePointsToAnalysis> points_to_analysis,
