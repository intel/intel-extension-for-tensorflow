  ...
  switch (instruction->opcode()) {
    case HloOpcode::kWhile: {
      if (computation == instruction->while_condition()) {
        ...
        ITEX_CHECK_EQ(computation, instruction->while_body());
        instruction->set_while_body(new_computation);
      }
      break;
    }
    case HloOpcode::kCall: {
      ITEX_CHECK_EQ(instruction->to_apply(), computation);
    ...
    case HloOpcode::kConditional: {
      for (int b = 0; b < instruction->branch_count(); ++b) {
        if (b == instruction->branch_count() - 1) {
          ITEX_CHECK_EQ(computation, instruction->branch_computation(b));
        }
    ...
    default:
      ITEX_LOG(FATAL) << "unexpected opcode: "
...
Status FlattenNode(const CallGraphNode& node) {
  ...
  for (int i = 0; i < node.caller_callsites().size(); ++i) {
    ...
    ITEX_CHECK_EQ(call_site.context(), CallContext::kControlFlow);
...
StatusOr<bool> FlattenCallGraph::Run(
  ...
  ITEX_XLA_VLOG_LINES(3, "Before flatten call graph:\n" + module->ToString());

  std::unique_ptr<CallGraph> call_graph = CallGraph::Build(module);
  TF_RETURN_IF_ERROR(call_graph->VisitNodes(FlattenNode));

  ITEX_XLA_VLOG_LINES(3, "After flatten call graph:\n" + module->ToString());
  return true;
}
