...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_MEMORY_SPACE_ASSIGNMENT_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_MEMORY_SPACE_ASSIGNMENT_H_
  ...
  class Allocation {
   ...
   public:
    Allocation(HloPosition defining_position, MemorySpace memory_space,
      ...
      ITEX_CHECK(!is_scoped_allocation ||
                 defining_position.index == ShapeIndex({}));
    }
...
class AsynchronousCopyResource {
 public:
  ...
  // Adds the given asynchronous copy and updates the current resources.
  // ITEX_CHECK fails if there aren't enough resources to satisfy this copy (the
  // caller should use HasEnoughResource first to ensure there is enough
  // resource).
...
class AlternateMemoryBestFitHeap
 ...
 private:
  ...
  // This sets a required assignment. ITEX_CHECK fails if there is a conflicting
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_MEMORY_SPACE_ASSIGNMENT_H_
