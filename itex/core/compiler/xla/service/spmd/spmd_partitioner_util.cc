...
HloInstruction* CreateConstant(const Shape& shape, Literal value,
  ...
  auto identity = [](Literal value, PrimitiveType primitive_type) {
    ITEX_CHECK(
        ShapeUtil::IsScalarWithElementType(value.shape(), primitive_type));
    return value;
  };
...
HloInstruction* PadToShape(HloInstruction* hlo, const Shape& padded_shape,
  ...
  ITEX_CHECK(b == nullptr || computation == nullptr);
...
int64_t OffsetCalculation::Calculate(int64_t shard_ordinal) const {
  switch (opcode_) {
    ...
    case HloOpcode::kMultiply:
      return lhs_->Calculate(shard_ordinal) * rhs_->Calculate(shard_ordinal);
    default:
      ITEX_LOG(FATAL) << "Should not happen";
  }
}
...
absl::optional<HloInstruction*> ExchangeHalo(
    ...
    const SPMDCollectiveOpsCreator& collective_ops_creator,
    int64_t* next_channel_id, SpmdBuilder* b) {
  ITEX_CHECK(left_halo_size_functions.size() == hlo->shape().rank());
  ITEX_CHECK(right_halo_size_functions.size() == hlo->shape().rank());
...
absl::optional<HloInstruction*> ExchangeHaloAndGetValidData(
  ...
  if (shard_size_with_halo != concat->shape().dimensions(dim)) {
    ...
    ITEX_CHECK_LT(shard_size_with_halo, concat->shape().dimensions(dim));
    auto slice_shape = concat->shape();
...
HloInstruction* HaloExchangeToPadOnLeft(PartitionedHlo& original,
  ...
  ITEX_CHECK(!reshard_window->dynamic_slice_index_on_output.has_value());
...
absl::optional<GroupedSharding> AlignGroupsWithInternal(
  ...
  auto get_permutation = [](absl::Span<const int64_t> src,
    ...
    ITEX_CHECK_EQ(src.size(), dst.size());
    ...
    for (int64_t i = 0; i < src.size(); ++i) {
      ...
      ITEX_CHECK(it != dst_reverse_map.end());
      permutation[i] = it->second;
    }
    return permutation;
  };
  ITEX_CHECK_EQ(grouped_sharding.device_groups.size(),
...
HloSharding CreateMatchingShardingOnDims(
  ...
  ITEX_CHECK(target_dims.size() == source_dims.size())
  ...
  if (num_tiles != source_sharding.tile_assignment().num_elements()) {
    ITEX_CHECK_EQ(source_sharding.tile_assignment().num_elements() % num_tiles,
                  0);
...
GatherOperandsShardedAcrossParallelDims(
  ...
  if (idx_parallel_tiles_num != op_parallel_tiles_num) {
    ...
    ITEX_CHECK_GE(new_tile_assignment_dims.back(), 1);
