...
#include "itex/core/compiler/xla/service/hlo_sharding_util.h"
#include "itex/core/compiler/xla/service/pattern_matcher.h"
#include "itex/core/compiler/xla/service/shape_inference.h"
#include "itex/core/compiler/xla/service/spmd/custom_call_handler.h"
#include "itex/core/compiler/xla/service/spmd/spmd_partitioner_util.h"
...
std::string SpmdLogger::MakeReport() {
  ...
  for (int64_t i = 0;
    ...
    absl::StrAppend(&report, "\n  ",
                    itex::strings::HumanReadableNumBytes(entries_[i].first),
                    " : ", entries_[i].second, "\n");
  }
...
PartitionedHlo PartitionedHlo::ReshardNoCache(const HloSharding& target,
  ...
  ITEX_CHECK_EQ(target.IsManualSubgroup(), sharding().IsManualSubgroup());
  if (sharding().IsManualSubgroup()) {
    ...
    ITEX_CHECK(target_grouped.has_value())
...
HloInstruction* PartitionedHlo::PadWithValueHlo(
  ...
  const Shape& shape = hlo_->shape();
  ITEX_CHECK(!shape.IsTuple() && shape.element_type() != TOKEN);
  if (sharding.IsReplicated() || EvenlyPartitions(base_shape_, sharding)) {
    return hlo_;
  }
  ITEX_CHECK(!sharding.IsTileMaximal());
...
PartitionedHlo::ReshardAsWindowedInput(const Window& window,
  ...
  ITEX_VLOG(2) << "ReshardAsWindowedInput()\n"
               << "\twindow:" << window_util::ToString(window)
               << "\ttarget sharding:" << target.ToString();

  ITEX_CHECK(!target.IsTileMaximal());
  ...
  for (int64_t i = 0; i < base_shape_.rank(); ++i) {
    ...
    if (full_size < dilated_size) {
      ITEX_VLOG(2)
          << "Failed to reshard window operand because the window size is "
    ...
    if (wd.stride() != 1 &&
      ...
      ITEX_VLOG(2)
          << "Failed to reshard window operand due to non-trivial dilation";
      return absl::nullopt;
    }
    ...
    if (wd.base_dilation() != 1) {
      ...
      ITEX_CHECK_EQ(get_first_valid_element_offset_on_dilated_shard(0),
                    swd->padding_low());
  ...
  for (int dim = 0; dim < base_shape_.rank(); ++dim) {
    ...
    if (!resharded) {
      ITEX_VLOG(1)
          << "ReshardAsWindowedInput failed without replicate first: halo "
...
PartitionedHlo PartitionedHlo::Broadcast() const {
  const Shape& shape = hlo_->shape();
  const HloSharding& sharding = hlo_->sharding();
  ITEX_CHECK(sharding.HasUniqueDevice());
  ITEX_CHECK(!shape.IsTuple() && shape.element_type() != TOKEN);
...
PartitionedHlo PartitionedHlo::ReshardWithCollectivePermute(
  ...
  ITEX_CHECK(CanReshardWithCollectivePermute(sharding(), target))
      << sharding().ToString() << " to " << target.ToString();
...
Status SpmdPartitioningVisitor::DefaultAction(HloInstruction* hlo) {
  ...
  if (!hlo->sharding().IsTileMaximal()) {
    ITEX_VLOG(1) << "Not partitioned in SPMD mode (DefaultAction):"
                 << hlo->ToString();
    for (int64_t i = 0; i < hlo->operand_count(); ++i) {
      ITEX_VLOG(1) << "  operand " << i
    ...
    if (has_manual_sharding && !hlo->IsCustomCall("SPMDFullToShardShape")) {
      ...
      if (has_manual_subgroup && !hlo->IsCustomCall("SPMDFullToShardShape")) {
        auto get_grouped_sharding =
          ...
          ITEX_CHECK(!sharding.tuple_elements().empty());
    ...
    if (!ShapeUtil::Compatible(per_branch_partitioned_shapes[i], shard_shape)) {
      std::function<HloInstruction*(const ShapeIndex&, HloInstruction*)>
        ...
        if (element_shape.IsArray()) {
          ITEX_CHECK(pad_shape.IsArray());
          return PadToShape(infeed_element, pad_shape, &branch_b);
        }
        ITEX_CHECK(element_shape.IsTuple());
        ITEX_CHECK(element_shape.tuple_shapes().empty());
  ...
  for (int64_t i = 0; i < branches.size(); ++i) {
    ...
    if (!ShapeUtil::Compatible(per_branch_partitioned_shapes[i], shard_shape)) {
      std::function<HloInstruction*(const ShapeIndex&, HloInstruction*)>
        ...
        if (element_shape.IsArray()) {
          ITEX_CHECK(slice_shape.IsArray());
...
Status SpmdPartitioningVisitor::HandleReduceWindow(HloInstruction* hlo) {
  ...
  SetPartitionedHlo(hlo, [&]() {
    ...
    if (!sharded_results[0].dynamic_slice_index_on_output.has_value()) {
      ITEX_CHECK(ShapeUtil::Compatible(shard_shape, sharded_rw->shape()))
...
StatusOr<bool> SpmdPartitioningVisitor::DoPartition(
  ...
  ITEX_VLOG(2) << "Partitioning computation " << computation->name() << " for "
...
StatusOr<bool> SpmdPartitioner::Run(
  ...
  ITEX_XLA_VLOG_LINES(1, SpmdLogger::ReportBeforePartition(
  ...
  for (int64_t i = 0; i < num_parameters; ++i) {
    ...
    ITEX_CHECK(param->has_sharding())
        << "Missing sharding in entry parameter " << i;
  ...
  ITEX_CHECK(entry_root->has_sharding()) << "Missing sharding in entry root.";
  ...
  ITEX_XLA_VLOG_LINES(1, SpmdLogger::ReportAfterPartition(
                             *module, options_.report_instruction_count));
  ITEX_XLA_VLOG_LINES(1, logger.MakeReport());
