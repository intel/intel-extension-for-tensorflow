...
/* static */ void TransferManager::RegisterTransferManager(
  ...
  ITEX_CHECK(managers->find(platform_id) == managers->end());
...
Status TransferManager::WriteTupleIndexTablesAsync(
    se::Stream* stream, const ShapedBuffer& device_buffer) {
  ITEX_VLOG(2) << "Writing tuple index tables for " << device_buffer;
...
itex_xla::Shape TransferManager::ChooseGoodInfeedLayout(
