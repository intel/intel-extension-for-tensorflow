...
absl::string_view HloLexer::StringViewFromPointers(const char* begin,
  ...
  ITEX_CHECK(begin <= end);
  ITEX_CHECK(begin == buf_.end() || CanDereference(begin));
  ITEX_CHECK(end == buf_.end() || CanDereference(end));
...
absl::optional<int64_t> HloLexer::LexNanPayload(absl::string_view& consumable) {
  ...
  ITEX_CHECK(absl::StartsWith(slice, "(0x"));
  slice.remove_prefix(std::strlen("(0x"));
  ITEX_CHECK(absl::EndsWith(slice, ")"));
  slice.remove_suffix(std::strlen(")"));
  itex::uint64 payload_value;
  if (itex::strings::HexStringToUint64(slice, &payload_value)) {
    if (payload_value <= 0 || payload_value > NanPayloadBitMask<double>()) {
      ITEX_LOG(ERROR) << "NaN payload out of range: " << payload_value;
      return absl::nullopt;
    }
...
TokKind HloLexer::LexNumberOrPattern() {
  ...
  if (RE2::Consume(&consumable, *float_pattern)) {
    current_ptr_ = consumable.begin();
    ITEX_CHECK(
        absl::SimpleAtod(std::string(token_state_.token_start, current_ptr_),
  ...
  if (RE2::Consume(&consumable, *int_pattern)) {
    ...
    ITEX_LOG(ERROR) << "Failed to parse int literal: " << slice;
    return TokKind::kError;
  }
...
TokKind HloLexer::LexString() {
  ...
  if (RE2::Consume(&consumable, *escaping_pattern)) {
    ...
    if (!absl::CUnescape(raw, &token_state_.str_val, &error)) {
      ITEX_LOG(ERROR) << "Failed unescaping string: " << raw
                      << ". error: " << error;
      return TokKind::kError;
    }
