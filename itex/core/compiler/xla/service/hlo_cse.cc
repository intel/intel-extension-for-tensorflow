...
struct ConstantKey {
  ...
  friend bool operator==(const ConstantKey& lhs, const ConstantKey& rhs) {
    return lhs.domain == rhs.domain &&
           (kIsLayoutSensitive
                ? Shape::Equal()
                : Shape::Equal().IgnoreLayout())(lhs.hlo->shape(),
                                                 rhs.hlo->shape()) &&
...
StatusOr<bool> CombineConstants(HloComputation* computation) {
  ...
  while (inst_it != computation->instructions().end()) {
    ...
    if (match != nullptr) {
      ...
      ITEX_CHECK_OK(instruction->ReplaceAllUsesWith(match));
      ITEX_CHECK_OK(computation->RemoveInstruction(instruction));
      ++combined;
    }
  }
  ITEX_VLOG(4) << "Combined " << combined << " constants and iotas in "
...
struct CseKey {
  ...
  friend H AbslHashValue(H h, const CseKey& key) {
    ...
    if (HloOpcodeIsBinaryCommutative(instruction->opcode())) {
      ITEX_CHECK_EQ(instruction->operand_count(), 2);
