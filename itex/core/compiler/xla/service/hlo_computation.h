...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_HLO_COMPUTATION_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_HLO_COMPUTATION_H_
...
class HloComputation {
 public:
  ...
  HloInstruction* parameter_instruction(int64_t param_no) const {
    ...
    ITEX_CHECK_LT(param_no, static_cast<int64_t>(param_instructions_.size()))
  ...
  using InstructionSequence = itex::gtl::iterator_range<
      UnwrappingIterator<std::list<std::unique_ptr<HloInstruction>>::iterator>>;

  using ConstInstructionSequence = itex::gtl::iterator_range<UnwrappingIterator<
  ...
  void SetFusionInstruction(HloInstruction* fusion_instruction) {
    ITEX_CHECK(!IsCustomCallComputation() && !IsAsyncComputation());
    fusion_instruction_ = fusion_instruction;
  ...
  void SetCustomCallInstruction(HloInstruction* custom_call_instruction) {
    ITEX_CHECK(!IsFusionComputation() && !IsAsyncComputation());
  ...
  void AddAsyncInstruction(HloInstruction* async_instruction) {
    ITEX_CHECK(async_instruction != nullptr)
        << "Nullptr shouldn't be added as commputation's async instruction. ";
    ITEX_CHECK(!IsFusionComputation() && !IsCustomCallComputation());
    ITEX_CHECK(async_instruction->opcode() == HloOpcode::kAsyncStart ||
...
Status HloComputation::Accept(
  ...
  for (HloInstruction* root : CollectUnreachableRoots()) {
    ITEX_VLOG(3) << "Traversing unreachable root: " << root->ToString();
...
Status HloComputation::AcceptOrdered(
    ...
    absl::Span<HloInstruction* const> order) const {
  ITEX_VLOG(3) << "Accepting visitor with order.";
  for (HloInstruction* root : CollectUnreachableRoots()) {
  ...
  for (const HloInstruction* instruction : order) {
    ITEX_VLOG(3) << "Visiting ordered: " << instruction->ToString();
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_HLO_COMPUTATION_H_
