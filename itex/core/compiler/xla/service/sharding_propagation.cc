...
#include "itex/core/compiler/xla/service/dot_as_convolution_util.h"
...
#include "itex/core/compiler/xla/sharding_op_util.h"
    ...
    if (!allow_aggressive_resharding && instruction->shape().IsArray() &&
      ...
      for (int64_t i = 0; i < instruction->shape().rank(); ++i) {
        ...
        if (instruction->sharding().tile_assignment().dim(i) != 1) {
          ITEX_VLOG(10) << "Not merging because of dim i = " << i
                        << " sharded differently";
          ITEX_VLOG(10) << "Instr sharding: "
                        << instruction->sharding().ToString();
          ITEX_VLOG(10) << "New sharding " << sharding.ToString();
          return false;
        }
      ...
      if (hlo_sharding_util::PartiallyReplicateTiledShardingOnDims(
        ...
        ITEX_VLOG(10) << "Not merging because of different device distribution";
        ITEX_VLOG(10) << "Instr sharding: "
                      << instruction->sharding().ToString();
        ITEX_VLOG(10) << "New sharding " << sharding.ToString();
        return false;
...
bool IsConvolutionKernelSmall(const HloInstruction* instruction) {
  ITEX_CHECK_EQ(instruction->opcode(), HloOpcode::kConvolution);
...
bool InferConvolutionShardingFromOperands(HloInstruction* instruction,
  ...
  auto get_tiled_sharding_based_on_lhs = [&] {
    ITEX_CHECK(!lhs->sharding().IsTileMaximal());
...
Status CheckAndUpdateDeviceAssignmentsInWhileBody(
  ...
  ITEX_CHECK_EQ(while_instruction->opcode(), HloOpcode::kWhile);
...
bool InferUnspecifiedDimsFromOperand(HloInstruction* annotate_op,
  ...
  if (!annotate_op->IsCustomCall("Sharding")) {
    ITEX_CHECK_EQ(annotate_op->opcode(), HloOpcode::kCopy);
  }
  ...
  *man_conversion_op_after = man_conversion_op;
  ...
  ITEX_CHECK(man_conversion_op->IsCustomCall("SPMDShardToFullShape"));
...
absl::optional<HloSharding> ShardingPropagation::GetShardingFromUser(
  ...
  switch (user.opcode()) {
    ...
    case HloOpcode::kScatter: {
      ...
      ITEX_CHECK_EQ(&instruction, user.operand(2));
  ...
  switch (instruction->opcode()) {
    ...
    case HloOpcode::kReduceWindow: {
      ...
      if (absl::c_any_of(instruction->window().dimensions(), has_dilation)) {
        ITEX_VLOG(2)
            << "Not applying sharding to reduce window because dilatation "
               "isn't supported yet: "
    ...
    case HloOpcode::kSelectAndScatter: {
      ...
      if (absl::c_any_of(instruction->window().dimensions(),
                         has_base_dilation)) {
        ITEX_VLOG(2) << "Not applying sharding to select-and-scatter because "
...
StatusOr<bool> ShardingPropagation::Run(
              ...
              if (absl::c_count(related_instructions, instruction)) {
                for (HloInstruction* inst : related_instructions) {
                  if (!inst->has_sharding() ||
                    ...
                    ITEX_VLOG(2) << "Add computation sharding: " << inst->name()
                                 << " " << instruction->sharding().ToString();
  ...
  auto run_to_fix_point = [&](int64_t aggressiveness) {
    ...
    while (changed_last_iter) {
      ...
      for (const HloComputation* computation :
           module->computations(execution_threads)) {
        ITEX_VLOG(2) << "Consider computation: " << computation->name();
        ...
        for (HloInstruction* instruction : instructions) {
          ...
          if (provided_shardings.contains(instruction)) {
            ...
            if (it != unspecified_dims.end() &&
              ...
              ITEX_VLOG(2) << "Refined partial sharding (forward-pass): "
                           << instruction->ToString();
          ...
          if (InferShardingFromOperands(instruction, computation_map,
            ...
            ++inferred_from_operand_counter;
            any_changed = true;
            ITEX_VLOG(2) << "Add sharding (forward-pass): "
                         << instruction->ToString();
        ...
        for (auto it = instructions.rbegin(); it != instructions.rend(); ++it) {
          ...
          if (provided_shardings.contains(*it)) {
            ...
            if (uit != unspecified_dims.end() &&
              ...
              ITEX_VLOG(2) << "Refined partial sharding (backward-pass): "
                           << (*it)->ToString();
          ...
          if (InferShardingFromUsers(*it, computation_map, aggressiveness,
            ...
            ++inferred_from_user_counter;
            any_changed = true;
            ITEX_VLOG(2) << "Add sharding (backward-pass): "
                         << (*it)->ToString();
            absl::flat_hash_set<HloInstruction*> changed_in_comp_prop;
      ...
      ITEX_VLOG(1) << "Sharding propagation iteration " << iterations << ";";
      ITEX_VLOG(1) << "  total instructions: " << instruction_counter;
      ITEX_VLOG(1) << "  instructions already sharded: "
                   << already_sharded_counter;
      ITEX_VLOG(1) << "  shardings inferred from operands: "
                   << inferred_from_operand_counter;
      ITEX_VLOG(1) << "  shardings inferred from users: "
                   << inferred_from_user_counter;
      ITEX_VLOG(1) << "  aggressiveness: " << aggressiveness;
      ++iterations;
    }
    return Status::OK();
  };
