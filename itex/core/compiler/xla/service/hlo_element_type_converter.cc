...
#include "itex/core/compiler/xla/service/hlo_element_type_converter.h"
  ...
  for (auto* computation : module->computations(execution_threads)) {
    for (auto* hlo : computation->MakeInstructionPostOrder()) {
      ...
      if (!should_eliminate_type) {
        // If this ITEX_CHECK fires, then this was an instruction that does not
        // take the elimination type as an operand but it does return it. This
        // pass does not have a feature to change the output type in that case,
        // so instead of silently failing to eliminate the type, it fails
        // loudly.
