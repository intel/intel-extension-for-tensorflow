...
class SubcomputationInsertionVisitor : public DfsHloVisitorWithDefault {
 public:
  ...
  explicit SubcomputationInsertionVisitor(HloInstruction* call)
    ...
    ITEX_CHECK_EQ(HloOpcode::kCall, call_->opcode());
  }
  ...
  Status DefaultAction(HloInstruction* hlo) override {
    ...
    ITEX_VLOG(1) << "Cloning HLO and adding to caller: " << hlo->ToString();
  ...
  Status FinishVisit(HloInstruction* root) override {
    ...
    ITEX_VLOG(1) << "Replacing all uses of " << call_->ToString()
...
StatusOr<bool> CallInliner::Run(HloModule* module) {
  ...
  TF_RETURN_IF_ERROR(call_graph->VisitNodes([&](const CallGraphNode& node)
                                                -> Status {
    ITEX_VLOG(1) << "Visiting node: " << node.ToString();
      ...
      if (instruction->opcode() == HloOpcode::kCall) {
        ...
        if (!single_call_site_ || call_graph->GetNode(instruction->to_apply())
          ...
          if (update_domain_) {
            ...
            TF_RETURN_IF_ERROR(isolator.UpdateDomains(inlined_root).status());
          }
          did_mutate = true;
        }
      }
