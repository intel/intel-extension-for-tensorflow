...
#include "itex/core/compiler/xla/service/buffer_value_containers.h"
#include "itex/core/compiler/xla/service/heap_simulator.h"
#include "itex/core/compiler/xla/service/hlo_alias_analysis.h"
#include "itex/core/compiler/xla/service/hlo_buffer.h"
#include "itex/core/compiler/xla/service/hlo_live_range.h"
#include "itex/core/compiler/xla/service/hlo_op_metadata.h"
#include "itex/core/compiler/xla/service/hlo_opcode.h"
#include "itex/core/compiler/xla/service/hlo_value.h"
...
using ::itex::strings::HumanReadableNumBytes;
...
std::vector<int64_t> ColorInterferenceGraph(
  ...
  for (int64_t node : nodes) {
    ...
    ITEX_CHECK_NE(color, kColorUnassigned);
...
const BufferAllocation& BufferAssignment::GetAssignedAllocation(
  ...
  ITEX_CHECK(HasAllocation(value));
...
const BufferAllocation& BufferAssignment::GetAllocation(
  ...
  ITEX_CHECK_LT(index, allocations_.size());
...
StatusOr<BufferAllocation::Slice> BufferAssignment::GetUniqueSlice(
    const HloInstruction* instruction, const ShapeIndex& index) const {
  ITEX_VLOG(3) << "Trying to find unique slice for " << instruction->name()
               << " [" << index << "]";
  BufferAllocation::Slice result;
  for (const HloValue* value :
       dataflow_analysis().GetValueSet(instruction, index).values()) {
    ITEX_VLOG(3) << "Examining value " << *value;
    if (HasAllocation(*value)) {
      ITEX_VLOG(3) << "Has allocation";
...
void BufferAssignment::AddAssignment(BufferAllocation* allocation,
                                     ...
                                     int64_t size) {
  ITEX_CHECK(allocation->is_reusable() ||
             allocation->assigned_buffers().empty())
      << "Non-reusable allocation already assigned a buffer: "
      << allocation->ToString();

  for (const HloValue* buffer_value : buffer.values()) {
    ITEX_CHECK(!allocation_index_for_value_.contains(buffer_value))
  ...
  if (alias_analysis().BufferLivesOut(buffer)) {
    ITEX_VLOG(3) << "HloBuffer lives out: " << buffer.ToString();
    ITEX_VLOG(3) << "Set maybe live out: " << allocation->ToString();
...
void BufferAssignment::AddAssignment(BufferAllocation* allocation,
  ...
  const HloValue& hlo_value =
      *ITEX_CHECK_NOTNULL(dynamic_cast<const HloValue*>(&value));
  if (alias_analysis().ValueLivesOut(hlo_value)) {
    ITEX_VLOG(3) << "HloValue lives out: " << hlo_value.ToString();
    ITEX_VLOG(3) << "Set maybe live out: " << allocation->ToString();
    allocation->set_maybe_live_out(true);
  }
}
...
void BufferAssignment::CombineTempAllocations() {
  ITEX_VLOG(1) << "CombineTempAllocations()";
  ...
  if (first_temp_it != allocations_.end()) {
    for (auto it = first_temp_it; it != allocations_.end(); ++it) {
      ...
      if (combined_it == combined_allocation_map.end()) {
        ...
        // size constraint.
        ITEX_VLOG(1) << "Combined temp allocation for color " << color
                     << " is: " << temp_allocation;
      ...
      if (combined_it->second->size() + it->size() >=
        ...
        // combined_it.
        ITEX_VLOG(1)
            << "Due to size constraint, reset temp allocation for color "
      ...
      ITEX_VLOG(1) << "Combined allocation absorbing temp allocation: "
                   << temp_allocation;
      ...
      if (!temp_allocation.HeapTraces().empty()) {
        ITEX_CHECK_EQ(temp_allocation.HeapTraces().size(), 1);
...
std::string BufferAssignment::ToVerboseString() const {
  ...
  for (size_t i = 0; i < std::min(kMaxBuffersToShow, peak_buffers.size());
    ...
    buf_strs.push_back(absl::StrCat("\n\tBuffer ", i + 1, ":\n\t\tSize: ",
                                    itex_xla::HumanReadableNumBytes(size)));
    if (!instr->metadata().op_name().empty()) {
      buf_strs.push_back(absl::StrCat(
          "\n\t\tOperator: ", itex_xla::OpMetadataToString(instr->metadata())));
    }
    if (instr->opcode() == HloOpcode::kParameter &&
        (instr->parent() == instr->parent()->parent()->entry_computation())) {
...
bool BufferAssigner::LiveRangeInterferes(const HloValue* buffer1,
  ...
  ITEX_CHECK((assignment->hlo_live_range().total_order_scheduled()));
  ...
  if (!(live_range_1.start > live_range_2.end ||
    ...
    if (live_range_1.end == live_range_2.start) {
      ...
      if (!can_share_as_operand(user_value, operand_value, live_range_1)) {
        ITEX_VLOG(4) << "End of live range of " << buffer1->ToShortString()
                     << " is equal to the start of live range of "
                     << buffer2->ToShortString()
      ...
      if (!can_share_as_operand(user_value, operand_value, live_range_2)) {
        ITEX_VLOG(4) << "End of live range of " << buffer2->ToShortString()
                     << " is equal to the start of live range of "
                     << buffer1->ToShortString()
                     << ", buffer cannot be shared.";
        return true;
      }
    } else {
      ITEX_VLOG(4) << "Can't assign: assignee " << *buffer1
                   << " may interfere with " << *buffer2;
      ITEX_VLOG(4) << "assigned_buffer.start: " << live_range_1.start;
      ITEX_VLOG(4) << "assigned_buffer.end: " << live_range_1.end;
      ITEX_VLOG(4) << "live_range_2.start" << live_range_2.start;
      ITEX_VLOG(4) << "live_range_2.end" << live_range_2.end;
      return true;
    }
  }
  return false;
...
bool BufferAssigner::MaybeAssignBuffer(BufferAllocation* allocation,
  ...
  if (allocation->is_readonly()) {
    ITEX_VLOG(4) << "Can't assign: allocation is readonly";
    return false;
  }
  ...
  if (must_not_live_out_.has_value()) {
    if (allocation->maybe_live_out()) {
      ...
      for (const HloValue* value : hlo_buffer.values()) {
        if ((*must_not_live_out_)(value->instruction(), value->index())) {
          ITEX_VLOG(4) << "Can't assign: " << value->instruction()->ToString()
    ...
    if (assignment->alias_analysis().BufferLivesOut(hlo_buffer)) {
      for (const auto& buffer_offset_size : allocation->assigned_buffers()) {
        const HloValue* value = buffer_offset_size.first;
        if ((*must_not_live_out_)(value->instruction(), value->index())) {
          ITEX_VLOG(4) << "Can't assign: "
                       << buffer_offset_size.first->instruction()
                       << " cannot live out of the module";
          return false;
        }
      }
  ...
  if (!allocation->is_reusable()) {
    ITEX_VLOG(4) << "Can't assign: allocation is not reusable";
    return false;
  }

  for (const auto& buffer_offset_size : allocation->assigned_buffers()) {
    // Pairwise compare.
    const HloValue& assigned_buffer = *ITEX_CHECK_NOTNULL(
    ...
    for (const HloValue* new_value : hlo_buffer.values()) {
      if (assignment->hlo_live_range().total_order_scheduled()) {
        if (LiveRangeInterferes(new_value, &assigned_buffer, assignment)) {
          ITEX_VLOG(4) << "Can't assign: assignee " << assigned_buffer
  ...
  if (assignment->alias_analysis().BufferLivesOut(hlo_buffer) &&
    ...
    ITEX_VLOG(4) << "Can't assign: buffer " << hlo_buffer
...
Status BufferAssigner::AssignSingleHloBuffer(
  ...
  for (const HloValue* value : hlo_buffer->values()) {
    if (value->instruction()->opcode() == HloOpcode::kConstant) {
      if (allocate_buffers_for_constants_) {
        ...
        ITEX_VLOG(3) << "New allocation #" << allocation->index()
                     << " for constant " << *hlo_buffer
                     << " value ptr: " << value;
      }
      ITEX_VLOG(3) << "Not allocating buffer for constant";
      return Status::OK();
    }
      ...
      for (auto* operand : instruction->operands()) {
        for (const auto& operand_slice :
          ...
          if (MaybeAssignBuffer(allocation, *hlo_buffer, assignment)) {
            ITEX_VLOG(3) << "Reusing (operand) allocation #"
                         << allocation->index() << " for: " << *hlo_buffer;
            return Status::OK();
          }
        }
      }
  ...
  for (int allocation_index = allocation_indices->size() - 1;
    ...
    if (MaybeAssignBuffer(allocation, *hlo_buffer, assignment)) {
      ITEX_VLOG(3) << "Reusing allocation #" << allocation->index()
                   << " for: " << *hlo_buffer;
      return Status::OK();
    }
  }
  ...
  if (!assignment->HasAllocation(*hlo_buffer) &&
    ...
    if (all_computations_have_sequential_order) {
      for (const HloValue* hlo_value : hlo_buffer->values()) {
        ...
        ITEX_VLOG(3) << "Delaying assignment of temp buffer: " << *hlo_value;
      }
      return Status::OK();
    }
  }
...
Status BufferAssigner::AssignBuffersForComputations(
  ...
  for (const HloBuffer& buffer : alias_analysis.buffers()) {
    ...
    if (preset_assigned_buffers.find(&buffer) !=
      ...
      ITEX_VLOG(3) << "Skip allocation for buffer: " << buffer;
      continue;
    }
  ...
  for (const HloBuffer* buffer : sorted_buffers) {
    ITEX_VLOG(3) << "=================================================";
    ITEX_VLOG(3) << "Assigning buffer for " << *buffer;
...
Status BufferAssigner::AssignPresetBuffers(
  ...
  for (auto& color_and_info : preset_assignments_->assignment_informations()) {
    ...
    ITEX_VLOG(3) << "Created preset buffer allocation "
  ...
  for (auto& position_and_chunk : preset_assignments_->chunks()) {
    ...
    for (const HloValue* value : buffer.values()) {
      ITEX_VLOG(3) << "Preset allocation for value: " << value->ToShortString();
      ...
      ITEX_CHECK(preset_allocations_iter != preset_allocations.end())
...
Status BufferAssigner::AssignBuffersWithSequentialOrdering(
  ...
  if (run_whole_module_heap_simulation) {
    ...
    // instructions.
    ITEX_VLOG(1) << "Running whole-module heap simulation";
    HloSchedule schedule(&assignment->module());
    ...
    for (const auto& pair : buffers_to_assign_sequentially) {
      ...
      const HloInstructionSequence* instruction_sequence =
          hlo_ordering.SequentialOrder(*computation);
      ITEX_CHECK(instruction_sequence != nullptr) << computation->name();
      schedule.set_sequence(computation, *instruction_sequence);
    ...
    for (auto& single_colored_set : color_map) {
      ...
      ITEX_VLOG(2) << "Simulating heap for color " << color;
    ...
    ITEX_VLOG(1) << "Running per-computation heap simulation";
    for (const auto& pair : buffers_to_assign_sequentially) {
      const HloComputation* computation = pair.first;
      const flat_hash_set<const HloValue*>& buffers_to_assign = pair.second;
      const HloInstructionSequence* instruction_sequence =
...
std::vector<const HloValue*> ComputePeakMemoryLogicalBuffers(
  ...
  ITEX_VLOG(1) << "Compute peak memory logical buffers";
  ...
  auto memory_delta = [&](const HeapSimulatorTrace::Event& event) -> int64_t {
    ...
    ITEX_LOG(FATAL) << "Unknown event kind: " << event.kind();
  };
  ...
  for (const auto& event : heap_trace.events()) {
    ...
    if (delta > 0) {
      ...
      ITEX_CHECK(ContainsKey(live_values, value));
  ...
  ITEX_CHECK_EQ(live_size, max_live_size);
  ...
  ITEX_VLOG(4) << "Peak memory buffer:";
  for (auto value : live_values_vector) {
    ITEX_VLOG(4) << "  " << value->ToString();
  }
...
void BufferAssigner::AssignBuffersFromHeapSimulator(
  ...
  ITEX_VLOG(1) << "Result size from heap simulator: " << result.heap_size;
  ...
  for (const HeapSimulator::HeapResult<HloValue>& heap_result :
    ...
    ITEX_XLA_VLOG_LINES(2, allocation->ToString());
...
StatusOr<std::unique_ptr<BufferAssignment>> BufferAssigner::CreateAssignment(
  ...
  ITEX_VLOG(1) << "Assigning buffers to module " << module->name();
  ITEX_XLA_VLOG_LINES(3, module->ToString());
  ITEX_XLA_VLOG_LINES(3, alias_analysis->ToString());
  ITEX_XLA_VLOG_LINES(3, alias_analysis->dataflow_analysis().ToString());
  ITEX_VLOG(1) << "Number of buffers to assign: "
  ...
  ITEX_VLOG(3) << "After coloring:";
  ITEX_XLA_VLOG_LINES(
      3, assignment->alias_analysis().dataflow_analysis().ToString());
  ...
  ITEX_VLOG(2) << "Running whole module heap simulation: "
  ...
  ITEX_VLOG(2) << "Multiheap per heap size limit: "
  ...
  for (const HloBuffer* buffer :
    ...
    ITEX_VLOG(3) << "maybe_live_out LogicalBuffer: " << *buffer;
    if (assignment->HasAllocation(*buffer)) {
      BufferAllocation* alloc =
          assignment->GetMutableAssignedAllocation(*buffer);
      alloc->set_maybe_live_out(true);
      ITEX_VLOG(3) << "maybe_live_out BufferAllocation: " << *alloc;
    }
  }
  ...
  ITEX_XLA_VLOG_LINES(2, assignment->ToString());
  TF_RETURN_IF_ERROR(assignment->ComputeSummaryStats());
  ITEX_XLA_VLOG_LINES(1, assignment->GetStats().ToString());
  ITEX_VLOG(1) << "Buffer assignment done.";
