...
#include "itex/core/compiler/xla/service/graphcycles/graphcycles.h"
...
bool IsCanonicalDot(HloInstruction* dot) {
  ...
  auto is_permutation_of_iota =
      [](const itex::protobuf::RepeatedField<int64_t>& vals) {
...
StatusOr<HloInstruction*> TryMergeSameOperand(HloInstruction* a,
  ...
  if (a->operand(0) != b->operand(0) && a->operand(1) != b->operand(1)) {
    ITEX_VLOG(4) << "Can't merge dots because they don't share an operand.\n"
                 << "\t" << a->ToString() << "\n"
                 << "\t" << b->ToString();
    return nullptr;
  }
  ...
  if (!absl::c_equal(dnums_a.lhs_batch_dimensions(),
    ...
    ITEX_VLOG(3) << "Can't merge dots because they have mismatching dnums.\n"
                 << "\t" << a->ToString() << "\n"
                 << "\t" << b->ToString() << "\n"
  ...
  if (!absl::c_equal(a->precision_config().operand_precision(),
    ...
    ITEX_VLOG(3) << "Can't merge dots because they have mismatching operand "
                    "precisions:\n"
                 << "\t" << a->ToString() << "\n"
                 << "\t" << b->ToString();
    return nullptr;
  ...
  ITEX_VLOG(2) << "Merging dots sharing an operand:\n"
               << "\t" << a->ToString() << "\n"
               << "\t" << b->ToString();
...
StatusOr<bool> MergeDots(HloComputation* comp, int64_t max_size_to_merge) {
  ...
  for (HloInstruction* instr : comp->instructions()) {
    ...
    for (HloInstruction* operand : instr->operands()) {
      ITEX_CHECK(graph.InsertEdge(graph_id(operand), id));
    }
    for (HloInstruction* control_pred : instr->control_predecessors()) {
      ITEX_CHECK(graph.InsertEdge(graph_id(control_pred), id));
    }
  }
