...
#include "itex/core/compiler/xla/stream_executor/device_description.h"
...
static ExecuteAsyncOnStreamWrapperState ExecuteWrapperBeforeExecution(
  ...
  ITEX_VLOG(1) << "enqueueing executable on stream...";
...
Status ExecuteWrapperAfterExecution(
  ...
  if (!return_status.ok()) {
    if (state.profile != nullptr) {
      ...
      // We already have a failure status to return, so just ITEX_LOG this if it
      // fails.
      Status status = stream->BlockHostUntilDone();
      if (!status.ok()) {
        ITEX_LOG(ERROR) << "Failed to BlockHostUntilDone: " << status;
      }
    }
  ...
  if (state.profile != nullptr) {
    ITEX_VLOG(1) << "enqueueing 'stop timer' and profiling callback...";
