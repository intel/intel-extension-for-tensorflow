...
bool BatchNormExpanderVisitor::Run(HloComputation* computation,
  ...
  ITEX_CHECK_OK(computation->Accept(&visitor));
...
Status BatchNormExpanderVisitor::HandleBatchNormTraining(
  ...
  if (batch_norm->has_sharding()) {
    ...
    ITEX_CHECK_EQ(instruction_count_after,
  ...
  ITEX_CHECK_OK(ReplaceWithNewInstruction(batch_norm, std::move(tuple)));
  return Status::OK();
}
...
Status BatchNormExpanderVisitor::HandleBatchNormInference(
  ...
  ITEX_CHECK_OK(ReplaceInstruction(batch_norm, shifted_normalized));
  return Status::OK();
}
...
StatusOr<bool> BatchNormExpander::Run(
  ...
  ITEX_XLA_VLOG_LINES(
      2, "BatchNormExpander::Run(), before:\n" + module->ToString());
  bool changed = false;
