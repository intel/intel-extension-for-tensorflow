...
#include "itex/core/compiler/xla/service/service.h"
...
#include "itex/core/compiler/xla/service/hlo_module_group.h"
#include "itex/core/compiler/xla/service/hlo_module_util.h"
...
Service::Service(const ServiceOptions& options,
  ...
  ITEX_CHECK_GT(options_.number_of_replicas(), 0);
  if (execute_backend_) {
    if (execute_backend_->device_count() > 0) {
      ITEX_CHECK_GE(execute_backend_->device_count(),
                    options_.number_of_replicas())
          << "Requested more replicas than there are devices.";
    }
    ITEX_LOG(INFO) << StrFormat(
    ...
    for (int i = 0; i < execute_backend_->device_count(); ++i) {
      ...
      ITEX_LOG(INFO) << StrFormat("  StreamExecutor device (%d): %s, %s", i,
                                  description.name(),
                                  description.platform_version());
    }
  } else {
    ITEX_VLOG(1) << "XLA compile-only service constructed";
  }
}
...
Service::ResolveAndValidateArguments(
  ...
  ITEX_CHECK_EQ(options_.number_of_replicas(), stream_executors.size());
  ...
  for (size_t i = 0; i < arguments.size(); ++i) {
    ...
    ITEX_CHECK_EQ(options_.number_of_replicas(), replicated_buffers.size());
...
StatusOr<std::unique_ptr<HloModuleConfig>> Service::CreateModuleConfig(
  ...
  return itex_xla::CreateModuleConfig(program_shape, argument_shapes,
...
StatusOr<std::vector<std::unique_ptr<Executable>>> Service::BuildExecutables(
  ...
  ITEX_VLOG(1) << StrFormat("BuildExecutable on service %p", this);

  ITEX_VLOG(1) << "Computations:";
  for (const HloModuleProto* proto : module_protos) {
    ITEX_VLOG(1) << proto->name();
  }

  ITEX_CHECK_EQ(module_protos.size(), module_configs.size());
  auto module_group =
...
Service::BuildAotResults(
    ...
    Backend* backend, std::vector<std::vector<se::StreamExecutor*>> executors,
    const Compiler::CompileOptions& options, bool run_backend_only) {
  ITEX_VLOG(1) << StrFormat("BuildAotResults on service %p", this);
...
Service::ExecuteParallelAndRegisterResult(
  ...
  for (int64_t i = 0; i < executables.size(); i++) {
    ...
    ITEX_CHECK_EQ(replicas.size(), arguments[i].size());
  ...
  for (int64_t i = 0, end = executables.size(); i < end; i++) {
    ...
    for (int64_t replica = 0; replica < n; ++replica) {
      ...
      if (replica == 0 && profile != nullptr) {
        ...
        ITEX_CHECK(timers.front() != nullptr);
      }
...
Status Service::ExecuteGraphParallel(const ExecuteGraphParallelRequest* arg,
  ...
  ITEX_VLOG(1) << "running execute-graph-parallel request";
    ...
    for (auto& args : replicated_arguments) {
      for (auto& arg : args) {
        auto update_shape_with_empty_tiles =
            [this](Shape* subshape, const itex_xla::ShapeIndex& index) {
              if (subshape->IsArray() && subshape->layout().tiles().empty()) {
                *subshape = execute_backend_->transfer_manager()
                                ->HostShapeToDeviceShape(*subshape);
              }
            };
  ...
  ITEX_VLOG(1) << "successfully completed 'execute-graph-parallel' request";
  return OkStatus();
}
...
StatusOr<std::unique_ptr<Executable>> Service::BuildExecutable(
  ...
  if (hlo_proto_before_opt != nullptr && hlo_proto_after_opt != nullptr) {
    ITEX_CHECK(DumpingEnabledForHloModule(executable->module()));
...
Status Service::Compile(const CompileRequest* arg, CompileResponse* result) {
  ITEX_VLOG(1) << "running compile request";
  ...
  ITEX_VLOG(3) << "Compile created HloModuleConfig computation layout: "
               << module_config->entry_computation_layout().ToString();
  ...
  *result->mutable_handle() = compilation_cache_.Insert(std::move(executable));
  ...
  ITEX_VLOG(1) << "successfully completed 'compile' request";
  return OkStatus();
}

Status Service::Execute(const ExecuteRequest* arg, ExecuteResponse* result) {
  ITEX_VLOG(1) << "running execute request";
  if (!arg->has_handle()) {
  ...
  // ITEX_CHECK that the replicated_arguments has the same shape and layout as
  // the
  ...
  ITEX_VLOG(1) << "successfully completed 'execute' request";
  return OkStatus();
}
...
Status Service::WaitForExecution(const WaitForExecutionRequest* arg,
  ...
  *result->mutable_profile() = execution->profile();
  ...
  ITEX_VLOG(1) << "successfully completed 'wait-for-execution' request";
  return OkStatus();
}
...
Status Service::ComputeConstantGraph(const ComputeConstantGraphRequest* arg,
  ...
  ITEX_DCHECK_OK(ShapeUtil::ValidateShape(program_shape.result()));
