...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_HLO_MODULE_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_HLO_MODULE_H_
...
#include "itex/core/compiler/xla/service/hlo_module_metadata.h"
...
class HloModule {
 public:
  ...
  HloComputation* entry_computation() const {
    ITEX_CHECK_NE(nullptr, entry_computation_);
  ...
  //
  itex::gtl::iterator_range<UnwrappingIterator<
  ...
  itex::gtl::iterator_range<FilteringUnwrappingIterator<
      std::vector<std::unique_ptr<HloComputation>>::const_iterator,
  ...
  HloComputation* mutable_computation(int64_t idx) {
    ITEX_CHECK(idx >= 0 && idx < computations_.size());
  ...
  // Returns the schedule of the module. ITEX_CHECK fails if no schedule is set.
  ...
  const std::vector<HloSharding>& spmd_parameters_shardings() const {
    ITEX_CHECK(spmd_parameters_shardings_.has_value());
  ...
  const HloSharding& spmd_output_sharding() const {
    ITEX_CHECK(spmd_output_sharding_.has_value());
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_HLO_MODULE_H_
