...
void ScopedShapedBuffer::Deallocate() {
  ...
  for (auto& pair : buffers_) {
    ...
    if (!memory_base.is_null() &&
      ...
      ITEX_CHECK_OK(allocator_->Deallocate(device_ordinal(), memory_base));
    }
  }
}

ScopedShapedBuffer ScopedShapedBuffer::TakeSubTree(ShapeIndexView index) {
  const itex_xla::Shape& sub_on_device_shape =
      itex_xla::ShapeUtil::GetSubshape(on_device_shape(), {index});
