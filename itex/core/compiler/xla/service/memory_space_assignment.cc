...
#include "itex/core/compiler/xla/service/memory_space_assignment.h"
...
#include "itex/core/compiler/xla/service/memory_space_assignment_tuning_utils.h"
#include "itex/core/compiler/xla/service/memory_space_assignment_utils.h"
#include "itex/core/compiler/xla/service/tuple_util.h"
...
FindCrossProgramPrefetchCandidate(const HloAliasAnalysis& alias_analysis,
  ...
  for (const HloBuffer& buffer : alias_analysis.buffers()) {
    ITEX_CHECK_GE(buffer.values().size(), 1);
  ...
  ITEX_VLOG(3) << "Cross-program prefetch candidate picked: "
...
Status InsertInstructionAndEnsureOperandsInserted(
  ...
  ITEX_VLOG(4) << "inserting: " << new_instruction->ToShortString();
...
int MemorySpaceAssignmentCostAnalysis::CalculateComputationNestLevel(
  ...
  while (!computation->IsEntryComputation()) {
    ...
    ITEX_CHECK_EQ(callsites.size(), 1) << "The module is not flattened!";
...
int64_t InstructionCountPrefetchIntervalPicker::Next() {
  ITEX_CHECK(!Done())
      << "Prefetch interval picker's Next() is called even though "
         "Done() is false";
...
CostAnalysisPrefetchIntervalPicker::CostAnalysisPrefetchIntervalPicker(
  ...
  ITEX_CHECK_EQ(size, while_nest_level_.size());
...
void CostAnalysisPrefetchIntervalPicker::Begin(const HloUse& use,
  ...
  ITEX_VLOG(4) << "Interval min/max/preferred = " << min_interval << " "
...
float CostAnalysisPrefetchIntervalPicker::GetLogicalIntervalElapsed(
    int64_t start_time, int64_t end_time) const {
  ITEX_CHECK_LE(start_time, end_time);
...
AlternateMemoryBestFitHeap::AlternateMemoryBestFitHeap(
  ...
  if (options.cost_analysis) {
    ...
    for (int i = 0; i < flattened_instructions.size(); ++i) {
      ...
      ITEX_VLOG(2) << "Initial resource[" << i << "] = " << initial_resources[i]
                   << " (" << inst->name() << ")";
    }
  }
...
void AlternateMemoryBestFitHeap::CreateAllocationValues(
  ...
  ITEX_VLOG(3) << "Creating AllocationValues for: " << value->ToString();
  ...
  for (const HloUse& use : uses) {
    ...
    ITEX_CHECK(last_allocation_value != nullptr);
  ...
  for (int i = beginning_idx; i < allocation_values.size(); ++i) {
    ...
    if (HloDataflowAnalysis::IsAsynchronousOperationStart(
            allocation_value.defining_instruction()->opcode())) {
      ITEX_CHECK_EQ(allocation_value.uses().size(), 1);
      ITEX_CHECK(HloDataflowAnalysis::IsAsynchronousOperationDone(
          allocation_value.uses().at(0).hlo_use.instruction->opcode()));
      ITEX_VLOG(3) << "Mark " << allocation_value.ToShortString()
                   << " to require contiguous allocation.";
      allocation_value.set_requires_contiguous_allocation(true);
    }
    ITEX_VLOG(3) << "Created allocation value: "
  ...
  auto maybe_add_alias_with_instruction = [&](const HloInstruction* instruction,
    ...
    if (aliased_values_it != values_by_defining_inst.end()) {
      for (const AllocationValue* aliased_value : aliased_values_it->second) {
        ITEX_VLOG(3) << "Adding aliasing for use " << use->hlo_use.ToString()
  ...
  for (AllocationValue& value : *allocation_values) {
    for (AllocationValue::Use& use : value.uses()) {
      ...
      if (use.hlo_use.instruction->opcode() == HloOpcode::kWhile) {
        ...
        ITEX_VLOG(3) << "Adding while body root aliasing for use "
...
bool AlternateMemoryBestFitHeap::IsUseAllowedInAlternateMemory(
  ...
  if (use.instruction->opcode() == HloOpcode::kWhile) {
    ...
    if (min_use_time == root_time) {
      ITEX_VLOG(4) << "While allocation not allowed in alternate memory. "
    ...
    if (existing_required_assignment &&
      ...
      ITEX_VLOG(4)
          << "While allocation not allowed in alternate memory because "
    ...
    for (const AllocationValue::Use& other_use : value.uses()) {
      ...
      if (options_.prefetch_interval_picker->CanAllocateInAlternateMemoryNoCopy(
        ...
        ITEX_VLOG(4)
            << "Conditional allocation allowed in alternate memory for "
               "computation = "
        ...
        ITEX_VLOG(4)
            << "Conditional allocation not allowed in alternate memory for "
               "computation = "
            << called_computation->name()
            << ", parameter time = " << parameter_time
            << ", min use time = " << min_use_time;
...
HeapSimulator::Result<HloValue> AlternateMemoryBestFitHeap::Finish() {
  if (options_.autotuning_config.has_value()) {
    ITEX_CHECK_EQ((*options_.autotuning_config).size(),
                  buffer_intervals_.size());
  }
  ...
  ITEX_VLOG(1) << "Memory pressure = " << memory_pressure_;
  ...
  ITEX_VLOG(1) << "Assigning buffers to alternate memory. Max heap size = "
               << options_.max_size_in_bytes;

  AddInputAndOutputRequiredAssignments();

  if (ITEX_VLOG_IS_ON(3)) {
    ITEX_VLOG(3) << "Flattened instruction sequence:";
  ...
  ITEX_VLOG(2) << "Total reserved bytes = " << reserved_in_bytes_;
  ...
  for (auto& interval : sorted_buffer_intervals) {
    ...
    if (inst->opcode() == HloOpcode::kParameter &&
      ...
      ITEX_VLOG(3) << "Skip " << interval.buffer->ToShortString()
    ...
    if (AreIntervalsReservedInAlternateMemory(colocated_intervals)) {
      ITEX_VLOG(3) << "Interval " << interval.buffer->ToShortString()
      ...
      for (const BufferInterval* colocated_interval : colocated_intervals) {
        ...
        for (auto& position : value->positions()) {
          ITEX_VLOG(4) << "Coloring " << position.ToString();
          Shape* shape = ShapeUtil::GetMutableSubshape(
              position.instruction->mutable_shape(), position.index);
          ITEX_CHECK(shape->IsArray())
              << "Coloring a shape that is not an array: "
              << position.ToString();
    ...
    if (colocated_intervals.size() > 1 &&
      ...
      ITEX_VLOG(4) << "Not allocating " << interval.buffer->ToShortString()
    ...
    if (options_.dump_fn != nullptr || ITEX_VLOG_IS_ON(3)) {
    ...
    for (int retry_number = 0; retry_number < options_.max_retries;
      ...
      if (result_requires_uncommit(result)) {
        ...
        ITEX_VLOG(2) << "Couldn't allocate. Retry number " << retry_number;
        ...
        ++num_repacks_;
        repacked = true;
        ITEX_CHECK_NE(options_.repacker, nullptr);
        ...
        ITEX_VLOG(2) << "Repacking.";
        auto repack_status =
            options_.repacker->Repack(absl::MakeSpan(repack_allocation_blocks));
        ITEX_CHECK_EQ(repack_status.status(), Status::OK());
        ITEX_VLOG(2) << "Repack complete. Modified = " << *repack_status;
        if (*repack_status) {
  ...
  ITEX_VLOG(3) << "Debug buffer info: ";
  ITEX_XLA_VLOG_LINES(3, buffer_info_str_);
  ITEX_VLOG(3) << "Debug allocation info: ";
  ITEX_XLA_VLOG_LINES(3, allocation_info_str_);
  ...
  for (const BufferInterval* colocated_interval : colocated_intervals) {
    ...
    for (const auto& position : value->positions()) {
      if (position.instruction->opcode() == HloOpcode::kConditional) {
        ITEX_VLOG(3) << "Adding required assignment for condition output: "
                     << value->ToShortString();
  ...
  for (int i = 0; i < allocation_values.size() - 1; ++i) {
    for (int j = i + 1; j < allocation_values.size(); ++j) {
      ...
      if (create_instruction_vector(allocation_value_1) ==
        ...
        ITEX_VLOG(3) << "Allocation values "
                     << allocation_value_1.ToShortString() << " and "
                     << allocation_value_2.ToShortString()
    ...
    for (int use_idx = 0; use_idx < allocation_value.uses().size(); ++use_idx) {
      ...
      if (!IsUseAllowedInAlternateMemory(allocation_value, hlo_use)) {
        ...
        if (previous_use.hlo_use.instruction->opcode() ==
          ...
          ITEX_VLOG(3) << "Previous use (" << previous_use.hlo_use.ToString()
      ...
      if (hlo_use.instruction->opcode() == HloOpcode::kWhile &&
        ...
        if (options_.enable_while_redundant_eviction_elimination &&
          ...
          if (prev_allocation_in_default_mem_it !=
            ...
            ITEX_VLOG(3)
                << "Found a prev allocation in default mem for while use: "
            ...
            ITEX_CHECK_NE(body_allocation_value_it, allocation_values.end());
            ITEX_VLOG(3) << "Body allocation value: "
            ...
            ITEX_CHECK_NE(after_while_allocation_value_it,
                          allocation_values.end());
            ITEX_VLOG(3) << "After while allocation value: "
...
bool AsynchronousCopyResource::ConsumeResource(
  ...
  ITEX_VLOG(3) << "Consume resource: " << start_time << ", " << end_time << ", "
  ...
  if (resource > 0) {
    ITEX_VLOG(3) << "Doesn't have enough resource; leftover resource = "
                 << resource;
    return false;
  }
...
void AsynchronousCopyResource::AddCopy(const AsynchronousCopy& copy) {
  ITEX_CHECK(ConsumeResource(copy.start_time, copy.end_time, copy.resource,
...
void AsynchronousCopyResource::RemoveCopy(const AsynchronousCopy& copy) {
  ITEX_CHECK(ConsumeResource(copy.start_time, copy.end_time, /*resource=*/0,
                             /*update_current_resource=*/true,
  ...
  ITEX_CHECK(async_copy_time_it != async_copy_time_map_.end());
  auto it = async_copy_time_it->second;
  for (; it != async_copies_.end() && *it != copy; ++it) {
  }
  ITEX_CHECK(it != async_copies_.end());
...
AlternateMemoryBestFitHeap::GetAliasedOffset(
  ...
  ITEX_CHECK(aliased_offset_it != aliased_offset_map_.end());
...
void AlternateMemoryBestFitHeap::CreateOrAddToAliasedOffset(
  ...
  ITEX_CHECK(allocation.memory_space() == MemorySpace::kAlternate);
  ITEX_CHECK(!aliased_offset_map_.contains(&allocation));
  if (!aliased_offset) {
    aliased_offsets_.push_back({allocation.chunk().offset});
    aliased_offset = &aliased_offsets_.back();
  }
  ITEX_CHECK_EQ(allocation.chunk().offset, aliased_offset->offset);
  ITEX_CHECK(aliased_offset->allocations.insert(&allocation).second);
...
void AlternateMemoryBestFitHeap::AllocateCrossProgramPrefetchBuffer(
  ...
  ITEX_VLOG(2) << "last use time = " << last_use_time
  ...
  ITEX_VLOG(2) << "Total execution time = " << total_execution_time
  ...
  if (free_buffer) {
    ITEX_VLOG(2) << "Adding an end-of-program prefetch for freed "
    ...
    AddAsyncCopy(*allocations.front(), MemorySpace::kAlternate, chunk_candidate,
                 ...
                 /*resource=*/0.0);
    ITEX_CHECK_EQ(cross_program_prefetch_offset->offset,
...
void AlternateMemoryBestFitHeap::AllocateReservedScopedAllocations() {
  ...
  for (int i = 0; i < instruction_sequence.size(); ++i) {
    ...
    if (reserved_scoped_memory != 0) {
      ITEX_VLOG(1) << "Allocate reserved scoped memory at " << i << " ("
                   << instruction_sequence[i]->name()
      ...
      ITEX_CHECK_EQ(chunk_candidate.offset, 0);
  ...
  if (required_assignment_it != required_assignments_.end()) {
    for (const RequiredMemoryAssignment& required_assignment :
      ...
      if (required_assignment.time == time) {
        ...
        ITEX_CHECK(!required_assignment_at_time);
...
AlternateMemoryBestFitHeap::AliasedRequiredAssignmentForUse(
  ...
  for (const HloPosition& position : use.aliases) {
    ...
    if (required_assignment == absl::nullopt) {
      ...
      ITEX_CHECK(required_assignment_for_alias == absl::nullopt ||
...
void AlternateMemoryBestFitHeap::AddRequiredAssignment(
  ...
  if (existing_required_assignment) {
    ITEX_CHECK(memory_space == existing_required_assignment->memory_space)
        << "inst = " << instruction->ToString() << " at " << time;
    ITEX_CHECK((!offset && !existing_required_assignment->offset) ||
               offset == existing_required_assignment->offset);
    ITEX_VLOG(3)
        << "Not adding required assignment because there is one already: "
        << value->ToShortString() << " at " << time << " at "
        << (memory_space == MemorySpace::kDefault ? "def" : "alt");
  } else {
    ITEX_VLOG(3) << "Adding required assignment: " << value->ToShortString()
                 << " at " << time << " at "
                 << (memory_space == MemorySpace::kDefault ? "def" : "alt");
    ...
    ShapeUtil::ForEachSubshape(
        ...
        [&](const Shape& subshape, const ShapeIndex& index) {
          ...
          for (const HloBuffer* buffer :
            ...
            for (const HloValue* value : buffer->values()) {
              ITEX_VLOG(3)
                  << "Adding required assignment for parameter value = "
                  << value->ToShortString()
  ...
  ShapeUtil::ForEachSubshape(
      ...
      [&](const Shape& subshape, const ShapeIndex& index) {
        ...
        for (const HloBuffer* buffer :
          ...
          for (const HloValue* value : buffer->values()) {
            ITEX_VLOG(3) << "Adding required assignment for output value = "
                         << value->ToShortString()
      ...
      if (instruction->opcode() == HloOpcode::kConstant) {
        ...
        for (const auto& indexed_shape :
          ...
          for (const HloBuffer* buffer :
            ...
            for (const HloValue* value : buffer->values()) {
              ITEX_VLOG(3) << "Adding required assignment for constant value = "
                           << value->ToShortString()
...
void AlternateMemoryBestFitHeap::ImportRepackedAllocations() {
  ...
  for (RepackAllocationBlock& allocation_block : repack_allocation_blocks_) {
    ...
    ITEX_VLOG(3) << "Moved " << allocation->ToString() << ", size "
...
void AlternateMemoryBestFitHeap::UncommitPendingChunks(
  ...
  for (const auto& interval_and_chunk : pending_chunks_) {
    ...
    ITEX_VLOG(3) << "Uncommitting: (" << interval.start << ", " << interval.end
                 << ") off = " << chunk.offset << " size = " << chunk.size;
    for (int i = interval.start; i <= interval.end; ++i) {
      peak_memory_usage_[i] -= chunk.size;
      ITEX_CHECK_GE(peak_memory_usage_[i], 0)
...
void AlternateMemoryBestFitHeap::AddToPendingChunks(
  ...
  ITEX_VLOG(3) << "Committing chunk: " << buffer_interval.start << "-"
  ...
  for (int i = buffer_interval.start; i <= buffer_interval.end; ++i) {
    ...
    ITEX_CHECK_LE(peak_memory_usage_[i], options_.max_size_in_bytes)
        << "Peak memory usage at " << i
...
AlternateMemoryBestFitHeap::Result AlternateMemoryBestFitHeap::AllocateSegment(
  ...
  if (request.start_time == request.end_time) {
    ...
    ITEX_CHECK_NE(allocation, nullptr);
  ...
  ITEX_VLOG(2) << "Finding allocation for "
  ...
  ITEX_CHECK_LE(request.start_time, request.end_time);
  if (ITEX_VLOG_IS_ON(3) && options_.cost_analysis) {
    ITEX_VLOG(3) << "Definition benefit = "
  ...
  if (required_assignment_at_end != aliased_required_assignment_at_end) {
    if (required_assignment_at_end == absl::nullopt) {
      ...
      ITEX_CHECK(aliased_required_assignment_at_end == absl::nullopt ||
  ...
  ITEX_CHECK(prev_allocation_in_default_mem_it != allocation_sequence->rend());
  ITEX_CHECK((*prev_allocation_in_default_mem_it)->memory_space() ==
             MemorySpace::kDefault);
...
void AlternateMemoryBestFitHeap::AddAsyncCopy(
  ...
  ITEX_VLOG(3) << "Copy to "
               ...
               << copy_done_schedule_before_time << " keeping until "
               << end_time << ", estimated copy resource is " << resource;
  ITEX_CHECK_LT(start_time, copy_done_schedule_before_time);
...
AlternateMemoryBestFitHeap::AllocateInAlternateMemoryNoCopy(
  ...
  ITEX_VLOG(3)
      << "We can eliminate copy to alternate memory. Preferred offset = "
  ...
  if (chunk_candidate) {
    ITEX_VLOG(3) << "Keep the buffer in alternate memory. Offset = "
                 << chunk_candidate->offset
                 << ", size = " << chunk_candidate->size << ", heap_size = "
                 << result_.UpdatedHeapSize(*chunk_candidate)
                 << ", prefetch picker = "
...
AlternateMemoryBestFitHeap::Result AlternateMemoryBestFitHeap::Evict(
    const AllocationRequest& request) {
  ITEX_CHECK_GT(request.allocation_value->allocation_sequence()->size(), 0);
  ...
  ITEX_CHECK(eviction_start_time <= eviction_end_time);
  ...
  ITEX_VLOG(3) << "Eviction (" << eviction_start_time << ", "
               << eviction_end_time
  ...
  ITEX_VLOG(3) << "Evicting buffer at " << prev_allocation->chunk().offset
  ...
  if (eviction_violates_resource) {
    ...
    if (options_.prefetch_interval_picker->retry_number() ==
      ...
      ITEX_VLOG(3) << "Violates resource in last retry, setting resource = 0";
      eviction_resource = 0;
    }
  ...
  if (!eviction_interval_too_short && !eviction_violates_outstanding_copies &&
    ...
    if (eviction_violates_outstanding_copies) {
      ITEX_VLOG(3) << "This violates the maximum async copies.";
    } else if (eviction_violates_resource) {
      ITEX_VLOG(3) << "This violates resource.";
    } else {
      ITEX_VLOG(3) << "Eviction interval is too short (" << eviction_start_time
                   << ", " << eviction_end_time << ").";
    }
    ...
    if (!eviction_scheduled) {
      ...
      ITEX_VLOG(3)
          << "Bailing: Could not evict " << request.use->hlo_use.ToString()
...
AlternateMemoryBestFitHeap::Result AlternateMemoryBestFitHeap::Prefetch(
  ...
  if (!earliest_non_oom_prefetch_time) {
    ITEX_VLOG(3) << "Any prefetch in range (" << earliest_prefetch_time << ", "
                 << prefetch_end_time << ") for size " << request.size
                 << " would go out of memory.";
    result_mark(Result::kFailOutOfMemory, result);
    return result;
  }
  ITEX_VLOG(4) << "After peak memory check, prefetch range is ("
  ...
  ITEX_VLOG(3) << "Trying prefetch picker = "
  ...
  if (!chunk_candidate) {
    ITEX_VLOG(3) << "The latest prefetch (" << alternate_mem_interval.start
                 << ", " << request.end_time
                 << ") cannot find a valid chunk. Giving up.";
    result_mark(Result::kFailOutOfMemory, result);
    return result;
  }
  ...
  while (!options_.prefetch_interval_picker->Done()) {
    ...
    ITEX_CHECK_LT(alternate_mem_interval.start, prefetch_end_time);
    if (out_of_mem_start.has_value() &&
        alternate_mem_interval.start <= *out_of_mem_start) {
      ITEX_VLOG(4) << "This would OOM (cached).";
      result_mark(Result::kFailOutOfMemory, result);
      continue;
    }
    ...
    ITEX_VLOG(4) << "Trying alternate memory allocation ("
    ...
    if (!prefetch_async_copy_resource_.HasEnoughResource(
            ...
            prefetch_resource)) {
      ITEX_VLOG(4) << "This would violate asynchronous copy resource = "
                   << prefetch_resource;
    ...
    if (ViolatesMaximumOutstandingAsyncCopies(
      ...
      ITEX_VLOG(4) << "This would violate the outstanding async copy limit.";
    ...
    if (chunk_candidate) {
      ITEX_VLOG(3) << "Move the buffer to alternate memory at "
                   << alternate_mem_interval.start
...
AlternateMemoryBestFitHeap::FindBestChunkCandidate(
  ...
  if (!preferred_offset) {
    ...
    ITEX_CHECK(use_time_it != use_times.end());
    ...
    if (last_chunk_candidate.has_value()) {
      ITEX_VLOG(3) << "FindBestChunkCandidate earliest use = " << earliest_use
...
MemorySpaceAssignment::Run(HloModule* module,
                           ...
                           const Options& options) {
  ITEX_CHECK(module->has_schedule());
  ITEX_VLOG(3) << "Module before memory space assignment: ";
  ITEX_XLA_VLOG_LINES(3, module->ToString());
  ITEX_VLOG(3) << "Schedule: " << module->schedule().ToString();
...
MemorySpaceAssignment::RunMemorySpaceAssignment(
  ...
  if (options_.cost_analysis) {
    ...
    ITEX_VLOG(1) << "Estimated elapsed time (sec): " << estimated_time;
  }
  ...
  ITEX_VLOG(3) << "Module after memory space assignment: ";
  ITEX_XLA_VLOG_LINES(3, module_->ToString());
  ITEX_CHECK_OK(module_->schedule().Verify());
  TF_ASSIGN_OR_RETURN(AsyncCopyStats stats, CalculateAsyncCopyStats());
  ITEX_VLOG(1) << "Maximum number of outstanding async copies: "
               << stats.max_outstanding_async_copies;
  ITEX_VLOG(1) << "Number of prefetches: " << stats.num_prefetches
               << ", in bytes: " << stats.prefetch_bytes;
  ITEX_VLOG(1) << "Number of evictions: " << stats.num_evictions
...
Status MemorySpaceAssignment::Allocation::Process() {
  ...
  for (const HloUse& use : uses_) {
    ...
    if (operand_shape.IsTuple()) {
      ...
      ITEX_VLOG(4) << "Old shape = " << operand_shape.ToString()
                   << ", new shape = "
                   << producing_instruction->shape().ToString()
                   << "; inserting a bitcast.";
...
StatusOr<HloInstruction*> MemorySpaceAssignment::Allocation::ReplaceTupleWith(
  ...
  ITEX_CHECK(tuple->shape().IsTuple())
  ...
  if (equivalent && instruction == tuple) {
    ITEX_VLOG(4) << "Instruction " << new_instruction->ToShortString()
                 << " already exists at index " << shape_index.ToString()
                 << " of " << tuple->ToShortString();
    return tuple;
  }

  HloComputation* computation = new_instruction->parent();
  std::vector<HloInstruction*> tuple_args(tuple_shape.tuple_shapes_size());
  ITEX_CHECK_GE(tuple_shape.tuple_shapes_size(), shape_index[0]);
    ...
    if (i == shape_index[0]) {
      ...
      if (subshape.IsTuple()) {
        ...
        if (subshape != new_instruction->shape()) {
          ITEX_VLOG(4) << "Old shape = " << subshape.ToString()
                       << ", new shape = "
                       << new_instruction->shape().ToString()
                       << "; inserting a bitcast.";
          ...
          // tuple.
          ITEX_VLOG(4) << "Tuple already contains the new instruction = "
...
HloInstruction* MemorySpaceAssignment::Allocation::AddGetTupleElements() const {
  ...
  ITEX_CHECK_NE(producing_instruction, nullptr);

  Shape shape = defining_position().shape();
  ITEX_CHECK(shape.IsArray())
      << "Allocation shape is not an array. Shape = " << shape.ToString()
...
Status MemorySpaceAssignment::CopyAllocation::Process() {
  ...
  ITEX_VLOG(4) << "Created " << copy_start_->name()
...
Status MemorySpaceAssignment::Process() {
  ITEX_VLOG(1) << "Processing assigned buffers...";
  ...
  for (auto& allocation : allocations_) {
    ITEX_VLOG(3) << "Processing: " << allocation->ToString();
    if (!needed_allocations.contains(allocation.get())) {
      ITEX_VLOG(3) << "Allocation not needed.";
      continue;
    }
  ...
  for (auto& allocation : allocations_) {
    if (needed_allocations.contains(allocation.get())) {
      ITEX_VLOG(3) << "Post-Processing: " << allocation->ToString();
...
Status MemorySpaceAssignment::ExportAndColorBuffers() {
  ...
  ITEX_VLOG(3) << "Exported alternate memory allocations:";
  ...
  ITEX_VLOG(3) << "Exported scoped allocations in alternate memory:";
  ...
  ITEX_VLOG(3) << "Exported alternate memory sizes:";
  for (auto& pair : preset_assignments_->assignment_informations()) {
    ITEX_VLOG(3) << "  space: " << pair.first << ", size: " << pair.second.size;
  }

  ITEX_VLOG(1) << "Coloring buffers...";
...
void MemorySpaceAssignment::RemoveAssignmentForInstruction(
  ...
  while (it != end) {
    ...
    if (position.instruction == instruction) {
      ITEX_VLOG(3) << "Removing instruction from alternate memory assignments.";
      if (std::next(it) == end) {
...
Status MemorySpaceAssignment::SimplifyGraph() {
  ...
  for (HloComputation* computation : module_->MakeNonfusionComputations()) {
    ...
    if (!computations_in_schedule_.contains(computation)) {
      ITEX_VLOG(4) << "Not simplifying " << computation->name()
    ...
    while (computation_modified) {
      ...
      ITEX_VLOG(4) << "Running simplify graph loop over "
                   << computation->name();
      for (HloInstruction* instruction :
           computation->MakeInstructionPostOrder()) {
        if (computation->IsSafelyRemovable(instruction) &&
          ...
          ITEX_VLOG(4) << "Instruction removed: " << instruction->ToString();
          ...
          if (operand->opcode() == HloOpcode::kTuple) {
            ...
            ITEX_VLOG(4) << "Replacing uses of " << instruction->ToString()
...
void MemorySpaceAssignment::ScheduleAsynchronousCopies() {
  ITEX_VLOG(1) << "Scheduling asynchronous copies...";
  for (MemorySpace memory_space :
    ...
    for (CopyAllocation* copy_allocation : copy_allocations) {
      ...
      while (copy_allocation->defining_position().instruction->parent() !=
        ...
        ITEX_VLOG(4) << "Delaying CopyStart (" << copy_start_schedule_after
...
Status MemorySpaceAssignment::FixSchedule() {
  ...
  for (const HloComputation* computation :
    ...
    // modified.
    if (!computations_in_schedule_.contains(computation)) {
      ITEX_VLOG(4) << "Not scheduling " << computation->name()
                   << " because it's not in the schedule.";
      continue;
    }
    ...
    ITEX_VLOG(4) << "Scheduling: " << computation->ToString();
    ...
    for (int64_t instruction_index = 0;; ++instruction_index) {
      ...
      if (insts_before_iter != schedule_before_.end()) {
        for (HloInstruction* new_instruction : insts_before_iter->second) {
          if (new_instruction->parent() == computation) {
            ITEX_VLOG(4) << "before " << instruction_index << ": "
      ...
      if (instruction != nullptr && instruction->parent() == computation &&
        ...
        ITEX_VLOG(4) << "inst " << instruction_index << ": "
                     << instruction->name();
        TF_RETURN_IF_ERROR(InsertInstructionAndEnsureOperandsInserted(
      ...
      if (insts_after_iter != schedule_after_.end()) {
        for (HloInstruction* new_instruction : insts_after_iter->second) {
          if (new_instruction->parent() == computation) {
            ITEX_VLOG(4) << "after " << instruction_index << ": "
                         << new_instruction->name();
            TF_RETURN_IF_ERROR(InsertInstructionAndEnsureOperandsInserted(
                new_instruction, &new_sequence, &inserted_instructions));
          }
    ...
    ITEX_CHECK_EQ(new_sequence.size(), computation->instruction_count())
...
Status MemorySpaceAssignment::VerifyAndExportHeapSimulatorTrace() {
      ...
      if (instruction->opcode() == HloOpcode::kCopyStart) {
        ...
        int64_t to_memory_space =
            ShapeUtil::GetSubshape(instruction->shape(), {0})
                .layout()
                .memory_space();
        ITEX_CHECK_NE(from_memory_space, to_memory_space)
  ...
  for (const auto& position_and_chunk : preset_assignments_->chunks()) {
    ...
    ITEX_CHECK(!seen_buffers.contains(buffer.id()))
    ...
    for (const HloValue* value : buffer.values()) {
      ...
      split_conditional_buffer = [&](const HloInstruction* use_instruction,
        ...
        ITEX_VLOG(3) << indent_string
                     << " from beginning until first computation: ("
                     << start_time << ", "
      ...
      if (last_use_instruction &&
        ...
        ITEX_VLOG(3) << " buffer: " << buffer.ToString()
                     << " value: " << value->ToShortString() << ": ("
  ...
  for (const auto& event : events) {
    ...
    if (kind == HeapSimulatorTrace::Event::ALLOC) {
      ...
      ITEX_CHECK_EQ(kind, HeapSimulatorTrace::Event::FREE);
      memory_usage -= chunk.size;
    }
    max_memory_usage = std::max(max_memory_usage, memory_usage);
    ITEX_VLOG(4) << "Memory usage: " << memory_usage << " at time: " << time;
  }
  ITEX_VLOG(1) << "Max memory usage ignoring fragmentation: "
               << max_memory_usage;
