...
#include "itex/core/compiler/xla/service/mapped_ptr_container_sorter.h"
...
StatusOr<std::unique_ptr<HloInstruction>> HloInstruction::CreateFromProto(
  ...
  switch (opcode) {
    ...
    case HloOpcode::kConstant: {
      // TODO(b/110214922): Revert this to ITEX_CHECK(proto.has_literal()).
    ...
    case HloOpcode::kFusion: {
      ...
      auto* fused_computation =
          itex::gtl::FindPtrOrNull(computation_map, fusion_id);
    ...
    case HloOpcode::kCollectivePermuteStart: {
      ...
      if (proto.dynamic_slice_sizes_size() == 0) {
        if (opcode == HloOpcode::kCollectivePermute) {
          ...
          ITEX_LOG(FATAL)
              << "Expect CollectivePermute or CollectivePermuteStart, "
          ...
          if (input_start_indices->shape()
            ...
            for (int i = 0; i < input->shape().tuple_shapes_size(); ++i) {
              ...
              for (int j = 0;
                ...
                ITEX_CHECK_GE(proto.dynamic_slice_sizes_size(), proto_index);
    ...
    case HloOpcode::kDynamicSlice: {
      ...
      TF_RET_CHECK(proto.operand_ids_size() >= 1)
          << "DynamicSlice instruction should have at least 1 operands but "
             "sees "
          << proto.operand_ids_size();
      // TODO(b/118437727): Old form, make the ITEX_CHECK unconditional.
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateUnary(
  ...
  switch (opcode) {
    ...
    case HloOpcode::kTanh:
      break;
    default:
      ITEX_LOG(FATAL) << "Invalid unary instruction opcode "
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateBinary(
  ...
  switch (opcode) {
    ...
    case HloOpcode::kStochasticConvert:
      break;
    default:
      ITEX_LOG(FATAL) << "Invalid binary instruction opcode "
                      << HloOpcodeString(opcode);
  }
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateTernary(
  ...
  switch (opcode) {
    ...
    case HloOpcode::kSelect:
      break;
    default:
      ITEX_LOG(FATAL) << "Invalid ternary instruction opcode "
                      << HloOpcodeString(opcode);
  }
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateVariadic(
    const Shape& shape, HloOpcode opcode,
    absl::Span<HloInstruction* const> operands) {
  ITEX_CHECK_EQ(HloOpcode::kTuple, opcode);
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateReplicaId(
    const Shape& shape) {
  ITEX_CHECK(
      Shape::Equal().IgnoreLayout()(shape, ShapeUtil::MakeShape(U32, {})))
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateSendDone(
  ...
  ITEX_CHECK(send_operand != nullptr)
      << "SendDone must take the context operand from Send";
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateRecvDone(
  ...
  ITEX_CHECK(recv_operand != nullptr)
      << "RecvDone must take the context operand from Recv";
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateConditional(
  ...
  ITEX_CHECK_EQ(branch_computations.size(), branch_computation_args.size());
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateReduce(
  ...
  if (!tuple_of_instructions->shape().IsTuple()) {
    ITEX_CHECK_EQ(init_values.size(), 1)
...
HloInstruction::CreateBroadcastSequence(
    ...
    const std::function<HloInstruction*(std::unique_ptr<HloInstruction>)>&
        adder) {
  ITEX_CHECK(ShapeUtil::IsScalar(operand->shape()) ||
  ...
  for (int i = 0; i < operand->shape().rank(); i++) {
    if (operand->shape().dimensions(i) == output_shape.dimensions(i)) {
      ...
      ITEX_CHECK_EQ(operand->shape().dimensions(i), 1)
...
/* static */ std::unique_ptr<HloInstruction> HloInstruction::CreateReshape(
  ...
  ITEX_CHECK_EQ(ShapeUtil::ElementsIn(shape),
...
HloInstruction::CreateDynamicReshape(
  ...
  ITEX_CHECK_EQ(shape.rank(), dim_sizes.size());
...
void HloInstruction::set_single_sharding(const HloSharding& sharding) {
  ITEX_CHECK(!sharding.IsTuple()) << sharding;
  if (shape().IsTuple()) {
...
bool HloInstruction::HasSideEffect() const {
  ...
  // ITEX_CHECK if any of the called computations has a side effect.
...
std::unique_ptr<HloInstruction> HloInstruction::CloneWithNewOperands(
  ...
  ITEX_VLOG(3) << "CloneWithNewOperands:\n  " << ToString();
  ITEX_VLOG(3) << "  new operands:";
  for (const HloInstruction* new_operand : new_operands) {
    ITEX_VLOG(3) << "    %" << new_operand->name();
  }
  ...
  switch (opcode_) {
    ...
    case HloOpcode::kCbrt:
    case HloOpcode::kTanh:
      ITEX_CHECK_EQ(new_operands.size(), 1);
    ...
    case HloOpcode::kShiftRightLogical:
    case HloOpcode::kStochasticConvert:
      ITEX_CHECK_EQ(new_operands.size(), 2);
      clone = CreateBinary(shape, opcode_, new_operands[0], new_operands[1]);
      break;
    // Ternary ops.
    case HloOpcode::kClamp:
    case HloOpcode::kSelect:
      ITEX_CHECK_EQ(new_operands.size(), 3);
    ...
    case HloOpcode::kWhile:
      ...
      break;
    case HloOpcode::kConditional:
      ITEX_CHECK_EQ(new_operands.size(), branch_count() + 1);
    ...
    case HloOpcode::kAddDependency:
      ...
      break;
    case HloOpcode::kReplicaId:
      ITEX_CHECK_EQ(new_operands.size(), 0);
...
void HloInstruction::DetachFromOperandsAndUsers() {
  ...
  // avoid calling RemoveUser twice on the same operand, ITEX_CHECK before
  // remove.
...
HloInstruction* HloInstruction::mutable_operand(int64_t i) {
  ITEX_CHECK(operands_[i] != nullptr);
  return operands_.at(i);
}
...
int64_t HloInstruction::operand_index(const HloInstruction* target) const {
  ...
  ITEX_LOG(FATAL) << "target was not an operand: " << target->ToString();
}
...
bool HloInstruction::IdenticalInternal(
  ...
  // ITEX_CHECK that operands are equal.
  //
...
void HloInstruction::AppendOperand(HloInstruction* operand) {
  if (operand->parent() != nullptr) {
    ITEX_DCHECK(!operand->parent()->IsMarkedAsDead(operand))
...
void HloInstruction::RemoveOperandsAtAscendingIndices(
  ...
  for (int to_remove : ascending_indices) {
    ...
    ITEX_CHECK_LT(to_remove, operands_.size());
    ++removed_count;
    ++next_index;
  }
  ...
  ITEX_CHECK_EQ(removed_count, ascending_indices.size());
...
int64_t HloInstruction::UserId(HloInstruction* user) {
  ...
  ITEX_CHECK(result != user_map_.end());
...
bool HloInstruction::IdenticalSlowPath(
  ...
  // Perform opcode specific ITEX_CHECKs.
  switch (opcode()) {
    ...
    case HloOpcode::kTriangularSolve:
    case HloOpcode::kCholesky:
      ITEX_LOG(FATAL) << "Base class impl called for opcode with subclass: "
                      << opcode();
  }
  return false;
}
...
void HloInstruction::RemoveUser(HloInstruction* user) {
  ...
  ITEX_CHECK(map_it != user_map_.end());

  const int64_t index = map_it->second;
  ITEX_CHECK_EQ(users_[index], user);
...
Status HloInstruction::ReplaceUseWithDifferentShape(
  ...
  ITEX_VLOG(3) << "Replacing uses of " << name() << " in " << user->name()
...
Status HloInstruction::ReplaceUseWithDifferentShape(
  ...
  ITEX_VLOG(3) << "Replacing operand " << operand_number << " of " << name()
...
Status HloInstruction::ReplaceOperandWithDifferentShape(
  ...
  ITEX_VLOG(3) << "Replacing operand " << operand_num << " of " << name()
               << " with " << new_operand->name() << ", was "
               << old_operand->name();
...
HloComputation* HloInstruction::while_condition() const {
  ITEX_CHECK_EQ(HloOpcode::kWhile, opcode_);
...
HloComputation* HloInstruction::true_computation() const {
  ITEX_CHECK_EQ(HloOpcode::kConditional, opcode_);
  ITEX_CHECK_EQ(PRED, operand(0)->shape().element_type());
...
const std::vector<HloComputation*>& HloInstruction::branch_computations()
    const {
  ITEX_CHECK(HloOpcode::kConditional == opcode_);
...
HloComputation* HloInstruction::branch_computation(int b) const {
  ITEX_CHECK(HloOpcode::kConditional == opcode_);
  ITEX_CHECK_GE(b, 0);
  ITEX_CHECK_LT(b, called_computations_.size());
...
std::string HloInstruction::ToStringWithCanonicalNameMap(
  ...
  // ITEX_LOGic to print the instruction name (e.g. "%foo = ").
  if (options.canonicalize_instruction_names()) {
    if (options.is_in_nested_computation()) {
      ...
      ITEX_DCHECK(
          !options.print_percent());  // no need to call PrintNameInternal
    ...
    std::string suffix = [&]() {
      switch (opcode()) {
        ...
        case HloOpcode::kAsyncUpdate:
          return "-update";
        default:
          ITEX_CHECK(opcode() == HloOpcode::kAsyncDone)
  ...
  if (options.print_metadata() &&
    ...
    StrAppend(&result, ", metadata={", itex_xla::OpMetadataToString(metadata_),
              "}");
  }
...
HloInstruction::HloInstruction(HloOpcode opcode, const Shape& shape)
  ...
  ITEX_DCHECK_OK(ShapeUtil::ValidateShapeWithOptionalLayout(shape_));
}
...
inline bool PushDFSChild(Visitor* visitor, DFSStack* dfs_stack,
  ...
  ITEX_CHECK(child != nullptr);
  const int id = child->unique_id();
  ITEX_CHECK_GE(id, 0) << "instruction may not have a parent computation";
...
static Status PostOrderDFS(HloInstruction* root, Visitor* visitor,
  ...
  do {
    ...
    if (visit_state == Visitor::kVisited) {
      dfs_stack.pop_back();
      ITEX_VLOG(3) << "Not visiting HLO (id = " << current_id
    ...
    if (visit_state == Visitor::kVisiting) {
      ...
      ITEX_VLOG(2) << "Visiting HLO %" << current_node->name();
...
Status HloInstruction::Accept(DfsHloVisitorBase<HloInstructionPtr>* visitor,
                              ...
                              bool ignore_control_predecessors) {
  ITEX_VLOG(3) << "HloInstruction::Accept(%" << name() << ")";
  TF_RETURN_IF_ERROR(
...
Status HloInstruction::AcceptWithOperandOrder(
    ...
    bool call_finish_visit) {
  ITEX_VLOG(2) << "HloInstruction::AcceptWithOperandOrder(%" << name() << ")";
  ...
  if (call_finish_visit) {
    ITEX_VLOG(3)
        << "HloInstruction::AcceptWithOperandOrder BEFORE FINISH VISIT";
    TF_RETURN_IF_ERROR(visitor->FinishVisit(this));
    ITEX_VLOG(3) << "HloInstruction::AcceptWithOperandOrder AFTER FINISH VISIT";
  }
  ITEX_VLOG(2) << "HloInstruction::AcceptWithOperandOrder EXIT";
  return OkStatus();
}
...
UseKind FusionReusesParamElements::ComputeInternal(
  ...
  for (int64_t operand_num = 0; operand_num < hlo.operands().size();
    ...
    // have to ITEX_CHECK all the remaining operands.
...
std::string ConvolutionDimensionNumbersToString(
  ...
  // lhs_dims[i] is the symbol of the ITEX_LOGical dimension i for the lhs
...
bool HloPtrComparator::operator()(const HloInstruction* const& lhs,
  ...
  ITEX_CHECK((lhs_module == nullptr && rhs_module == nullptr) ||
...
Status HloInstruction::GetBackendConfigInternal(
    itex::protobuf::Message* proto) const {
  proto->Clear();
  ...
  TF_RETURN_IF_ERROR(itex::HumanReadableJsonToProto(raw_string, proto));
...
HloInstruction::BackendConfigRep& HloInstruction::BackendConfigRep::operator=(
    const itex::protobuf::Message& proto) {
  SetProto(proto);
  raw_string_.clear();
  return *this;
}
...
bool HloInstruction::BackendConfigRep::operator==(
  ...
  if (proto_a != nullptr && proto_b != nullptr) {
    using ::itex::protobuf::util::MessageDifferencer;
...
/* static */ StatusOr<std::string> HloInstruction::BackendConfigToRawString(
  ...
  TF_RETURN_IF_ERROR(itex::ProtoToHumanReadableJson(
...
const PrecisionConfig& HloInstruction::precision_config() const {
  ...
  ITEX_LOG(FATAL) << "Unimplemented method.";
}
...
void HloInstruction::SortInstructionUsersAndControlLists(
  ...
  if (!status.ok()) {
    ITEX_LOG(ERROR) << "Failed to sort instruction users for " << name() << "; "
                    << status;
  }
  user_map_.clear();
  ...
  if (!status.ok()) {
    ITEX_LOG(ERROR) << "Failed to sort instruction control predecessors for "
                    << name() << "; " << status;
  }
  status =
  ...
  if (!status.ok()) {
    ITEX_LOG(ERROR) << "Failed to sort instruction control successors for "
                    << name() << "; " << status;
  }
}
