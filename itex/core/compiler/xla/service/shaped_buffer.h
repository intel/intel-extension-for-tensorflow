...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_SHAPED_BUFFER_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_SHAPED_BUFFER_H_
...
class ShapedBuffer {
 public:
  ...
  explicit ShapedBuffer(const ScopedShapedBuffer&) = delete;
  ...
  void set_buffers(ShapeTree<se::DeviceMemoryBase> buffers) {
    ITEX_CHECK(ShapeUtil::Equal(buffers.shape(), on_device_shape_));
  ...
  void set_shapes(const Shape& on_device_shape) {
    ITEX_CHECK(ShapeUtil::EqualStructure(on_device_shape, on_device_shape_))
...
class ScopedShapedBuffer : public ShapedBuffer {
 public:
  ...
  void set_buffer(se::OwningDeviceMemory buffer, const ShapeIndex& index) {
    if (!buffer.is_null()) {
      ITEX_CHECK_EQ(buffer.device_ordinal(), device_ordinal());
      ITEX_CHECK_EQ(buffer.allocator(), allocator_);
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_SHAPED_BUFFER_H_
