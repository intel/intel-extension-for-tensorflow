...
StatusOr<std::vector<HloInstruction*>> GatherLoopBody(
  ...
  ITEX_CHECK_EQ(incoming_loop_state.size(), 3);
  ...
  ITEX_CHECK_EQ(has_scalar_indices,
...
StatusOr<HloInstruction*> GatherExpander::ExpandInstruction(
  ...
  ITEX_CHECK(!ShapeUtil::IsZeroElementArray(gather_instr->shape()));
  ...
  ITEX_CHECK_EQ(gather_loop_trip_count,
