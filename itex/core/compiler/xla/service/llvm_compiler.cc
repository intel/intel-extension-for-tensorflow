...
StatusOr<std::vector<std::unique_ptr<Executable>>> LLVMCompiler::Compile(
  ...
  // //
  // //  - Denormals are zero (DAZ): roughly, operations treat denormal floats
  // as
  // //    zero.
  // //  - Flush denormals to zero (FTZ): roughly, operations produce zero
  // instead
  // //    of denormal floats.
  // //
  // // In theory enabling these shouldn't matter since the compiler should
  // ideally
  // // not leak its environment into generated code, but we turn off DAZ and
  // FTZ
  // // to get some defense-in-depth.
  // itex::port::ScopedDontFlushDenormal dont_flush_denormals;
