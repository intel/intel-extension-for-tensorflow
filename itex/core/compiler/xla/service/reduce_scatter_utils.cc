...
#include "itex/core/compiler/xla/service/reduce_scatter_utils.h"
...
int64_t GetIndexForId(const HloInstruction* index, int64_t id,
  ...
  if (!IsTableLookup(index)) {
    ITEX_VLOG(2) << "Index is not table lookup " << index->ToString();
    return -1;
  }
  ...
  if (inner_index < 0) {
    ITEX_VLOG(2) << "Failed to get inner index.";
    return -1;
  }
...
bool IsPerIdOffset(const HloInstruction* offset, int64_t shard_size,
  ...
  if (offset->opcode() == HloOpcode::kMultiply) {
    ...
    if (offset->shape().rank() != 0) {
      ITEX_VLOG(2) << "Offset is not a scalar " << offset->ToString();
      return false;
    }
    ...
    if (offset->operand(0)->IsConstant()) {
      ...
      ITEX_VLOG(2) << "Offset is not multiple(const, ...) "
                   << offset->ToString();
      return false;
    }
    auto multiplier =
    ...
    if (!multiplier || shard_size % *multiplier != 0) {
      ITEX_VLOG(2)
          << "Multiplier is unknown or cannot evenly divide shard size "
  ...
  if (offset->opcode() == HloOpcode::kClamp) {
    ...
    if (!lower_bound || !upper_bound || *lower_bound != 0 ||
      ...
      ITEX_VLOG(2) << "Boundaries of the clamp is not legal: "
                   << offset->ToString();
      return false;
    }
  ...
  if (IsTableLookup(offset)) {
    ...
    for (int64_t i = 0; i < num_groups; ++i) {
      for (int64_t j = 0; j < group_size; ++j) {
        ...
        if (table_index < 0) {
          ITEX_VLOG(2) << "Failed to infer table index from "
        ...
        if (table_entry != shard_size * j) {
          ITEX_VLOG(2) << "Unexpected offset from table.";
          return false;
        }
      }
    }
  ...
  for (int64_t i = 0; i < num_groups; ++i) {
    for (int64_t j = 0; j < group_size; ++j) {
      ...
      if (mapped_id != shard_size * j) {
        ITEX_VLOG(2) << "Mapping of " << id << " to " << mapped_id
...
absl::optional<ReduceScatterSpec> MatchReduceScatter(
  ...
  if (!ar->shape().IsArray() || ar->constrain_layout() ||
    ...
    ITEX_VLOG(2) << "Unsupported all-reduce: " << ar->ToString();
    return absl::nullopt;
  }
  if (ar->shape().rank() - absl::c_count(ar->shape().dimensions(), 1) <
      min_rank) {
    ITEX_VLOG(2) << " Should be at least rank-" << min_rank
                 << " excluding trivial dimensions " << ar->ToString();
    return absl::nullopt;
  }
  if (ar->user_count() != 1) {
    ITEX_VLOG(2) << "All-reduce user_count > 1 " << ar->ToString();
    return absl::nullopt;
  }
  if (ar->replica_groups().size() > 1) {
    ...
    if (!has_uniform_size) {
      ITEX_VLOG(2) << "Unsupported non-uniform replica group size "
                   << ar->ToString();
      return absl::nullopt;
    }
  }
  ...
  if (allow_intervening_reshape && user->opcode() == HloOpcode::kReshape) {
    ...
    if (reshape->user_count() != 1) {
      ITEX_VLOG(2) << "Reshape following all-reduce has user count > 1"
                   << reshape->ToString();
      return absl::nullopt;
    }
  ...
  if (user->opcode() != HloOpcode::kDynamicSlice) {
    ITEX_VLOG(2) << "All-reduce user is not dynamic slice " << user->ToString();
    return absl::nullopt;
  }
  ...
  if (!ar->IsCrossModuleAllReduce()) {
    ...
    if (ar->replica_groups().size() != num_replicas ||
      ...
      ITEX_VLOG(2)
          << "Unsupported size > 1 replica groups for cross-partition, "
             "non-global ID "
          << ar->ToString();
      return absl::nullopt;
    }
  ...
  if (group_size < 2) {
    ITEX_VLOG(2) << "Group_size < 2, nothing to do " << ar->ToString();
    return absl::nullopt;
  }
    ...
    for (int64_t dim = 0; dim < shape.rank(); ++dim) {
      ...
      if (spec.split_dim != -1) {
        if (!allow_multiple_split_dims || spec.split_dim != (dim - 1)) {
          ITEX_VLOG(2) << "Only support split on consecutive dims "
                       << user->ToString();
          return absl::nullopt;
        }
        continue;
  ...
  if (Product(group_sizes) != group_size) {
    ITEX_VLOG(2) << "Group size mismatch " << user->ToString() << " vs "
                 << ar->ToString();
    return absl::nullopt;
  }
  ...
  if (split_dims.size() > 1) {
    ...
    if (!IsPerIdOffsets(absl::MakeSpan(offsets), shard_size, map_id,
                        group_sizes, ar)) {
      ITEX_VLOG(2) << "IsPerIdOffsets() failed " << ar->ToString();
      return absl::nullopt;
    }
  } else {
  ...
  if (reshape) {
    ...
    if (!all_split_dims_unmodified) {
      ITEX_VLOG(2) << "Split dimensions are modified by reshape";
      return absl::nullopt;
    }
