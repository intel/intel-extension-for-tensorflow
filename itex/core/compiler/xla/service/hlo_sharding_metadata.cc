...
void SetSingleSharding(HloInstruction* instruction,
  ...
  ITEX_VLOG(4) << "  " << instruction->name() << " to " << sharding;
...
std::vector<PassThrough> LocatePassThroughDomainLinks(
  ...
  for (HloInstruction* instruction : domain.enter_domains) {
    ITEX_CHECK(instruction->opcode() == HloOpcode::kDomain)
    ...
    for (HloInstruction* user : instruction->users()) {
      if (user->opcode() == HloOpcode::kDomain &&
        ...
        ITEX_VLOG(2) << "Found passthrough domain link:";
        ITEX_VLOG(2) << "  " << user->ToString();
...
Status ApplyDomainSingleSharding(const DomainMetadata::Domain& domain,
                                 const HloSharding& sharding) {
  ITEX_VLOG(4) << "Applying " << sharding << " sharding";
  for (HloInstruction* instruction : domain.instructions) {
    ...
    if (!instruction->has_sharding()) {
      ...
      ITEX_VLOG(4) << "  " << instruction->name() << " already has sharding "
...
Status ApplyDomainSharding(const DomainMetadata::Domain& domain,
  ...
  ITEX_VLOG(1) << "Assigning non-trivial sharding " << sharding;
  TF_RETURN_IF_ERROR(ApplyDomainShardingPass(domain, sharding).status());

  int64_t unassigned = 0;
  for (HloInstruction* instruction : domain.instructions) {
    if (!instruction->has_sharding()) {
      ITEX_LOG(WARNING) << "Unassigned instruction: "
...
StatusOr<std::shared_ptr<const HloSharding>> ExtractOriginalCommonSharding(
  ...
  ITEX_VLOG(4) << "Extracted sharding is " << *sharding;
...
Status ShardingMetadata::NormalizeShardingDomain(
  ...
  if (metadata != nullptr) {
    ...
    if (sharding != nullptr) {
      ITEX_VLOG(4) << "Normalizing sharding to " << sharding->ToString() << ":";
    ...
    if (sharding != nullptr) {
      ITEX_VLOG(4) << "Normalizing sharding-less domain to "
                   << sharding->ToString();
      TF_RETURN_IF_ERROR(ApplyDomainSharding(domain, *sharding));
    } else {
      ITEX_VLOG(1) << "Unable to find common sharding";
    }
  }
  return Status::OK();
}
...
HloInstruction* ShardingDomainCreator::operator()(HloInstruction* instruction,
  ...
  ITEX_VLOG(3) << "Creating domain:";
  ITEX_VLOG(3) << "  Instruction: " << instruction->name();
  ITEX_VLOG(3) << "  Operand: " << operand->name();
  ITEX_VLOG(3) << "    User side sharding: "
               << (instruction_sharding != nullptr
                       ? instruction_sharding->ToString()
                       : "None");
  ITEX_VLOG(3) << "    Operand side sharding: "
               << (root_sharding != nullptr ? root_sharding->ToString()
                                            : "None");
