...
#include "itex/core/compiler/xla/literal_comparison.h"
#include "itex/core/compiler/xla/literal_util.h"
#include "itex/core/compiler/xla/overflow_util.h"
...
#include "itex/core/compiler/xla/service/hlo_evaluator.h"
...
bool IsAll(const HloInstruction* op, const Literal& scalar) {
  ITEX_CHECK(ShapeUtil::IsScalar(scalar.shape()));
  switch (op->opcode()) {
    case HloOpcode::kBroadcast:
...
bool TransposeIsBitcast(const HloInstruction* transpose) {
  ITEX_CHECK_EQ(HloOpcode::kTranspose, transpose->opcode());
...
HloInstruction* BitcastingOperandOfReshapeOrCopyChain(
  ...
  ITEX_CHECK(HloOpcode::kReshape == instr->opcode() ||
...
PrecisionConfig SwapOperandsInDotPrecisionConfig(PrecisionConfig config) {
  ITEX_CHECK_EQ(config.operand_precision_size(), 2);
...
bool ValidateTilingOfBitcast(
  ...
  ITEX_VLOG(2) << "op shape:" << op_shape.ToString(true) << "\n";
  ITEX_VLOG(2) << "bitcast shape:" << bitcast_shape.ToString(true) << "\n";
  ITEX_VLOG(2) << "operand_map size:" << operand_map.size() << "\n";
  ...
  for (auto op_dim : op_shape.layout().minor_to_major()) {
    ITEX_VLOG(3) << "op_dim = " << op_dim << "\n";
    ITEX_VLOG(3) << "tiled_dim_idx = " << tiled_dim_idx << "\n";
    ITEX_VLOG(3) << "tiled_dim_size = " << op_tile.dimension(tiled_dim_idx)
                 << ":" << bitcast_tile.dimension(tiled_dim_idx) << "\n";
    if (op_tile.dimensions()[tiled_dim_idx] !=
        bitcast_tile.dimensions()[tiled_dim_idx]) {
      ITEX_VLOG(2) << "Abort b/c tiled dimension " << op_dim
    ...
    if (operand_map.size() <= op_dim || operand_map[op_dim].empty()) {
      ...
      ITEX_VLOG(2)
          << "Abort because the bitcasted dimensions are not aligned!\n";
      return false;
    } else if (bitcast_shape.dimensions(operand_map[op_dim][0]) <
               op_shape.dimensions(op_dim)) {
      if (operand_map[op_dim].size() == 1) {
        ITEX_VLOG(2)
            << "Abort b/c a dimension (possibly padded) is shrank to a "
               "smaller size.\n";
        return false;
      }
      if (tiled_dim_idx > 0) {
        ITEX_VLOG(2) << "Abort b/c a non-major tiled dimension is split.\n";
        return false;
      }
      if (bitcast_shape.dimensions(operand_map[op_dim][0]) %
        ...
        return false;
               ...
               op_shape.dimensions(op_dim)) {
      if (tiled_dim_idx > 0) {
        ITEX_VLOG(2) << "Abort b/c a non-major tiled dimension is combined.\n";
        return false;
      }
      if (bitcast_shape.dimensions(operand_map[op_dim][0]) %
                  op_shape.dimensions(op_dim) !=
...
bool AlgebraicSimplifierVisitor::Run(HloComputation* computation,
  ...
  ITEX_CHECK_OK(computation->Accept(this));
...
std::unique_ptr<HloInstruction> MakeScalarInstruction(HloInstruction* target,
  ...
  switch (target->shape().element_type()) {
    ...
    case F32:
      ...
      break;
    default:
      ITEX_LOG(FATAL) << "Unsupported data type: "
...
Status AlgebraicSimplifierVisitor::ScalarMultiplyReduction(
  ...
  while (!operands.empty()) {
    ...
    if (Match(inst, m::MultiplyAnyOrder(
      ...
      ITEX_CHECK_LT(index, user->operand_count());
      ITEX_CHECK_EQ(inst, user->operands()[index]);
  ...
  ITEX_CHECK_EQ(new_multiply->shape(), target->shape());
...
void AlgebraicSimplifierVisitor::ReplaceWithBitcast(HloInstruction* instruction,
                                                    HloInstruction* operand) {
  ITEX_CHECK_EQ(1, instruction->operand_count());
  if (operand == nullptr) {
    operand = instruction->mutable_operand(0);
  }
  ITEX_CHECK_EQ(ShapeUtil::ElementsIn(instruction->shape()),
                ShapeUtil::ElementsIn(operand->shape()));
  ITEX_CHECK_EQ(ShapeUtil::ByteSizeOf(instruction->shape()),
  ...
  ITEX_CHECK_OK(ReplaceInstruction(instruction, bitcast));
}
...
bool AlgebraicSimplifierVisitor::ReplaceInstructionIfCompatible(
  ...
  ITEX_CHECK(!new_instructions.empty());
...
Status AlgebraicSimplifierVisitor::HandleAbs(HloInstruction* abs) {
  ...
  ITEX_VLOG(10) << "trying transform [Abs(A) => A] " << abs->ToString()
...
Status AlgebraicSimplifierVisitor::HandleAdd(HloInstruction* add) {
  ...
  ITEX_CHECK(Match(add, m::Add(m::Op(&lhs), m::Op(&rhs))));

  // A + 0 => A
  ITEX_VLOG(10) << "trying transform [A + 0 => A]: " << add->ToString();
  if (IsAll(rhs, 0) && ReplaceInstructionIfCompatible(add, lhs)) {
    return Status::OK();
  }
  // 0 + A => A
  ITEX_VLOG(10) << "trying transform [0 + A => A]: " << add->ToString();
  ...
  // rules below simpler.
  ITEX_VLOG(10) << "trying transform [Const + A => A + Const]";
  ...
  //   (A + C1) + (B + C2) =>  A + B + (C1 + C2).
  //
  ITEX_VLOG(10) << "trying transform [(A + C1) + C2 => A + (C1 + C2)]";
...
Status AlgebraicSimplifierVisitor::HandleAnd(HloInstruction* logical_and) {
  HloInstruction *lhs, *rhs;
  ITEX_CHECK(Match(logical_and, m::And(m::Op(&lhs), m::Op(&rhs))));
  // Simplify logical and
  if (ShapeUtil::HasPrimitiveType(lhs->shape(), itex_xla::PRED) &&
      ShapeUtil::HasPrimitiveType(rhs->shape(), itex_xla::PRED)) {
    // A && True => A
    ITEX_VLOG(10) << "trying transform [A && True => A]: "
                  << logical_and->ToString();
    if (IsAll(rhs, 1) && ReplaceInstructionIfCompatible(logical_and, lhs)) {
      return Status::OK();
    }
    // True && A => A
    ITEX_VLOG(10) << "trying transform [True && A => A]: "
                  << logical_and->ToString();
  ...
  ITEX_VLOG(10) << "trying transform [A && False => False]: "
                << logical_and->ToString();
  ...
  ITEX_VLOG(10) << "trying transform [False && A => False]: "
                << logical_and->ToString();
  ...
  for (int64_t bitcast_pos = 0; bitcast_pos < bitcast_rank; ++bitcast_pos) {
    ...
    if (operand_pos >= operand_rank) {
      if (bitcast_shape.dimensions(bitcast_dim) != 1) {
        ITEX_VLOG(3)
            << "Abort b/c bitcasted size is bigger than operand size.\n";
        return absl::nullopt;
      }
      continue;
    }
    ITEX_CHECK_LT(bitcast_dim, bitcast_shape.dimensions_size());
    int64_t bitcast_dim_size = bitcast_shape.dimensions()[bitcast_dim];
    cur_bitcast_size *= bitcast_dim_size;
    ITEX_VLOG(2) << "bitcast pos = " << bitcast_pos << "\n";
    ITEX_VLOG(2) << "bitcast size = " << cur_bitcast_size << "\n";
    while (operand_pos < operand_rank) {
      if (operand_pos < 0 || cur_operand_size < cur_bitcast_size) {
        ITEX_VLOG(2) << "operand size < bitcase size\n";
        operand_pos++;
        if (operand_pos >= operand_rank) {
          ITEX_VLOG(2)
        ...
        ITEX_VLOG(3) << "operand size = " << cur_operand_size << "\n";
        if (cur_operand_size > cur_bitcast_size &&
          ...
          ITEX_VLOG(3)
              << "Abort b/c re-partitioning a group of dimensions is not "
                 "supported. \n";
          return absl::nullopt;
        }
      }
      ITEX_CHECK_GE(operand_dim, 0);
      if (operand_shape.dimensions(operand_dim) > 1) {
        ITEX_CHECK_LT(operand_dim, operand_dim_map.size());
        operand_dim_map[operand_dim].push_back(bitcast_dim);
        ITEX_VLOG(3) << "operand dim_map[operand_dim] add " << bitcast_dim
                     << " at " << operand_dim << "\n";
      }
      if (cur_operand_size >= cur_bitcast_size) {
        ITEX_VLOG(3) << cur_operand_size << ">=" << cur_bitcast_size << "\n";
        ITEX_CHECK_GE(operand_dim, 0);
...
absl::optional<Shape> AlgebraicSimplifierVisitor::ReshapeLayoutDimensions(
  ...
  for (int64_t op_pos = 0; op_pos < original_dimensions.size(); ++op_pos) {
    ...
    ITEX_VLOG(3) << "op_pos = " << op_pos << "\n";
    if (original_map.size() <= op_dim) {
      ITEX_VLOG(3) << "Skip due to original_map has too few dimensions.\n";
      continue;
    }
    ...
    for (int64_t bitcast_dim : bit_dims) {
      ...
      ITEX_VLOG(3) << "Add new reshaped dimension:" << bitcast_dim << "\n";
      if (bitcast_pos < 0 ||
        ...
        if (bitcast_pos >= reshaped_dimensions->size()) {
          ITEX_VLOG(3) << "bitcast pos is over incremented:" << bitcast_pos
                       << "\n";
          return absl::nullopt;
        }
        (*reshaped_dimensions)[bitcast_pos] = bitcast_dim;
      ...
      if (op_dims.size() > 1 && op_pos > 0 && op_dims[0] != op_dim) {
        ...
        if (original_map[op_dim_prev].empty() ||
          ...
          ITEX_VLOG(2) << "Abort b/c op dimensions that are combined into "
        ...
        for (int i = 0; i < op_dims.size(); ++i) {
          if (op_dims[i] == op_dim_prev) {
            if (i == op_dims.size() - 1 || op_dims[i + 1] != op_dim) {
              ITEX_VLOG(2)
                  << "Abort b/c op dimensions that are combined into "
  ...
  ITEX_CHECK_EQ(bitcast_pos + 1, result_shape.rank());
  return new_shape;
}
...
AlgebraicSimplifierVisitor::InvertBitcastDimMap(
  ...
  for (auto i = 0; i < original_shape.rank(); ++i) {
    ...
    ITEX_VLOG(3) << "traversing minor to major (" << i << ")=" << j;
    for (auto k : original_map[j]) {
      ITEX_VLOG(3) << "setting result_map[" << k << "] = " << j << "\n";
...
bool AlgebraicSimplifierVisitor::SwapCopyBitcastCopy(
  ...
  ITEX_VLOG(2) << "Processing " << copy->ToString() << "\n"
               << bitcast->ToString() << "\n"
               << root_copy->ToString() << "\n";
  ...
  if (!ValidateTilingOfBitcast(bitcast->shape(), copy->shape(), operand_map)) {
    ITEX_VLOG(2) << "Abort because bitcast changes tiling assignment.\n";
    return false;
  }
  ...
  if (ValidateTilingOfBitcast(bitcast->shape(), op->shape(), operand_map)) {
    ...
    ITEX_VLOG(2) << "Replace with " << repl->operand(0)->ToString() << "\n"
                 << repl->ToString() << "\n";
    ITEX_CHECK_OK(ReplaceWithNewInstruction(root_copy, std::move(repl)));
    return true;
  }
...
Status AlgebraicSimplifierVisitor::HandleCopy(HloInstruction* copy) {
  ...
  if (!EnableGpuMlirLowering() && bitcast_operand) {
...
Status AlgebraicSimplifierVisitor::HandleConcatenate(
  ...
  if (nonempty_operands.size() < operands.size()) {
    ...
    ITEX_VLOG(10) << "trying to replace " << concatenate->ToString() << " with "
...
AlgebraicSimplifierVisitor::TrySimplifyTautologicalBitcastConvert(
    HloInstruction* bitcast) {
  ITEX_CHECK_EQ(bitcast->opcode(), HloOpcode::kBitcastConvert);
...
Status AlgebraicSimplifierVisitor::HandleSubtract(HloInstruction* sub) {
  HloInstruction *lhs, *rhs;
  ITEX_CHECK(Match(sub, m::Subtract(m::Op(&lhs), m::Op(&rhs))));
  // A - 0 => A
  ITEX_VLOG(10) << "trying transform [A - 0 => A]: " << sub->ToString();
  if (IsAll(rhs, 0) && ReplaceInstructionIfCompatible(sub, lhs)) {
    return Status::OK();
  }

  // Canonicalize subtraction of a constant to addition.
  ITEX_VLOG(10) << "trying transform [A - Const => A + (-Const)]";
  ...
  // A - A => 0 for integer A.
  ITEX_VLOG(10) << "trying transform [A - A => 0] for integer A.";
...
std::unique_ptr<HloInstruction> TryDivideToShift(
  ...
  ITEX_CHECK(Match(divide, m::Divide(m::Op(&a), m::Op(&b))));
...
Status AlgebraicSimplifierVisitor::HandleDivide(HloInstruction* divide) {
  ...
  ITEX_CHECK(Match(divide, m::Divide(m::Op(&a), m::Op(&b))));
  // A/1 => A
  ITEX_VLOG(10) << "trying transform [A/1 => A]: " << divide->ToString();
  ...
  if (Match(divide, m::Divide(m::Exp(m::Op(&a)), m::Exp(m::Op(&b)))
    ...
    ITEX_VLOG(10) << "transform [exp(A)/exp(B) => exp(A-B)]: "
                  << divide->ToString();
  ...
  if (Match(divide, m::Divide(m::Op(&a), m::Exp(m::Op(&b))))) {
    ITEX_VLOG(10) << "transform [A/exp(B) => A*exp(-B)]: "
                  << divide->ToString();
  ...
  if (Match(divide, m::Divide(m::Op(&a), m::Power(m::Op(&b), m::Op(&c))))) {
    ITEX_VLOG(10) << "transform [A/pow(B,C) => A*pow(B,-C)]: "
                  << divide->ToString();
...
StatusOr<HloInstruction*> AlgebraicSimplifierVisitor::OptimizeDotOfConcat(
  ...
  HloInstruction *lhs, *rhs;
  ITEX_CHECK(Match(dot, m::Dot(m::Op(&lhs), m::Op(&rhs))));
...
StatusOr<HloInstruction*> AlgebraicSimplifierVisitor::OptimizeDotOfGather(
  ...
  if (dnums.lhs_contracting_dimensions_size() != 1 ||
      ...
      dot->shape().dimensions_size() != 2) {  // dot output 2D
    ITEX_VLOG(10)
        << "DotOfGather: Can only optimize 2D, non-batch dot operations.";
    return nullptr;
  }
  ...
  if (!is_dynamic_slice_constant_combination(
    ...
    ITEX_VLOG(10) << "DotOfGather: Can only optimize dot(DS(ctA), ctB)) or "
...
AlgebraicSimplifierVisitor::OptimizeDotOfReorderContractingDims(
  ...
  ITEX_CHECK_EQ(lhs_contracting_dims.size(), 1);
  ...
  ITEX_CHECK(IsPermutation(permutation));
  ...
  // Invert reshape.
  ITEX_CHECK_EQ(rhs_contracting_dims.size(), 1);
...
Status AlgebraicSimplifierVisitor::HandleDot(HloInstruction* dot) {
  ITEX_CHECK(computation_ == dot->parent());
  const auto& dnums = dot->dot_dimension_numbers();
  ...
  if (options_.enable_dot_strength_reduction() &&
    ...
    int64_t rhs_outer_dims =
        rhs->shape().rank() - (dnums.rhs_batch_dimensions_size() +
                               dnums.rhs_contracting_dimensions_size());
    ITEX_CHECK(lhs_outer_dims == 0 || rhs_outer_dims == 0);
    if (rhs_outer_dims > 0) {
  ...
  if (dot_of_reorder_optimized) {
    ITEX_VLOG(10) << " Replaced dot " << dot->ToString()
  ...
  if (dot_of_concat_optimized) {
    ITEX_VLOG(10) << "Replaced dot(concat(...), constant) with add(dot(..., "
                     "constant)...)";
  ...
  if (dot_of_gather_optimized) {
    ITEX_VLOG(10) << "Replaced dot(constA, gather(i, constB)) with "
...
Status AlgebraicSimplifierVisitor::HandleMaximum(HloInstruction* maximum) {
  HloInstruction *lhs, *rhs;
  ITEX_CHECK(Match(maximum, m::Maximum(m::Op(&lhs), m::Op(&rhs))));
...
Status AlgebraicSimplifierVisitor::HandleMinimum(HloInstruction* minimum) {
  HloInstruction *lhs, *rhs;
  ITEX_CHECK(Match(minimum, m::Minimum(m::Op(&lhs), m::Op(&rhs))));
...
Status AlgebraicSimplifierVisitor::HandleClamp(HloInstruction* clamp) {
  ...
  HloInstruction* clamp_upper_bound;
  HloInstruction* to_clamp;
  ITEX_CHECK(Match(clamp, m::Clamp(m::Op(&clamp_lower_bound), m::Op(&to_clamp),
                                   m::Op(&clamp_upper_bound))));
...
Status AlgebraicSimplifierVisitor::HandleMultiply(HloInstruction* multiply) {
  HloInstruction *lhs, *rhs;
  ITEX_CHECK(Match(multiply, m::Multiply(m::Op(&lhs), m::Op(&rhs))));
  // LHS*1 => LHS
  ITEX_VLOG(10) << "trying transform [LHS*1 => LHS]: " << multiply->ToString();
  if (IsAll(rhs, 1) && ReplaceInstructionIfCompatible(multiply, lhs)) {
    return Status::OK();
  }
  // 1*RHS => RHS
  ITEX_VLOG(10) << "trying transform [1*RHS => RHS]: " << multiply->ToString();
  ...
  ITEX_VLOG(10) << "trying to transform exp(LHS) * exp(RHS) => exp(LHS+RHS) "
                << multiply->ToString();
  ...
  ITEX_VLOG(10) << "trying transform [rsqrt(B) * rsqrt(B) => 1/B] "
                << multiply->ToString();
  HloInstruction* b;
...
Status AlgebraicSimplifierVisitor::HandleOr(HloInstruction* logical_or) {
  HloInstruction *lhs, *rhs;
  ITEX_CHECK(Match(logical_or, m::Or(m::Op(&lhs), m::Op(&rhs))));

  // Simplify logical or
  if (ShapeUtil::HasPrimitiveType(lhs->shape(), itex_xla::PRED) &&
      ShapeUtil::HasPrimitiveType(rhs->shape(), itex_xla::PRED)) {
    // A || True => True
    ITEX_VLOG(10) << "trying transform [A || True => True]: "
                  << logical_or->ToString();
    if (IsAll(rhs, 1) && ReplaceInstructionIfCompatible(logical_or, rhs)) {
      return Status::OK();
    }
    // True || A => True
    ITEX_VLOG(10) << "trying transform [True || A => True]: "
                  << logical_or->ToString();
  ...
  // A || False => A and A | 0 => A
  ITEX_VLOG(10) << "trying transform [A || False => A]: "
                << logical_or->ToString();
  ...
  // False || A => A and 0 | A => A
  ITEX_VLOG(10) << "trying transform [False || A => A]: "
                << logical_or->ToString();
...
Status AlgebraicSimplifierVisitor::HandleLog(HloInstruction* log) {
  // ln(exp(A)) => A
  ITEX_VLOG(10) << "trying transform [ln(exp(A)) => A]: " << log->ToString();
  HloInstruction *a, *b;
...
Status AlgebraicSimplifierVisitor::HandleOptimizationBarrier(
  ...
  *barrier->mutable_shape() = new_operand->shape();
  for (auto use : barrier->users()) {
    ITEX_CHECK_EQ(use->opcode(), HloOpcode::kGetTupleElement);
...
absl::optional<std::vector<int64_t>> ReshapeLeavesDimensionsUnmodified(
  ...
  ITEX_CHECK_EQ(hlo->opcode(), HloOpcode::kReshape);
...
bool OutputIsPermutationOfOperandElements(HloInstruction* instruction,
                                          HloInstruction* operand) {
  ITEX_DCHECK(!instruction->OperandIndices(operand).empty());
...
bool OutputIsSubsetOfOperandElements(HloInstruction* instruction,
  ...
  ITEX_CHECK(!operand_indices.empty());
...
Status AlgebraicSimplifierVisitor::HandleBroadcast(HloInstruction* broadcast) {
  HloInstruction* operand;
  ITEX_CHECK(Match(broadcast, m::Broadcast(m::Op(&operand))));
  ...
  if (std::is_sorted(dims.begin(), dims.end()) &&
      ShapeUtil::ElementsIn(broadcast->shape()) ==
          ShapeUtil::ElementsIn(operand->shape())) {
    ITEX_VLOG(10) << "transform broadcast(X) -> reshape(X) where "
                     "n(broadcast(X)) == n(X)";
    return ReplaceWithNewInstruction(
  ...
  if (broadcast->shape().rank() == operand->shape().rank() &&
      ShapeUtil::ElementsIn(broadcast->shape()) ==
          ShapeUtil::ElementsIn(operand->shape())) {
    ITEX_VLOG(10) << "transform broadcast(X) -> transpose(X) where "
                     "n(broadcast(X)) == n(X)";
    return ReplaceWithNewInstruction(
        broadcast,
  ...
  if (ShapeUtil::IsScalar(operand->shape())) {
    for (HloInstruction* user : broadcast->users()) {
      ...
      if (OutputIsPermutationOfOperandElements(user, broadcast) ||
        ...
        ITEX_VLOG(10)
            << "transform permuting/subset  of a scalar broadcast into "
            << "a single broadcast";
...
Status AlgebraicSimplifierVisitor::HandleCompare(HloInstruction* compare) {
  HloInstruction* lhs;
  HloInstruction* rhs;
  ITEX_CHECK(Match(compare, m::Compare(m::Op(&lhs), m::Op(&rhs))));
  {
...
Status AlgebraicSimplifierVisitor::HandlePower(HloInstruction* power) {
  ITEX_VLOG(10) << "trying transform [pow(A, 0) => 1]: " << power->ToString();
  HloInstruction *lhs, *rhs;
  ITEX_CHECK(Match(power, m::Power(m::Op(&lhs), m::Op(&rhs))));
  if (IsAll(rhs, 0)) {
    return ReplaceInstruction(power, MakeScalarLike(power, 1));
  }

  ITEX_VLOG(10) << "trying transform [pow(A, 1) => A]: " << power->ToString();
  ...
  ITEX_VLOG(10) << "trying transform [pow(A, 2) => A*A]: " << power->ToString();
  if (IsAll(rhs, 2)) {
    return ReplaceWithNewInstruction(
  ...
  ITEX_VLOG(10) << "trying transform [pow(A, 3) => A*A*A]: "
                << power->ToString();
  if (IsAll(rhs, 3)) {
  ...
  ITEX_VLOG(10) << "trying transform [pow(A, -1) => 1/A]: "
                << power->ToString();
  if (IsAll(rhs, -1)) {
    return ReplaceWithNewInstruction(
  ...
  for (HloInstruction* user : broadcast->users()) {
    ...
    for (HloInstruction* user_operand : user->operands()) {
      ...
      if (is_compatible_broadcast(user_operand)) {
        ...
        ITEX_CHECK_EQ(broadcast, user_operand);
        new_operands.push_back(operand);
      }
    }
    ITEX_VLOG(4) << "Sinking broadcast after user:";
    ITEX_VLOG(4) << "  old broadcast: " << broadcast->ToString();
    ITEX_VLOG(4) << "  old user: " << user->ToString();
    ...
    ITEX_VLOG(4) << "  new user: " << new_user->ToString();
    ...
    ITEX_VLOG(4) << "  new broadcast: " << new_broadcast->ToString();
...
std::unique_ptr<HloInstruction> TryRemainderToAnd(
    ...
    AlgebraicSimplifier* simplifier) {
  HloInstruction *a, *b, *c;
  ITEX_CHECK(Match(remainder, m::Remainder(m::Op(&a), m::Op(&b))));
...
Status AlgebraicSimplifierVisitor::HandleReshape(HloInstruction* reshape) {
  ...
  if (SameShape(reshape, operand)) {
    ITEX_VLOG(3) << "deleting no-op reshape";
  ...
  // Make this a bitcast if possibly
  HloInstruction* bitcast_operand =
...
StatusOr<bool> AlgebraicSimplifierVisitor::TrySimplifyScalarSlice(
  ...
  if (slice->operand(0)->opcode() == HloOpcode::kConcatenate) {
    ITEX_VLOG(10) << "Trying to simplify scalar slice of concat";
    // Only do this for R1, there's no chance of this being useful otherwise.
    if (slice->shape().rank() != 1) {
      ITEX_VLOG(10) << "Not folding, slice is not rank 1";
      return false;
    }
    ...
    if (replaced) {
      ITEX_VLOG(10) << "Folding scalar slice of concat into concat operand";
    } else {
      ITEX_VLOG(10)
          << "Folding scalar slice of concat into slice of concat operand";
      TF_RETURN_IF_ERROR(ReplaceWithNewInstruction(
...
StatusOr<bool> AlgebraicSimplifierVisitor::TryToReorderSliceAndReshape(
    HloInstruction* slice) {
  ITEX_CHECK_EQ(slice->opcode(), HloOpcode::kSlice);
...
StatusOr<bool> AlgebraicSimplifierVisitor::TryToReorderSliceAndReverse(
    HloInstruction* slice) {
  ITEX_VLOG(2) << "Entered TryToReorderSliceAndReverse for slice:"
               << slice->ToString();
  if (Match(slice, m::Slice(m::Reverse()))) {
    ...
    for (auto rdim : reverse->dimensions()) {
      ...
      ITEX_VLOG(2) << "Analyzing dim:" << rdim << " (start,limit):" << start
                   << "," << limit
                   << " and new (start, limit):" << new_starts[rdim] << ","
                   << new_limits[rdim];
    }
...
Status AlgebraicSimplifierVisitor::HandleSlice(HloInstruction* slice) {
  ...
  if (Match(slice,
    ...
    ITEX_VLOG(3) << "Sink broadcast through slice";
    ITEX_VLOG(3) << "Original slice: " << slice->ToString();
    ITEX_VLOG(3) << "Original broadcast: " << broadcast->ToString();
    ...
    ITEX_VLOG(3) << "New slice: " << slice->ToString();
    ITEX_VLOG(3) << "New broadcast: " << new_broadcast->ToString();
...
Status AlgebraicSimplifierVisitor::HandleRsqrt(HloInstruction* rsqrt) {
  ITEX_VLOG(10) << "trying transform [rsqrt(Pow(A, -2)) => |A|] "
                << rsqrt->ToString();
  ...
  ITEX_VLOG(10) << "trying transform [rsqrt(Divide(1, A)) => sqrt(A)] "
                << rsqrt->ToString();
...
Status AlgebraicSimplifierVisitor::HandleDynamicSlice(
  ...
  if (Match(operand, m::Broadcast(m::Op(&broadcast_operand)))) {
    ...
    ITEX_VLOG(3) << "Sink broadcast through dynamic slice";
    ITEX_VLOG(3) << "Original dynamic slice: " << dynamic_slice->ToString();
    ITEX_VLOG(3) << "Original broadcast: " << operand->ToString();
    ...
    ITEX_VLOG(3) << "New dynamic slice: " << dynamic_slice->ToString();
    ITEX_VLOG(3) << "New broadcast: " << new_broadcast->ToString();
  ...
  if (Match(updated,
    ...
    if (compatible) {
      for (int64_t dim = 0; dim < updated_shape.rank(); ++dim) {
        ...
        ITEX_VLOG(2) << "slice: " << slice_dim_start->ToString();
        ...
        *beg = std::min<int64_t>(bcast_width - update_width, *beg);
        ITEX_VLOG(2) << "adjusted beg value: " << *beg;
    ...
    if (compatible) {
      ...
      ITEX_VLOG(2) << dynamic_update_slice->ToString();
      ITEX_VLOG(2) << " with pad:" << pad->ToString();
      ITEX_VLOG(2) << " Computation before rewrite is: "
...
Status AlgebraicSimplifierVisitor::HandleReduceWindow(HloInstruction* hlo) {
  ...
  auto replace_with_span = [&](const std::vector<HloInstruction*>& elements) {
    ITEX_CHECK(multi_output_reduce_window || elements.size() == 1);
  ...
  if (operand->opcode() == HloOpcode::kPad) {
    ...
    ITEX_VLOG(10) << "Not folding pad into reduce-window as there is no pad.";
    return Status::OK();
  }

  ITEX_VLOG(10) << "Considering folding Pad: " << pad->ToString()
  ...
  if (HasInteriorPadding(pad_config) && window_util::HasBaseDilation(window)) {
    ITEX_VLOG(10)
        << "Not folding interior pad into base-dilated reduce-window.";
    return Status::OK();
  }
  ...
  if (pad_value != reduce_init_value) {
    ...
    if (pad_value->opcode() != HloOpcode::kConstant ||
      ...
      ITEX_VLOG(10)
          << "Not folding pad into reduce-window due to different pad "
             "values.";
      return Status::OK();
    }
  }
  ...
  auto is_effective_broadcast = [&] {
    if (window_util::HasStride(window)) {
      ITEX_VLOG(10) << "Window has stride.";
      return false;
    }
    if (!window_util::HasSymmetricPadding(pad_config)) {
      ITEX_VLOG(10) << "Window has uneven padding.";
      return false;
    }
    if (HasInteriorPadding(pad_config)) {
      ITEX_VLOG(10) << "Window has interior padding.";
      return false;
    }
    for (int64_t i = 0; i < pad_config.dimensions_size(); ++i) {
      ...
      if ((pad_dimension.edge_padding_low() != 0 ||
        ...
        ITEX_VLOG(10) << "Found non-trivial dimension being padded: " << i;
        return false;
      }
    }
    ITEX_VLOG(10) << "Found to be padding trivial dimensions only.";
    ...
    for (int64_t i = 0; i < window.dimensions_size(); ++i) {
      ...
      if (dimension_has_padding &&
        ...
        ITEX_VLOG(10)
            << "Found window did not cover single unpadded element in "
               "dimension: "
            << i;
        return false;
      }
      if (pad_operand->shape().dimensions(i) != 1 &&
        ...
        ITEX_VLOG(10)
            << "Found window covers more than one element in non-trivial "
               "dimension: "
            << i;
        return false;
      }
    }
    ITEX_VLOG(10) << "Found window covers a single unpadded element.";
    return true;
  };
  ...
  if (is_effective_broadcast()) {
    ITEX_VLOG(10) << "Replacing pad/reduce-window with broadcast.";
  ...
  ITEX_VLOG(10) << "Folding pad into reduce-window.";
  ...
  for (int64_t i = 0; i < rank; ++i) {
    ...
    if (pad_dim.interior_padding() != 0) {
      ITEX_CHECK_EQ(window_dim.base_dilation(), 1);
...
Status AlgebraicSimplifierVisitor::HandleSqrt(HloInstruction* sqrt) {
  ITEX_VLOG(10) << "trying transform [sqrt(A*A) => |A|] " << sqrt->ToString();
...
Status AlgebraicSimplifierVisitor::HandleTranspose(HloInstruction* transpose) {
  ...
  if (std::is_sorted(transpose->dimensions().begin(),
    ...
    ITEX_VLOG(10) << "deleting no-op transpose";
  ...
  if (!EnableGpuMlirLowering() && options_.is_layout_sensitive() &&
  ...
  if (operand->opcode() == HloOpcode::kReshape &&
    ...
    ITEX_VLOG(2) << "trying depth-to-space transform";
...
StatusOr<bool> AlgebraicSimplifierVisitor::FoldConvInputPad(
  ...
  if (Match(convolution,
    ...
    for (int64_t dim = 0; dim < dnums.input_spatial_dimensions_size(); ++dim) {
      ...
      if (p.interior_padding() != 0) {
        ITEX_CHECK_EQ(w.base_dilation(), 1);
...
StatusOr<bool> AlgebraicSimplifierVisitor::FoldConvFilterPad(
  ...
  for (int64_t dim = 0; dim < dnums.kernel_spatial_dimensions_size(); ++dim) {
    ...
    ITEX_CHECK_EQ(w.window_dilation(), 1);
...
Status AlgebraicSimplifierVisitor::HandleConvolution(
  ...
  // Disable SimplifyConv to avoid bitcast op when MLIR Lowering Enabled.
