...
#include "itex/core/compiler/xla/service/while_loop_analysis.h"
...
//
// ITEX_CHECK-fails if instr doesn't have exactly one unique non-constant
// operand.
...
static optional<int64_t> GetGTEOperandIndex(const HloInstruction* instr,
  ...
  ITEX_VLOG(2) << "GetGTEOperandIndex(" << instr->ToString() << ", "
...
std::vector<const HloInstruction*> GetAuxiliaryLoopInductionVars(
  ...
  ITEX_CHECK_EQ(while_op->opcode(), HloOpcode::kWhile);
  auto* while_body = while_op->while_body();
  auto* while_body_param = while_body->parameter_instruction(0);
  ITEX_VLOG(2) << "Aux Induction Variables for loop:"
               << while_op->ToShortString();
  ITEX_VLOG(2) << "the parameter instr:" << while_body_param->ToShortString();
  ITEX_VLOG(2) << "the parameter user count:"
               << while_body_param->users().size();
  ...
  for (const HloInstruction* indx_instr : while_body_param->users()) {
    ...
    if (it != extractions.end()) {
      it->second = nullptr;
      ITEX_VLOG(2) << "two extractions at same index:"
                   << indx_instr->ToString();
    } else {
      extractions.insert(std::make_pair(indx_instr->tuple_index(), indx_instr));
      ITEX_VLOG(2) << "inserting extraction :" << indx_instr->ToString();
    }
  }
  ITEX_VLOG(2) << "total extractions size:" << extractions.size() << std::endl;
  ...
  if (while_body_root->opcode() != HloOpcode::kTuple) {
    ITEX_VLOG(2) << "While body root is not a tuple:"
  ...
  for (const HloInstruction* operand : while_body_root->operands()) {
    ...
    if (!operand->IsConstant()) {
      ...
      if (it != insertions.end()) {
        it->second = nullptr;
        ITEX_VLOG(2) << "two insertions at same index:" << operand->ToString();
      } else {
        insertions.insert(std::make_pair(index, operand));
        ITEX_VLOG(2) << "inserting insertions:" << operand->ToString();
      }
    }
  }
  if (insertions.empty()) {
  ...
  ITEX_VLOG(2) << "total candidate pairs:" << candidate_pairs.size()
               << std::endl;
  ...
  for (auto candidates : candidate_pairs) {
    ITEX_VLOG(2) << "are reachable?:" << (candidates.second.first)->ToString()
  ...
  ITEX_VLOG(2) << "num auxiliary candidates :" << aux_ind_gte.size();
  return aux_ind_gte;
}
...
optional<int64_t> GetLoopInductionVarTupleIdx(const HloInstruction* while_op) {
  ITEX_CHECK_EQ(while_op->opcode(), HloOpcode::kWhile);
  ITEX_VLOG(2) << "Finding induction variable for loop "
               << while_op->ToShortString();
  ...
  if (!indvar_tuple_idx) {
    ITEX_VLOG(2) << "Induction variable not found in loop condition: "
  ...
  auto* while_body_root = while_body->root_instruction();
  if (while_body_root->opcode() != HloOpcode::kTuple) {
    ITEX_VLOG(2) << "While body's root is not a tuple instruction: "
                 << while_body_root->ToString();
    return nullopt;
  }
  ...
  if (while_body_indvar_tuple_idx != indvar_tuple_idx) {
    ITEX_VLOG(2)
        << "Tuple index of induction variable does not match between loop "
           "condition ("
  ...
  if (while_init->opcode() != HloOpcode::kTuple) {
    ITEX_VLOG(2) << "While init expected to be a tuple: "
                 << while_init->ToString();
    return nullopt;
  }

  ITEX_VLOG(2) << "Induction variable's tuple index: " << *indvar_tuple_idx;
...
static optional<int64_t> LiteralAsScalarInt64(const Literal& l) {
  if (!ShapeUtil::IsEffectiveScalar(l.shape())) {
    ITEX_VLOG(2) << "literal is not an effective scalar: " << l.ToString();
    return nullopt;
  }
  switch (l.shape().element_type()) {
    ...
    case U64: {
      ...
      if (v > static_cast<uint64_t>(std::numeric_limits<int64_t>::max())) {
        ITEX_VLOG(2) << "uint64_t literal is out of range for int64_t: " << v;
        return nullopt;
      }
      return v;
    }
    default:
      ITEX_VLOG(2) << "literal is of non-integral type "
                   << l.shape().ToString();
      return nullopt;
  }
}
...
static optional<int64_t> PatternMatchLoopTripCount(HloInstruction* while_op,
  ...
  if (!indvar_init_val) {
    ITEX_VLOG(2) << "Pattern-match failed: induction variable init is not a "
  ...
  if (!Match(while_body_indvar_update,
    ...
    ITEX_VLOG(2)
        << "Pattern-match failed: induction variable does not go as i++: "
  ...
  if (!Match(while_cond_root,
    ...
    ITEX_VLOG(2) << "Pattern-match failed: while condition is not of the form "
                    "op(i, N) or op(N, i).";
    return nullopt;
  }
  ...
  if (!while_cond_bound_val) {
    ITEX_VLOG(2)
        << "Pattern-match failed: while condition induction variable is "
  ...
  if (Match(while_cond_root,
    ...
    ITEX_VLOG(2) << "Pattern-match succeeded: loop condition is i < N: "
    ...
    if (trips) {
      ...
      ITEX_VLOG(2) << "Pattern-match failed: Trip count exceeds INT64_MAX.";
      return nullopt;
    }
  }
  ...
  ITEX_VLOG(2)
      << "Pattern-match failed: while condition follows unknown pattern: "
      << while_cond_root->ToString();
  return nullopt;
}
...
optional<int64_t> ComputeWhileLoopTripCount(HloInstruction* while_op,
  ...
  ITEX_VLOG(2) << "Getting trip count for loop " << while_op->ToString();
  ...
  if (!indvar_init_result.ok()) {
    ITEX_VLOG(2) << "Couldn't evaluate induction variable init, "
  ...
  for (int64_t trip_count = 0; trip_count != max_brute_force_iters + 1;
    ...
    if (!result.ok()) {
      ITEX_VLOG(2) << "Couldn't evaluate while cond: " << result.status();
      return nullopt;
    }
    if (result.ValueOrDie().data<bool>() == absl::Span<const bool>{false}) {
      ITEX_VLOG(2) << "Loop has static trip count of " << trip_count;
      return trip_count;
    }
    ...
    if (!indvar_next_result.ok()) {
      ITEX_VLOG(2) << "Couldn't evaluate induction variable update: "
  ...
  ITEX_VLOG(2) << "Loop has unknown trip count.";
  return nullopt;
}
...
optional<int64_t> ComputeWhileLoopTripCountUpperBound(
  ...
  if (exact_trip_count) {
    ITEX_VLOG(2) << "Loop has exact trip count.";
  ...
  auto* while_body_root = while_body->root_instruction();
  if (while_body_root->opcode() != HloOpcode::kTuple) {
    ITEX_VLOG(3) << "While body's root is not a tuple instruction: "
                 << while_body_root->ToString();
    return nullopt;
  }
  ...
  if (while_body_indvar->opcode() != HloOpcode::kConstant) {
    ITEX_VLOG(3) << "While body does not set the IV to a constant: "
  ...
  ITEX_CHECK_OK(fake_input.CopyFrom(while_body_indvar->literal(),
  ...
  if (!eval_result.ok()) {
    ITEX_VLOG(2) << "Couldn't evaluate while loop condition.";
    return nullopt;
  }

  Literal cond_result_pred = std::move(eval_result.ValueOrDie());
  ITEX_CHECK(Shape::Equal().IgnoreLayout()(cond_result_pred.shape(),
  ...
  if (!cond_returns_true) {
    ITEX_VLOG(2) << "Upper bound on the trip count is 1";
    return 1;
  }

  ITEX_VLOG(2) << "Loop has no known upper bound on the trip count.";
  return nullopt;
}
