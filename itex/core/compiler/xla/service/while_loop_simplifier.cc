...
static StatusOr<HloInstruction*> RemoveDeadTupleIndices(
  ...
  for (int64_t new_idx = 0; new_idx < new_to_old_tuple_idx.size(); ++new_idx) {
    ...
    ITEX_VLOG(2) << "Remapping tuple index " << old_idx << " to " << new_idx;
  }
  ...
  auto make_while_computation_replacements = [&](const HloComputation* comp) {
    ...
    for (const auto* user : materialized_users) {
      ...
      ITEX_CHECK_EQ(user->opcode(), HloOpcode::kGetTupleElement)
...
static StatusOr<bool> TryRemoveDeadWhileParams(HloInstruction* while_op) {
  ...
  if (!while_op->parent()->IsSafelyRemovable(while_op)) {
    ITEX_VLOG(2) << "Can't remove dead parameters from non-removable while op.";
    return false;
  }
  ...
  if (!while_init->shape().IsTuple()) {
    ITEX_VLOG(2) << "While op's carried value isn't tuple shaped.";
    return false;
  }
  ...
  for (const HloInstruction* instr : {while_body->parameter_instruction(0),
    ...
    for (const HloInstruction* user : instr->users()) {
      if (user->opcode() != HloOpcode::kGetTupleElement) {
        ITEX_VLOG(2) << "Cowardly refusing to analyze while loop with "
  ...
  if (tuple_size == 0) {
    ITEX_VLOG(2)
        << "Can't remove elements from while loop's tuple -- it's already "
           "empty.";
    return false;
  }
  ...
  absl::flat_hash_map<HloInstruction*, itex::UnionFind<HloInstruction*>>
      disjoint_sets;
  // Initialize.
  ...
  for (int64_t i = 0; i < tuple_size; ++i) {
    if (!indices_affecting_others.contains(i) &&
      ...
      ITEX_VLOG(2) << "Remove with dependencies " << i;
  ...
  for (const auto& group : groups) {
    ...
    ITEX_VLOG(2) << "Remove with groups:";
    for (int64_t index : group.second) {
      ITEX_VLOG(2) << "    index " << index;
      used_tuple_indices.erase(index);
    }
  }

  if (used_tuple_indices.size() == tuple_size) {
    ITEX_VLOG(2) << "Loop " << while_op->ToString(print_no_metadata)
  ...
  ITEX_CHECK_LT(used_tuple_indices.size(), tuple_size);

  ITEX_VLOG(1) << "Eliminating " << tuple_size - used_tuple_indices.size()
...
static StatusOr<HloInstruction*> TryRemoveRepeatedWhileTupleIndicesHelper(
  ...
  ITEX_VLOG(2) << "while_init " << while_init->ToString() << " operands "
               << while_init->operand_count();
  ITEX_VLOG(2) << "while_body_root "
               << while_body->root_instruction()->ToString() << " operands "
               << while_body->root_instruction()->operand_count();
...
static StatusOr<bool> TryRemoveRepeatedWhileTupleIndices(
  ...
  while (index_to_investigate < while_init->shape().tuple_shapes_size()) {
    ...
    ITEX_VLOG(2) << "Iterating " << index_to_investigate;
    ...
    if (pivot_body_elem->opcode() == HloOpcode::kGetTupleElement &&
      ...
      if (pivot_body_elem->tuple_index() != index_to_investigate) {
        ITEX_VLOG(2) << "Mismatch between pivot_body_elem->tuple_index() "
                     << pivot_body_elem->tuple_index()
    ...
    for (int64_t i = index_to_investigate + 1;
      ...
      if (body_elem->opcode() == HloOpcode::kGetTupleElement &&
        ...
        if (body_elem->tuple_index() != i) {
          ITEX_VLOG(2) << "Mismatch between body_elem->tuple_index() "
      ...
      if (pivot_init_elem == init_elem) {
        ITEX_VLOG(2) << "init_elem " << init_elem->ToString()
                     << " pivot_init_elem " << pivot_init_elem->ToString();
        ITEX_VLOG(2) << "body_elem " << body_elem->ToString()
                     << " pivot_body_elem " << pivot_body_elem->ToString();
        duplicates.insert(i);
      }
    }
...
static StatusOr<bool> TryRemoveConstantParams(HloInstruction* while_op) {
  ...
  auto remove_constant_elems = [&](HloInstruction* instr) {
    ITEX_CHECK(ShapeUtil::Compatible(instr->shape(), while_shape));
  ...
  auto add_constant_elems = [&](HloInstruction* instr) {
    ITEX_CHECK(ShapeUtil::Compatible(instr->shape(), new_while_shape));
...
static StatusOr<bool> TryRemoveWhileLoop(HloInstruction* while_op) {
  ...
  if (!while_op->parent()->IsSafelyRemovable(while_op)) {
    ITEX_VLOG(2)
        << "Not attempting to remove while loop that is not removable: "
  ...
  if (trip_count && *trip_count == 1 && !skip_trip_count_one_simplification) {
    ...
    if (!has_side_effects) {
      ...
      ITEX_VLOG(2)
          << "Not attempting to simplify while loop because it contains a "
             "side-effecting node: "
          << while_op->ToShortString();
    }
  }
...
static StatusOr<bool> TryPropagateConstant(HloInstruction* while_op) {
  ...
  for (int i = 0; i < root_operands.size(); i++) {
    ...
    if (Match(root_operands[i],
      ...
      ITEX_VLOG(3) << "Found loop invariant tuple element " << i << " "
  ...
  auto propagate_constant = [&](HloComputation* computation) -> StatusOr<bool> {
    ...
    for (auto instr : param->users()) {
      ...
      if (instr->opcode() == HloOpcode::kGetTupleElement) {
        ITEX_VLOG(3) << "tuple index " << instr->tuple_index() << " "
                     << instr->ToString();
        ...
        if (iter != index_to_constant.end()) {
          ...
          ITEX_VLOG(3) << "Replace use of " << instr->ToString() << " with "
...
static std::unique_ptr<HloInstruction> UnflattenTupleInstr(
    ...
    std::vector<std::unique_ptr<HloInstruction>>* new_instrs) {
  ITEX_CHECK(desired_shape.IsTuple()) << ShapeUtil::HumanString(desired_shape);
...
static StatusOr<bool> TryFlattenNestedTuples(HloInstruction* while_op) {
  ...
  auto nested = [&](HloInstruction* instr) {
    ...
    ITEX_CHECK(ShapeUtil::Compatible(nested_instr->shape(), while_shape))
...
static StatusOr<HloInstruction*> TryMergeInductionVariables(
  ...
  ITEX_CHECK(primitive_util::IsIntegralType(elem_ty))
  ...
  for (int64_t i = 0; i < while_body_root->operand_count(); ++i) {
    ...
    if (!trip_counter && constant->literal().IsAll(1) &&
      ...
      ITEX_VLOG(10) << "Found existing trip counter at index " << i;
      trip_counter = i;
    } else {
      ITEX_VLOG(10) << "Found induction variable at index " << i;
  ...
  if (!trip_counter) {
    ITEX_VLOG(10) << "Adding new trip counter to end of loop's tuple.";
  ...
  auto convert_to_new_form = [&](HloInstruction* old_root,
    ...
    ITEX_CHECK(ShapeUtil::Compatible(old_root->shape(), while_shape));
    std::vector<HloInstruction*> tuple_elems;
  ...
  auto get_new_while_init = [&](HloInstruction* init) {
    ITEX_CHECK(ShapeUtil::Compatible(init->shape(), while_shape));
    if (!added_trip_counter) {
      return init;
    }
    std::vector<HloInstruction*> tuple_elems;
...
StatusOr<bool> WhileLoopSimplifier::Run(
  ...
  ITEX_XLA_VLOG_LINES(
      3, "WhileLoopSimplifier::Run(), before:\n" + module->ToString());
  bool changed = false;
  ...
  ITEX_XLA_VLOG_LINES(
      3, "WhileLoopSimplifier::Run(), after:\n" + module->ToString());
  return changed;
}
