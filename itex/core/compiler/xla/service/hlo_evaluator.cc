...
#include "itex/core/compiler/xla/index_util.h"
...
#include "itex/core/compiler/xla/service/hlo_evaluator_typed_visitor.h"
...
StatusOr<Literal> Compare<complex64>(const Shape& shape,
  ...
  switch (direction) {
    ...
    case ComparisonDirection::kNe:
      ...
      break;
    default:
      ITEX_LOG(FATAL) << "unhandled direction for conversion to Comparison: "
...
Status MakeEvalErrorDueToParamOrInfeed() {
  Status error = itex::errors::FailedPrecondition(
...
absl::optional<EvalErrorDetail> ParseEvalErrorDetail(const Status& error) {
  absl::optional<itex::StringPiece> error_detail =
...
absl::optional<ParsedWhileLoop> PatternMatchParseWhileLoop(
  ...
  if (loop_comparison_or_noop->index() == 1) {
    ...
    ITEX_CHECK(parameter_index_and_value.param_index.has_value());
  ...
  ITEX_CHECK_EQ(loop_comparison_or_noop->index(), 0);
  WhileCondComparison loop_comparison =
      absl::get<WhileCondComparison>(*loop_comparison_or_noop);
  ITEX_CHECK(loop_comparison.lhs.IsValid() && loop_comparison.rhs.IsValid());
  ...
  ITEX_CHECK(loop_comparison.lhs.value.has_value());
  ITEX_CHECK(loop_comparison.rhs.value.has_value());

  if (loop_comparison.lhs.param_index.has_value()) {
    ITEX_VLOG(3) << __func__
                 << " lhs index: " << *loop_comparison.lhs.param_index;
  }

  ITEX_VLOG(3) << __func__ << " lhs bound: " << *loop_comparison.lhs.value;

  if (loop_comparison.rhs.param_index.has_value()) {
    ITEX_VLOG(3) << __func__
                 << " rhs index: " << *loop_comparison.rhs.param_index;
  }

  ITEX_VLOG(3) << __func__ << " rhs bound: " << *loop_comparison.rhs.value;
...
StatusOr<Literal> HloEvaluator::Evaluate(
  ...
  ITEX_CHECK(computation.parent() != nullptr);
  ITEX_XLA_VLOG_LINES(
  ...
  if (ITEX_VLOG_IS_ON(100)) {
    for (const HloInstruction* instr : computation.instructions()) {
      ITEX_VLOG(100) << instr->name() << " = " << GetEvaluatedLiteralFor(instr);
    }
  }
  if (!result.IsKnown()) {
...
bool HloEvaluator::TryEvaluate(HloInstruction* instruction, Literal* result,
                               bool recursively_evaluate_nonconstant_operands) {
  ITEX_CHECK(result != nullptr);
  auto result_or =
      Evaluate(instruction, recursively_evaluate_nonconstant_operands);
  if (!result_or.ok()) {
    ITEX_VLOG(1) << "TryEvaluate failed:" << result_or.status();
    return false;
  }
...
Status HloEvaluator::HandleParameter(HloInstruction* parameter) {
  ...
  // arg_literals_.
  ITEX_CHECK_LT(parameter->parameter_number(), arg_literals_.size());

#ifndef NDEBUG
  const Literal* input_literal = arg_literals_[parameter->parameter_number()];
  ITEX_VLOG(2) << "Parameter evaluated to: " << input_literal->ToString();
  ITEX_DCHECK(Shape::Equal().MinorToMajorOnlyInLayout()(parameter->shape(),
                                                        input_literal->shape()))
      << "parameter shape is: "
...
Status HloEvaluator::HandleConcatenate(HloInstruction* concatenate) {
  ...
  ITEX_CHECK(reference_shape.IsArray());
  const int64_t rank = reference_shape.rank();
  const int64_t concat_dim = concatenate->dimensions()[0];
  ITEX_CHECK_GE(concat_dim, 0);
  ITEX_CHECK_LT(concat_dim, rank);
  ...
  for (int64_t i = 1; i < operands.size(); ++i) {
    ...
    ITEX_CHECK(operand_shape.IsArray());
...
Status HloEvaluator::HandleReal(HloInstruction* real) {
  ...
  switch (operand->shape().element_type()) {
    ...
    default:
      ITEX_LOG(FATAL) << "HandleReal: unknown/unhandled primitive type: "
...
Status HloEvaluator::HandleImag(HloInstruction* imag) {
  ...
  switch (operand->shape().element_type()) {
    ...
    default:
      ITEX_LOG(FATAL) << "HandleImag: unknown/unhandled primitive type: "
                      << PrimitiveType_Name(operand->shape().element_type());
  }
...
Status HloEvaluator::HandleComplex(HloInstruction* complex) {
  ...
  switch (complex->shape().element_type()) {
    ...
    default:
      ITEX_LOG(FATAL) << "HandleComplex: unknown/unhandled primitive type: "
...
Status HloEvaluator::HandleCompare(HloInstruction* compare) {
  ...
  ITEX_DCHECK(ShapeUtil::SameDimensions(compare->shape(), rhs->shape()) &&
  ...
  switch (lhs->shape().element_type()) {
    ...
    default:
      ITEX_LOG(FATAL) << "HandleCompare: unknown primitive type: "
...
Status HloEvaluator::HandleTuple(HloInstruction* tuple) {
  ...
  if (evaluated_.contains(tuple)) {
    ...
    ITEX_CHECK(new_result.IsDetermined(visitor_shape_index_));
    TF_RETURN_IF_ERROR(
...
class FftTransform {
 ...
 private:
  ...
  static bool GatherToBuffer(absl::Span<ComplexType> data, int64_t length,
    ...
    ITEX_CHECK_GE(buffer.size(), length);
    bool input_is_zero = true;
    const int64_t ub = expand_input ? length / 2 + 1 : length;
    ITEX_CHECK_GE(data.size(), start + (ub - 1) * stride);
  ...
  static void Fft1D(int64_t length, int64_t start, int64_t stride, bool inverse,
                    ...
                    absl::Span<ComplexType> data,
                    absl::Span<ComplexType> buffer) {
    ITEX_CHECK(absl::has_single_bit(static_cast<uint64_t>(length)));
    const bool input_is_zero =
        GatherToBuffer(data, length, start, stride, expand_input, buffer);
  ...
  static std::vector<int64_t> ComputeStrides(
    ...
    ITEX_CHECK_EQ(num_dimensions, layout.minor_to_major_size());
  ...
  static void GenerateIndices(const absl::Span<const int64_t> dst_lengths,
    ...
    ITEX_CHECK_EQ(dst_lengths.size() + 1, dst_strides.size());
    ITEX_CHECK_GE(dst_lengths.size(), rank);
    ITEX_CHECK_EQ(src_lengths.size() + 1, src_strides.size());
    ITEX_CHECK_GE(src_lengths.size(), rank);
  ...
  bool CopyDataFromInput(const Literal& input_literal, int64_t input_start,
                         ...
                         absl::Span<ComplexType> data) {
    ITEX_CHECK_GE(data.size(), fft_size);
  ...
  Status CheckParameters(const Shape& input_shape, const Shape& output_shape) {
    ...
    ITEX_CHECK_OK(ShapeUtil::ValidateShape(input_shape));
    ...
    ITEX_CHECK_OK(ShapeUtil::ValidateShape(output_shape));
...
Status HloEvaluator::HandleGather(HloInstruction* gather) {
  ...
  auto gather_inner_loop_body =
    ...
    for (int i = 0, e = output_index.size(); i < e; i++) {
      ...
      ITEX_DCHECK_LT(output_index[i], shape.dimensions(i));
    }
    ...
    for (int i = 0, e = input_index.size(); i < e; i++) {
      ...
      ITEX_DCHECK_GE(input_index[i], 0);
      ITEX_DCHECK_LT(input_index[i], operand_shape.dimensions(i));
    }
    TF_RETURN_IF_ERROR(
...
Status HloEvaluator::HandleScatter(HloInstruction* hlo) {
  ...
  auto scatter_inner_loop_body =
    ...
    for (int i = 0, e = update_index.size(); i < e; i++) {
      ...
      ITEX_DCHECK_LT(update_index[i], updates_dims[i]);
    }
...
Status HloEvaluator::HandleWhile(HloInstruction* while_hlo) {
  ...
  while (keep_going) {
    ...
    if (keep_going) {
      ...
      ITEX_VLOG(3) << "Loop iteration result: " << body_val.ToString();
...
Status HloEvaluator::HandleSort(HloInstruction* sort) {
  ...
  if (ITEX_VLOG_IS_ON(3)) {
    for (int64_t i = 0; i < sort->operand_count(); ++i) {
      ITEX_VLOG(3) << "HandleSort operand " << i << " literal: "
  ...
  if (sort->operand_count() == 1) {
    ...
    ITEX_VLOG(3) << "HandleSort result_tuple: " << result_tuple.ToString();
...
Status HloEvaluator::HandleReduce(HloInstruction* instr) {
  ...
  for (int64_t i = 0; i < num_args; ++i) {
    ...
    ITEX_VLOG(3) << "HandleReduce arg_literal: " << input_args[i]->ToString();
    init_values[i] = &GetEvaluatedLiteralFor(reduce->init_values()[i]);
    ITEX_VLOG(3) << "HandleReduce init_literal: " << init_values[i]->ToString();
  ...
  if (is_tuple) {
    ...
    for (int64_t i = 0; i < num_args; ++i) {
      ITEX_CHECK_OK(tuple_result.MoveFrom(std::move(results[i]), {i}));
    }
    evaluated_[reduce] = std::move(tuple_result);
  } else {
    ITEX_CHECK_EQ(results.size(), 1);
...
Status HloEvaluator::Preprocess(HloInstruction* hlo) {
  ITEX_VLOG(2) << "About to visit HLO: " << hlo->ToString();
  if (!enable_partial_evaluation_) {
...
Status HloEvaluator::Postprocess(HloInstruction* hlo) {
  ITEX_VLOG(2) << "Finished visiting " << hlo->ToString()
               << "; evaluated value is: "
               << GetEvaluatedLiteralFor(hlo).ToString();
  ...
  itex_xla::Shape hlo_shape = hlo->shape();
...
std::unique_ptr<Array2D<T>> MatmulArray2DImpl(
    ...
    const std::function<void(const void* run_options_ptr, T* out, T* lhs,
        ...
        impl_fn) {
  ITEX_CHECK_EQ(lhs.width(), rhs.height());
  int m = lhs.height();
  int n = rhs.width();
  int k = lhs.width();
