...
Status HloInputOutputAliasConfig::SetUpAlias(
  ...
  ITEX_VLOG(4) << "Set up alias between output index "
               << output_index.ToString() << " and parameter " << param_index
               << " at index " << param_index.ToString();
  return Status::OK();
}
...
bool HloInputOutputAliasConfig::ParameterMustAlias(
  ...
  bool result = false;
  alias_.ForEachElement(
      [&](const itex_xla::ShapeIndex&, absl::optional<Alias> alias) {
...
absl::optional<ShapeIndex> HloInputOutputAliasConfig::GetAliasedOutput(
  ...
  alias_.ForEachElement([&](const itex_xla::ShapeIndex& output_index,
                            absl::optional<Alias> alias) {
    if (alias && alias->parameter_number == param_number &&
...
HloInputOutputAliasConfig::GetAliasedParameter(
    const ShapeIndex& output_index) const {
  ITEX_CHECK(ShapeUtil::IndexIsValid(alias_.shape(), output_index))
