...
HloOrdering::ExecutionConstraint HloOrdering::GetExecutionConstraint(
  ...
  if (a_ancestor == nullptr) {
    ITEX_VLOG(4)
        << "Ancestors in a common computation could not be found between"
        << a->ToString() << "\n and \n"
  ...
  ITEX_CHECK_NE(b_ancestor, nullptr);
  ITEX_CHECK_EQ(a_ancestor->parent(), b_ancestor->parent());
  ...
  if (a_ancestor == b_ancestor &&
    ...
    if (a_branch == -1 && b_branch == -1) {
      ITEX_CHECK_EQ(a, a_ancestor);
      ITEX_CHECK_EQ(b, b_ancestor);
      ITEX_CHECK_EQ(a, b);
      return ExecutionConstraint::kIsSame;
    }
  ...
  ITEX_VLOG(1) << "Cannot determine order between:" << a->ToString() << "\n"
               << "and " << b->ToString()
               << " which are in the same computation\n";
  return ExecutionConstraint::kUnordered;
}
...
bool HloOrdering::UsesBeforeValueDefinition(
  ...
  auto UseIsBeforeValueDefinition = [&](const HloUse& use) {
    ITEX_VLOG(4) << "UseIsBeforeValueDefinition(use=" << use
    ...
    switch (
      ...
      case HloOrdering::ExecutionConstraint::kRunExclusiveAfter:
        ...
        ITEX_VLOG(4) << " use and value def are in exclusive branches.";
        if (!has_escaped_use_in_conditional) {
          has_use_in_exclusive_branches = true;
          ITEX_VLOG(4) << "Allowing them to share buffer.\n";
          return true;
        }
        ITEX_VLOG(4) << "value def has escaped use in conditional. \n";
        break;
    ...
    if (use.instruction->opcode() == HloOpcode::kWhile) {
      ...
      if (call_graph_->InstructionIsNestedIn(value.defining_instruction(),
        ...
        ITEX_VLOG(4) << "  use is while " << use.instruction->name()
                     << " and def is in body";
        return true;
      }
      if (call_graph_->InstructionIsNestedIn(value.defining_instruction(),
    ...
    if (use.instruction->opcode() == HloOpcode::kCall) {
      ...
      if (call_graph_->InstructionIsNestedIn(value.defining_instruction(),
                                             call->to_apply())) {
        ITEX_VLOG(4) << "  use is call " << use.instruction->name()
    ...
    if (use.instruction->opcode() == HloOpcode::kConditional) {
        ...
        if (call_graph_->InstructionIsNestedIn(
          ...
          if (!dataflow.ValueIsDefinedAt(
            ...
            for (auto value_use : value.GetUses()) {
              ITEX_VLOG(4) << "def have use:" << value_use << "\n";
              if (value_use.instruction ==
                  value_use.instruction->parent()->root_instruction()) {
                ITEX_VLOG(4) << "def use is conditional root \n";
          ...
          if (!has_use_in_exclusive_branches) {
            ITEX_VLOG(4) << "  use is conditional " << use.instruction->name()
      ...
      if (value.defining_instruction() == use.instruction) {
        ITEX_VLOG(4) << "  use is conditional " << use << " and def is "
                     << value.ToShortString();
        return true;
      }
    }

    ITEX_VLOG(4) << "  use is not before value definition";
    return false;
  };
  for (auto* use : uses) {
...
bool HloOrdering::LiveRangeStrictlyBefore(
    ...
    bool use_is_always_before_def_in_same_instr) const {
  ITEX_VLOG(4) << "LiveRangeStrictlyBefore(a = " << a.ToShortString()
               << ", b = " << b.ToShortString() << ")";
  ITEX_VLOG(4) << "Parent:" << a.instruction()->parent()->ToString() << "\n";
  if (!IsDefinedBefore(a, b)) {
    ITEX_VLOG(4) << a << " not defined before " << b;
    return false;
  }

  if (a.live_out_of_module()) {
    ITEX_VLOG(4) << a << " is live out of module and not defined before " << b;
    return false;
  }
  ...
  if (!UsesBeforeValueDefinition(uses, b, dataflow,
    ...
    ITEX_VLOG(4) << "uses of " << a << "not before " << b << " is defined";
    return false;
  }

  if (a.IsRootOf(b.instruction()->parent())) {
    ITEX_VLOG(4) << a << " is live out of computation and defined before " << b
...
bool PredecessorHloOrdering::ExecutesBeforeInSameComputation(
    const HloInstruction* a, const HloInstruction* b) const {
  ITEX_CHECK_EQ(a->parent(), b->parent());
...
void SequentialHloOrdering::Initialize() {
  ...
  ITEX_DCHECK_OK(schedule_.Verify());
