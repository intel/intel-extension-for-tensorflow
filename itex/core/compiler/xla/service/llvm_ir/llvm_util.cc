...
// (e.g. constants) context it will ITEX_CHECK fail.
llvm::Module* ModuleFromIRBuilder(llvm::IRBuilder<>* b) {
  auto block = ITEX_CHECK_NOTNULL(b->GetInsertBlock());
  auto fn = ITEX_CHECK_NOTNULL(block->getParent());
  auto module = ITEX_CHECK_NOTNULL(fn->getParent());
  return module;
}
...
llvm::Value* EmitBufferIndexingGEP(llvm::Value* array, llvm::Type* element_type,
  ...
  ITEX_CHECK(array_type->isPointerTy());
  llvm::PointerType* array_type_as_pointer =
      llvm::cast<llvm::PointerType>(array_type);
  ITEX_CHECK(array_type_as_pointer->isOpaqueOrPointeeTypeMatches(element_type));
  ITEX_VLOG(2) << "EmitBufferIndexingGEP with type="
...
llvm::Type* PrimitiveTypeToIrType(PrimitiveType element_type,
  ...
  switch (element_type) {
    ...
    default:
      ITEX_LOG(FATAL) << "unsupported type " << element_type;
  }
}
...
int GetSizeInBits(llvm::Type* type) {
  ...
  ITEX_CHECK_GT(bits, 0) << "type is not sized";
  return bits;
}
...
llvm::Constant* ConvertLiteralToIrConstant(const Literal& literal,
  ...
  ITEX_CHECK_EQ(module->getDataLayout().isLittleEndian(),
                itex::port::kLittleEndian);
...
static void LogS64(const char* tag, int64_t value) {
  ITEX_LOG(INFO) << tag << " (int64_t): " << value;
}
...
llvm::Instruction* AddRangeMetadata(int32_t lower, int32_t upper,
  ...
  //          llvm::ConstantAsMetadata::get(llvm::ConstantInt::get(i32,
  //          upper))}));
  return inst;
}
...
static Status CreateAndWriteStringToFile(const std::string& directory_name,
  ...
  TF_RETURN_IF_ERROR(
      itex::Env::Default()->RecursivelyCreateDir(directory_name));
  TF_RETURN_IF_ERROR(itex::Env::Default()->NewWritableFile(file_name, &f));
...
std::pair<llvm::Value*, llvm::Value*> UMulLowHigh32(llvm::IRBuilder<>* b,
                                                    llvm::Value* src0,
                                                    llvm::Value* src1) {
  ITEX_CHECK_EQ(src0->getType()->getPrimitiveSizeInBits(), 32);
  ITEX_CHECK_EQ(src1->getType()->getPrimitiveSizeInBits(), 32);
...
std::pair<llvm::Value*, llvm::Value*> SplitInt64ToInt32s(
  ...
  ITEX_CHECK_EQ(value_64bits->getType()->getPrimitiveSizeInBits(), 64);
