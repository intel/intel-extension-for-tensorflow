...
bool CanUpdateDynamicSliceInPlace(HloInstruction* dynamic_update_slice,
  ...
  ITEX_CHECK_EQ(HloOpcode::kDynamicUpdateSlice, dynamic_update_slice->opcode());
...
bool CanEmitFusedDynamicUpdateSliceInPlace(HloInstruction* fusion,
                                           const BufferAssignment& assignment) {
  ITEX_CHECK_EQ(fusion->opcode(), HloOpcode::kFusion);
  ...
  ITEX_CHECK_EQ(fusion_operand->opcode(), HloOpcode::kParameter);
...
Status EmitDynamicUpdateSliceInPlace(absl::Span<const IrArray> operand_arrays,
                                     ...
                                     llvm::IRBuilder<>* b) {
  ITEX_VLOG(2) << "EmitDynamicUpdateSliceInPlace for " << name;
...
static Status EmitFusedDynamicUpdateSliceInPlaceImpl(
  ...
  ITEX_VLOG(2) << "EmitFusedDynamicUpdateSliceInPlace for "
               << fusion->ToString();
