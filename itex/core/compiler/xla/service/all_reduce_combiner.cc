...
#include "itex/core/compiler/xla/service/all_reduce_key.h"
#include "itex/core/compiler/xla/service/collective_combiner_utils.h"
...
Status CombineAllReduces(absl::Span<HloInstruction* const> to_combine) {
  ...
  ITEX_VLOG(1) << "Combined " << to_combine.size() << " CRS ops";
  ...
  ITEX_VLOG(1) << "Combining set";
  for (HloInstruction* hlo : to_combine) {
    ITEX_VLOG(1) << "Set element: " << hlo->ToString();
  ...
  ITEX_VLOG(1) << "Replacing with : " << combined->ToString();
...
StatusOr<bool> AllReduceCombiner::Run(
  ...
  ITEX_VLOG(1) << "Running AllReduceCombiner with threshold of "
               << combine_threshold_in_bytes_ << " bytes";

  if (combine_threshold_in_bytes_ <= 0 || combine_threshold_count_ <= 0) {
    ITEX_VLOG(1) << "Skip AllReduceCombiner because the threshold is zero";
    return false;
  }

  if (hlo_query::ContainsLayoutConstrainedAllReduce(*module)) {
    ITEX_VLOG(1)
        << "Skip AllReduceCombiner because the module contains all-reduce "
