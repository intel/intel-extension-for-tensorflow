...
llvm::Value* EmitIntegralToFloating(llvm::Value* integer_value,
  ...
  if (primitive_util::IsSignedIntegralType(from_type)) {
    ...
    ITEX_CHECK(primitive_util::IsUnsignedIntegralType(from_type) ||
               from_type == PRED);
...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitIntegerUnaryOp(
  ...
  switch (op->opcode()) {
    case HloOpcode::kConvert: {
      ...
      ITEX_CHECK(primitive_util::IsIntegralType(from_type) || from_type == PRED)
          << from_type;
    ...
    case HloOpcode::kBitcastConvert: {
      PrimitiveType from_type = op->operand(0)->shape().element_type();
      PrimitiveType to_type = op->shape().element_type();
      ITEX_CHECK(primitive_util::IsIntegralType(from_type));
      if (from_type == to_type) {
        return operand_value;
      }
...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitFloatUnaryOp(
  ...
  switch (op->opcode()) {
    case HloOpcode::kConvert: {
      PrimitiveType from_type = op->operand(0)->shape().element_type();
      PrimitiveType to_type = op->shape().element_type();
      ITEX_CHECK(primitive_util::IsFloatingPointType(from_type)) << from_type;
      if (from_type == to_type) {
        return operand_value;
      }
      if (from_type == BF16) {
    ...
    case HloOpcode::kBitcastConvert: {
      PrimitiveType from_type = op->operand(0)->shape().element_type();
      PrimitiveType to_type = op->shape().element_type();
      ITEX_CHECK(primitive_util::IsFloatingPointType(from_type));
      if (from_type == to_type) {
        return operand_value;
      }
      if (primitive_util::BitWidth(from_type) ==
...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitElementalConcatenate(
  ...
  if (b_->GetInsertPoint() != init_block->end()) {
    ...
    ITEX_CHECK(init_block->getTerminator());
    exit_block =
  ...
  ITEX_CHECK_EQ(current_offset, hlo->shape().dimensions(concat_dim));
...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitElementalDynamicSlice(
  ...
  for (int64_t i = 0; i < rank; ++i) {
    ...
    ITEX_CHECK_GE(largest_valid_start_index, 0);
...
llvm_ir::ElementGenerator ElementalIrEmitter::MakeElementGenerator(
  ...
  switch (hlo->opcode()) {
    ...
    case HloOpcode::kBitcast:
      ITEX_CHECK_EQ(ShapeUtil::ElementsIn(hlo->shape()),
...
llvm::Value* ElementalIrEmitter::EmitMulAdd(
    llvm::Value* lhs, llvm::Value* rhs, llvm::Value* accumulator,
...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitElementalReduceWindow(
  ...
  ITEX_CHECK_EQ(window_index.size(), index.size());
...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitElementalReduce(
  ...
  ITEX_CHECK(results.size() == accumulators_count);
  for (int i = 0; i < accumulators_count; i++) {
...
StatusOr<llvm::Value*> ElementalIrEmitter::EmitAccumResult(
  ...
  if (is_variadic) {
    ...
    ITEX_CHECK_EQ(accumulator_addrs.size(), 1);
