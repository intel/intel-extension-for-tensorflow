...
bool MayUseOperandValue(int64_t operand_number, const ShapeIndex& index,
  ...
  switch (user->opcode()) {
    ...
    case HloOpcode::kCopy:
      ...
      // transparently.
      ITEX_CHECK_EQ(operand_number, 0);
      return index.empty();
...
void HloValue::SetPositions(absl::Span<const HloPosition> positions) {
  ITEX_CHECK_EQ(positions_.size(), 1)
      << "SetPositions should only be called once.";
  ...
  for (const HloPosition& position_a : positions) {
    ITEX_DCHECK_NE(position_a, defining_position());
    for (const HloPosition& position_b : positions) {
      if (&position_a != &position_b) {
        ITEX_DCHECK_NE(position_a, position_b);
      }
    }
  }
...
bool InstructionValueSet::AssignUnionOf(
  ...
  ITEX_CHECK_GT(inputs.size(), 0);
  for (int i = 1; i < inputs.size(); ++i) {
    ITEX_DCHECK(ShapeUtil::Compatible(inputs[0]->shape(), inputs[i]->shape()));
  }
  bool changed = false;
  for (auto& pair : *this) {
