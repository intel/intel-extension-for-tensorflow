...
StatusOr<HloInstruction*> ComparisonExpander::ExpandInstruction(
    HloInstruction* instruction) {
  ITEX_CHECK(instruction->opcode() == HloOpcode::kCompare);
  HloCompareInstruction* compare =
      static_cast<HloCompareInstruction*>(instruction);
  ITEX_CHECK(compare->type() == Comparison::Type::kFloatTotalOrder);
  ...
  ITEX_CHECK(primitive_util::IsFloatingPointType(compare_type));
  ...
  ITEX_VLOG(2) << "New comparison instruction for total order:"
