...
static Status CheckCommonAllGatherInvariants(HloInstruction* hlo,
  ...
  ITEX_CHECK_NE(computed_shard_count, nullptr)
      << "Expected a shard count as input";
...
Status ShapeVerifier::CheckShape(const HloInstruction* instruction,
  ...
  if (!inferred_shape_status.ok()) {
    ...
    // itex::errors::AppendToMessage(&s, ", for instruction ",
    //                                     instruction->ToString());
    return s;
  }
...
// ITEX_CHECKs various invariants of a fusion instruction.
Status CheckFusionInstruction(HloInstruction* fusion) {
  ...
  ITEX_CHECK_EQ(fusion->operands().size(), fused_parameters.size());
