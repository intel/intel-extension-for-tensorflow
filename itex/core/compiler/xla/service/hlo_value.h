...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_HLO_VALUE_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_HLO_VALUE_H_
...
class HloValueSet {
 public:
  ...
  // Return the unique HLO value in the set. ITEX_CHECKs if the set does not
  // contain exactly one value.
  const HloValue& GetUniqueValue() const {
    ITEX_CHECK_EQ(values_.size(), 1);
    return *values_[0];
  }
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_HLO_VALUE_H_
