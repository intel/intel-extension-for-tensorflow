...
void DfsHloVisitorBase<HloInstructionPtr>::SetVisiting(
    const HloInstruction& instruction) {
  ITEX_VLOG(3) << "marking HLO " << &instruction << " as visiting: ";
  ITEX_DCHECK(NotVisited(instruction));
...
void DfsHloVisitorBase<HloInstructionPtr>::SetVisited(
    const HloInstruction& instruction) {
  ITEX_VLOG(3) << "marking HLO " << &instruction << " as visited: ";
  ITEX_DCHECK(NotVisited(instruction) || IsVisiting(instruction));
