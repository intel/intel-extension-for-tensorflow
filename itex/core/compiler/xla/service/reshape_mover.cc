...
HloInstruction* FirstNonScalarAndNonTrivialReshapeOperand(
  ...
  for (HloInstruction* operand : hlo->operands()) {
    if (IsNontrivialReshape(operand)) {
      ITEX_VLOG(5) << "Found first non-trivial reshape operand of "
                   << hlo->ToString(HloPrintOptions().set_print_metadata(false))
                   << ":\n\t"
                   << operand->ToString(
                          HloPrintOptions().set_print_metadata(false));
      return operand;
    }
  }
  return nullptr;
  ...
  switch (operand->opcode()) {
    case HloOpcode::kConstant: {
      if (first_reshape_operand->opcode() == HloOpcode::kReshape) {
        ITEX_VLOG(5) << "Adding reshape to kConstant operand";
        return computation->AddInstruction(
            HloInstruction::CreateReshape(new_shape, operand));
      } else {
        ITEX_CHECK(first_reshape_operand->opcode() == HloOpcode::kTranspose);
        ITEX_VLOG(5) << "Adding transpose to kConstant operand";
    ...
    case HloOpcode::kRng: {
      ITEX_CHECK_EQ(operand->user_count(), 1);
      ITEX_VLOG(5) << "Cloning kRng operand with new shape";
      return computation->AddInstruction(
          operand->CloneWithNewOperands(new_shape, operand->operands()));
    }
    case HloOpcode::kReshape:
    case HloOpcode::kTranspose: {
      ITEX_VLOG(5) << "Using existing operand of kReshape or kTranspose";
      return operand->mutable_operand(0);
    }
    case HloOpcode::kBroadcast: {
      ITEX_CHECK(ShapeUtil::IsScalar(operand->operand(0)->shape()));
      HloInstruction* inst = computation->AddInstruction(
          operand->CloneWithNewOperands(new_shape, operand->operands()));
      ITEX_VLOG(5) << "Changing broadcast from " << operand->ToString()
                   << " to " << inst->ToString();
      return inst;
    }

    default:
      ITEX_LOG(FATAL) << "Unexpected operand opcode during update: " << operand;
  }
}
...
StatusOr<bool> PerformSinkReshapeOrTranspose(
  ...
  ITEX_VLOG(3) << "** Sinking reshape or transpose: "
  ...
  for (size_t i = 0; i < operands.size(); ++i) {
    ...
    ITEX_VLOG(3) << "Updating operand #" << i << ": "
  ...
  switch (first_reshape_operand->opcode()) {
    case HloOpcode::kReshape:
      ITEX_VLOG(3) << "Creating new reshape for new elementwise op: "
...
bool IsReshapeMoveCandidate(HloInstruction* instruction) {
  auto print_no_metadata = HloPrintOptions().set_print_metadata(false);
  ITEX_VLOG(5) << "** Checking instruction: "
               << instruction->ToString(print_no_metadata);
  ...
  for (const HloInstruction* operand : instruction->operands()) {
    if (!ShapeUtil::SameDimensions(operand->shape(), instruction->shape())) {
      ITEX_VLOG(5) << "Operand shape differs from output shape; so preventing "
                      "movement\n\toperand: "
    ...
    if (CanTriviallyChangeShape(operand)) {
      ITEX_VLOG(5) << "Operand can trivially change shape: "
                   << operand->ToString(print_no_metadata);
      continue;
    }

    if (!IsNontrivialReshape(operand)) {
      ITEX_VLOG(5) << "Operand can't trivially change shape: "
                   << operand->ToString(print_no_metadata);
      return false;
    }
    ...
    if (first_reshape_operand == nullptr) {
      ...
      ITEX_VLOG(5) << "First reshape operand "
                   << operand->ToString(print_no_metadata);
      ...
      // well.
      ITEX_VLOG(5) << "Operand is a reshape but is not equivalent to the first "
                      "Reshape operand"
                   << operand->ToString(print_no_metadata);
      return false;
    }
  ...
  if (first_reshape_operand) {
    ITEX_VLOG(5) << "All operands have easy shape changes: "
                 << instruction->ToString(print_no_metadata);
  }
...
StatusOr<bool> TryReshapeMoveOnCandidates(
  ...
  while (!reshape_candidates->empty() && removed) {
    if (ITEX_VLOG_IS_ON(5)) {
      for (const HloInstruction* instruction : *reshape_candidates) {
        ITEX_VLOG(5) << "candidate " << instruction->ToString();
      }
    }
