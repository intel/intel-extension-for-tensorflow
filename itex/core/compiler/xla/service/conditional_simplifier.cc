  ...
  *param->mutable_shape() = tuple_shape;
  ...
  for (HloInstruction* conditional : calling_conditionals) {
    ...
    for (int64_t branch = 0; branch < conditional->branch_count(); ++branch) {
      ...
      ITEX_CHECK(
          ShapeUtil::Compatible(conditional->operand(branch + 1)->shape(),
                                conditional->branch_computation(branch)
                                    ->parameter_instruction(0)
                                    ->shape()));
      ITEX_CHECK(ShapeUtil::Compatible(
          conditional->shape(),
...
bool RemoveUnusedTupleElements(HloInstruction* conditional_op) {
  if (conditional_op->user_count() == 0 ||
    ...
    ITEX_VLOG(3) << "Skip RemoveUnusedTupleElements due to non-tuple result:\n"
  ...
  for (const HloInstruction* user : conditional_op->users()) {
    ...
    if (user->opcode() != HloOpcode::kGetTupleElement) {
      ITEX_VLOG(3) << "Skip RemoveUnusedTupleElements due to non-GTE user:\n"
                   << user->ToShortString();
      return false;
    }
  ...
  if (new_tuple_shapes_size == old_tuple_shapes_size) {
    ITEX_VLOG(3)
        << "Skip RemoveUnusedTupleElements due to every index is in use.";
    return false;
  }
  ...
  for (HloComputation* branch : conditional_op->branch_computations()) {
    ...
    if (!root->shape().IsTuple() ||
        !ShapeUtil::Compatible(branch->root_instruction()->shape(),
                               old_shape)) {
      ITEX_VLOG(3) << "Skip RemoveUnusedTupleElements due to some branch "
...
bool MergeDuplicateTupleElements(HloInstruction* conditional) {
  if (conditional->user_count() == 0 ||
    ...
    ITEX_VLOG(3)
        << "Skip MergeDuplicateTupleElements due not tuple shape nor root "
           "instruction:\n"
  ...
  for (const HloInstruction* user : conditional->users()) {
    if (user->opcode() != HloOpcode::kGetTupleElement) {
      ITEX_VLOG(3) << "Skip MergeDuplicateTupleElements due not all users are "
                      "kGetTupleElement:\n"
                   << conditional->ToShortString();
      return false;
    }
  ...
  for (const HloComputation* branch : conditional->branch_computations()) {
    if (branch->root_instruction()->opcode() != HloOpcode::kTuple) {
      ITEX_VLOG(3)
          << "Skip MergeDuplicateTupleElements due not all branch roots "
             "are kTuple:\n"
          << conditional->ToShortString();
      return false;
    }
...
StatusOr<bool> ConditionalSimplifier::TryRemoveConditional(
  ...
  if (!conditional->parent()->IsSafelyRemovable(conditional) ||
    ...
    ITEX_VLOG(2)
        << "Not attempting to remove conditional as it is not removable or "
           "has side effect: "
        << conditional->ToShortString();
    return false;
  }
...
StatusOr<bool> ConditionalSimplifier::Run(HloModule* module) {
  ...
  for (auto* comp : calling_computationals_vector) {
    ...
    ITEX_CHECK(entry != calling_conditionals.end());
  ...
  ITEX_XLA_VLOG_LINES(
      3, "ConditionalSimplifier::Run(), after:\n" + module->ToString());
  return changed;
}
