...
void HloSchedule::set_sequence(const HloComputation* computation,
                               HloInstructionSequence sequence) {
  ITEX_CHECK(computation->parent() == module_);
  ...
  auto schedule_worklist = [&]() {
    while (!worklist.empty()) {
      ...
      std::vector<HloInstruction*>* new_users =
          itex::gtl::FindOrNull(new_instruction_uses, instruction);
      if (new_users != nullptr) {
        ...
        for (HloInstruction* new_user : *new_users) {
          ...
          ITEX_CHECK_GE(unscheduled_operand_count.at(new_user), 0);
...
Status HloSchedule::Update() {
  ...
  ITEX_CHECK_EQ(sequences_.size(), nonfusion_computations.size());
...
Status HloSchedule::Verify() const {
  ITEX_VLOG(2) << "VerifySchedule()";
  ITEX_XLA_VLOG_LINES(2, ToString());
