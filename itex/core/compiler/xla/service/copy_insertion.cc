...
#include "itex/core/compiler/xla/service/compile_time_cap.h"
#include "itex/core/compiler/xla/service/dump.h"
#include "itex/core/compiler/xla/service/hlo_alias_analysis.h"
...
#include "itex/core/compiler/xla/service/hlo_ordering.h"
...
DeepCopyAndAddControlEdges(HloInstruction* from, HloInstruction* to,
  ...
  ITEX_DCHECK(ShapeUtil::Compatible(from->shape(), to->shape()));
...
bool IndicesToCopyForWhile(const HloDataflowAnalysis& dataflow,
  ...
  ITEX_DCHECK(
      ShapeUtil::Compatible(indices_to_copy->shape(), xla_while->shape()));
...
bool IndicesToCopyForConditional(const HloDataflowAnalysis& dataflow,
                                 ...
                                 ShapeTree<bool>* indices_to_copy) {
  ITEX_DCHECK(ShapeUtil::Compatible(indices_to_copy->shape(),
  ...
  for (auto& pair : *indices_to_copy) {
    ...
    ITEX_CHECK_EQ(dataflow.GetValueSet(xla_conditional, index).values().size(),
                  1);
...
Status AddCopiesForWhile(const HloAliasAnalysis& alias_analysis,
  ...
  ITEX_VLOG(2) << "Adding copies for kWhile instruction " << xla_while->name();
  ...
  if (!IndicesToCopyForWhile(alias_analysis.dataflow_analysis(), xla_while,
                             &indices_to_copy)) {
    ITEX_VLOG(2) << "No copies necessary for kWhile instruction "
                 << xla_while->name();
    return Status::OK();
  }

  ITEX_VLOG(2) << "Adding copies for " << xla_while->name() << " at indices:";
  for (auto& pair : indices_to_copy) {
    if (pair.second) {
      ITEX_VLOG(2) << "  " << pair.first;
    }
  }
...
Status AddCopiesForInPlaceOperation(const HloAliasAnalysis& alias_analysis,
  ...
  ITEX_VLOG(2) << "Adding copies for in-place operation "
               << in_place_op->name();
...
class LiveRangeRegions {
 public:
  ...
  const InstructionMap& operator[](const HloComputation* computation) const {
    ...
    ITEX_CHECK(p != computation_map_.end());
    return p->second;
  }
  ...
  int64_t size() const {
    ITEX_CHECK_EQ(computation_vector_.size(), computation_map_.size());
...
class Relation {
 public:
  ...
  void UnionRelationFromSameSource(const Relation& rel) {
    ITEX_CHECK_LE(orders_.size(), 1);
    ITEX_CHECK_EQ(rel.orders_.size(), 1);
    if (orders_.empty()) {
...
class ComputeRelativeLocation {
 public:
  ...
  Relation Compute(const InstructionEntry& entry1,
    ...
    if (def->opcode() == HloOpcode::kParameter &&
      ...
      ITEX_VLOG(3) << "Setting interception due to parameter/root relation\n";
    ...
    if (Relation::UseImpliesInterception(order)) {
      ...
      if (Relation::DefinitionImpliesInterception(order2)) {
        ITEX_VLOG(3) << "Setting interception for " << def->ToString()
    ...
    for (int64_t index = 0; index < range1.size(); index++) {
      ...
      for (const auto& computation_entry2 : range2) {
        ...
        for (auto instr_entry2 : computation_entry2.second) {
          ...
          ITEX_VLOG(3) << "Locationing " << instr_entry2.first->ToString();
          ...
          for (auto instr_entry1 : range1[computation1]) {
            ...
            ITEX_VLOG(3) << "new relation with:"
                         << instr_entry1.first->ToString() << " = "
                         << rel.ToString() << "\n";
            ...
            ITEX_VLOG(3) << "instr2 relation:" << instr2_relation.ToString()
                         << "\n";
          }
          ...
          if (!ForceRuntimeOrder(unordered_ops, instr_entry2,
            ...
            ITEX_VLOG(3) << "Unable to force ordering of unordered ops\n";
          ...
          ITEX_VLOG(3) << "Resulting relation : " << dir_src_dest.ToString()
                       << "\n";
        }
      }
    }
    ...
    for (const auto& comp_it : ctrl_deps_) {
      ...
      for (const auto& instr_it : comp_it.second) {
        ...
        for (HloInstruction* entry2 : instr_it.second) {
          ITEX_VLOG(3) << "Add control dependence between "
                       << entry2->ToString();
          ITEX_VLOG(3) << "\n vs " << entry1->ToString() << "\n";
          ITEX_CHECK_OK(entry2->AddControlDependencyTo(entry1));
        }
        reachability_map.UpdateReachabilityThroughInstruction(entry1);
        for (HloInstruction* entry2 : instr_it.second) {
          ITEX_DCHECK(ordering_->GetExecutionConstraint(entry1, entry2) ==
 ...
 private:
  ...
  bool ForceRuntimeOrder(absl::Span<const InstructionEntry> unordered_ops,
    ...
    for (const InstructionEntry& entry1 : unordered_ops) {
      ...
      if (succ->opcode() == HloOpcode::kCopy &&
        ...
        ITEX_VLOG(3)
            << "Failed to force unordered op ordering due to copy ordering "
            << " between " << pred->ToString() << "\n";
        ITEX_VLOG(3) << " vs. " << succ->ToString() << "\n";
        return false;
      }
    }
    for (const InstructionEntry& entry1 : unordered_ops) {
  ...
  Relation::RuntimeOrder Save(HloInstruction* entry1, HloInstruction* entry2,
    ...
    ITEX_CHECK_EQ(AlreadyComputed(entry1, entry2).first, kNotComputed);
    // Do not save unordered relations.
    ITEX_CHECK_NE(relation, Relation::kBeforeStartOrAfterEnd);
    saved_relations_[entry2][entry1] = relation;
    if (is_unordered_originally) {
      ITEX_CHECK(relation == Relation::kBeforeStart ||
      ...
      ITEX_VLOG(3) << "Save unordered relation: " << pred->ToString() << "\n";
      ITEX_VLOG(3) << " vs " << succ->ToString() << "\n";
      ITEX_CHECK_EQ(succ->parent(), pred->parent());
      ...
      for (HloInstruction*& op : dep_vec) {
        ...
        if (rel.first != kNotComputed) {
          if (rel.second == Relation::kAfterEnd) {
            ...
            ITEX_CHECK(rel.second == Relation::kBeforeStart);
          }
          return relation;
        }
      }
      ITEX_VLOG(2) << "Forcing unordered:" << pred->ToString() << "\n";
      ITEX_VLOG(2) << " vs " << succ->ToString() << "\n";
  ...
  Relation::RuntimeOrder ComputeRuntimeOrdering(HloInstruction* instr1,
    ...
    if (saved_relation.first != kNotComputed) {
      ITEX_VLOG(3) << "Already computed between " << instr1->ToString()
                   << "\n vs " << instr2->ToString() << "\n";
      ...
      case HloOrdering::ExecutionConstraint::kUnordered: {
        ...
        if (!ctrl_deps_.empty()) {
          ...
          if (absl::c_any_of(ctrl_deps[instr2], [&](HloInstruction* pred2) {
            ...
            ITEX_VLOG(2) << "control-dependent: " << instr1->ToString() << "\n";
            ITEX_VLOG(2) << "vs " << instr2->ToString() << "\n";
            return Save(instr1, instr2, Relation::kBeforeStart);
          } else if (absl::c_any_of(
                         ctrl_deps[instr1], [&](HloInstruction* pred1) {
                           return ControlDependenceBefore(instr2, pred1);
                         })) {
            ITEX_VLOG(2) << "control-dependent: " << instr2->ToString() << "\n";
            ITEX_VLOG(2) << "vs " << instr1->ToString() << "\n";
            return Save(instr1, instr2, Relation::kAfterEnd);
          }
        }
...
class CopyRemover {
 public:
  ...
  CopyRemover(const HloModule& module, const HloAliasAnalysis& alias_analysis,
    ...
    ITEX_XLA_VLOG_LINES(3, ToString());
    ITEX_DCHECK_OK(Verify());
  }
  ...
  bool TryElideCopy(const HloInstruction* copy,
    ...
    ITEX_VLOG(2) << "Trying to remove " << copy->name();
    ITEX_CHECK_NE(region_analysis_limit, nullptr);

    if (!ContainsKey(copy_map_, copy)) {
      ITEX_VLOG(2) << copy->name() << " is not removable";
      return false;
    }
    if (!ShapeUtil::Equal(copy->shape(), copy->operand(0)->shape())) {
      ITEX_VLOG(2) << copy->name() << " is not removable (shape mismatch)";
      return false;
    }
    const CopyNodes& copy_node = copy_map_.at(copy);
    ITEX_DCHECK(copy_node.src != nullptr);
    ITEX_DCHECK(copy_node.dest != nullptr);
    ...
    *region_analysis_limit = 0;
    ITEX_VLOG(3) << copy->name() << " copies value "
                 << copy_node.src->value->ToShortString();
    ITEX_VLOG(3) << "Source buffer values: "
                 << ValueListToString(copy_node.src);
    ITEX_VLOG(3) << "Dest buffer values: " << ValueListToString(copy_node.dest);
      ...
      for (ValueNode* next_dest = dest; next_dest != nullptr;
        ...
        for (ValueNode* prev_src = src; prev_src != nullptr;
          ...
          if (!LiveRangeBefore(*prev_src, *next_dest)) {
            ITEX_VLOG(2) << "Live range of " << prev_src->value->ToShortString()
                         << " is not before "
    ...
    auto CheckLiveRangeInterference = [&](ValueNode* src, ValueNode* dest,
      ...
      ITEX_CHECK_NE(src, nullptr);
      ITEX_CHECK_NE(dest, nullptr);
      if (!use_region_analysis) {
        ITEX_VLOG(2) << "Configured to not use region-based analysis.\n";
        return true;
      }
      *region_analysis_limit += live_range_size1 * live_range_size2;
      if (ValuesInterfere(src, dest, option)) {
        ITEX_VLOG(2) << "Region-based interference is true. \n";
        return true;
      }
      ITEX_VLOG(2) << "Region-based interference is false. \n";
      return false;
    };
    ...
    if (copy_node.src->next == copy_node.dest) {
      ...
      // safely removed.
      ITEX_VLOG(2) << copy->name()
                   << " source and destination buffers are same.";
      ...
      //    used to be.
      ITEX_VLOG(2) << copy->name() << " defines the first value in its buffer";
      ...
      ITEX_VLOG(2) << copy->name() << " copies the last value ("
                   << copy_node.src->value->ToShortString()
                   << ") in its buffer";
      bool live_range_before =
      ...
      if (!live_range_before &&
        ...
        ITEX_VLOG(2) << "Region-based analysis concludes interference.\n";
        return false;
      }
      ITEX_VLOG(2) << "Splice src after prev of dest.";
    ...
    ITEX_XLA_VLOG_LINES(4, ToString());
    ITEX_DCHECK_OK(Verify());
  ...
  void RemoveCopyValue(ValueNode* copy_value_node) {
    ITEX_CHECK_EQ(copy_value_node->value->defining_instruction()->opcode(),
                  HloOpcode::kCopy);
    ValueNode* operand_node = copy_value_node->prev;
    ITEX_CHECK(operand_node != copy_value_node);

    ITEX_VLOG(2) << "Removing copy " << operand_node->value->ToShortString()
    ...
    ITEX_CHECK(it != operand_node->uses.end());
  ...
  bool LiveRangeBefore(const ValueNode& a, const ValueNode& b) {
    if (a.uses.empty()) {
      ITEX_VLOG(2) << "Empty uses for " << *a.value;
      return ordering_->IsDefinedBefore(*a.value, *b.value);
    }
    ITEX_VLOG(3) << "Checking live ranges before :" << ValueListToString(&a)
    ...
    if (a.value->IsRootOf(b.value->defining_instruction()->parent())) {
      ITEX_VLOG(3) << "Value is root of the same computation";
      return false;
    }
  ...
  bool ValuesInterfere(const ValueNode* src, const ValueNode* dest,
    ...
    if (rel1.RuntimeOrderOverlap() || rel2.RuntimeOrderOverlap()) {
      ...
      if (rel1.RuntimeOrderOverlap()) {
        ...
        ITEX_VLOG(3) << "rel2 is overlap, with interception = "
                     << rel2.InterceptDefUse() << "\n";
...
Status CopyInsertion::AddCopiesForConditional(
  ...
  ITEX_VLOG(2) << "Adding copies for kConditional instruction "
               << conditional->name();
  ...
  for (HloComputation* computation : module->computations()) {
    ...
    ShapeUtil::ForEachSubshape(root->shape(), [&](const Shape& /*subshape*/,
      ...
      if (buffer_seen_before && policy.copy_root_replicated_buffers &&
        ...
        ITEX_CHECK(alias) << "Alias does not exist";
        const ShapeIndex& other_index = seen[buffers_at_index[0]];
        ITEX_VLOG(2) << "Output indices " << index.ToString() << " and "
      ...
      if (buffers_at_index.size() > 1 ||
        ...
        ITEX_VLOG(2) << "Index " << index << " of computation "
    ...
    for (const auto& pair :
      ...
      for (const HloValue* value : value_set.values()) {
        if (ShouldCopyRootValue(*value, policy)) {
          ITEX_VLOG(2) << "Root of (" << root->name() << ") of computation("
                       << computation->name()
...
Status CopyInsertion::RemoveUnnecessaryCopies(HloOrdering* ordering,
  ...
  ITEX_XLA_VLOG_LINES(4, module->ToString());
  TF_ASSIGN_OR_RETURN(std::unique_ptr<HloAliasAnalysis> alias_analysis,
                      HloAliasAnalysis::Run(module, can_share_buffer_));
  CopyRemover copy_remover(*module, *alias_analysis, ordering,
                           check_live_range_ordering);
  if (ITEX_VLOG_IS_ON(3)) {
    ITEX_LOG(INFO) << "Removing unnecessary copies in " << module->name();
    ITEX_LOG(INFO) << "Buffer values, in dependency order: ";
    for (const HloBuffer& buffer : alias_analysis->buffers()) {
      ITEX_LOG(INFO) << "    HloBuffer " << buffer.id();
    }
  }
  ...
  ITEX_VLOG(6) << "Copy Insertion analyzing module with instructino count = "
               << module->instruction_count() << "\n";
  BoundNonLinearCompilerAnalysis allowance(module, name(), 10);
  while (changed) {
    ITEX_CHECK_LE(++num_iterations, num_existing_copies);
    changed = false;
    ITEX_VLOG(2) << "Running fixpoint iteration " << num_iterations
                 << " of copy elision";
    for (HloComputation* computation : module->computations()) {
      ITEX_VLOG(2) << "computation:" << computation->name() << "\n";
      for (HloInstruction* instruction : computation->instructions()) {
        ITEX_VLOG(2) << instruction->ToString() << "\n";
        ...
        if (instruction->opcode() == HloOpcode::kCopy) {
          if (copy_remover.TryElideCopy(instruction,
            ...
            ITEX_VLOG(6) << "succeeded in eliminating copy.\n";
          }
          if (allowance.ContinueAnalysis() && region_analysis_cost_now > 0) {
            ITEX_VLOG(6) << "Copy Insertion analyzing module cost: "
                         << region_analysis_cost_now << "\n";
            ITEX_VLOG(6) << "instruction:" << instruction->ToString() << "\n";
            allowance.DeductCost(region_analysis_cost_now);
            ITEX_VLOG(6) << "allowance:" << allowance.analysis_allowance()
                         << "\n";
          }
        }
      }
...
StatusOr<bool> CopyInsertion::Run(HloModule* module) {
  ...
  if (ITEX_VLOG_IS_ON(1)) {
    ...
    ITEX_VLOG(1) << "Num copies before copy-insertion: "
                 << GetNumExistingCopies(module);
    ITEX_VLOG(1) << "Num copies after copy-insertion: " << num_total_copies;
  }
