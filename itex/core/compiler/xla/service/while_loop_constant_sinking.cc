...
Status ReplaceUsesWhileKeepingLoopInvariance(HloInstruction* old_instr,
                                             ...
                                             int64_t tuple_index) {
  ITEX_CHECK_EQ(while_body_root->opcode(), HloOpcode::kTuple);
...
HloInstruction* CloneHelper(const HloInstruction* instruction,
  ...
  ITEX_LOG(FATAL) << "Unexpected instruction.";
}
...
StatusOr<bool> WhileLoopConstantSinking::Run(
  ...
  ITEX_VLOG(2) << "HLO module before WhileLoopConstantSinking:";
  ...
  if (changed) {
    ITEX_VLOG(2) << "HLO module after WhileLoopConstantSinking:";
    ITEX_XLA_VLOG_LINES(2, module->ToString());
  } else {
    ITEX_VLOG(2) << "HLO module unchanged after WhileLoopConstantSinking";
  }
