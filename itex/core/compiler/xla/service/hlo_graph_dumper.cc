...
#include "itex/core/utils/base64.h"
...
optional<std::string> MatchTrivialComputation(
  ...
  if (root->operand(0) == param1) {
    ITEX_CHECK_EQ(root->operand(1), param0);
...
>
  ...
  if (profile_ != nullptr) {
    ...
    absl::StrAppendFormat(&graph_label, "<br/>total cycles = %d (%s)", cycles,
                          itex::strings::HumanReadableNum(cycles));
  }
  ...
  for (const auto& kv : edge_ids_) {
    ...
    int64_t from_node_id = itex::gtl::FindWithDefault(node_ids_, from_node, -1);
    if (from_node_id == -1) {
      ITEX_LOG(FATAL) << from_node->name()
                      << " was added to edges but not to nodes";
    }
    int64_t to_node_id =
        to_node ? itex::gtl::FindWithDefault(node_ids_, to_node, -1)
                : root_node_id_;
    if (to_node != nullptr && to_node_id == -1) {
      ITEX_LOG(FATAL) << to_node->name()
                      << " was added to edges but not to nodes";
    }
    ...
    if (to_node) {
      ITEX_VLOG(3) << "Adding css for edge " << edge_id << " from node "
...
bool HloDotDumper::ShouldShowFusionSubcomputation(const HloInstruction* instr) {
  ITEX_CHECK_EQ(instr->opcode(), HloOpcode::kFusion);
...
std::string HloDotDumper::DumpSubcomputation(
  ...
  ITEX_VLOG(2) << "Dumping subcomputation " << subcomp->name();
  ...
  if (parent_instr->opcode() != HloOpcode::kFusion) {
    ...
    ITEX_VLOG(2) << "Edge: from " << from->name() << " to "
...
std::string HloDotDumper::DumpRootTag() {
  ...
  ITEX_VLOG(2) << "Adding root tag as node " << next_node_id_;
  root_node_id_ = next_node_id_++;

  ITEX_VLOG(2) << "Adding edge from " << from->name() << " to root tag as "
               << next_edge_id_;
...
std::string HloDotDumper::DumpInstruction(const HloInstruction* instr) {
  ...
  ITEX_VLOG(2) << "Adding node " << instr->name() << " as " << next_node_id_;
  ...
  std::string extended_opcode =
      StrCat(HloOpcodeString(instr->opcode()),
             instr->opcode() != HloOpcode::kFusion
                 ? ""
                 : StrCat(":", itex_xla::ToString(instr->fusion_kind())));
...
void HloDotDumper::AddInstructionIncomingEdges(const HloInstruction* instr) {
  auto add_edge = [&](const HloInstruction* from, const HloInstruction* to,
    ...
    ITEX_VLOG(2) << "Adding edge from " << from->name() << " to " << to->name()
                 << " as " << next_edge_id_;
    edge_ids_.insert({{from, to}, next_edge_id_++});
...
StatusOr<std::string> WrapDotInFormat(const HloComputation& computation,
  ...
  switch (format) {
    case RenderedGraphFormat::kUrl:
      ITEX_CHECK(url_renderer != nullptr)
...
static StatusOr<std::string> CompressAndEncode(absl::string_view input) {
  class WritableStringFile : public itex::WritableFile {
   public:
  ...
  itex::io::ZlibOutputBuffer gz_file(&f, gz_opts.input_buffer_size,
  ...
  std::string encoded;
  TF_RETURN_IF_ERROR(itex::Base64Encode(compressed, &encoded));
...
void RegisterGraphToURLRenderer(
  ...
  if (url_renderer != nullptr) {
    ITEX_LOG(WARNING)
        << "Multiple calls to RegisterGraphToURLRenderer.  Last call "
...
StatusOr<std::string> RenderAllPathsFromTo(
  ...
  ITEX_CHECK_EQ(from.parent(), to.parent())
      << "Nodes must be in same computation!";
