...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_HLO_INSTRUCTION_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_HLO_INSTRUCTION_H_
...
#include "itex/core/compiler/xla/iterator_util.h"
...
class HloInstruction {
 public:
  ...
  // ITEX_CHECKs whether the instruction has compatible sharding with the other
  // instruction.
  ...
  void SetUniqueId(int id) {
    ITEX_CHECK_EQ(unique_id_, -1);  // Should not be assigned already
    ITEX_CHECK_GE(id, 0);
    unique_id_ = id;
  }
  ...
  using EnableIfProto = typename std::enable_if_t<
      std::is_base_of<itex::protobuf::Message, T>::value>;
  ...
  Status set_backend_config(const itex::protobuf::Message& proto) {
 ...
 private:
  ...
  class BackendConfigRep {
   public:
    const itex::protobuf::Message* GetProtoPtr() const { return proto_.get(); }
    ...
    BackendConfigRep& operator=(const itex::protobuf::Message& proto);
    void SetProto(const itex::protobuf::Message& proto);

   private:
    std::unique_ptr<itex::protobuf::Message> proto_;
  ...
  // given proto.
  Status GetBackendConfigInternal(itex::protobuf::Message* proto) const;
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_HLO_INSTRUCTION_H_
