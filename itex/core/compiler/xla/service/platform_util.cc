...
std::string CanonicalPlatformName(const std::string& platform_name) {
  ...
  if (lowercase_platform_name == "sycl") {
    return "sycl";
  }
...
StatusOr<std::vector<se::Platform*>> GetSupportedPlatforms() {
  return se::MultiPlatformManager::PlatformsWithFilter(
      [](const se::Platform* platform) {
        ...
        if (!supported) {
          ITEX_LOG(INFO) << "platform " << platform->Name()
                         << " present but no "
...
PlatformUtil::GetSupportedPlatforms() {
  ...
  return itex_xla::GetSupportedPlatforms();
}
...
PlatformUtil::GetStreamExecutors(
  ...
  ITEX_VLOG(1) << "Initializing devices";
  {
    itex::thread::ThreadPool thread_pool(itex::Env::Default(),
    ...
    auto create_fn = [](se::Platform* platform,
      ...
      ITEX_VLOG(1) << "Started device init " << device_ordinal;
      ...
      if (executor_status.ok()) {
        ...
        ITEX_LOG(WARNING) << "unable to create StreamExecutor for "
                          << platform->Name() << ":" << device_ordinal << ": "
                          << executor_status.status().error_message();
      }
      ITEX_VLOG(1) << "Finished device init " << device_ordinal;
    };
  ...
  ITEX_VLOG(1) << "Device initialization complete";
