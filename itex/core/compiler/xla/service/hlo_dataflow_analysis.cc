...
bool AreTransitiveUsesEffectivelyElementwise(const HloInstruction* param,
  ...
  ITEX_CHECK_EQ(root_tuple->opcode(), HloOpcode::kTuple);
  ITEX_CHECK_EQ(out_shape_idx.size(), 1);
  absl::flat_hash_set<const HloInstruction*> visited;
  absl::InlinedVector<const HloInstruction*, 4> stack;
  ...
  while (!stack.empty()) {
    ...
    visited.insert(current);
    for (const HloInstruction* user : current->users()) {
      ITEX_VLOG(3) << "Visiting: " << user->ToString();
      switch (user->opcode()) {
        ...
        case HloOpcode::kConcatenate: {
          ...
          ITEX_CHECK(
              !visited.contains(concat_usage_info.slice_to_recover_opnd));
...
const HloValue& HloDataflowAnalysis::GetValueDefinedAt(
  ...
  ITEX_CHECK(ValueIsDefinedAt(instruction, index)) << instruction->ToString();
  return GetUniqueValueAt(instruction, index);
}

HloValue& HloDataflowAnalysis::GetValueDefinedAt(
    const HloInstruction* instruction, const ShapeIndex& index) {
  ITEX_CHECK(ValueIsDefinedAt(instruction, index));
  return GetUniqueValueAt(instruction, index);
}
...
HloValue* HloDataflowAnalysis::NewHloValue(HloInstruction* instruction,
  ...
  ITEX_CHECK(result.second);

  ITEX_VLOG(4) << "NewHloValue = " << result.first->second->ToShortString();
...
void HloDataflowAnalysis::MarkValueForDeletion(HloValue::Id value_id) {
  ...
  ITEX_VLOG(4) << "MarkValueForDeletion(" << value.ToShortString() << ")";
  ...
  for (const auto& pair : value_sets_) {
    ...
    for (const auto& index_value_set : instruction_value_set) {
      ...
      for (const HloValue* value : value_set.values()) {
        ITEX_DCHECK(!ContainsKey(id_set, value->id()))
...
bool HloDataflowAnalysis::Phi(
  ...
  ITEX_CHECK(ssa_form_);
  ITEX_VLOG(4) << "Phi(" << instruction->name() << ")";
  ITEX_VLOG(5) << "instruction value set = "
               << GetInstructionValueSet(instruction).ToString();
  for (const InstructionValueSet* input : inputs) {
    ITEX_VLOG(5) << "input value set = " << input->ToString();
  }

  if (bitcast_defines_value_) {
    absl::c_for_each(inputs, [&](const InstructionValueSet* input) {
      ITEX_DCHECK(ShapeUtil::Compatible(instruction->shape(), input->shape()));
    });
  } else {
    ...
    absl::c_for_each(inputs, [&](const InstructionValueSet* input) {
      ITEX_DCHECK(ty == input->shape().element_type() &&
  ...
  for (auto& pair : GetInstructionValueSet(instruction)) {
    ...
    // value set.
    ITEX_CHECK_LE(value_set.values().size(), 1);
    ...
    ITEX_VLOG(5) << "after input_value_ids.size = " << input_value_ids.size();
    if (input_value_ids.empty()) {
      ...
      ITEX_CHECK_EQ(value_set.values().size(), 0)
      ...
      // necessary.
      ITEX_CHECK_GT(input_value_ids.size(), 1);
      ...
      if (current_value == nullptr || !phi_defined_here) {
        ...
        if (!phi_graph_.InputsEqualTo(*current_value, new_inputs)) {
          ITEX_VLOG(1) << current_value->ToShortString()
                       << " has new phi inputs: ";
          // Update phi inputs.
...
bool HloDataflowAnalysis::UpdateBitcastValueSet(HloInstruction* bitcast) {
  ITEX_CHECK_EQ(bitcast->opcode(), HloOpcode::kBitcast);
...
bool HloDataflowAnalysis::UpdateSetDimensionSizeValueSet(
  ...
  ITEX_CHECK_EQ(set_dimension_size->opcode(), HloOpcode::kSetDimensionSize);
  const InstructionValueSet& operand_set =
...
bool HloDataflowAnalysis::UpdateSendValueSet(HloInstruction* send) {
  ITEX_CHECK_EQ(send->opcode(), HloOpcode::kSend);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateAsyncStartValueSet(
  ...
  ITEX_CHECK_EQ(async_start->opcode(), HloOpcode::kAsyncStart);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateAsyncUpdateValueSet(
  ...
  ITEX_CHECK_EQ(async_update->opcode(), HloOpcode::kAsyncUpdate);
  ITEX_CHECK_EQ(async_update->shape(), async_update->operand(0)->shape());
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateAsyncDoneValueSet(HloInstruction* async_done) {
  ITEX_CHECK_EQ(async_done->opcode(), HloOpcode::kAsyncDone);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateCopyStartValueSet(HloInstruction* copy_start) {
  ITEX_CHECK_EQ(copy_start->opcode(), HloOpcode::kCopyStart);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateCopyDoneValueSet(HloInstruction* copy_done) {
  ITEX_CHECK_EQ(copy_done->opcode(), HloOpcode::kCopyDone);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateRecvDoneValueSet(HloInstruction* recv_done) {
  ITEX_CHECK_EQ(recv_done->opcode(), HloOpcode::kRecvDone);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateCallValueSet(HloInstruction* call) {
  ITEX_CHECK_EQ(call->opcode(), HloOpcode::kCall);
...
bool HloDataflowAnalysis::UpdateConditionalValueSet(
  ...
  ITEX_CHECK_EQ(conditional->opcode(), HloOpcode::kConditional);
...
bool HloDataflowAnalysis::UpdateCopyValueSet(HloInstruction* copy) {
  ITEX_CHECK_EQ(copy->opcode(), HloOpcode::kCopy);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateOptimizationBarrierValueSet(
  ...
  ITEX_CHECK_EQ(barrier->opcode(), HloOpcode::kOptimizationBarrier);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateDomainValueSet(HloInstruction* domain) {
  ...
  // Unlike copies though we also propagate the top-level value.
  ITEX_CHECK_EQ(domain->opcode(), HloOpcode::kDomain);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateAddDependencyValueSet(
  ...
  ITEX_CHECK_EQ(add_dependency->opcode(), HloOpcode::kAddDependency);
  const InstructionValueSet& operand_set =
...
bool HloDataflowAnalysis::UpdateGetTupleElementValueSet(HloInstruction* gte) {
  ITEX_CHECK_EQ(gte->opcode(), HloOpcode::kGetTupleElement);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateParameterValueSet(HloInstruction* parameter) {
  ITEX_CHECK_EQ(parameter->opcode(), HloOpcode::kParameter);
  ...
  ITEX_CHECK_EQ(call_graph_node.context(), CallContext::kControlFlow);
  ...
  for (const CallSite& callsite : call_graph_node.caller_callsites()) {
    if (callsite.instruction()->opcode() == HloOpcode::kCall) {
      ...
      ITEX_CHECK_EQ(parameter->parameter_number(), 0);
      inputs.push_back(
      ...
      ITEX_CHECK(found_parent);
      need_phi = true;
    } else {
      ITEX_LOG(FATAL) << "CallContext::kSequential computations should only be "
...
bool HloDataflowAnalysis::UpdateTupleValueSet(HloInstruction* tuple) {
  ITEX_CHECK_EQ(tuple->opcode(), HloOpcode::kTuple);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateWhileValueSet(HloInstruction* xla_while) {
  ITEX_CHECK_EQ(xla_while->opcode(), HloOpcode::kWhile);
...
bool HloDataflowAnalysis::UpdateAllGatherStartValueSet(
  ...
  ITEX_CHECK_EQ(all_gather_start->opcode(), HloOpcode::kAllGatherStart);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateAllGatherDoneValueSet(
  ...
  ITEX_CHECK_EQ(all_gather_done->opcode(), HloOpcode::kAllGatherDone);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateAllReduceDoneValueSet(
  ...
  ITEX_CHECK_EQ(all_reduce_done->opcode(), HloOpcode::kAllReduceDone);
  bool changed = false;
...
bool HloDataflowAnalysis::UpdateCollectivePermuteStartValueSet(
  ...
  ITEX_CHECK_EQ(collective_permute_start->opcode(),
...
bool HloDataflowAnalysis::UpdateCollectivePermuteDoneValueSet(
  ...
  ITEX_CHECK_EQ(collective_permute_done->opcode(),
...
void HloDataflowAnalysis::Propagate() {
  ...
  while (!worklist.empty()) {
    ...
    ITEX_VLOG(3) << "Worklist top: " << instruction->name();
    ITEX_VLOG(3) << ToString();

    if (!UpdateInstructionValueSet(instruction)) {
      // No change to the instruction's value set.
      ITEX_VLOG(4) << "No change.";
      continue;
    }

    ITEX_VLOG(4) << "New value set for " << instruction->name() << ": "
                 << GetInstructionValueSet(instruction);
      ...
      switch (instruction->opcode()) {
        ...
        case HloOpcode::kCollectivePermuteStart:
          ...
          if (instruction->operand_count() > 1) {
            ITEX_CHECK_EQ(instruction->operand_count(), 4);
...
void HloDataflowAnalysis::OptimizePhiValues() {
  ...
  ITEX_VLOG(1) << "Before phi graph optimization";
  ITEX_XLA_VLOG_LINES(1, phi_graph_.ToString());
  phi_graph_.Optimize();
  ITEX_VLOG(1) << "After phi graph optimization";
  ITEX_XLA_VLOG_LINES(1, phi_graph_.ToString());
  ...
  for (const HloComputation* computation : module_.computations()) {
    for (HloInstruction* instruction : computation->instructions()) {
      InstructionValueSet& instruction_value_set =
          GetInstructionValueSet(instruction);
      ITEX_VLOG(1) << "inst: " << instruction->name();
      ITEX_VLOG(1) << instruction_value_set.ToString();
      instruction_value_set.ForEachMutableElement(
          [&](const itex_xla::ShapeIndex& index, HloValueSet* value_set) {
            ...
            if (new_id != phi_id) {
              ITEX_VLOG(1) << "Replacing " << values[0]->ToString() << " with "
...
StatusOr<std::unique_ptr<HloDataflowAnalysis>> HloDataflowAnalysis::Run(
    ...
    const CanShareBuffer& can_share_buffer) {
  ITEX_VLOG(1) << "HloDataflowAnalysis::Run on module " << module.name();
  ITEX_XLA_VLOG_LINES(2, module.ToString());
  ...
  ITEX_DCHECK_OK(dataflow_analysis->Verify());

  ITEX_XLA_VLOG_LINES(1, dataflow_analysis->ToString());
  ...
  for (const auto& fusion_output_array_shape :
    ...
    for (const auto& output_source_in_place_pair : in_place_pairs) {
      ...
      if (output_index == output_source_index) {
        ...
        ITEX_CHECK(in_place_input_source == nullptr);
...
bool HloDataflowAnalysis::CanShareOperandBufferWithUser(
  ...
  ITEX_CHECK(user->IsUserOf(operand))
