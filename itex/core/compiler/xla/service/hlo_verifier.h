...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_HLO_VERIFIER_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_HLO_VERIFIER_H_
...
class TargetVerifierMetadata {
 public:
  ...
  void SetShapeSize(std::function<int64_t(const Shape&)> shape_size_function) {
    ITEX_CHECK(shape_size_function_ == nullptr)
 ...
 public:
  explicit HloVerifier(
      ...
      : target_metadata_(absl::make_unique<DefaultVerifierMetadata>(
        ...
        context_("Unknown") {
    ITEX_CHECK(instruction_can_change_layout_func_ == nullptr ||
               layout_sensitive);
  }
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_HLO_VERIFIER_H_
