...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_HLO_PASS_FIX_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_HLO_PASS_FIX_H_
...
class HloPassFix : public Pass {
 public:
  ...
  StatusOr<bool> RunOnModuleGroup(HloModuleGroup* module_group) override {
    ...
    ITEX_VLOG(3) << "Running HloPassFix.";
    while (changed_this_iteration) {
      ...
      ITEX_VLOG(3) << "changed_this_iteration: " << changed_this_iteration;
      ++iteration_count;
      if (iteration_count == kIterationLimit) {
        ITEX_VLOG(1) << "Unexpectedly high number of iterations in HLO passes, "
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_HLO_PASS_FIX_H_
