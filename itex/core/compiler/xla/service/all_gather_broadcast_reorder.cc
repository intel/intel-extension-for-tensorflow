...
StatusOr<bool> AllGatherBroadcastReorder::Run(HloModule* module) {
  if (hlo_query::ContainsLayoutConstrainedCollective(*module,
    ...
    ITEX_VLOG(1)
        << "Skip AllGatherBroadcastReorder because the module contains "
  ...
  for (auto computation : module->computations()) {
    for (HloInstruction* inst : computation->MakeInstructionPostOrder()) {
      ...
      if (!all_gather_along_uniform_dim) {
        ...
        ITEX_VLOG(2) << "All-gather along non uniform dimension";
        ...
        ITEX_VLOG(2) << "All-gather along uniform dimension";
