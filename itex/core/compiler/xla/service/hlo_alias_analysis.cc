...
void ComputeWhileAliasedValues(const HloValue& value,
                               const HloDataflowAnalysis& dataflow,
                               FlatValueSet& aliased_values) {
  ITEX_VLOG(3) << "Compute kWhile aliases";
  ...
  for (const HloUse& use : value.GetUses()) {
    if (use.instruction->opcode() == HloOpcode::kWhile) {
      ...
      ITEX_VLOG(3)
          << "  value is init value to a while; must share buffer with "
             "while value "
          << while_value;
    }
  }
  ...
  if (value.defining_instruction()->opcode() == HloOpcode::kParameter) {
    ...
    for (const CallSite& callsite : call_graph_node.caller_callsites()) {
      if (callsite.instruction()->opcode() == HloOpcode::kWhile) {
        ...
        ITEX_CHECK_EQ(call_graph_node.caller_callsites().size(), 1);

        const HloValue& while_value = dataflow.GetUniqueValueAt(
            callsite.instruction(), value.defining_index());
        ITEX_VLOG(3)
            << "  value is parameter value of the body or condition of a "
  ...
  for (const HloPosition& position : value.positions()) {
    ...
    for (const CallSite& callsite : call_graph_node.caller_callsites()) {
      if (callsite.instruction()->opcode() == HloOpcode::kWhile &&
        ...
        ITEX_VLOG(3) << "  value @ " << position << " is root of "
...
void ComputeConditionalAliasedValues(const HloValue& value,
                                     const HloDataflowAnalysis& dataflow,
                                     FlatValueSet& aliased_values) {
  ITEX_VLOG(3) << "Compute kConditional aliases";
...
void ComputeInPlaceOperationAliasedValues(const HloValue& value,
                                          const HloDataflowAnalysis& dataflow,
                                          FlatValueSet& aliased_values) {
  ITEX_VLOG(3) << "Compute aliases for in-place operations (e.g. "
  ...
  for (const HloPosition& position : value.positions()) {
    ...
    for (const auto& operand_and_output_index :
      ...
      if (position.index == operand_and_output_index.second) {
        ...
        ITEX_VLOG(3) << " operand value " << operand_value << " aliases.";
  ...
  for (const HloUse& use : value.GetUses()) {
    for (const auto& operand_and_output_index :
      ...
      if (use.operand_number == operand_index.operand_number &&
        ...
        ITEX_VLOG(3) << " use value " << use_value << " aliases.";
...
FlatValueSet ComputeAliasedValues(const HloValue& value,
  ...
  if (ITEX_VLOG_IS_ON(2)) {
    for (const HloUse& use : value.GetUses()) {
      ITEX_VLOG(2) << "Use of value " << value << ": " << use;
    }
  }
...
std::vector<HloBuffer> CreateBuffers(const HloDataflowAnalysis& dataflow) {
  ...
  for (const HloValue* value : values) {
    ITEX_VLOG(3) << "Merging colocated values, value: " << *value;
    ...
    for (auto& aliased_set_and_id : aliased_sets) {
      ...
      if ((aliased_set != union_set) && !aliased_set->empty()) {
        for (const HloValue* aliased_value : *aliased_set) {
          ITEX_CHECK(union_set->insert(aliased_value).second);
  ...
  ITEX_CHECK_EQ(buffers.size(), num_buffers);
  return buffers;
}
...
StatusOr<std::unique_ptr<HloAliasAnalysis>> HloAliasAnalysis::Run(
  ...
  ITEX_VLOG(2) << "HloAliasAnalysis::Run on module " << module->name();
  ...
  ITEX_CHECK_EQ(alias_analysis->value_to_buffer_.size(), num_values);
  ITEX_DCHECK_OK(alias_analysis->Verify());
  ...
  ITEX_XLA_VLOG_LINES(2, alias_analysis->ToString());
