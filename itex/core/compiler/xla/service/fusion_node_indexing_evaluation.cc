...
void FusionNodeIndexingEvaluation::UpdateEvaluationCache(
  ...
  ITEX_CHECK(!indexing_users_.contains(producer));
...
FusionNodeIndexingEvaluation::RemoveFusionOperand(
  ...
  ITEX_CHECK(!index_usage_count_.contains(fusion_operand));
...
void FusionNodeIndexingEvaluation::UpdateIndexUsageCount(
  ...
  ITEX_CHECK(index_usage_count_.emplace(instruction, total).second);
}
