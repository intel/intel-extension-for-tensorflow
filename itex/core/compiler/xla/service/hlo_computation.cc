...
#include "itex/core/compiler/xla/service/hlo_clone_context.h"
...
HloComputation::HloComputation(
  ...
  for (auto& instruction : *instructions) {
    if (instruction->opcode() == HloOpcode::kParameter) {
      ...
      ITEX_CHECK(param_no >= 0 && param_no < parameter_count)
          << "\nERROR: invalid parameter number.  Expected [0, "
          << parameter_count << "), got " << param_no;
      ITEX_CHECK(param_instructions_[param_no] == nullptr)
  ...
  ITEX_CHECK(root_found)
      << "\nERROR: root instruction is not present in computation.";
}

HloComputation::~HloComputation() {
  if (fusion_instruction_ != nullptr) {
    ITEX_CHECK(fusion_instruction_->fused_instructions_computation() == this);
  ...
  if (IsAsyncComputation()) {
    for (auto* async_instr : async_instructions_) {
      ITEX_CHECK(async_instr->async_wrapped_computation() == this);
...
HloInstruction* HloComputation::AddInstruction(
  ...
  ITEX_CHECK(instruction->opcode() != HloOpcode::kParameter)
...
HloInstruction* HloComputation::AddParameter(
    std::unique_ptr<HloInstruction> instruction) {
  ITEX_CHECK(instruction->opcode() == HloOpcode::kParameter);
  ITEX_CHECK(!IsFusionComputation() || fusion_instruction_->operand_count() ==
...
HloInstruction* HloComputation::AddEntryComputationParameter(
    std::unique_ptr<HloInstruction> instruction) {
  ITEX_CHECK_EQ(instruction->opcode(), HloOpcode::kParameter);
  ITEX_CHECK_EQ(instruction->parameter_number(), num_parameters());
  ITEX_CHECK(parent()->entry_computation() == this);
...
Status HloComputation::ReplaceEntryComputationParameter(
    ...
    std::unique_ptr<HloInstruction> instruction) {
  ITEX_CHECK_GE(param_no, 0);
  ITEX_CHECK_LT(param_no, param_instructions_.size());
  ITEX_CHECK_EQ(instruction->opcode(), HloOpcode::kParameter);
  ITEX_CHECK(parent()->entry_computation() == this);
...
HloInstruction* HloComputation::ReplaceParameter(
  ...
  ITEX_CHECK(RemoveInstruction(old_instruction).ok());
...
Status HloComputation::RemoveUnusedParametersImpl(bool allow_non_fusion) {
  ITEX_CHECK(allow_non_fusion || IsFusionComputation());
  int64_t removed = 0;
...
Status HloComputation::RemoveInstructionImpl(HloInstruction* instruction,
  ...
  ITEX_VLOG(2) << "Removing instruction " << instruction->name()
...
void HloComputation::set_root_instruction(HloInstruction* new_root_instruction,
  ...
  if (!IsFusionComputation() && !accept_different_shape) {
    ITEX_CHECK(ShapeUtil::Compatible(new_root_instruction->shape(),
  ...
  while (!dfs_stack.empty()) {
    ...
    if (!result.second) {  // We've already seen this instruction.
      ...
      if (result.first->second != kVisited) {
        ITEX_DCHECK_EQ(current.parent(), this)
...
std::vector<HloInstruction*> HloComputation::MakeInstructionPostOrder() const {
  ...
  ITEX_CHECK_EQ(instructions_.size(), post_order.size())
...
std::vector<HloComputation*> HloComputation::MakeEmbeddedComputationsList()
  ...
  ITEX_CHECK_EQ(this, post_order.back());
  post_order.pop_back();
...
std::string HloComputation::ToString(
  ...
  ITEX_CHECK_EQ(instruction_order.size(), instruction_count());
...

  {
    ...
    for (const HloInstruction* const instruction : instruction_order) {
      ITEX_DCHECK_EQ(this, instruction->parent());
...
void HloComputation::AppendInstructionsIntoCalledComputation(
  ...
  ITEX_CHECK_OK(root->ReplaceAllUsesWith(caller));
  if (root == root_instruction()) {
    set_root_instruction(caller);
  }
  ITEX_CHECK_OK(RemoveInstruction(root));
  for (size_t i = 1; i < instructions_to_append.size(); ++i) {
    ...
    if (instruction->IsDead()) {
      ITEX_CHECK_OK(RemoveInstruction(instruction));
    }
  }
}
...
StatusOr<bool> HloComputation::ReplaceInstructionWithDifferentShape(
  ...
  if (preserve_sharding && new_instruction->has_sharding() &&
    ...
    ITEX_VLOG(10) << "Skipping replacement due to incompatible sharding";
    return false;
  }
  ITEX_VLOG(10) << "transformed " << old_instruction->ToString() << " to "
...
void SortClonedInstructions(
  ...
  if (!status.ok()) {
    ITEX_LOG(ERROR)
        << "Failed to reorder instructions while cloning computation: "
...
std::unique_ptr<HloComputation> HloComputation::CloneWithReplacements(
  ...
  ITEX_VLOG(1) << "Cloning " << name() << " --> " << suffix << "\n";
  ...
  for (const auto& instr : instructions_) {
    ...
    while (!dfs_stack.empty()) {
      ...
      if (it != visited.end()) {
        ...
        ITEX_CHECK_EQ(it->second, kVisiting);
  ...
  for (const auto& instr : extra_parameters) {
    ITEX_CHECK_EQ(instr->opcode(), HloOpcode::kParameter)
  ...
  for (auto instr : postorder) {
    ...
    for (auto operand : instr->operands()) {
      ...
      ITEX_CHECK_NE(replaced_operand, nullptr)
  ...
  for (auto instr : postorder) {
    ...
    for (auto successor : instr->control_successors()) {
      ...
      if (replaced_successor != nullptr) {
        ITEX_CHECK_OK(new_instr->AddControlDependencyTo(
