...
#include "itex/core/compiler/xla/service/scatter_expander.h"
...
static StatusOr<HloInstruction*> CheckIndexValidity(
  ...
  ITEX_DCHECK_EQ(operand_dims.size(), window_sizes.size());
...
static StatusOr<std::vector<HloInstruction*>> ScatterLoopBody(
  ...
  ITEX_CHECK_EQ(loop_state.size(), scatter->operand_count());
...
StatusOr<HloInstruction*> ScatterExpander::ExpandInstruction(
  ...
  ITEX_CHECK_EQ(scatter_loop_trip_count,
