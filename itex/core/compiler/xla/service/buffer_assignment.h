...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_BUFFER_ASSIGNMENT_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_BUFFER_ASSIGNMENT_H_
...
class BufferAllocation {
 public:
  ...
  // computation, this methods returns the parameter number. ITEX_CHECKs
  // otherwise.
  int64_t parameter_number() const {
    ITEX_CHECK(is_entry_computation_parameter_);
...
// Add stream operators for nicer output of ITEX_CHECK/RET_CHECK failures.
...
class BufferAssignment {
 public:
  ...
  // itex_xla::Executables, migrating off the use of addresses can be hard.
  ...
  // to. ITEX_CHECKs if buffer has not been assigned an allocation.
  ...
  // Returns the allocation with the given index. ITEX_CHECKs if no allocation
 ...
 private:
  ...
  int64_t HloBufferSize(const HloBuffer& buffer) {
    ...
    for (const HloValue* value : buffer.values()) {
      ITEX_DCHECK_EQ(result, buffer_size_(*value));
    }
    return result;
  }
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_BUFFER_ASSIGNMENT_H_
