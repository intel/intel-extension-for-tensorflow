...
#ifndef ITEX_CORE_COMPILER_XLA_SERVICE_EXECUTABLE_H_
#define ITEX_CORE_COMPILER_XLA_SERVICE_EXECUTABLE_H_
...
class ExecutionInput {
 public:
  explicit ExecutionInput(itex_xla::Shape shape) : buffers_(std::move(shape)) {
    SetHostShape(ShapeUtil::DeviceShapeToHostShape(buffers_.shape()));
  }
  // TODO(b/170310047): remove this overload.
  ExecutionInput(itex_xla::Shape shape, itex_xla::Shape host_shape)
  ...
  itex_xla::StatusOr<itex_xla::ShapedBuffer> ToShapedBuffer(
 ...
 private:
  void SetHostShape(itex_xla::Shape host_shape) {
    if (shape() != host_shape) {
...
class Executable {
 public:
  ...
  explicit Executable(
    ...
    ITEX_CHECK_EQ(hlo_profile_printer_data_.get() == nullptr,
  ...
  const HloProfilePrinterData& hlo_profile_printer_data() const {
    ITEX_CHECK(hlo_profiling_enabled());
  ...
  // Dumping helpers.
  void set_hlo_proto(std::unique_ptr<itex_xla::HloProto> hlo_proto) {
...
#endif  // ITEX_CORE_COMPILER_XLA_SERVICE_EXECUTABLE_H_
