...
StatusOr<bool> RemoveUnusedOperandFromSort(HloInstruction* sort) {
  ...
  if (new_sort->shape().IsTuple()) {
    ...
    ITEX_CHECK_EQ(used_indices.size(), 1);
...
StatusOr<bool> SortSimplifier::Run(
  ...
  ITEX_VLOG(2) << "HLO module before SortSimplifier:";
  ...
  if (changed) {
    ITEX_VLOG(2) << "HLO module after SortSimplifier:";
    ITEX_XLA_VLOG_LINES(2, module->ToString());
  } else {
    ITEX_VLOG(2) << "HLO module unchanged after SortSimplifier";
  }
