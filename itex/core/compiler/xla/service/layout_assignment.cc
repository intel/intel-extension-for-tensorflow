...
#include "itex/core/compiler/xla/service/call_graph.h"
...
#include "itex/core/compiler/xla/service/tuple_points_to_analysis.h"
#include "itex/core/compiler/xla/service/tuple_simplifier.h"
#include "itex/core/compiler/xla/shape_layout.h"
...
BufferLayoutConstraint::BufferLayoutConstraint(const Layout& layout,
    ...
    : LayoutConstraint(mandatory, dfs, priority),
      layout_(layout),
      buffer_(&buffer) {
  ITEX_CHECK(LayoutUtil::ValidateLayoutForShape(layout, buffer.shape()).ok());
}
...
OperandLayoutConstraint::OperandLayoutConstraint(
  ...
  ITEX_CHECK(shape_layout_.LayoutIsSet());
  ITEX_CHECK(ShapeUtil::Compatible(shape_layout.shape(),
...
Status LayoutAssignment::SetBufferLayout(const Layout& layout,
                                         ...
                                         bool mandatory, bool dfs,
                                         int64_t priority) {
  ITEX_VLOG(3) << "SetBufferLayout : " << buffer << " : "
               << LayoutUtil::HumanString(layout) << " with priority "
               << priority;
  ...
  if (unconstrained_buffer_ids_.find(buffer.id()) !=
    ...
    ITEX_VLOG(3) << "Erase buffer from unconstrained ids\n";
  ...
  if (iter != buffer_constraints_.end()) {
    ...
    if (curr_constraint.mandatory() && !mandatory) {
      ...
      if (curr_constraint.UpdateLayout(priority, layout, mandatory, dfs)) {
        ITEX_VLOG(3) << "Updating existing Buffer layout for "
                     << buffer.ToString() << " with new layout"
                     << LayoutUtil::HumanString(layout);
        iter = buffer_constraints_.insert_or_assign(&buffer, curr_constraint)
                   .first;
      } else {
        ITEX_VLOG(3) << "Unable to update existing Buffer layout for "
...
Status LayoutAssignment::SetOperandLayout(const Shape& shape_with_layout,
  ...
  ITEX_VLOG(3) << "SetOperandLayout : " << instruction->name() << ", operand "
               << operand_no << " : "
  ...
  if (curr_shape_layout != nullptr) {
    ...
    if (curr_shape_layout->mandatory() && !mandatory) {
      ...
      ITEX_VLOG(5) << "Existing layout has higher priority: "
...
Status LayoutAssignment::LayoutConstraints::SetResultLayout(
    ...
    int64_t priority) {
  ITEX_VLOG(3) << "  : " << ShapeUtil::HumanStringWithLayout(shape_with_layout)
               << "; priority = " << priority << ".\n";
...
Status LayoutAssignment::SetInstructionLayout(const Layout& layout,
  ...
  ITEX_CHECK(instruction->shape().IsArray() ||
  ...
  return ShapeUtil::ForEachSubshapeWithStatus(
      ...
      [this, layout, instruction, mandatory, allow_alias, priority](
        ...
        ITEX_CHECK_EQ(1, buffers.size());
        if (!allow_alias) {
          ITEX_CHECK_EQ(buffers[0]->instruction(), instruction);
        }
        if (subshape.IsArray()) {
...
Status LayoutAssignment::SetInstructionLayout(const Shape& shape_with_layout,
                                              ...
                                              bool allow_alias,
                                              int64_t priority) {
  ITEX_VLOG(3) << "SetInstructionLayout : " << instruction->name() << ", "
               << ShapeUtil::HumanStringWithLayout(shape_with_layout)
               << ": priority = " << priority << " : mandatory = " << mandatory
               << "; dfs = " << dfs << "\n";
...
Status PropagateParameterLayoutToUsers(const HloInstruction* instruction,
  ...
  for (auto* user : instruction->users()) {
    ...
    ITEX_VLOG(3) << "Setting  user layout : " << user->ToString();
    if (user->opcode() == HloOpcode::kGetTupleElement) {
      auto tuple_index = user->tuple_index();
      ITEX_CHECK(shape.IsTuple());
...
Status LayoutAssignment::AddMandatoryConstraints(
    ...
    LayoutConstraints* constraints) {
  ITEX_VLOG(2) << "Adding mandatory layout constraints to computation "
               << constraints->computation()->name();
  ...
  for (auto* instruction : constraints->computation()->instructions()) {
    if (instruction->opcode() == HloOpcode::kInfeed) {
      ...
      ITEX_CHECK(get_channel_constraints(instruction))
  ...
  for (auto* instruction : constraints->computation()->instructions()) {
    if (instruction->opcode() == HloOpcode::kCall &&
      ...
      ITEX_CHECK_EQ(1, body->num_parameters());
      ITEX_CHECK_EQ(1, condition->num_parameters());
      ITEX_DCHECK(ShapeUtil::Compatible(body_layout.result_shape(),
                                        body_layout.parameter_shape(0)));
      ITEX_DCHECK(ShapeUtil::Compatible(body_layout.result_shape(),
                                        condition_layout.parameter_shape(0)));
      ITEX_DCHECK(
          ShapeUtil::Compatible(body_layout.result_shape(), init->shape()));

      if (body_layout.result_layout() != body_layout.parameter_layout(0)) {
        ITEX_VLOG(2) << "Reset %while body parameter layout: body="
                     << body->name() << " while=" << instruction->name()
      ...
      if (condition_layout.parameter_layout(0) !=
        ...
        ITEX_VLOG(2) << "Reset %while condition parameter layout: cond="
  ...
  if (conditional_mismatch_.count(constraints->computation()) > 0) {
    ITEX_VLOG(5) << "Setting mismatching conditional result:"
                 << constraints->computation()->name() << "\n";
    ...
    if (result_layout != nullptr) {
      ITEX_VLOG(2) << "Setting computation result layout.\n";
      PushAddedConstraints(&constraints->computation_constraint());
    } else {
      ITEX_VLOG(2) << "Computation result layout is not set.\n";
    }
  }
  return Status::OK();
}
...
bool LayoutsInShapesEqual(const Shape& lhs, const Shape& rhs) {
  ...
  ITEX_CHECK(lhs.has_layout() && rhs.has_layout());
...
StatusOr<HloInstruction*> LayoutAssignment::CreateCopyWithNewLayout(
  ...
  ITEX_DCHECK(ShapeUtil::Compatible(shape_with_layout, instruction->shape()))
...
Status LayoutAssignment::CopyOperandIfLayoutsDiffer(
  ...
  if (Shape::Equal().MinorToMajorOnlyInLayout()(operand_layout.shape(),
                                                operand->shape())) {
    ITEX_VLOG(2) << "Operand " << operand->ToString() << " layout matches in "
                 << instruction->ToString();
    // Operand layout already matches our constraint. Nothing to do.
    return Status::OK();
  }
  ITEX_VLOG(2) << "Operand " << operand->ToString() << " layout does not match "
               << operand_layout.ToString() << " in "
               << instruction->ToString();
  ...
  if (!reverse_computation_order_ &&
    ...
    *param->mutable_shape() = operand->shape();
    ...
    ITEX_VLOG(2) << "New copy of " << operand->ToString() << " is "
                 << param_copy->ToString();
  ...
  ITEX_VLOG(4) << "New copy of " << operand->ToString() << " is "
...
LayoutAssignment::LayoutAssignment(
  ...
  ITEX_VLOG(1) << "Entry computation layout given to layout assignment: "
...
std::unique_ptr<Layout> LayoutAssignment::ChooseOperandLayoutFromOutputLayout(
  ...
  const HloInstruction* operand = instruction->operand(operand_no);
  ITEX_CHECK(instruction->shape().IsArray());
  ITEX_CHECK(operand->shape().IsArray());
...
static Layout GetReduceLayoutFromOperand(const Layout& operand_layout,
  ...
  ITEX_CHECK_EQ(hlo->opcode(), HloOpcode::kReduce);
...
std::unique_ptr<Layout> LayoutAssignment::ChooseOutputLayoutFromOperandLayout(
  ...
  ITEX_CHECK(user->shape().IsArray() && operand->shape().IsArray())
...
Status LayoutAssignment::PropagateConstraints(LayoutConstraints* constraints) {
  ...
  auto add_new_constraints_to_worklist = [this, &worklist]() {
    ...
    for (auto* constraint : ConsumeAddedConstraints()) {
      if (constraint->dfs()) {
        ...
        ITEX_VLOG(3) << "push back constraint for propagation : "
                     << constraint->ToString();
  ...
  while (!worklist.empty()) {
    ...
    worklist.pop_front();
    ITEX_VLOG(2) << "Propagating " << layout_constraint->ToString()
    ...
    if (auto* buffer_constraint =
      ...
      ITEX_LOG(FATAL) << "Invalid constraint type: " << *layout_constraint;
    }
    add_new_constraints_to_worklist();
  }
  return Status::OK();
...
std::vector<std::pair<const HloInstruction*, int64_t>> GetArrayUsesOfBuffer(
  ...
  for (const auto& buffer_alias : aliases) {
    ...
    ITEX_CHECK(buffer_alias.index().empty());
...
Status LayoutAssignment::PropagateOperandConstraint(
    ...
    LayoutConstraints* constraints) {
  ITEX_VLOG(3) << "Propagate Operand Constraint : "
               << operand_constraint.ToString() << "\n";
...
Status LayoutAssignment::PropagateBufferConstraintToOperands(
    ...
    LayoutConstraints* constraints) {
  ITEX_VLOG(5) << "PropagateBufferConstraintToOperands: "
  ...
  for (int64_t operand_no = 0; operand_no < instruction->operand_count();
    ...
    if (!InstructionCanChangeLayoutInstance(instruction)) {
      ...
      ITEX_VLOG(6) << "Propagating constraint to operand " << operand_no
                   << " of " << instruction->ToShortString();
      ...
      if (constraint == nullptr || !constraint->mandatory()) {
        ...
        ITEX_VLOG(6) << "Operand already has a constraint "
                     << constraint->ToString();
      }
    }
  }
...
Status LayoutAssignment::PropagateBufferConstraintToUses(
    const BufferLayoutConstraint& buffer_constraint,
    LayoutConstraints* constraints) {
  ITEX_VLOG(5) << "PropagateBufferConstraintToUses: "
               << buffer_constraint.ToString();
  const LogicalBuffer& buffer = buffer_constraint.buffer();
  ...
  for (HloInstruction* user : buffer.instruction()->users()) {
    ...
    if (user->parent()->root_instruction() == user) {
      ITEX_VLOG(3) << "Propagating layout through backedge"
...
StatusOr<Layout> LayoutAssignment::InferArrayLayout(
  ...
  for (const LogicalBuffer* source_buffer : source_buffers) {
    ITEX_VLOG(5) << "Logical buffer: " << source_buffer->ToString() << "\n";
...
Status SetFusionLayouts(HloInstruction* fusion) {
  ...
  for (auto* fused_instruction :
    ...
    if (fused_instruction->opcode() == HloOpcode::kParameter) {
      ...
      ITEX_DCHECK(ShapeUtil::Compatible(fusion_operand->shape(),
...
Status LayoutAssignment::AssignLayouts(LayoutConstraints& constraints) {
  ...
  ITEX_VLOG(2) << "Assigning layouts to computation: " << computation->name();

  ITEX_XLA_VLOG_LINES(2, ToString(constraints));
  ...
  for (HloInstruction* instruction : computation->MakeInstructionPostOrder()) {
    ...
    ITEX_VLOG(3) << "Instruction layout:" << instruction->ToString();
    ...
    for (int64_t operand_no = 0; operand_no < instruction->operand_count();
      ...
      if (operand_layout != nullptr) {
        ...
        ITEX_VLOG(2) << "operand " << operand_no << " has no constraint";
      }
    }
    ...
    ITEX_VLOG(3) << "Resulting instruction:" << instruction->ToString() << "\n";
  ...
  if (constraints.ResultLayout() != nullptr) {
    ...
    ITEX_VLOG(5) << "Computation result layout needs root copying\n";
    if (!constraints.ResultLayout()->MatchesLayoutInShape(
      ...
      auto copy_tiling = [&constraints](itex_xla::Shape* subshape,
                                        const itex_xla::ShapeIndex& index) {
        if (subshape->IsArray()) {
      ...
      itex_xla::ShapeUtil::ForEachMutableSubshape(
          computation->root_instruction()->mutable_shape(), copy_tiling);
    }
  }
  ITEX_VLOG(5) << "Final computation layout:" << computation->name() << ":"
               << constraints.computation_constraint().ToString() << "\n";
  ITEX_VLOG(5) << "Root instruction:"
  ...
  auto UpdateLayout = [this](const HloInstruction* operand,
    ...
    ShapeUtil::ForEachSubshape(
        operand->shape(), [this, &change, operand, update](
          ...
          if (param_layout.ok()) {
            ITEX_VLOG(5) << index << ":" << param_layout.ValueOrDie().ToString()
                         << "\n";
  ...
  auto SetCalleeLayout =
    ...
    if (callee_constraint->priority() < priority ||
      ...
      if (conditional_mismatch_.count(callee->computation()) == 0 &&
        ...
        ITEX_VLOG(2) << "Setting result layout from : " << result->ToString()
                     << "\n";
      }
      ...
      for (auto* operand : operands) {
        if (UpdateLayout(operand,
          ...
          ITEX_VLOG(2) << "Setting callee parameter: " << operand->ToString()
                       << "\n";
        }
        ++operand_no;
      }
      ITEX_VLOG(2) << "Set callee layout: " << callee->computation()->name()
    ...
    switch (instruction->opcode()) {
      ...
      case HloOpcode::kCall:
        if (reverse_computation_order_ &&
          ...
          ITEX_VLOG(2) << "Successfully propagated to callee layout\n";
        }
        break;
      case HloOpcode::kConditional:
        if (reverse_computation_order_) {
      ...
      case HloOpcode::kWhile:
        ...
        if (reverse_computation_order_) {
          ITEX_VLOG(2) << "Populating while loop constraints inside loop body.";
          ...
          TF_RETURN_IF_ERROR(SetCalleeLayout(
              ...
              current_priority_ + 1));
          ITEX_VLOG(2)
              << "Populating while loop constraints inside loop condition.";
          ITEX_VLOG(2) << instruction->ToString();
          TF_RETURN_IF_ERROR(SetCalleeLayout(
...
Status LayoutAssignment::RunOnComputation(
    ...
    ChannelLayoutConstraints* channel_constraints) {
  HloComputation* computation = constraints->computation();
  ITEX_VLOG(1) << "LayoutAssignment::RunOnComputation(" << computation->name()
               << ")";
  ITEX_VLOG(4) << computation->ToString() << "\n";
  ...
  while (!unconstrained_buffer_ids_.empty()) {
    ...
    ITEX_CHECK_LT(unconstrained_buffer_ids_.size(), unconstrained_count);
  }
...
Status LayoutAssignment::ConstrainChannelLayouts(
  ...
  for (HloInstruction* instruction : computation->MakeInstructionPostOrder()) {
    if (instruction->IsCrossModuleAllReduce()) {
      ...
      ITEX_VLOG(5) << "Constrain cross module all reduce: "
                   << op_layout.ToString() << "\n";
...
Status LayoutAssignment::PropagateComputationLayouts(
  ...
  for (int64_t i = 0; i < computed_computation_layout.parameter_count(); ++i) {
    ...
    if (needs_assign) {
      ITEX_VLOG(4)
          << "Assigning layout to parameter " << i << " of computation "
          << computation->name() << ": "
  ...
  if (!result_layout->LayoutIsSet()) {
    ITEX_VLOG(4) << "Assigning result layout of computation "
                 << computation->name() << ": "
...
StatusOr<bool> LayoutAssignment::Run(
    HloModule* module,
    const absl::flat_hash_set<absl::string_view>& execution_threads) {
  ITEX_VLOG(2) << "Running layout assignment on module " << module->name();
  ...
  if (reverse_computation_order_ && !computations_to_work.empty()) {
    ...
    ITEX_VLOG(2) << "reversing traversal order for computation:";
  }
  ...
  for (int64_t i = 0; i < kNumberOfPropagationRounds; ++i) {
    ITEX_VLOG(1) << "Running " << (i == 0 ? "un" : "") << "constrained pass";
  ...
  for (HloComputation* computation : module->computations()) {
    for (HloInstruction* instruction :
      ...
      if (instruction->opcode() == HloOpcode::kCopy &&
        ...
        ITEX_VLOG(5) << "Removing added copy: " << instruction->ToString();
        TF_RETURN_IF_ERROR(
...
Status LayoutAssignment::ClearPreviousPassSideEffects(
    HloModule* module,
    const absl::flat_hash_set<absl::string_view>& execution_threads) {
  ITEX_VLOG(5) << "Clearing previous side effects";
  for (HloComputation* computation : module->computations(execution_threads)) {
