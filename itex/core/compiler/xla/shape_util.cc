...
Shape MakeTupleShapeImpl(absl::Span<ShapePtrOrRef> shapes) {
  ...
  ITEX_DCHECK_OK(ShapeUtil::ValidateShapeWithOptionalLayout(result));
  return result;
}
...
/* static */ bool ShapeUtil::Equal(const Shape& lhs, const Shape& rhs) {
  ...
  if (!equal && ITEX_VLOG_IS_ON(3)) {
    ITEX_VLOG(3) << "ShapeUtil::Equal differ: lhs = " << lhs.ShortDebugString()
...
/* static */ bool ShapeUtil::EqualIgnoringElementType(const Shape& lhs,
  ...
  if (!equal && ITEX_VLOG_IS_ON(3)) {
    ITEX_VLOG(3) << "ShapeUtil::EqualIgnoringElementType differ: lhs = "
...
/* static */ bool ShapeUtil::EqualIgnoringFpPrecision(const Shape& lhs,
  ...
  if (!equal && ITEX_VLOG_IS_ON(3)) {
    ITEX_VLOG(3) << "ShapeUtil::EqualIgnoringFpPrecision differ: lhs = "
                 << lhs.ShortDebugString()
                 << ", rhs = " << rhs.ShortDebugString();
  }
...
/* static */ bool ShapeUtil::FillNewShape(PrimitiveType element_type,
  ...
  ITEX_DCHECK_EQ(dense_shape_size, ByteSizeOfPrimitiveType(element_type))
      << element_type;
...
/* static */ Shape ShapeUtil::MakeShape(PrimitiveType element_type,
                                        absl::Span<const int64_t> dimensions) {
  Shape shape;
  ITEX_CHECK(FillNewShape(element_type, dimensions, &shape));
  return shape;
}
...
/* static */ Shape ShapeUtil::MakeShapeWithLayout(
  ...
  auto ret = MakeShapeWithLayoutInternal(
      ...
      physical_shape);
  if (!ret.ok()) ITEX_LOG(ERROR) << ret.status();
  return ret.value();
}
...
/* static */ Shape ShapeUtil::MakeOpaqueShape() {
  ...
  ITEX_DCHECK_OK(ValidateShapeWithOptionalLayout(result));
  return result;
}
...
/* static */ bool ShapeUtil::ElementIsSigned(const Shape& shape) {
  switch (shape.element_type()) {
    ...
    default:
      ITEX_LOG(FATAL) << "Unhandled element type " << shape.element_type();
  }
}
...
/* static */ int64_t ShapeUtil::TupleElementCount(const Shape& shape) {
  ITEX_CHECK(shape.IsTuple()) << HumanString(shape);
...
/* static */ const Shape& ShapeUtil::GetTupleElementShape(const Shape& shape,
                                                          int64_t index) {
  ITEX_CHECK(shape.IsTuple());
  ITEX_CHECK_GT(TupleElementCount(shape), index);
  ITEX_DCHECK_OK(ValidateShapeWithOptionalLayout(shape.tuple_shapes(index)));
...
/* static */ Shape ShapeUtil::SliceTuple(const Shape& tuple, int64_t start,
                                         int64_t limit) {
  ITEX_DCHECK_OK(ValidateShapeWithOptionalLayout(tuple));
  ITEX_CHECK(tuple.IsTuple());
  ITEX_CHECK_LE(start, TupleElementCount(tuple));
  ITEX_CHECK_LE(limit, TupleElementCount(tuple));
...
/* static */ Shape ShapeUtil::ComplexComponentShape(
  ...
  ITEX_CHECK(ElementIsComplex(complex_shape)) << HumanString(complex_shape);
...
/* static */ int64_t ShapeUtil::ElementsIn(const Shape& shape) {
  ITEX_DCHECK(shape.IsArray()) << ShapeUtil::HumanString(shape);
  ITEX_DCHECK_EQ(shape.dimensions_size(), shape.rank());
...
/* static */ int64_t ShapeUtil::ElementsInRecursive(const Shape& shape) {
  ITEX_CHECK(shape.IsArray() || shape.IsTuple());
  if (shape.IsArray()) {
...
/* static */ bool ShapeUtil::SameDimensions(const Shape& lhs,
                                            const Shape& rhs) {
  ITEX_CHECK(lhs.IsArray());
  ITEX_CHECK(rhs.IsArray());
...
/* static */ int64_t ShapeUtil::GetDimensionNumber(const Shape& shape,
  ...
  ITEX_CHECK_GE(dimension_number, 0);
...
/* static */ int64_t ShapeUtil::ByteSizeOfPrimitiveType(
  ...
  switch (primitive_type) {
    ...
    case TUPLE:
    case OPAQUE_TYPE:
      ITEX_LOG(FATAL) << PrimitiveType_Name(primitive_type)
                      << " primitive type has no definitive size";
    default:
      ITEX_LOG(FATAL) << "Unhandled primitive type " << primitive_type;
  }
}
...
/* static */ int64_t ShapeUtil::ByteSizeOf(const Shape& shape,
  ...
  if (shape.element_type() == TUPLE) {
    ...
    ITEX_CHECK_GT(pointer_size, 0);
    return pointer_size;
  }
  ITEX_LOG(FATAL) << PrimitiveType_Name(shape.element_type())
                  << " primitive type has no definitive size";
}
...
/* static */ int64_t ShapeUtil::ByteSizeOfTupleIndexTable(
  ...
  ITEX_DCHECK_OK(ValidateShape(shape));
  ITEX_CHECK_EQ(TUPLE, shape.element_type());
  ITEX_CHECK_GT(pointer_size, 0);
...
/* static */ int64_t ShapeUtil::ByteSizeOfElements(const Shape& shape) {
  ...
  ITEX_CHECK(LayoutUtil::IsDenseArray(shape)) << shape.ShortDebugString();
...
/* static */ Status ShapeUtil::ValidateShapeSize(const Shape& shape) {
  ITEX_VLOG(3) << "Validating shape size: " << ShapeUtil::HumanString(shape);
  ...
  ITEX_VLOG(3) << "Shape size is valid: " << shape_size;
  return OkStatus();
}
...
/* static */ const Shape& ShapeUtil::GetSubshape(const Shape& shape,
  ...
  for (auto i : index) {
    ITEX_CHECK(return_shape->IsTuple())
...
/* static */ Shape ShapeUtil::PermuteDimensions(
  ...
  if (shape.has_layout()) {
    ...
    // permutation here.
    ITEX_CHECK(TransposeIsBitcast(shape, new_shape, permutation))
...
ShapeUtil::InsertedOrDeleted1SizedDimensions(const Shape& shape_pre,
  ...
  ITEX_CHECK(shape_pre.IsArray());
  ITEX_CHECK(shape_post.IsArray());
...
ShapeUtil::DimensionsUnmodifiedByReshape(const Shape& input_shape,
  ...
  ITEX_CHECK(input_shape.IsArray());
  ITEX_CHECK(output_shape.IsArray());
...
/* static */ bool ShapeUtil::TransposeIsBitcast(
  ...
  ITEX_CHECK(LayoutUtil::IsDenseArray(input_shape))
      << input_shape.ToString(true);
  ITEX_CHECK(LayoutUtil::IsDenseArray(output_shape))
      << output_shape.ToString(true);
  ITEX_CHECK(input_shape.has_layout()) << input_shape.ToString(true);
  ITEX_CHECK(output_shape.has_layout()) << output_shape.ToString(true);
...
/* static */ bool ShapeUtil::ReshapeIsBitcast(const Shape& input_shape,
  ...
  if (ElementsIn(input_shape) != ElementsIn(output_shape)) {
    ITEX_VLOG(3) << "input_shape=" << input_shape.ShortDebugString()
...
/* static */ std::optional<Shape> ShapeUtil::AlignLayouts(
  ...
  ITEX_CHECK(ReshapeIsBitcast(input_shape, output_shape_with_layout))
...
/* static */ bool ShapeUtil::DynamicArrayShapeIsCompatible(
    const itex_xla::Shape& dynamic_shape,
    const itex_xla::Shape& bounded_shape) {
...
/* static */ bool ShapeUtil::DynamicShapeIsCompatible(
  ...
  itex_xla::ShapeUtil::ForEachSubshape(
...
/* static */ void ShapeUtil::ForEachIndexParallel(
  ...
  ITEX_CHECK(
      ForEachIndexInternalParallel(shape, base, count, incr, visitor_function)
          .ok());
}
...
struct ParallelState {
  ParallelState() {
    const int kNumThreads = itex::port::MaxParallelism();
    pool.emplace(itex::Env::Default(), "foreach", kNumThreads);
  }
  ~ParallelState() {}
  void Wait() {
  ...
  absl::Mutex mu;
  std::optional<itex::thread::ThreadPool> pool;
...
ShapeUtil::ForEachState::ForEachState(const Shape& s,
  ...
  ITEX_CHECK_EQ(shape.rank(), base.size());
  ITEX_CHECK_EQ(incr.size(), base.size());
  ITEX_CHECK_EQ(count.size(), base.size());
}
