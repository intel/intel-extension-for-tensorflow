...
Shape::Shape(const ShapeProto& shape_proto) {
  ...
  if (shape_proto.dimensions_size() !=
    ...
    if (shape_proto.is_dynamic_dimension_size() != 0) {
      ...
      ITEX_LOG(WARNING)
          << "Malformed shape proto: is_dynamic_dimension is empty";
    }
  }
  ...
  if (shape_proto.has_layout()) {
    if (!IsArray()) {
      ITEX_LOG(ERROR) << "Malformed shape proto: element_type "
...
void Shape::DeleteDimension(int64_t dim_to_delete) {
  ITEX_CHECK(IsArray());
  ITEX_CHECK_GE(dim_to_delete, 0);
  ITEX_CHECK_LT(dim_to_delete, dimensions_.size());
...
bool Shape::Equal::operator()(const Shape& lhs, const Shape& rhs) {
  ...
  if (!ignore_element_type_) {
    if ((ignore_fp_precision_ &&
      ...
      ITEX_VLOG(3) << "CompareShapes: lhs element type != rhs element type";
      return false;
    }
  }
  ...
  if (!ignore_dimensions_) {
    if (!ShapeUtil::SameDimensions(lhs, rhs)) {
      ITEX_VLOG(3) << "CompareShapes: lhs dimensions != rhs dimensions";
      return false;
    }
  } else {
    if (!ShapeUtil::SameRank(lhs, rhs)) {
      ITEX_VLOG(3) << "CompareShapes: lhs rank != rhs rank";
      return false;
    }
  }
  ...
  if (!ignore_layout_) {
    if (lhs.IsArray()) {
      ...
      if (lhs.has_layout() || rhs.has_layout()) {
        if (!lhs.has_layout() || !rhs.has_layout()) {
          ITEX_VLOG(3) << "CompareShapes: both shapes do not have layouts";
          return false;
        }
        ...
        if (!equal(lhs.layout(), rhs.layout())) {
          ITEX_VLOG(3) << "CompareShapes: lhs layout != rhs layout";
          return false;
        }
      }
    }
