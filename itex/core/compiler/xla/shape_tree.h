...
#ifndef ITEX_CORE_COMPILER_XLA_SHAPE_TREE_H_
#define ITEX_CORE_COMPILER_XLA_SHAPE_TREE_H_
...
class ShapeTree {
 ...
 public:
  ...
  void replace_shape_ptr(const Shape& shape) {
    if (shape_storage_ != nullptr) {
      ITEX_DCHECK_EQ(shape, *shape_storage_);
  ...
  itex::gtl::iterator_range<leaf_iterator> leaves() {
    return itex::gtl::make_range(leaf_begin(), leaf_end());
  }
  itex::gtl::iterator_range<const_leaf_iterator> leaves() const {
    return itex::gtl::make_range(leaf_begin(), leaf_end());
  }
  ...
  void CopySubtreeFrom(const ShapeTree<T>& other, const ShapeIndex& src_index,
    ...
    ITEX_CHECK(ShapeUtil::Compatible(src_shape, dst_shape))
        << src_shape << ", " << dst_shape;
 ...
 private:
  ...
  ShapeTree(const Shape* shape, Nodes nodes)
    ...
    ITEX_DCHECK_EQ(nodes_.size(), ShapeUtil::SubshapeCount(*shape));
  }
...
#endif  // ITEX_CORE_COMPILER_XLA_SHAPE_TREE_H_
