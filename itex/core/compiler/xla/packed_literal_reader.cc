...
#include "itex/core/compiler/xla/packed_literal_reader.h"
...
PackedLiteralReader::PackedLiteralReader(itex::RandomAccessFile* file)
...
StatusOr<Literal> PackedLiteralReader::Read(const Shape& shape,
                                            const Layout* layout) {
  ITEX_VLOG(3) << "reading shape from file: " << ShapeUtil::HumanString(shape)
               << " layout: "
               << (layout == nullptr ? "<none>" : layout->ToString());
  ...
  if (!s.ok()) {
    ...
    ITEX_CHECK_EQ(sp.size(), bytes);
    if (sp.data() != data) {
      memcpy(data, sp.data(), sp.size());
    }
  }
  ITEX_VLOG(3) << "read shape from file: " << ShapeUtil::HumanString(shape);
