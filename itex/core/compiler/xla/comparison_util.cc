...
bool IsValidComparison(itex_xla::PrimitiveType type, Comparison::Order order) {
  switch (type) {
    case F16:
    case F32:
    case BF16:
...
Comparison::Order DefaultOrdering(PrimitiveType type) {
  switch (type) {
    ...
    case C128:
      return Comparison::Order::kPartial;
    default:
      ITEX_LOG(FATAL) << "Unsupported type: " << PrimitiveType_Name(type);
  }
}
...
std::string ComparisonDirectionToString(Comparison::Direction direction) {
  switch (direction) {
    ...
    default:
      ITEX_LOG(FATAL)
          << "Attempted to print uninitialized comparison direction";
  }
}
...
Comparison::Type Comparison::DefaultComparisonType(PrimitiveType type) {
  switch (type) {
    ...
    case C128:
      return Type::kFloat;
    default:
      ITEX_LOG(FATAL) << "Unexpected: " << PrimitiveType_Name(type);
  }
}
...
Comparison::Comparison(Direction dir, PrimitiveType type, Order order)
  ...
  ITEX_CHECK(IsValidComparison(primitive_type_, order_));
}
...
Comparison Comparison::Converse() const {
  return Comparison(itex_xla::Converse(dir_), primitive_type_, order_);
}
...
absl::optional<Comparison> Comparison::Inverse() const {
  ...
  switch (primitive_type_) {
    ...
    case U32:
    case U64:
      return Comparison(itex_xla::Inverse(dir_), primitive_type_, order_);
    case TUPLE:
    case OPAQUE_TYPE:
    case TOKEN:
    case PRIMITIVE_TYPE_INVALID:
