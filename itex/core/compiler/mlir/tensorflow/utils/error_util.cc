...
#include "itex/core/utils/managed_stack_trace.h"
  ...
  if (filter_stack) {
    this->shouldShowLocFn = [](Location loc) -> bool {
      ...
      if (FileLineColLoc fileLoc = loc.dyn_cast<FileLineColLoc>()) {
        return !itex::IsInternalFrameForFilename(fileLoc.getFilename().str());
      } else {
...
StatusScopedDiagnosticHandler::~StatusScopedDiagnosticHandler() {
  ...
  ITEX_DCHECK(all_errors_produced_were_consumed)
...
Status StatusScopedDiagnosticHandler::ConsumeStatus() {
  ...
  // up.
  Status s = itex::errors::Unknown(diag_str_);
  diag_str_.clear();
  return s;
}
...
Status StatusScopedDiagnosticHandler::Combine(Status status) {
  ...
  // there.
  status = ::itex::Status(status.code(),
                          absl::StrCat(status.error_message(), diag_str_));
  diag_str_.clear();
  return status;
}
...
LogicalResult StatusScopedDiagnosticHandler::handler(Diagnostic* diag) {
  ...
  // Emit non-errors to ITEX_VLOG instead of the internal status.
  if (diag->getSeverity() != DiagnosticSeverity::Error) {
    ITEX_VLOG(1) << diag_str_.substr(current_diag_str_size_);
