...
using ::int64_t;
using itex_xla::PrimitiveType;
using itex_xla::ShapeUtil;
...
StatusOr<Shape> TypeToShape(
  ...
  itex::PartialTensorShape partial_tensor_shape =
      itex::ConvertTypeToTensorShape(type);

  itex::TensorShape fully_defined_tensor_shape;
  ...
  itex::DataType dtype;
  TF_RETURN_IF_ERROR(itex::ConvertToDataType(type, &dtype));
...
Shape TypeToShape(mlir::Type type) {
  ...
  if (type.isIntOrFloat()) {
    ...
    llvm::SmallVector<int64_t, 4> dimensions(m.getShape().begin(),
                                             m.getShape().end());
    return ::itex_xla::ShapeUtil::MakeShapeWithLayout(
    ...
    // TODO(b/115638799): Update once itex_xla::Shape can support dynamic
    // shapes.
