...
static StringRef GetClientBuilder(const Operator& op) {
  ...
  // names in the dialect. For e.g., AddOp -> itex_xla::Add method.
...
static void BuildOperator(const Operator& op, raw_ostream& os) {
  ...
  for (int index : llvm::seq<int>(0, op.getNumArgs())) {
    ...
    if (auto* operand_cst = arg.dyn_cast<NamedTypeConstraint*>()) {
      ...
      os << "  std::vector<itex_xla::XlaOp> " << xla_arg << ";\n"
         << "  for (auto operand : op.getODSOperands(" << operand_number++
         << ")) {\n";
      os << "    itex_xla::XlaOp result;\n";
  ...
  os << "  auto xla_result = itex_xla::" << GetClientBuilder(op) << "(";
...
static bool OperatorWritersMain(raw_ostream& os, RecordKeeper& records) {
  ...
  os << "  itex_xla::XlaScopedShardingAssignment "
        "sharding(lowering_context.builder, "
  ...
  os << "  itex_xla::XlaScopedFrontendAttributesAssignment "
  ...
  os << "  itex_xla::XlaScopedOpMetadataAssignment "
