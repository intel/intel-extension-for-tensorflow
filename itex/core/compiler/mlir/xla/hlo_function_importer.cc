...
bool DotIsDefault(const HloInstruction* instruction) {
  ...
  return itex_xla::protobuf_util::ProtobufEquals(dnums,
...
Status HloFunctionImporter::ImportAsRegion(
    const itex_xla::HloComputation& computation, mlir::Region* region,
...
itex::Status HloFunctionImporter::ImportAsRegion(
...
StatusOr<mlir::Operation*> HloFunctionImporter::ImportInstructionImpl(
  ...
  const Shape& shape =
      mode == DynamicShapeHandlingMode::kConvertToStatic
          ? itex_xla::ShapeUtil::MakeStaticShape(instruction_shape)
          : instruction_shape;
  ...
  switch (instruction->opcode()) {
    ...
    case HloOpcode::kCustomCall: {
      ...
      //   } else {
      //     mlir::Attribute attr =
      //         mlir::parseAttribute(raw_backend_config,
      //         builder_->getContext());
      //     if (!attr.isa<mlir::DictionaryAttr>())
      //       return Internal(
      //           "Couldn't parse backend config into a dictionary attribute");
      //
      //     attributes.push_back(builder_->getNamedAttr("backend_config",
      //     attr));
      //   }
      // } else {
      //   attributes.push_back(builder_->getNamedAttr(
    ...
    case HloOpcode::kInfeed: {
      if (IsNestedTupleInData(result_type)) {
        llvm_unreachable(
            "Importing itex_xla::kInfeed with nested tuple shape not "
            "supported");
      }
    ...
    case HloOpcode::kRng: {
      ...
      switch (instruction->random_distribution()) {
        ...
        default:
          return itex::errors::InvalidArgument(absl::StrCat(
    ...
    case HloOpcode::kRngBitGenerator: {
      ...
      auto op = func_builder->create<mlir::mhlo::RngBitGeneratorOp>(
          ...
          static_cast<mlir::mhlo::RngAlgorithm>(
              func_builder->getI32IntegerAttr(rng_op->algorithm()).getInt()),
          operands[0]);
    ...
    case HloOpcode::kConvert: {
      ...
      itex_xla::Shape input_shape = instruction->operand(0)->shape();
...
#define NO_ATTRIBUTE_CASE(hlo_op_code, mlir_op)                               \
  case HloOpcode::hlo_op_code: {                                              \
    return func_builder                                                       \
      ...
      // defined in itex/core/compiler/xla/client/xla_builder.h. See
...
itex::Status HloFunctionImporter::GetMlirTypes(
...
StatusOr<Value> HloFunctionImporter::GetMlirValue(
  ...
  return itex::errors::Internal(absl::StrCat("Unable to find value for input: ",
...
mlir::NamedAttribute HloFunctionImporter::ConvertChannelHandle(
    const itex_xla::ChannelHandle& channel) {
  return builder_->getNamedAttr(
      "channel_handle",
      mlir::mhlo::ChannelHandleAttr::get(
          context_, builder_->getI64IntegerAttr(channel.handle()).getInt(),
          builder_->getI64IntegerAttr(channel.type()).getInt()));
}
...
Status HloFunctionImporter::ConvertShapeToMlirLayout(
  ...
  if (shape.IsArray()) {
    const itex_xla::Layout l = shape.layout();
  ...
  return itex::errors::Internal("Couldn't convert layout.");
}
