...
#ifndef ITEX_CORE_COMPILER_MLIR_XLA_HLO_UTILS_H_
#define ITEX_CORE_COMPILER_MLIR_XLA_HLO_UTILS_H_
...
static StatusOr<TypeT> ConvertTensorShapeToType(const Shape& shape,
  ...
  for (auto element : llvm::enumerate(shape.dynamic_dimensions())) {
    ...
    if (is_dynamic) {
      array[element.index()] = mlir::ShapedType::kDynamic;
    }
  }
...
::itex_xla::StatusOr<::itex_xla::HloOpcode> MhloToHloOpcode(
    mlir::Operation* op);

}  // namespace itex_xla

#endif  // ITEX_CORE_COMPILER_MLIR_XLA_HLO_UTILS_H_
