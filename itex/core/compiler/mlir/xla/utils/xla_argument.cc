...
bool XlaArgument::operator==(const XlaArgument& other) const {
  ...
  if (absl::holds_alternative<itex_xla::Shape>(shape)) {
    if (!absl::holds_alternative<itex_xla::Shape>(other.shape)) {
      return false;
    }
    if (!itex_xla::Shape::Equal()(absl::get<itex_xla::Shape>(shape),
                             absl::get<itex_xla::Shape>(other.shape))) {
      return false;
    }
  } else {
