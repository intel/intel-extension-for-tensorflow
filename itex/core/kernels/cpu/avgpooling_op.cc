...
#define REGISTER_CPU_POOL_KERNELS(T)                                         \
  REGISTER_KERNEL_BUILDER(                                                   \
      Name("_ITEXAvgPool").Device(DEVICE_CPU).TypeConstraint<T>("T"),        \
      PoolingOp<CPUDevice, T, dnnl::algorithm::pooling_avg_exclude_padding>) \
  REGISTER_KERNEL_BUILDER(                                                   \
      Name("_ITEXAvgPool3D").Device(DEVICE_CPU).TypeConstraint<T>("T"),      \
      PoolingOp<CPUDevice, T, dnnl::algorithm::pooling_avg_exclude_padding>)
...
#define REGISTER_CPU_POOL_GRAD_KERNELS(T)                             \
  REGISTER_KERNEL_BUILDER(                                            \
      Name("_ITEXAvgPoolGrad")                                        \
          .Device(DEVICE_CPU)                                         \
          .TypeConstraint<T>("T")                                     \
          .HostMemory("orig_input_shape"),                            \
      AvgPoolGradOp<CPUDevice, T, dnnl::prop_kind::forward_training>) \
  REGISTER_KERNEL_BUILDER(                                            \
      Name("_ITEXAvgPool3DGrad")                                      \
          .Device(DEVICE_CPU)                                         \
          .TypeConstraint<T>("T")                                     \
          .HostMemory("orig_input_shape"),                            \
      AvgPoolGradOp<CPUDevice, T, dnnl::prop_kind::forward_training>)
