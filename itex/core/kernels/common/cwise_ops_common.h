...
#ifndef ITEX_CORE_KERNELS_COMMON_CWISE_OPS_COMMON_H_
#define ITEX_CORE_KERNELS_COMMON_CWISE_OPS_COMMON_H_
...
struct BinaryFunctor<GPUDevice, Functor, NDIMS, false> {
  void operator()(const GPUDevice& d, typename Functor::tout_type out,
  ...
  void Left(const GPUDevice& d, typename Functor::tout_type out,
    ...
    // TODO(itex): explicit broadcast as Accuracy issur in Eign broadcast, we
  ...
  void BCast(const GPUDevice& d,
    ...
    if (bcast0_all_one && bcast1_all_one) {
      To32Bit(out).device(d) = To32Bit(in0).binaryExpr(To32Bit(in1), func);
    } else if (bcast0_all_one) {
      To32Bit(out).device(d) =
          To32Bit(in0).binaryExpr(To32Bit(in1).broadcast(bcast1), func);
    } else if (bcast1_all_one) {
      To32Bit(out).device(d) =
...
template <typename Functor, int NDIMS>
struct BinaryFunctor<GPUDevice, Functor, NDIMS, true> {
  void operator()(const GPUDevice& d, typename Functor::tout_type out,
                  typename Functor::tin_type in0,
                  typename Functor::tin_type in1, bool* error) {
    To32Bit(out).device(d) =
        To32Bit(in0).binaryExpr(in1, typename Functor::func(error));
  }
  ...
  void Right(const GPUDevice& d, typename Functor::tout_type out,
    ...
    Eigen::Sizes<1> scalar_dim;
    out.device(d) = in.binaryExpr(
        scalar.reshape(scalar_dim).broadcast(in.dimensions()), Binary(error));
  }
...
class BinaryOpShared : public OpKernel {
 ...
 protected:
  struct BinaryOpState {
    ...
    explicit BinaryOpState(OpKernelContext* ctx, const std::string& op,
...
class BinaryOp : public BinaryOpShared {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    BinaryOpCompute(ctx, input_0, input_1);
  }

  void BinaryOpCompute(OpKernelContext* ctx, const Tensor& input_0,
                       const Tensor& input_1) {
    // const Device& eigen_device = ctx->eigen_gpu_device();
    ...
    BinaryOpState state(ctx, op_name, has_attr, incompatible_shape_error);
    if (ctx->status().code() == TF_Code::TF_RESOURCE_EXHAUSTED) {
...
#endif  // ITEX_CORE_KERNELS_COMMON_CWISE_OPS_COMMON_H_
