...
#undef NDIMS_CASE
  ...
  static Status ReduceOperand(OpKernelContext* ctx, const Tensor& input,
    ...
    TF_RETURN_IF_ERROR(
        ctx->allocate_temp(DataTypeToEnum<T>::value, output_shape, output));
    if (reshape[kReduce] == 0) {
      functor::SetZeroFunctor<Device, T> func;
      func(ctx->eigen_device<Device>(), output->flat<T>());
      return Status::OK();
    }
  ...
  static Status ContractOperands(OpKernelContext* ctx,
    ...
    // start BatchMatMul with oneDNN
    const auto adj_x = false;
    const auto adj_y = false;
    const Tensor& in_x = inputs[0];
    const Tensor& in_y = inputs[1];
    Tensor* dst_tensor = &output_reshaped;

    const int kSrcDims = in_x.dims();
    const auto m = adj_x || trans_x ? in_x.dim_size(kSrcDims - 1)
                                    : in_x.dim_size(kSrcDims - 2);
    const int kWeightsDims = in_y.dims();
    const auto n = adj_y || trans_y ? in_y.dim_size(kWeightsDims - 2)
                                    : in_y.dim_size(kWeightsDims - 1);

    TensorShape dst_shape = bcast.output_batch_shape();
    dst_shape.AddDim(m);
    dst_shape.AddDim(n);
    if (dst_tensor->NumElements() == 0) {
      return Status::OK();
    }
    ...
    try {
      ...
      auto params = MatMulBaseUtil::CreateMatMulParams(
          in_x.shape(), in_y.shape(), dst_shape, trans_x, trans_y);
      ...
      TF_ABORT_IF_ERROR(ctx->allocate_temp(DataTypeToEnum<T>::v(),
                                           TensorShape({scratchpad_size}),
                                           &scratchpad_tensor));
      auto scratchpad_mem =
          dnnl::memory(matmul_pd.scratchpad_desc(), dnnl_engine,
      ...
      return Status(TF_INTERNAL, error_msg);
    }
    return Status::OK();
  }
};
...
class EinsumOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    OpInputList inputs(ctx, 0, ctx->num_inputs());
