...
#ifndef ITEX_CORE_KERNELS_GPU_TRAINING_OPS_H_
#define ITEX_CORE_KERNELS_GPU_TRAINING_OPS_H_
...
template <typename Device, typename T>
struct ApplyRMSPropComputeRMS {
  void operator()(const Device& d, typename TTypes<T>::ConstFlat ms,
                  typename TTypes<T>::ConstScalar rho,
                  typename TTypes<T>::ConstFlat grad,
...
template <typename T>
inline T rsqrt(const T& v) {
  return 1 / Eigen::numext::sqrt(v);
}

template <>
inline float rsqrt<float>(const float& v) {
  return sycl::rsqrt(v);
}

template <>
inline Eigen::half rsqrt<Eigen::half>(const Eigen::half& v) {
  sycl::half cv(v.x);
  return Eigen::half(sycl::rsqrt(cv));
}

template <>
inline Eigen::bfloat16 rsqrt<Eigen::bfloat16>(const Eigen::bfloat16& v) {
  float f = v;
  return Eigen::bfloat16(sycl::rsqrt(f));
}

}  // end namespace functor
}  // end namespace itex

#endif  // ITEX_CORE_KERNELS_GPU_TRAINING_OPS_H_
