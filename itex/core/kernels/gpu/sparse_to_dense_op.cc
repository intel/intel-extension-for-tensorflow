  ...
  void Compute(OpKernelContext* c) override {
    ...
    if (indices.dtype() == DT_INT64 && indices.dims() == 2) {
      ...
      if (indices.dtype() == DT_INT64) {
        ITEX_CHECK(indices_shaped_holder->CopyFrom(indices, ix_shape));
      } else {
...
class SparseToDenseGPU : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* c) final {
    ...
    OP_REQUIRES_OK(
        c, c->allocate_temp(DataTypeToEnum<Index>::value,
                            {output_shape_vec.size()}, &output_shape_tensor));
    const Eigen::GpuDevice& device = c->eigen_device<Eigen::GpuDevice>();
    sycl::queue* stream = device.stream();
    stream
        ->memcpy(output_shape_tensor.template flat<Index>().data(),
                 output_shape_vec.data(),
                 output_shape_tensor.NumElements() * sizeof(Index))
        .wait();
    functor::LaunchSparseToDense<T, Index>()(
        c, validate_indices_, indices, sparse_values, output_shape_tensor,
        default_value.scalar<T>()(), output);
  }
