...
#define DECLARE_TYPE(T)                             \
  extern template struct Tile<GPUDevice, T, int32>; \
  extern template struct Tile<GPUDevice, T, int64>;
...
#undef HANDLE_TYPE
 ...
 private:
  ...
  void HandleCaseImpl(OpKernelContext* context,
    ...
    functor::Tile<Device, T, Tmultiples>()(context->eigen_gpu_device(), result,
...
inline void TileOp<Device, Tmultiples>::HandleCase(
  ...
  // having to use RTTI.
  ITEX_LOG(FATAL) << "TileOp: Invalid combination of Device, DT: "
                  // << typeid(Device).name() << ", "
                  << DataTypeString(DT);
}
...
inline void TileGradientOp<Device, Tmultiples>::HandleCase(
    ...
    const gtl::ArraySlice<Tmultiples>& multiples_array, Tensor* result) {
  ITEX_LOG(FATAL)
      << "TileGradientOp: Invalid combination of Device, DT and NDIM: "
