...
class DynamicPartitionOpShared : public OpKernel {
 public:
  explicit DynamicPartitionOpShared(OpKernelConstruction* c) : OpKernel(c) {
  ...
  void ValidateAndAllocateOutputs(OpKernelContext* c, Tensor** data,
                                  Tensor** partitions, OpOutputList* Tout) {
...
template <class T>
class DynamicPartitionOp : public DynamicPartitionOpShared {
 public:
  explicit DynamicPartitionOp(OpKernelConstruction* c)
      : DynamicPartitionOpShared(c) {}
...
#define REGISTER_DYNAMIC_PARTITION(T)                   \
  REGISTER_KERNEL_BUILDER(Name("DynamicPartition")      \
                              .Device(DEVICE_GPU)       \
                              .TypeConstraint<T>("T")   \
                              .HostMemory("data")       \
                              .HostMemory("partitions") \
                              .HostMemory("outputs"),   \
                          DynamicPartitionOp<T>)
TF_CALL_GPU_NUMBER_TYPES(REGISTER_DYNAMIC_PARTITION);
