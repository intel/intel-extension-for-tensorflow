...
#include <limits>
#include <vector>
// TODO(itex): Fix non-standard include order.
...
#include "itex/core/utils/ptr_util.h"
...
class TensorArrayCreationOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    std::pair<std::string, std::string> output_handle_string;
    TensorArray* output_tensor_array;
    OP_REQUIRES_OK(ctx, CreateTensorArray(ctx, rm, &output_handle_string,
                                          &output_tensor_array));

    if (IsRefType(ctx->expected_output_dtype(0)) ||
        ctx->expected_output_dtype(0) == DT_STRING) {
      ITEX_VLOG(FATAL)
          << "Error: running unsupported TensorArray/TensorArrayV2, only"
             " TensorArrayV3/_ITEXTensorArray is GPU supported,"
             "please change to supported ones.";
    } else {
      Tensor* handle;
 ...
 protected:
  virtual Status CreateTensorArray(
      OpKernelContext* ctx, ResourceMgr* rm,
      //  Tensor* output_handle_string,
      std::pair<std::string, std::string>* output_handle_string,
...
class TensorArrayOp : public TensorArrayCreationOp {
 public:
  explicit TensorArrayOp(OpKernelConstruction* context)
    ...
    if (context->HasAttr("num_dims_of_element_shape"))
      OP_REQUIRES_OK(context, context->GetAttr("num_dims_of_element_shape",
                                               &num_dims_of_element_shape_));
    if (num_dims_of_element_shape_ == -1) {
      element_shape_ = PartialTensorShape();
    } else {
      OP_REQUIRES_OK(context, context->GetAttr("dims_of_element_shape",
                                               &dims_of_element_shape_));
      element_shape_ = PartialTensorShape(gtl::ArraySlice<int64_t>(
          dims_of_element_shape_.data(), num_dims_of_element_shape_));
    }
  ...
  Status CreateTensorArray(
    ...
    output_handle_string->first = "_tensor_arrays";
    output_handle_string->second = unique_tensor_array_name;

    auto key =
        strings::StrCat(output_handle_string->first, unique_tensor_array_name);

    TensorArray* tensor_array = new TensorArray(
        key, dtype_, *output_handle_string, size, element_shape_,
        identical_element_shapes_, dynamic_size_,
        false /* multiple_writes_aggregate */, false /* is_grad */,
        -1 /* marked_size */, clear_after_read_);
    int64_t current_step_id = ctx->step_id();
    TF_RETURN_IF_ERROR(rm->Create(key, tensor_array, current_step_id));
 ...
 private:
  DataType dtype_;
  int32_t num_dims_of_element_shape_ = -1;
  std::vector<int64_t> dims_of_element_shape_;
...
#define REGISTER_GPU(type)                                   \
  REGISTER_KERNEL_BUILDER(Name("_ITEXTensorArray")           \
                              .Device(DEVICE_GPU)            \
...
class TensorArrayGradOp : public TensorArrayCreationOp {
 public:
  ...
  Status CreateTensorArray(
    ...
    const Tensor& input1_t = ctx->input(0);
    if (ctx->input_dtype(0) != DT_RESOURCE) {
      return errors::Unimplemented(
          "TensorArray/TensorArrayV2 is not supported, please check is it "
          "rewritted as _ITEXTensorArray or directly use TensorArrayV3.");
      ...
      const auto& resource = input1_t.flat<ResourceHandle>()(0);
    ...
    // auto output_handle = output_handle_string->flat<tstring>();
    output_handle_string->first = "_tensor_array_grads";
    output_handle_string->second =
        strings::StrCat(tensor_array_name, "@", source_);

    TensorArray* tensor_array;
    TF_RETURN_IF_ERROR(rm->Lookup(strings::StrCat(container, tensor_array_name),
                                  &tensor_array));
    ...
    if (ctx->num_inputs() > 2) {
      TF_RETURN_IF_ERROR(MakeShape(ctx->input(2), &shape_to_prepend));
    ...
    const auto key = strings::StrCat(output_handle_string->first,
                                     output_handle_string->second);
    auto creator = [key, tensor_array, array_size, marked_size, element_shape,
                    shape_to_prepend,
                    output_handle_string](TensorArray** ret) -> Status {
      *ret = new TensorArray(
          key, tensor_array->ElemType(), *output_handle_string, array_size,
          element_shape, tensor_array->HasIdenticalElementShapes(),
    ...
    Status s =
        rm->LookupOrCreate<TensorArray>(key, output_tensor_array, creator);
...
REGISTER_KERNEL_BUILDER(Name("_ITEXTensorArrayGrad")
                            .Device(DEVICE_GPU)
                            .HostMemory("handle")
                            .HostMemory("grad_handle"),
                        TensorArrayGradOp);
REGISTER_KERNEL_BUILDER(Name("_ITEXTensorArrayGradWithShape")
                            .Device(DEVICE_GPU)
                            .HostMemory("handle")
...
class TensorArrayWriteOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    const Tensor* tensor_index = &(ctx->input(1));
    const Tensor* tensor_value = &(ctx->input(2));
...
#define REGISTER_GPU(type)                                      \
  REGISTER_KERNEL_BUILDER(Name("_ITEXTensorArrayWrite")         \
                              .Device(DEVICE_GPU)               \
...
#define REGISTER_GPU(type)                                     \
  REGISTER_KERNEL_BUILDER(Name("_ITEXTensorArrayRead")         \
                              .Device(DEVICE_GPU)              \
                              .TypeConstraint<type>("dtype")   \
                              .HostMemory("handle")            \
                              .HostMemory("index"),            \
 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (std::is_same<Device, GPUDevice>::value) {
      Concat<T>(ctx, input_tensors_flat, &output_flat);
      return;
    }
  }
  ...
  explicit TensorArrayConcatOp(OpKernelConstruction* context)
    ...
    if (context->HasAttr("num_dims_of_element_shape"))
      OP_REQUIRES_OK(context, context->GetAttr("num_dims_of_element_shape",
                                               &num_dims_of_element_shape_));
    if (num_dims_of_element_shape_ == -1) {
      element_shape_except0_ = PartialTensorShape();
    } else {
      OP_REQUIRES_OK(context, context->GetAttr("dims_of_element_shape",
                                               &dims_of_element_shape_));
      element_shape_except0_ = PartialTensorShape(gtl::ArraySlice<int64_t>(
          dims_of_element_shape_.data(), num_dims_of_element_shape_));
    }
  }
...
#define REGISTER_GPU(type)                                       \
  REGISTER_KERNEL_BUILDER(Name("_ITEXTensorArrayConcat")         \
                              .Device(DEVICE_GPU)                \
                              .TypeConstraint<type>("dtype")     \
                              .HostMemory("lengths")             \
                              .HostMemory("handle"),             \
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    for (int i = 0; i < num_values; ++i) {
      ...
      if (element_shape.num_elements() > 0) {
        functor::Split<T, 3>()(ctx->eigen_device<Device>(), tensor_value_i_t,
...
#define REGISTER_GPU(type)                                      \
  REGISTER_KERNEL_BUILDER(Name("_ITEXTensorArraySplit")         \
                              .Device(DEVICE_GPU)               \
                              .TypeConstraint<type>("T")        \
                              .HostMemory("lengths")            \
                              .HostMemory("handle"),            \
...
class TensorArraySizeOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES_OK(
        ctx, tensor_array->Size(&(output->scalar<int32>()())));  //  NOLINT
  }
};

REGISTER_KERNEL_BUILDER(Name("_ITEXTensorArraySize")
                            .Device(DEVICE_GPU)
                            .HostMemory("handle")
                            .HostMemory("size"),
                        TensorArraySizeOp);
...
REGISTER_KERNEL_BUILDER(
    Name("_ITEXTensorArrayClose").Device(DEVICE_GPU).HostMemory("handle"),
    TensorArrayCloseOp);
REGISTER_KERNEL_BUILDER(
