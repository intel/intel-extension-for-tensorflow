...
template <typename T>
Status ResizeNearestNeighborNHWC(const GPUDevice& d, int total_count,
                                 const T* bottom_data, int in_height,
                                 int in_width, int channels, int out_height,
                                 int out_width, float height_scale,
                                 float width_scale, T* top_data) {
...
template <typename T, bool align_corners>
Status LegacyResizeNearestNeighborNHWC(const GPUDevice& d, int total_count,
                                       const T* bottom_data, int in_height,
                                       int in_width, int channels,
                                       int out_height, int out_width,
                                       float height_scale, float width_scale,
                                       T* top_data) {
  auto stream = d.stream();
...
struct LegacyResizeNearestNeighborGradKernelTask {
  LegacyResizeNearestNeighborGradKernelTask(int input_size, const T* top_diff,
                                            int in_height, int in_width,
                                            int channels, int out_height,
                                            int out_width, float height_scale,
                                            float width_scale, T* bottom_diff)
      : input_size(input_size),
        top_diff(top_diff),
        in_height(in_height),
        in_width(in_width),
...
template <typename T>
Status ResizeNearestNeighborGradNHWCKernel(const GPUDevice& d, int input_size,
                                           const T* top_diff, int in_height,
                                           int in_width, int channels,
                                           int out_height, int out_width,
                                           float height_scale,
                                           float width_scale, T* bottom_diff) {
...
Status LegacyResizeNearestNeighborGradNHWCKernel(
    ...
    float height_scale, float width_scale, T* bottom_diff) {
  auto& stream = d.stream();
  auto wg_size =
      (*stream)
          .get_device()
    ...
    if (half_pixel_centers) {
      ...
      auto status =
          LegacyResizeNearestNeighborGradNHWCKernel<float, align_corners>(
              d, input_size, static_cast<float*>(input_fp32.data()), in_height,
              in_width, channels, out_height, out_width, height_scale,
