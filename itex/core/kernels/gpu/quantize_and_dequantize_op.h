...
#ifndef ITEX_CORE_KERNELS_GPU_QUANTIZE_AND_DEQUANTIZE_OP_H_
#define ITEX_CORE_KERNELS_GPU_QUANTIZE_AND_DEQUANTIZE_OP_H_
...
// TODO(itex): 'signed_input' should really be called 'signed_output'.
...
struct QuantizeAndDequantizePerChannelImpl {
  static void Compute(const Device& d, typename TTypes<T, 3>::ConstTensor input,
                      ...
                      QuantizerRoundMode round_mode, bool narrow_range,
                      typename TTypes<T, 3>::Tensor output) {
    using Index = typename TTypes<T>::ConstTensor::Index;
    ...
    if (!range_given) {
      ...
      // Copy the range values from their respective tensors on the host.
      std::copy_n(
          reinterpret_cast<const char*>(input_min_tensor->vec<T>().data()),
          num_channels * sizeof(T), reinterpret_cast<char*>(min_range.data()));
      std::copy_n(
          reinterpret_cast<const char*>(input_max_tensor->vec<T>().data()),
          num_channels * sizeof(T), reinterpret_cast<char*>(max_range.data()));
    }
...
#endif  // ITEX_CORE_KERNELS_GPU_QUANTIZE_AND_DEQUANTIZE_OP_H_
