...
#ifndef ITEX_CORE_KERNELS_GPU_DATA_FORMAT_OPS_H_
#define ITEX_CORE_KERNELS_GPU_DATA_FORMAT_OPS_H_
...
template <typename T>
struct VecPermute {
  explicit VecPermute(const Eigen::DSizes<Eigen::DenseIndex, 8>& dst)
      : dst_(dst) {}
  ...
  void eval(typename TTypes<T>::ConstFlat input,
            Output& output,  // NOLINT(runtime/references)
...
#endif  // ITEX_CORE_KERNELS_GPU_DATA_FORMAT_OPS_H_
