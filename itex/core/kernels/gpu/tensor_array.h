...
#ifndef ITEX_CORE_KERNELS_GPU_TENSOR_ARRAY_H_
#define ITEX_CORE_KERNELS_GPU_TENSOR_ARRAY_H_
...
class TensorArray : public ResourceBase {
 public:
  ...
  TensorArray(const string& key, const DataType& dtype,
              const std::pair<std::string, std::string>& handle, int32_t N,
  ...
  mutex* mu() { return &mu_; }
  std::pair<std::string, std::string>* handle() { return &handle_; }

  ResourceHandle resource_handle(OpKernelContext* ctx) {
    return ctx->resource_manager()->MakeResourceHandle<TensorArray>(
        key_, ctx->GetDeviceStream()->get_device());
  }
 ...
 private:
  ...
  Status LockedReturnIfClosed() const TF_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    if (closed_) {
      return errors::InvalidArgument("TensorArray ", handle_.first,
  ...
  // Tensor handle_;
  std::pair<std::string, std::string> handle_;
...
Status TensorArray::LockedWriteOrAggregate(OpKernelContext* ctx,
  ...
  if (index < 0 || (!dynamic_size_ && index_size >= tensors_.size())) {
    return errors::InvalidArgument(
        "TensorArray ", handle_.first, ": Tried to write to index ", index,
        " but array is not resizeable and size is: ", tensors_.size());
  }
  if (dynamic_size_) {
  ...
  if (t.written) {
    ITEX_DCHECK(multiple_writes_aggregate_);
...
#endif  // ITEX_CORE_KERNELS_GPU_TENSOR_ARRAY_H_
