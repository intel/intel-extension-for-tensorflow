...
#ifndef ITEX_CORE_KERNELS_GPU_IMAGE_IMAGE_OPS_H_
#define ITEX_CORE_KERNELS_GPU_IMAGE_IMAGE_OPS_H_
...
enum Interpolation : unsigned int { NEAREST, BILINEAR };
...
class ProjectiveGenerator {
 ...
 public:
  ...
  nearest_interpolation(const DenseIndex batch, const float y, const float x,
    ...
    return read_with_fill_value(batch, DenseIndex(sycl::round(y)),
                                DenseIndex(sycl::round(x)), channel,
                                fill_value);
  }
  ...
  bilinear_interpolation(const DenseIndex batch, const float y, const float x,
                         const DenseIndex channel, const T fill_value) const {
    const float y_floor = sycl::floor(y);
    const float x_floor = sycl::floor(x);
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE T read_with_fill_value(
    ...
    // the input.
    // return T(0);
    const int height = input_.dimension(1);
    const int width = input_.dimension(2);
    const int channel_size = input_.dimension(3);
    const int index =
        ((batch * height + y) * width + x) * channel_size + channel;
    const T* ptr = input_.data();
    // TODO(itex): the commented code below is the original Eigen code in
    // official tensorflow, but it caused segment fault in our side. Still work
    // on to find the root cause
    return (0 <= y && y < height && 0 <= x && x < width)
               ? ptr[index]  // input_(array<DenseIndex, 4>{batch, y, x,
                             // channel})
               : fill_value;
  }
};
...
#endif  // ITEX_CORE_KERNELS_GPU_IMAGE_IMAGE_OPS_H_
