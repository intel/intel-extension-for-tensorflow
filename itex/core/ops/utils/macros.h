...
#ifndef ITEX_CORE_OPS_UTILS_MACROS_H_
#define ITEX_CORE_OPS_UTILS_MACROS_H_
...
// (for instance, a ITEX_CHECK failure), and use that information in static
...
#define ITEX_PREDICT_FALSE(x) (__builtin_expect(x, 0))
#define ITEX_PREDICT_TRUE(x) (__builtin_expect(!!(x), 1))
#else
#define ITEX_PREDICT_FALSE(x) (x)
#define ITEX_PREDICT_TRUE(x) (x)
#endif
...
#define TF_UNUSED_VARIABLE(x) \
  itex::internal::remove_unused_variable_compiler_warning(x)

#endif  // ITEX_CORE_OPS_UTILS_MACROS_H_
