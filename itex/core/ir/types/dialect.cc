...
#include "itex/core/ir/types/attributes.cc.inc"
#include "itex/core/ir/types/attributes_enum.cc.inc"

#define GET_TYPEDEF_CLASSES
#include "itex/core/ir/types/types.cc.inc"

// Generated definitions.
#include "itex/core/ir/types/dialect.cpp.inc"
...
void ITEXTypeDialect::initialize() {
  addAttributes<
#define GET_ATTRDEF_LIST
#include "itex/core/ir/types/attributes.cc.inc"  //NOLINT
      >();
  addTypes<ControlType, OpaqueTensorType,
#define HANDLE_TF_TYPE(tftype, enumerant, name) tftype##Type,
#define HANDLE_LAST_TF_TYPE(tftype, enumerant, name) tftype##Type
#include "itex/core/ir/types/types.def"
           >();
}

namespace {
template <typename TypeWithSubtype>
Type ParseTypeWithSubtype(MLIRContext* context,
                          DialectAsmParser& parser) {  // NOLINT
...
Type ParseResourceType(MLIRContext* context,
                       DialectAsmParser& parser) {  // NOLINT
  return ParseTypeWithSubtype<ResourceType>(context, parser);
}

void PrintResourceType(ResourceType ty, DialectAsmPrinter& os) {  // NOLINT
  return PrintTypeWithSubtype("resource", ty, os);
}

Type ParseVariantType(MLIRContext* context,
                      DialectAsmParser& parser) {  // NOLINT
  return ParseTypeWithSubtype<VariantType>(context, parser);
}

void PrintVariantType(VariantType ty, DialectAsmPrinter& os) {  // NOLINT
...
Type ITEXTypeDialect::parseType(DialectAsmParser& parser) const {
  StringRef type_tag;
...
// Entry point for Type parsing, TableGen generated code will handle the
// dispatch to the individual classes.
void ITEXTypeDialect::printType(Type type, DialectAsmPrinter& printer) const {
#define HANDLE_TF_TYPE(tftype, enumerant, name)          \
...
Attribute VersionAttr::parse(AsmParser& parser, Type) {
  ...
  if (parser.parseKeyword("producer", " in itex_type version") ||
      parser.parseEqual() || parser.parseInteger(producer) ||
      parser.parseComma() ||
      parser.parseKeyword("min_consumer", " in itex_type version") ||
      parser.parseEqual() || parser.parseInteger(min_consumer))
    return {};

  SmallVector<int32_t, 4> bad_consumers;
  if (!parser.parseOptionalComma()) {
    if (parser.parseKeyword("bad_consumers", " in itex_type version") ||
...
FailureOr<FullTypeAttr> RawFullTypeAttrParser(AsmParser& parser) {  // NOLINT
  ...
  parser.parseCommaSeparatedList(
      AsmParser::Delimiter::OptionalLessGreater, [&]() {
        FailureOr<itex_type::FullTypeAttr> arg = RawFullTypeAttrParser(parser);
...
Attribute FullTypeAttr::parse(AsmParser& parser, Type odsType) {
  if (failed(parser.parseLess())) return {};
  FailureOr<itex_type::FullTypeAttr> ret = RawFullTypeAttrParser(parser);
  if (succeeded(ret) && failed(parser.parseGreater())) return {};
  return ret.getValueOr(FullTypeAttr());
}

static void RawFullTypeAttrPrint(FullTypeAttr tfattr,
                                 AsmPrinter& printer) {  // NOLINT
  printer << stringifyFullTypeId(itex_type::FullTypeId(tfattr.getType_id()));
...
SubElementAttrInterface FuncAttr::replaceImmediateSubAttribute(
    ArrayRef<std::pair<size_t, Attribute>> replacements) const {  // NOLINT
...
bool TensorFlowType::classof(Type type) {
  return llvm::isa<ITEXTypeDialect>(type.getDialect());
}
...
bool BroadcastCompatible(TypeRange lhs, TypeRange rhs) {
  ...
  for (auto types : llvm::zip(lhs, rhs)) {
    ...
    // `tensor<!itex_type.f32ref>` and `tensor<f32>` should be considered
