...
#include "itex/core/ir/utility.h"
...
#include "itex/core/ir/dialect.cc.inc"
...
struct TFGraphOpAsmInterface
  ...
  void getAsmBlockArgumentNames(Operation* op, Region& region,  // NOLINT
...
void TFGraphDialect::initialize() {
  getContext()->getOrLoadDialect<itex_type::ITEXTypeDialect>();
  addOperations<
#define GET_OP_LIST
#include "itex/core/ir/ops.cc.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "itex/core/ir/attributes.cc.inc"
      >();
...
#include "itex/core/ir/tf_op_names.inc"
...
static void PrintKeywordAttributes(Operation* op,
                                   OpAsmPrinter& printer,  // NOLINT
...
static ParseResult ParseKeywordAttributes(OpAsmParser& parser,       // NOLINT
...
//           name("<node_name>") device("<device>") { attribute-dict } :
//           (input types) -> (result_types)
static ParseResult ParseCustomTfOp(OpAsmParser& parser,       // NOLINT
                                   OperationState& result) {  // NOLINT
...
static bool VerifyGenericTFGOperation(Operation& op) {  // NOLINT
...
LogicalResult GraphFuncOp::verifyBody() {
  ...
  for (auto& indexed_type : llvm::enumerate(type.getResults())) {
    ...
    if (!itex_type::AreCastCompatible({expected_type, actual_type})) {
...
static LogicalResult VerifySignature(GraphFuncOp func, Operation* op,
  ...
  for (auto& it : llvm::enumerate(operands)) {
    ...
    if (!itex_type::HasCompatibleElementTypes(arg_type, op_type)) {
      return attach_func(
  ...
  for (auto& it : llvm::enumerate(results)) {
    ...
    if (!itex_type::HasCompatibleElementTypes(ret_type, res_type)) {
      return attach_func(
          op->emitOpError(func_name)
...
static LogicalResult VerifyTypeArray(Operation* op, ValueRange values,
  ...
  for (auto it :
    ...
    if (!itex_type::HasCompatibleElementTypes(type,
...
static LogicalResult VerifyIfLikeOp(
    IfLikeOp op,
    SymbolTableCollection& symbol_table) {  // NOLINT
...
static LogicalResult VerifyCaseLikeOp(
    CaseLikeOp op,
    SymbolTableCollection& symbol_table) {  // NOLINT
  if (failed(op.verifyInvariants())) return failure();
  FailureOr<TypeRange> ins = VerifyOperands(op);
...
static LogicalResult VerifyWhileLikeOp(
    WhileLikeOp op,
    SymbolTableCollection& symbol_table) {  // NOLINT
  if (failed(op.verifyInvariants())) return failure();
  FailureOr<TypeRange> ins = VerifyOperands(op);
...
LogicalResult ForOp::verifySymbolUses(
    SymbolTableCollection& symbolTable) {  // NOLINT
...
static bool TerminatedByYield(Block& block) {  // NOLINT
...
void GetIfLikeRegionOpSuccessorRegions(
    ...
    SmallVectorImpl<RegionSuccessor>& regions) {  // NOLINT
...
// types.
static LogicalResult VerifyLoopRegionArgs(Operation* op,
...
#include "itex/core/ir/ops.cc.inc"  //NOLINT
#define GET_ATTRDEF_CLASSES
#include "itex/core/ir/attributes.cc.inc"  //NOLINT
