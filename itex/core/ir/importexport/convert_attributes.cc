...
Status ConvertAttribute(Type type, bool remove_ref_type, AttrValue* value) {
  ...
  if (itex::IsRefType(dtype)) dtype = itex::RemoveRefType(dtype);
...
Status ConvertAttributes(ArrayRef<NamedAttribute> attrs,
  ...
  for (const NamedAttribute& named_attr : attrs) {
    ...
    ITEX_CHECK_LE(name_tokens.size(), 2);
...
StatusOr<itex_type::FullTypeAttr> ConvertAttribute(
  ...
  switch (full_type.attr_case()) {
    case itex::FullTypeDef::AttrCase::kS:
      attr = builder.getStringAttr(full_type.s());
      break;
    case itex::FullTypeDef::AttrCase::kI:
      attr = builder.getI64IntegerAttr(full_type.i());
      break;
    case itex::FullTypeDef::ATTR_NOT_SET:
      break;
    default:
...
StatusOr<itex::FullTypeDef> ConvertAttribute(
  ...
  FullTypeDef ret;
  for (itex_type::FullTypeAttr it : full_type.getArgs()) {
