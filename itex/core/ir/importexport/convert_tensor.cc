...
static TensorProto ConvertToProto(const Tensor& input_tensor,
  ...
  if (use_tensor_content)
    ...
    ITEX_LOG(FATAL) << "Tensor::AsProtoField is not supported.";
  return tensor_proto;
}
...
template <typename T>
itex::StatusOr<ElementsAttr> ConvertFlatTensor(const Tensor& input_tensor,
                                               ShapedType type) {
...
itex::StatusOr<ElementsAttr> ConvertStringTensor(const Tensor& input_tensor,
                                                 ShapedType type) {
...
itex::StatusOr<ElementsAttr> ConvertTensor(const Tensor& input_tensor,
                                           Builder builder) {
...
#define CONVERT_FLAT(DTYPE, CTYPE) \
  ...
  switch (input_dtype) {
    ...
    default:
      ...
      // calls.
      return ElementsAttr(
          itex_type::TensorProtoAttr::get(type, MangleTensor(input_tensor)));
  }
...
itex::StatusOr<ElementsAttr> ConvertTensorProto(const TensorProto& input_tensor,
                                                Builder builder) {
...
Status ConvertTensorProtoAttr(const mlir::itex_type::TensorProtoAttr attr,
...
void ConvertHalfElementsAttr(const DenseElementsAttr attr,
  ...
  if (attr.isSplat()) {
    ...
    if (std::signbit(static_cast<float>(value))) output->Add(value);
  } else {
    output->Reserve(attr.getNumElements());
...
Status ConvertToTensorProto(const ElementsAttr attr, TensorProto* output) {
  ...
  if (auto tensor_attr = attr.dyn_cast<mlir::itex_type::TensorProtoAttr>())
