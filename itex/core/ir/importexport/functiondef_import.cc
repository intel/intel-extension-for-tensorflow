...
Status ImportNodes(ValueMapManager value_manager,
                   const RepeatedPtrField<NodeDef>& nodes,
  ...
  for (const NodeDef& node : nodes) {
    ITEX_DVLOG(1) << "Processing node " << node.name() << "\n";
...
Status ImportGenericFunction(
    ...
    llvm::StringMap<llvm::StringMap<SmallVector<Value, 1>>>&
        values_map,        // NOLINT
    OpBuilder& builder) {  // NOLINT
