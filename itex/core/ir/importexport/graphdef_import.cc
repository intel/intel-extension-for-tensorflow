...
class GraphDefImporter {
 public:
  ...
  GraphDefImporter(TFGraphDialect* dialect, const GraphDef& graph_def,
                   const GraphDebugInfo& debug_info)
      : ctx_(dialect->getContext()),
        dialect_(dialect),
        b_(ctx_),
        // registry_(registry),
        function_library_(FunctionLibraryDefinition(graph_def)),
 ...
 private:
  ...
  struct ResultId {
    ...
    bool IsControl() const { return index == -1; }
  };
  ...
  Status ConvertNodes(OpBuilder& builder, ConversionState& s,  // NOLINT
                      const itex::protobuf::RepeatedPtrField<NodeDef>& nodes,
                      Block* block);
  // Convert a node to an operation.
  Status ConvertNodeDef(OpBuilder& builder, ConversionState& s,  // NOLINT
                        const NodeDef& node);
  ...
  StatusOr<Result> GetResult(ConversionState& s, StringPiece name);  // NOLINT

  // Convert TF datatypes to unranked MLIR tensor types.
  Status ConvertDataTypesToUnrankedTensorTypes(
      const DataTypeVector& dtypes,
      SmallVectorImpl<Type>& results);  // NOLINT
  ...
  StatusOr<unsigned> ArgNumType(const NamedAttrList& attrs,
                                ...
                                SmallVectorImpl<Type>& types);  // NOLINT
  // Convert function attributes to MLIR attributes.
  Status ConvertFunctionAttributes(
      const absl::flat_hash_map<StringPiece, StringPiece>& gradient_map,
      const FunctionDef& function, GraphFuncOp op,
      NamedAttrList& attrs);  // NOLINT
...
StatusOr<OwningOpRef<ModuleOp>> GraphDefImporter::ConvertGraphDef(
  ...
  for (const itex::GradientDef& gradient : graph.library().gradient())
...
StatusOr<GraphDefImporter::Result> GraphDefImporter::GetResult(
  ...
  TensorId tensor_id = itex::ParseTensorName(name);
...
Status GraphDefImporter::ConvertNodes(
    ...
    const itex::protobuf::RepeatedPtrField<NodeDef>& nodes, Block* block) {
...
Status GraphDefImporter::ConvertNodeDef(OpBuilder& builder, ConversionState& s,
  ...
  OpDef op_def_lookup;
  Status status = function_library_.LookUpOpDef(node.op(), &op_def_lookup);

  // if ((op_reg_data = registry_.LookUp(node.op()))) {
  //   op_def = &op_reg_data->op_def;
  if (status.ok()) {
    op_def = &op_def_lookup;
  } else {
  ...
  //   TF_RETURN_IF_ERROR(
  //       itex::full_type::SpecializeType(node, *op_def, full_type_def));
  //   ITEX_LOG(INFO) << "222 " << full_type_def.DebugString();
