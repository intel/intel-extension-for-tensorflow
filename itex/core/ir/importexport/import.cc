...
using itex::AttrSlice;
using itex::AttrValue;
using itex::AttrValueMap;
using itex::DataType;
// using itex::Edge;
// using itex::FunctionBody;
using itex::FunctionDef;
using itex::FunctionLibraryDefinition;
// using itex::FunctionLibraryRuntime;
// using itex::Graph;
// using itex::GraphConstructorOptions;
using itex::GraphDebugInfo;
using itex::GraphDef;
using itex::NameAttrList;
// using itex::Node;
using itex::NodeDef;
using itex::OpDef;
using itex::OpDef_ArgDef;
// using itex::OpRegistry;
// using itex::PartialTensorShape;
using itex::ResourceHandleProto_DtypeAndShape;
// using itex::StackFrame;
using itex::Status;
using itex::TensorProto;
using itex::VersionDef;
using itex::errors::AppendToMessage;
using itex::errors::Internal;
using itex::errors::InvalidArgument;
using itex::errors::NotFound;
using itex::protobuf::MapPair;
using itex::protobuf::RepeatedPtrField;
// using itex::shape_inference::InferenceContext;
// using itex::shape_inference::ShapeAndType;
// using itex::shape_inference::ShapeHandle;
...
//   // Converts the prepared graph to a Function and adds it to the module. A
//   set
//   // of nodes from the graph are given to converted to the arguments and
//   returns
//   // of the function.
...
//                           const Node& node);
//   // Try to infer the output types of a node from one of its attributes.
//   Certain
...
//   // Returns `None` if the node has inputs, if it is unregistered, or if it
//   does
...
//   itex::StatusOr<TensorType> ConvertDataTypeAndShape(
...
//   // Converts the inferred shape referred to by 'handle' in 'context', with
//   // given element type, and returns an MLIR tensor type.
//   itex::StatusOr<TensorType> ConvertElementTypeAndShape(
//       Type element_type, const ShapeHandle& handle, InferenceContext*
//       context, Builder builder);

//   // Converts the inferred subtypes for an element type to corresponding MLIR
//   // types in 'context'.
//   itex::StatusOr<ElementSubtypes> ConvertSubtypes(
//       const std::vector<ShapeAndType>* handle_subtypes,
//       InferenceContext* context, Builder builder);
...
//   // "original_node_name" in the NodeDef to get the corresponding file
//   location
...
//     // Check for a generic `_output_shapes` attribute. Only use it if it
//     matches
...
//   // The output shapes attribute is required. It may also be empty. Handle
//   the
...
//                         ConvertToMlirTensorType(it.value(), dtype,
//                         &builder));
...
//     Builder& builder, OperationState& result, const Node& node) {
//   // Below we only try and do some shape inference for "source" ops which
//   have
//   // no inputs.
//   if (node.num_inputs() > 0) return {};

//   const itex::OpRegistrationData* op_reg_data;
//   TF_RETURN_IF_ERROR(
//       graph_->op_registry()->LookUp(node.type_string(), &op_reg_data));
//   if (!op_reg_data) {
//     DVLOG(3) << "Skipping inference for unregistered op " <<
//     node.type_string(); return {};
//   }
...
// Status GraphImporter::InferOutputTypes(Builder& builder, OperationState&
// result,
//                                        const Node& node) {
...
// itex::StatusOr<TensorType> GraphImporter::ConvertDataTypeAndShape(
//     DataType dtype, const ShapeHandle& handle,
//     const std::vector<ShapeAndType>* handle_subtypes, InferenceContext*
//     context, Builder builder) {
//   TF_ASSIGN_OR_RETURN(auto subtypes,
//                       ConvertSubtypes(handle_subtypes, context, builder));

//   Type element_type;
//   if (dtype == itex::DT_VARIANT)
//     element_type = VariantType::get(subtypes, context_);
//   else if (dtype == itex::DT_RESOURCE)
...
// itex::StatusOr<TensorType> GraphImporter::ConvertElementTypeAndShape(
...
//       llvm::makeArrayRef(dimensions.begin(), dimensions.end()),
//       element_type);
// }

// itex::StatusOr<GraphImporter::ElementSubtypes>
// GraphImporter::ConvertSubtypes(const std::vector<ShapeAndType>*
// handle_subtypes,
//                                InferenceContext* context, Builder builder) {
...
//     Type element_type;
//     TF_RETURN_IF_ERROR(ConvertDataType(subtype.dtype, builder,
//     &element_type)); TF_ASSIGN_OR_RETURN(TensorType type,
//                         ConvertElementTypeAndShape(element_type,
//                         subtype.shape,
//                                                    context, builder));
...
//   // Create the graph operation in which we will convert the individual
//   nodes.
...
//     // Use the catenation of function and node names as the lookup key into
//     the
//     // debug info. This matches the way that the key is formed on the python
//     // side.
//     //
//     // We also use this as the name for the NameLoc for ops in function,
//     since
//     // otherwise our names could collide across functions.
//     // For ops in the main graph, we omit the "@function_name" (which, would
//     be
//     // just "@" since function_name would be empty) because some code seems
//     to
...
//         function_name.empty() ? name.str() : (name + "@" +
//         function_name).str();
//     auto name_loc_id = StringAttr::get(context_, name_for_name_loc);

//     llvm::SmallVector<Location, 4> locations;
//     // Prefer stack traces if available, fallback to debug info if not, and
//     then
...
//           auto file_line_loc =
//               FileLineColLoc::get(file_name, location.line(),
//               location.col());
//           locations.push_back(file_line_loc);
//         }
//       }
...
//     // If there are more locations then generate a stack trace, otherwise
//     just
//     // return the name loc.
...
//     // call sites, and then fuse them to a single fused location, with the
//     name
//     // of the node_def.
...
//   auto getResult = [&]() {
//     if (edge.IsControlEdge()) return inst->getResult(inst->getNumResults() -
//     1); return inst->getResult(resultId);
//   };
//   if (inst) return getResult();

//   // We use placeholders during the import to create "fake" operations to
//   break
...
//     ITEX_VLOG(4) << "Ignore " << node.name() << " on import";
//     return Status::OK();
//   }
...
//   // Input edges can be nondeterministically ordered, sort them here. First
//   the
...
//         ConvertAttribute(node.def().experimental_type(), builder_,
//         dialect_));
...
//   // Register the mapping between the TF node and the newly created
//   operation. Operation* operation = builder_.create(result); Operation*&
//   cached_operation = node_values_[node.id()]; if (cached_operation) {
...
// itex::StatusOr<std::string> MangleName(const FunctionDef& fdef,
//                                              AttrSlice attrs) {
...
//     }
//     if (arg_def.type() == itex::DT_INVALID &&
...
// itex::StatusOr<GraphFuncOp> ImportFunctionDef(
...
//     builder.getStringAttr(signature.description()));
...
//       if (!ret_op) return Internal("Missing mapping for return #",
//       ret->id()); if (ret_op->getName().getStringRef() != "tfg._Retval" &&
...
//       Type output_type;
//       if (output.type() != itex::DT_INVALID) {
//         TF_RETURN_IF_ERROR(
...
//             "' not found in the graph for signature control output '",
//             sig_name,
//             "'");
//       // Find node in the graph using the node id instead of using `arg`
//       // directly because the graph has been cloned.
...
itex::StatusOr<OwningOpRef<mlir::ModuleOp>> ImportGraphDefToMlir(
  ...
  ITEX_VLOG(4) << "ConvertGraphdefToMlir begin";
