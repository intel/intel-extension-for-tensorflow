...
class GraphDefExporter {
 public:
  GraphDefExporter(TFGraphDialect* dialect,
                   SymbolTable& table,  // NOLINT
...
static Status ConvertAttributes(
    itex::protobuf::Map<std::string, AttrValue>* map,
...
StatusOr<std::pair<StringRef, unsigned>> GraphDefExporter::GetOutputSegment(
  ...
  OpDef op_def;
  Status status = function_library_.LookUpOpDef(op_name, &op_def);
  if (status.ok()) {
