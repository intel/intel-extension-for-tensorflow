...
// Replace Sum to BiasAddGrad.
struct ReplaceableSum {
  ReplaceableSum() = default;
  explicit ReplaceableSum(int bias_sum) : bias_sum(bias_sum) {}

  int bias_sum = kMissingIndex;
};
...
// Find Sum if the reduction happened on last dim.
//
//        Sum
//       /   \(port)
...
bool FindSum(const RemapperContext& ctx, int node_index,
  ...
  const auto* node_view = ctx.graph_view.GetNode(node_index);
  const auto* bias_sum = node_view->node();

  if (!IsSum(*bias_sum) || HasControlFaninOrFanout(*node_view)) return false;

  // Only find Sum generated by AddGrad with BroadcastGradientArgs.
  ...
  if (!IsBroadcastGradientArgs(*broadcast)) return false;
  ...
  for (int i = 0; i < broadcast_view->GetRegularFanouts().size(); ++i) {
    for (const auto& fanout_i : broadcast_view->GetRegularFanout(i)) {
      if (fanout_i.node_view()->node_index() == node_index) {
        port = i;
        break;
      }
    }
  ...
  if (IsAnyConst(*shape)) {
    ...
    if (input_shape.dim_size() != 1 || IsUnknown(input_shape.dim(0)))
      return false;
    reduction_indices = input_shape.dim(0).size();
  } else {
    return false;
  }
  ...
  std::vector<OpInfo_TensorProperties> props;
  TF_ABORT_IF_ERROR(
      ctx.graph_properties.GetInputProperties(bias_sum->name(), &props));
  ...
  const ReplaceableSum pattern{node_index};
  *matched = pattern;
...
Status AddSum(RemapperContext* ctx, const ReplaceableSum& matched,
              ...
              std::vector<bool>* nodes_to_delete) {
  const GraphDef* graph = ctx->graph_view.graph();
  const NodeDef& bias_sum = graph->node(matched.bias_sum);
  ITEX_VLOG(2) << "Replace " << bias_sum.name() << " to BiasAddGrad";

  NodeDef bias_add_grad;
  bias_add_grad.set_op("BiasAddGrad");
  bias_add_grad.set_name(bias_sum.name());
  bias_add_grad.set_device(bias_sum.device());
  bias_add_grad.add_input(bias_sum.input(0));
  AddNodeAttr("T", bias_sum.attr().at("T"), &bias_add_grad);
  // Hard coded format `NHWC` here since only matched `reduction_indices` = -1.
...
Status RunRemapper(OptimizerContext* opt_ctx, const GrapplerItem& item,
  ...
  for (int i = num_nodes - 1; i >= 0;) {
    ...
    // is true or false.
    {
      ...
      // Replace Sum to BiasAddGrad if the reduction happened on last dim.
      ReplaceableSum bias_sum;
      if (FindSum(ctx, i, &bias_sum)) {
        TF_ABORT_IF_ERROR(
