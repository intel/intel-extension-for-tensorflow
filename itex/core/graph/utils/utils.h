...
#ifndef ITEX_CORE_GRAPH_UTILS_UTILS_H_
#define ITEX_CORE_GRAPH_UTILS_UTILS_H_
...
// ITEX_DUMP_GRAPH_PREFIX environment variable if 'dirname' is empty, and
// suffixes 'name' with ".pbtxt" to form a name. If a graph has already been
...
class NodeMapInternal {
 public:
  ...
  explicit NodeMapInternal(GraphDefT* graph) {
    ...
    for (int i = 0; i < graph->node_size(); i++) {
      ...
      if (!rslt.second) {
        ...
        ITEX_LOG(WARNING) << "Duplicated node in the graph: " << node_name;
      }
  ...
  void AddNode(const string& node_name, NodeDefT* node) {
    ITEX_DCHECK(node != nullptr);
    auto ret = nodes_.emplace(node_name, node);
    ITEX_DCHECK(ret.second)
        << "Pair (" << node_name << "," << node
  ...
  NodeDefT* GetNode(const string& name) const {
    ...
    if (it == nodes_.end()) {
      ITEX_VLOG(1) << "Node could not be found: " << name;
      return nullptr;
    }
    return it->second;
  }
  ...
  void AddOutput(const string& node_name, const string& output_name) {
    ...
    ITEX_DCHECK(output_node)
        << "Output node " << output_name << " is missing in NodeMap.";
...
#endif  // ITEX_CORE_GRAPH_UTILS_UTILS_H_
