...
class LogMessage : public std::basic_ostringstream<char> {
 public:
  ...
  // E.g., if MinVLogLevel() is 2, then ITEX_VLOG(2) statements will produce
  // output, but ITEX_VLOG(3) will not. Defaults to 0.
  ...
  //
  // It is expected that the result of this query will be cached in the
  // ITEX_VLOG-ing call site to avoid repeated lookups. This routine performs a
  // hash-map access against the ITEX_VLOG-ing specification provided by the env
  // var.
...
class TFLogSink {
 public:
  ...
  // Implementations should be careful not to call`ITEX_LOG` or `ITEX_CHECK` or
  // take any locks that might be held by the `ITEX_LOG` caller, to avoid
  // deadlock.
  //
...
#endif  // ITEX_CORE_UTILS_LOGGING_H_
