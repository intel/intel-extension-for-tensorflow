...
#ifndef ITEX_CORE_UTILS_TENSOR_FORMAT_H_
#define ITEX_CORE_UTILS_TENSOR_FORMAT_H_
...
inline int GetTensorSpatialDims(int num_dims, TensorFormat format) {
  switch (format) {
    ...
    default:
      ITEX_LOG(FATAL) << "Unknown format " << format;
...
inline int GetFilterTensorSpatialDimIndex(int num_dims,
  ...
  ITEX_CHECK(dim >= 0 && dim < GetFilterTensorSpatialDims(num_dims, format))
...
inline int GetFilterTensorInnerInputChannelsDimIndex(
  ...
  ITEX_DCHECK_EQ(format, FORMAT_OIHW_VECT_I);
  return num_dims - 1;
}
...
inline int32 GetTensorDimIndex(TensorFormat format, char dimension) {
  if (format == FORMAT_NHWC || format == FORMAT_NHWC_VECT_W) {
    ...
    switch (dimension) {
      ...
      default:
        ITEX_LOG(FATAL) << "Invalid dimension: " << dimension;
        return -1;  // Avoid compiler warning about missing return value
    }
    ...
    ITEX_LOG(FATAL) << "Invalid format: " << static_cast<int>(format);
    return -1;  // Avoid compiler warning about missing return value
  }
  // clang-format on
}
...
inline int GetFilterDimIndex(FilterTensorFormat filter_tensor_format,
  ...
  if (filter_tensor_format == FORMAT_HWIO) {
    ...
    ITEX_LOG(FATAL) << "Invalid format: "
                    << static_cast<int>(filter_tensor_format);
    return -1;  // Avoid compiler warning about missing return value
  }
  // clang-format on
}
...
inline int32 GetTensorDimIndex(TensorFormat format, char dimension,
  ...
  ITEX_CHECK(index >= 0 && index < num_total_dims)  // Crash OK.
...
inline TensorShape ShapeFromFormat(TensorFormat format, int64 N,
  ...
  if (format == FORMAT_NCHW_VECT_C) {
    ITEX_CHECK_EQ(0, C % 4)
        << "NCHW_VECT_C requires C to be a multiple of 4, but C=" << C;
    C /= 4;
    dim_sizes[GetTensorInnerFeatureDimIndex(dims, format)] = 4;
  }
  ITEX_CHECK(feature_index >= 0 && feature_index < 6);
...
inline TensorShape ShapeFromFilterTensorFormat(FilterTensorFormat format,
  ...
  if (format == FORMAT_OIHW_VECT_I) {
    ITEX_CHECK_EQ(0, I % 4)
        << "OIHW_VECT_I requires I to be a multiple of 4, but I=" << I;
    I /= 4;
...
#endif  // ITEX_CORE_UTILS_TENSOR_FORMAT_H_
