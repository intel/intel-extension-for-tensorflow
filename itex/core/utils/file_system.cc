...
Status FileSystem::IsDirectory(const string& name, TransactionToken* token) {
  ...
  // TODO(ITEX): Forward token to other methods once migration is complete.
  ...
  return Status(TF_FAILED_PRECONDITION, "Not a directory");
}
...
Status FileSystem::DeleteRecursively(const string& dirname,
  ...
  ITEX_CHECK_NOTNULL(undeleted_files);
  ITEX_CHECK_NOTNULL(undeleted_dirs);
...
Status FileSystem::RecursivelyCreateDir(const string& dirname,
  ...
  while (!remaining_dir.empty()) {
    ...
    if (exists_status.ok()) {
      ...
      } else if (directory_status.code() == TF_UNIMPLEMENTED) {
        return directory_status;
      } else {
        return errors::FailedPrecondition(remaining_dir, " is not a directory");
      }
    }
    if (exists_status.code() != TF_NOT_FOUND) {
      return exists_status;
    }
  ...
  for (const StringPiece sub_dir : sub_dirs) {
    ...
    if (!status.ok() && status.code() != TF_ALREADY_EXISTS) {
      return status;
    }
  }
  return Status::OK();
