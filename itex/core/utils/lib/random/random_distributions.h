...
#ifndef ITEX_CORE_UTILS_LIB_RANDOM_RANDOM_DISTRIBUTIONS_H_
#define ITEX_CORE_UTILS_LIB_RANDOM_RANDOM_DISTRIBUTIONS_H_
...
// [0..1).
PHILOX_DEVICE_INLINE Eigen::bfloat16 Uint16ToBfloat16(uint16 x);
// Helper function to convert a 16-bit integer to a bfloat16 between [1..2).
PHILOX_DEVICE_INLINE Eigen::bfloat16 InternalUint16ToBfloat16(uint16 x);
...
// TODO(itex): This is for sycl-std2020, to update the member in const
// member function
template <typename T>
static void update_const_member_by_val(const T* ptr, const T val) {
  T* dst_ptr = const_cast<T*>(ptr);
  *dst_ptr = val;
}
 ...
 public:
  ...
  ResultType operator()() const {
    if (used_result_index_ == Generator::kResultElementCount) {
      ...
      update_const_member_by_val(&unused_results_, (*generator_)());
      // used_result_index_ = 0;
      update_const_member_by_val(&used_result_index_, 0);
    }

    // return unused_results_[used_result_index_++];
    auto ret = unused_results_[used_result_index_];
    update_const_member_by_val(&used_result_index_, used_result_index_ + 1);
    return ret;
  }
...
class TruncatedNormalDistribution<SingleSampleGenerator, Eigen::half> {
 public:
  ...
  PHILOX_DEVICE_INLINE
  ResultType operator()(const SingleSampleGenerator* gen) const {
    ResultType results;
    int index = 0;
    while (true) {
...
#else
  const float v1 = 2.0f * M_PIf32 * Uint32ToFloat(x1);
#endif
...
// [0..1). This can create a uniform distribution of values between [1..2).
PHILOX_DEVICE_INLINE Eigen::bfloat16 InternalUint16ToBfloat16(uint16 x) {
...
#else
  std::copy_n(reinterpret_cast<const char*>(&val), sizeof(val),
              reinterpret_cast<char*>(&result));
#endif
  // The mantissa has an implicit leading 1, so the above code creates a value
  // in [1, 2). Instead it will just be close to 1.
  return result;
}
...
PHILOX_DEVICE_INLINE Eigen::bfloat16 Uint16ToBfloat16(uint16 x) {
  ...
  return InternalUint16ToBfloat16(x) - Eigen::bfloat16(1.0);
}
...
PHILOX_DEVICE_INLINE float InternalUint32ToFloat(uint32 x) {
  ...
  float result;
#if defined(__SYCL_DEVICE_ONLY__)
  result = *(const_cast<float*>(reinterpret_cast<const float*>(&val)));
#else
  std::copy_n(reinterpret_cast<const char*>(&val), sizeof(val),
              reinterpret_cast<char*>(&result));
#endif
...
PHILOX_DEVICE_INLINE double Uint64ToDouble(uint32 x0, uint32 x1) {
  ...
  double result;
#if defined(__SYCL_DEVICE_ONLY__)
  result = *(const_cast<double*>(reinterpret_cast<const double*>(&val)));
#else
  std::copy_n(reinterpret_cast<const char*>(&val), sizeof(val),
              reinterpret_cast<char*>(&result));
#endif
...
#endif  // ITEX_CORE_UTILS_LIB_RANDOM_RANDOM_DISTRIBUTIONS_H_
