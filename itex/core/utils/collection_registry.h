...
#ifndef ITEX_CORE_UTILS_COLLECTION_REGISTRY_H_
#define ITEX_CORE_UTILS_COLLECTION_REGISTRY_H_
...
#include "itex/core/utils/collected_metrics.h"
#include "itex/core/utils/env.h"
#include "itex/core/utils/logging.h"
#include "itex/core/utils/macros.h"
#include "itex/core/utils/metric_def.h"
#include "itex/core/utils/monitoring_types.h"
...
//   // This is inefficient. If and when we hit snags, we can change the API to
//   do
...
MetricCollector<metric_kind, Value, NumLabels> MetricCollectorGetter::Get(
  ...
  if (allowed_metric_def_ != metric_def) {
    ITEX_LOG(FATAL) << "Expected collection for: "
                    << allowed_metric_def_->name()
...
#define REGISTER_TF_METRICS_EXPORTER_UNIQ(ctr, exporter)                 \
  static ::itex::monitoring::exporter_registration::ExporterRegistration \
      exporter_registration_##ctr(new exporter())

}  // namespace monitoring
}  // namespace itex

#endif  // ITEX_CORE_UTILS_COLLECTION_REGISTRY_H_
