...
#include "itex/core/utils/host_info.h"
...
#undef DeleteFile
#undef CopyFile
#include "itex/core/utils/windows/wide_char.h"
...
string Env::GetExecutablePath() {
  ...
  ITEX_CHECK(realpath(unresolved_path.data(), exe_path));
...
Status FileSystemCopyFile(FileSystem* src_fs, const string& src,
  ...
  while (s.ok()) {
    ...
    if (!(s.ok() || s.code() == TF_OUT_OF_RANGE)) {
      return s;
    }
