...
#ifndef ITEX_CORE_UTILS_BUFFERED_INPUTSTREAM_H_
#define ITEX_CORE_UTILS_BUFFERED_INPUTSTREAM_H_
...
class BufferedInputStream : public InputStreamInterface {
 public:
  ...
  itex::Status ReadNBytes(int64_t bytes_to_read, tstring* result) override;

  itex::Status SkipNBytes(int64_t bytes_to_skip) override;
  ...
  itex::Status Seek(int64_t position);
  ...
  itex::Status ReadLine(std::string* result);
  itex::Status ReadLine(tstring* result);
  ...
  template <typename T>
  itex::Status ReadAll(T* result);

  itex::Status Reset() override;

 private:
  itex::Status FillBuffer();
  template <typename StringType>
  itex::Status ReadLineHelper(StringType* result, bool include_eol);
  ...
  // buffer allocations.
  itex::Status file_status_ = Status::OK();
...
#ifndef SWIG
extern template itex::Status BufferedInputStream::ReadAll<std::string>(
    std::string* result);
extern template itex::Status BufferedInputStream::ReadAll<tstring>(
    tstring* result);
#endif  // SWIG
...
#endif  // ITEX_CORE_UTILS_BUFFERED_INPUTSTREAM_H_
