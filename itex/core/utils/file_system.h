...
#ifndef ITEX_CORE_UTILS_FILE_SYSTEM_H_
#define ITEX_CORE_UTILS_FILE_SYSTEM_H_
...
#include "itex/core/utils/file_statistics.h"
...
class FileSystem {
 public:
  ...
  virtual itex::Status NewRandomAccessFile(
  ...
  /// and the object should be deleted when is not used.
  virtual itex::Status NewWritableFile(const std::string& fname,
  ...
  /// The ownership of the returned WritableFile is passed to the caller
  /// and the object should be deleted when is not used.
  virtual itex::Status NewAppendableFile(
  ...
  /// and the object should be deleted when is not used.
  virtual itex::Status NewReadOnlyMemoryRegionFromFile(
  ...
  virtual itex::Status FileExists(const std::string& fname) {
  ...
  virtual itex::Status GetChildren(const std::string& dir,
  ...
  ///                    implemented
  virtual itex::Status GetMatchingPaths(const std::string& pattern,
  ...
  virtual itex::Status Stat(const std::string& fname, FileStatistics* stat) {
    return Stat(fname, nullptr, stat);
  }

  virtual itex::Status Stat(const std::string& fname, TransactionToken* token,
                            FileStatistics* stat) {
    return Status::OK();
  }

  /// \brief Deletes the named file.
  virtual itex::Status DeleteFile(const std::string& fname) {
  ...
  virtual itex::Status CreateDir(const std::string& dirname) {
  ...
  virtual itex::Status RecursivelyCreateDir(const std::string& dirname) {
  ...
  virtual itex::Status DeleteDir(const std::string& dirname) {
  ...
  ///                    implemented
  virtual itex::Status DeleteRecursively(const std::string& dirname,
  ...
  virtual itex::Status GetFileSize(const std::string& fname,
                                   uint64* file_size) {
  ...
  virtual itex::Status RenameFile(const std::string& src,
  ...
  virtual itex::Status CopyFile(const std::string& src,
                                const std::string& target) {
  ...
  virtual itex::Status IsDirectory(const std::string& fname) {
  ...
  bool IsAbsolutePath(itex::StringPiece path) const;
  ...
  std::string JoinPathImpl(std::initializer_list<itex::StringPiece> paths);
  ...
  virtual itex::Status StartTransaction(TransactionToken** token) {
    *token = nullptr;
    return Status::OK();
  }

  /// \brief Adds `path` to transaction in `token`
  virtual itex::Status AddToTransaction(const std::string& path,
                                        TransactionToken* token) {
    return Status::OK();
  }

  /// \brief Ends transaction
  virtual itex::Status EndTransaction(TransactionToken* token) {
    return Status::OK();
  }

  /// \brief Get token for `path` or start a new transaction and add `path` to
  /// it.
  virtual itex::Status GetTokenOrStartTransaction(const std::string& path,
                                                  TransactionToken** token) {
    *token = nullptr;
    return Status::OK();
  }
  ...
  virtual itex::Status GetTransactionForPath(const std::string& path,
                                             TransactionToken** token) {
    *token = nullptr;
    return Status::OK();
  }
  ...
  virtual itex::Status SetOption(const std::string& name,
...
class WrappedFileSystem : public FileSystem {
 public:
  ...
  itex::Status FileExists(const std::string& fname,
  ...
  itex::Status GetChildren(const std::string& dir, TransactionToken* token,
                           std::vector<string>* result) override {
    return fs_->GetChildren(dir, (token ? token : token_), result);
  }

  itex::Status GetMatchingPaths(const std::string& pattern,
                                TransactionToken* token,
  ...
  itex::Status Stat(const std::string& fname, TransactionToken* token,
                    FileStatistics* stat) override {
    return fs_->Stat(fname, (token ? token : token_), stat);
  }

  itex::Status DeleteFile(const std::string& fname,
                          TransactionToken* token) override {
    return fs_->DeleteFile(fname, (token ? token : token_));
  }

  itex::Status CreateDir(const std::string& dirname,
                         TransactionToken* token) override {
    return fs_->CreateDir(dirname, (token ? token : token_));
  }

  itex::Status RecursivelyCreateDir(const std::string& dirname,
                                    TransactionToken* token) override {
    return fs_->RecursivelyCreateDir(dirname, (token ? token : token_));
  }

  itex::Status DeleteDir(const std::string& dirname,
                         TransactionToken* token) override {
    return fs_->DeleteDir(dirname, (token ? token : token_));
  }

  itex::Status DeleteRecursively(const std::string& dirname,
                                 TransactionToken* token,
                                 int64_t* undeleted_files,
  ...
  itex::Status GetFileSize(const std::string& fname, TransactionToken* token,
                           uint64* file_size) override {
    return fs_->GetFileSize(fname, (token ? token : token_), file_size);
  }

  itex::Status RenameFile(const std::string& src, const std::string& target,
                          TransactionToken* token) override {
    return fs_->RenameFile(src, target, (token ? token : token_));
  }

  itex::Status CopyFile(const std::string& src, const std::string& target,
                        TransactionToken* token) override {
  ...
  itex::Status IsDirectory(const std::string& fname,
                           TransactionToken* token) override {
  ...
  itex::Status StartTransaction(TransactionToken** token) override {
    return fs_->StartTransaction(token);
  }

  itex::Status AddToTransaction(const std::string& path,
                                TransactionToken* token) override {
    return fs_->AddToTransaction(path, (token ? token : token_));
  }

  itex::Status EndTransaction(TransactionToken* token) override {
    return fs_->EndTransaction(token);
  }

  itex::Status GetTransactionForPath(const std::string& path,
                                     TransactionToken** token) override {
    return fs_->GetTransactionForPath(path, token);
  }

  itex::Status GetTokenOrStartTransaction(const std::string& path,
                                          TransactionToken** token) override {
...
class RandomAccessFile {
 public:
  ...
  /// filesystem.
  virtual itex::Status Name(StringPiece* result) const {
  ...
  virtual itex::Status Read(uint64 offset, size_t n, StringPiece* result,
                            char* scratch) const = 0;

#if defined(TF_CORD_SUPPORT)
  /// \brief Read up to `n` bytes from the file starting at `offset`.
  virtual itex::Status Read(uint64 offset, size_t n, absl::Cord* cord) const {
...
class WritableFile {
 public:
  ...
  virtual itex::Status Append(StringPiece data) = 0;

#if defined(TF_CORD_SUPPORT)
  // \brief Append 'data' to the file.
  virtual itex::Status Append(const absl::Cord& cord) {
  ...
  virtual itex::Status Tell(int64_t* position) {
    *position = -1;
...
class FileSystemRegistry {
 public:
  ...
  virtual itex::Status Register(const std::string& scheme, Factory factory) = 0;
  virtual itex::Status Register(const std::string& scheme,
                                std::unique_ptr<FileSystem> filesystem) = 0;
  virtual FileSystem* Lookup(const std::string& scheme) = 0;
  virtual itex::Status GetRegisteredFileSystemSchemes(
      std::vector<std::string>* schemes) = 0;
};

}  // namespace itex

#endif  // ITEX_CORE_UTILS_FILE_SYSTEM_H_
