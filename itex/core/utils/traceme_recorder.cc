...
class EventQueue {
 public:
  ...
  ~EventQueue() {
    ...
    ITEX_DCHECK(Empty());
    delete end_block_;
    if (start_block_ && (end_block_ != start_block_)) {
      delete start_block_;
    }
  }
  ...
  void Push(TraceMeRecorder::Event&& event) {
    ...
    if (ITEX_PREDICT_FALSE(end - end_block_->start == Block::kNumSlots)) {
 ...
 private:
  ...
  TraceMeRecorder::Event Pop() {
    ...
    if (ITEX_PREDICT_FALSE(start_ - start_block_->start == Block::kNumSlots)) {
      auto* next_block = start_block_->next;
      delete start_block_;
      start_block_ = next_block;
      ITEX_DCHECK_EQ(start_, start_block_->start);
    }
    return out;
  }
