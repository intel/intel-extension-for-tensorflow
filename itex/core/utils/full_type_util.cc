...
OpTypeConstructor NoOp() { return nullptr; }
...
inline Status SubstituteFromAttrs(AttrMap& attrs, FullTypeDef& t);  // NOLINT

Status SubstituteVar(AttrMap& attrs, FullTypeDef& t) {  // NOLINT
  ITEX_DCHECK_EQ(t.args_size(), 0);
  ...
  if (attr_type == AttrValue::kType) {
    ...
    if (attr_list.type_size() != 1) {
      return Status(TF_UNIMPLEMENTED,
...
Status SubstituteForEach(AttrMap& attrs, FullTypeDef& t) {  // NOLINT
  ITEX_DCHECK_EQ(t.args_size(), 3);
...
Status SubstituteGeneric(AttrMap& attrs, FullTypeDef& t) {  // NOLINT
...
Status SpecializeType(const AttrSlice& attrs, const OpDef& op_def,
  ...
  for (int i = 0; i < nargs; i++) {
    ...
    //     t.DebugString(), "\nfrom\n", attrs.SummarizeNode());
    TF_RETURN_WITH_CONTEXT_IF_ERROR(SubstituteFromAttrs(map, t),
                                    "while substituting");
  }
