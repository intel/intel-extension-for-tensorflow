...
#ifndef ITEX_CORE_UTILS_TRACEME_H_
#define ITEX_CORE_UTILS_TRACEME_H_
...
#include "itex/core/utils/traceme_encode.h"  // IWYU pragma: export
#include "itex/core/utils/types.h"

#if !defined(IS_MOBILE_PLATFORM)
#include "itex/core/utils/time_utils.h"
#include "itex/core/utils/traceme_recorder.h"
#endif
...
class TraceMe {
 public:
...
#if !defined(IS_MOBILE_PLATFORM)
    if (ITEX_PREDICT_FALSE(TraceMeRecorder::Active(level))) {
      new (&no_init_.name) std::string(name);
      start_time_ = profiler::GetCurrentTimeNanos();
    }
#endif
  }
...
#if !defined(IS_MOBILE_PLATFORM)
    if (ITEX_PREDICT_FALSE(other.start_time_ != kUntracedActivity)) {
...
#if !defined(IS_MOBILE_PLATFORM)
    if (ITEX_PREDICT_FALSE(start_time_ != kUntracedActivity)) {
      if (ITEX_PREDICT_TRUE(TraceMeRecorder::Active())) {
        TraceMeRecorder::Record({std::move(no_init_.name), start_time_,
  ...
  static int64_t ActivityStart(NameGeneratorT&& name_generator, int level = 1) {
    ...
    if (ITEX_PREDICT_FALSE(TraceMeRecorder::Active(level))) {
      ...
      TraceMeRecorder::Record({std::forward<NameGeneratorT>(name_generator)(),
                               profiler::GetCurrentTimeNanos(), -activity_id});
      return activity_id;
    }
#endif
  ...
  static int64_t ActivityStart(absl::string_view name, int level = 1) {
    ...
    if (ITEX_PREDICT_FALSE(TraceMeRecorder::Active(level))) {
      ...
      TraceMeRecorder::Record(
          {std::string(name), profiler::GetCurrentTimeNanos(), -activity_id});
      return activity_id;
    }
#endif
    return kUntracedActivity;
  ...
  static void ActivityEnd(int64_t activity_id) {
    ...
    if (ITEX_PREDICT_FALSE(activity_id != kUntracedActivity)) {
      if (ITEX_PREDICT_TRUE(TraceMeRecorder::Active())) {
        TraceMeRecorder::Record(
            {std::string(), -activity_id, profiler::GetCurrentTimeNanos()});
      }
    }
#endif
  }
...
#if !defined(IS_MOBILE_PLATFORM)
    if (ITEX_PREDICT_FALSE(TraceMeRecorder::Active(level))) {
      int64_t now = profiler::GetCurrentTimeNanos();
      TraceMeRecorder::Record({std::forward<NameGeneratorT>(name_generator)(),
...
#endif  // ITEX_CORE_UTILS_TRACEME_H_
