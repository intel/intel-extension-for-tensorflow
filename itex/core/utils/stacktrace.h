...
#ifndef ITEX_CORE_UTILS_STACKTRACE_H_
#define ITEX_CORE_UTILS_STACKTRACE_H_
...
inline std::string CurrentStackTrace() {
  ...
  for (int i = 0; i < buffer_size; ++i) {
    ...
    std::string demangled = itex::port::MaybeAbiDemangle(symbol);
    if (demangled.length()) {
...
#endif  // ITEX_CORE_UTILS_STACKTRACE_H_
