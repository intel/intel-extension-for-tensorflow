...
#ifndef ITEX_CORE_UTILS_BITS_H_
#define ITEX_CORE_UTILS_BITS_H_
...
inline uint32 NextPowerOfTwo(uint32 value) {
  ...
  ITEX_CHECK_LT(exponent, std::numeric_limits<uint32>::digits);
  return 1 << exponent;
}

inline uint64 NextPowerOfTwo64(uint64 value) {
  int exponent = Log2Ceiling(value);
  ITEX_CHECK_LT(exponent, std::numeric_limits<uint64>::digits);
  return 1LL << exponent;
}

}  // namespace itex

#endif  // ITEX_CORE_UTILS_BITS_H_
