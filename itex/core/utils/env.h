...
#ifndef ITEX_CORE_UTILS_ENV_H_
#define ITEX_CORE_UTILS_ENV_H_
...
#include "itex/core/utils/numa.h"
  ...
  Register(Env* env, const std::string& scheme, bool try_modular_filesystems) {
    ...
    if (try_modular_filesystems) {
      ...
      if (load_plugin == "true" || load_plugin == "1") {
        ...
        ITEX_LOG(WARNING)
            << "Using modular file system for '" << scheme << "'."
    ...
    // TODO(b/32704451): Don't just ignore the ::itex::Status object!
...
#define REGISTER_FILE_SYSTEM_UNIQ(ctr, env, scheme, factory, modular)        \
  static ::itex::register_file_system::Register<factory> register_ff##ctr    \
      TF_ATTRIBUTE_UNUSED = ::itex::register_file_system::Register<factory>( \
          env, scheme, modular)

#define REGISTER_FILE_SYSTEM(scheme, factory) \
  REGISTER_FILE_SYSTEM_ENV(::itex::Env::Default(), scheme, factory, false);

#define REGISTER_LEGACY_FILE_SYSTEM(scheme, factory) \
  REGISTER_FILE_SYSTEM_ENV(::itex::Env::Default(), scheme, factory, true);

#endif  // ITEX_CORE_UTILS_ENV_H_
