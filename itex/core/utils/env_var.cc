...
Status ReadBoolFromEnvVar(StringPiece env_var_name, bool default_val,
                          bool* value) {
  *value = default_val;
  const char* tf_env_val = std::getenv(std::string(env_var_name).c_str());
  if (tf_env_val == nullptr) {
    return Status::OK();
  }
  std::string str_value = absl::AsciiStrToLower(tf_env_val);
  ...
  return errors::InvalidArgument(strings::StrCat(
      ...
      "} into bool: ", tf_env_val, ". Use the default value: ", default_val));
}
