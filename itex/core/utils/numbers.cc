...
size_t DoubleToBuffer(double value, char* buffer) {
  ...
  if (std::abs(value) <= kDoublePrecisionCheckMax) {
    ...
    ITEX_CHECK(snprintf_result > 0 && snprintf_result < kFastToBufferSize);
...
string FpToString(Fprint fp) {
  char buf[17];
  ITEX_CHECK(snprintf(buf, sizeof(buf), "%016lx", static_cast<int64>(fp)))
...
StringPiece Uint64ToHexString(uint64 v, char* buf, int buf_size) {
  ...
  ITEX_CHECK(buf_size >= kFastToBufferSize);
  buf[num_byte] = '\0';
...
string HumanReadableNum(int64 value) {
  ...
  if (value < 1000) {
    Appendf(&s, "%ld", static_cast<int64>(value));
    ...
    while (value >= static_cast<int64>(1000000)) {
      ...
      ++unit;
      ITEX_CHECK(unit < units + TF_ARRAYSIZE(units));
    }
...
string HumanReadableNumBytes(int64 num_bytes) {
  ...
  if (num_bytes < 1024) {
    ...
    ITEX_CHECK(snprintf(buf, sizeof(buf), "%s%ldB", neg_str,
  ...
  // We use SI prefixes.
  char buf[16];
  ITEX_CHECK(snprintf(buf, sizeof(buf),
                      ((*unit == 'K') ? "%s%.1f%ciB" : "%s%.2f%ciB"), neg_str,
