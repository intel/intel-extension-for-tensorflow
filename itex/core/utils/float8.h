...
#ifndef ITEX_CORE_UTILS_FLOAT8_H_
#define ITEX_CORE_UTILS_FLOAT8_H_
...
class float8_base {
 ...
 public:
  ...
  // We set true as default value of kSaturate, but tf set false.
  template <bool kSaturate = true, bool kTruncate = false, typename From>
  static inline EIGEN_DEVICE_FUNC Derived ConvertFrom(const From& from);

  template <typename To, bool kSaturate = true, bool kTruncate = false>
...
template <>
struct numeric_limits<itex::float8_internal::float8_e4m3fn>
    : public itex::float8_internal::numeric_limits_float8<
          itex::float8_internal::float8_e4m3fn> {};

template <>
struct numeric_limits<itex::float8_internal::float8_e4m3b11>
    : public itex::float8_internal::numeric_limits_float8<
          itex::float8_internal::float8_e4m3b11> {};

template <>
struct numeric_limits<itex::float8_internal::float8_e5m2>
    : public itex::float8_internal::numeric_limits_float8<
          itex::float8_internal::float8_e5m2> {};
}  // namespace std
...
struct Traits : public TraitsBase<Float> {
  ...
  static EIGEN_DEVICE_FUNC Float ConstructFromSignAndBits(
    ...
    return bit_cast<Float>(static_cast<typename Base::BitsType>(bits | sign));
  }
};
...
struct Traits<float8_e4m3b11> : public TraitsBase<float8_e4m3b11> {
  ...
  static EIGEN_DEVICE_FUNC float8_e4m3b11 ConstructFromSignAndBits(
    ...
    return bit_cast<float8_e4m3b11>(
...
// tf exists template partial specialization for these template argument:
// From            To            Saturate            Truncate
// float8_e4m3fn   float8_e4m2   true                true/false
// half            float8_e5m2   true/false          false
// half            float8_e5m2   true                true/false
// float8_e5m2     half          true/false          true/false
// We remove them at first, and will restore if needed.
...
struct ConvertImpl<From, To, kSaturate, kTruncate,
  ...
  static EIGEN_DEVICE_FUNC inline To run(const From& from) {
    ...
    FromBits from_bits = bit_cast<FromBits>(from);
    const FromBits from_sign =
        from_bits ^ bit_cast<FromBits>(Eigen::numext::abs(from));
    ToBits sign;
    ...
    if (Eigen::numext::isinf(from)) {
      return sign != 0 ? -std::numeric_limits<To>::infinity()
                       : std::numeric_limits<To>::infinity();
    }
    if (Eigen::numext::isnan(from)) {
      return sign != 0 ? -std::numeric_limits<To>::quiet_NaN()
    ...
    if constexpr (kExponentOffset > 0) {
      ...
      // source type.
      constexpr ToBits kToHighest = std::numeric_limits<To>::max().rep();
      constexpr FromBits kHighest = ToFromBits<From, To>(kToHighest);

      if (rounded_from_bits > kHighest) {
        ToBits bits =
            kSaturate ? kToHighest : std::numeric_limits<To>::infinity().rep();
        return ToTraits::ConstructFromSignAndBits(/*sign=*/sign, /*bits=*/bits);
      }
      ...
      if (rounded_from_bits < kLowestNormal) {
        ...
        rounded_from_bits = ((from_bits & FromTraits::kMantissaMask) |
                             (FromBits{1} << kFromMantissaBits));  // Hidden bit
        ToBits bits = 0;
...
// We should implement our own bit cast func though tf use eigen bit cast.
// Eigen bit cast has bug for sycl kernel.
template <typename Tgt, typename Src>
EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Tgt bit_cast(const Src& src) {
  return *reinterpret_cast<Tgt*>(&(const_cast<Src&>(src)));
}

template <>
EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC itex::float8_e4m3fn
bit_cast<itex::float8_e4m3fn, uint8_t>(const uint8_t& src) {
  return itex::float8_e4m3fn::FromRep(src);
}

template <>
EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC uint8_t
bit_cast<uint8_t, itex::float8_e4m3fn>(const itex::float8_e4m3fn& src) {
  return src.rep();
}

template <>
EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC itex::float8_e5m2
bit_cast<itex::float8_e5m2, uint8_t>(const uint8_t& src) {
  return itex::float8_e5m2::FromRep(src);
}

template <>
EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC uint8_t
bit_cast<uint8_t, itex::float8_e5m2>(const itex::float8_e5m2& src) {
  return src.rep();
}
...
// Eigen specific-override
namespace Eigen {

// Work-around for isinf/isnan/isfinite issue on aarch64.
namespace internal {
template <>
EIGEN_DEVICE_FUNC inline bool isinf_impl<itex::float8_e4m3fn>(
    const itex::float8_e4m3fn& x) {
  return itex::float8_internal::isinf(x);
}

template <>
EIGEN_DEVICE_FUNC inline bool isinf_impl<itex::float8_e4m3b11>(
    const itex::float8_e4m3b11& x) {
  return itex::float8_internal::isinf(x);
}

template <>
EIGEN_DEVICE_FUNC inline bool isinf_impl<itex::float8_e5m2>(
    const itex::float8_e5m2& x) {
  return itex::float8_internal::isinf(x);
}

template <>
EIGEN_DEVICE_FUNC inline bool isnan_impl<itex::float8_e4m3fn>(
    const itex::float8_e4m3fn& x) {
  return itex::float8_internal::isnan(x);
}

template <>
EIGEN_DEVICE_FUNC inline bool isnan_impl<itex::float8_e4m3b11>(
    const itex::float8_e4m3b11& x) {
  return itex::float8_internal::isnan(x);
}

template <>
EIGEN_DEVICE_FUNC inline bool isnan_impl<itex::float8_e5m2>(
    const itex::float8_e5m2& x) {
  return itex::float8_internal::isnan(x);
}

template <>
EIGEN_DEVICE_FUNC inline bool isfinite_impl<itex::float8_e4m3fn>(
    const itex::float8_e4m3fn& x) {
  return itex::float8_internal::isfinite(x);
}

template <>
EIGEN_DEVICE_FUNC inline bool isfinite_impl<itex::float8_e4m3b11>(
    const itex::float8_e4m3b11& x) {
  return itex::float8_internal::isfinite(x);
}

template <>
EIGEN_DEVICE_FUNC inline bool isfinite_impl<itex::float8_e5m2>(
    const itex::float8_e5m2& x) {
  return itex::float8_internal::isfinite(x);
}
...
#endif  // ITEX_CORE_UTILS_FLOAT8_H_
