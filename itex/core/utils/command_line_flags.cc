...
#include "itex/core/utils/command_line_flags.h"
...
bool ParseStringFlag(itex::StringPiece arg, itex::StringPiece flag,
...
bool ParseInt32Flag(itex::StringPiece arg, itex::StringPiece flag,
  ...
  *value_parsing_ok = true;
  if (absl::ConsumePrefix(&arg, "--") && absl::ConsumePrefix(&arg, flag) &&
    ...
    if (sscanf(arg.data(), "%d%c", &parsed_int32, &extra) != 1) {
      ITEX_LOG(ERROR) << "Couldn't interpret value " << arg << " for flag "
                      << flag << ".";
...
bool ParseInt64Flag(itex::StringPiece arg, itex::StringPiece flag,
...
bool ParseBoolFlag(itex::StringPiece arg, itex::StringPiece flag,
...
bool ParseFloatFlag(itex::StringPiece arg, itex::StringPiece flag,
...
Flag::Flag(const char* name, itex::int32* dst, const std::string& usage_text,
           bool* dst_updated)
    : name_(name),
      type_(TYPE_INT32),
  ...
  for (const Flag& flag : flag_list) {
    ...
    if (flag.type_ == Flag::TYPE_INT32) {
      ...
      flag_string = strings::Printf(
          ...
          static_cast<long long>(flag.int64_default_for_display_));  // NOLINT
