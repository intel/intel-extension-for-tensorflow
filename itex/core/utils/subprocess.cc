...
#include "itex/core/utils/subprocess.h"
  ...
  for (int i = 0; i < kNFds; i++) {
    if (parent_pipe_[i] >= 0) {
      if (close(parent_pipe_[i]) < 0) {
        ITEX_LOG(ERROR) << "close() failed: " << strerror(errno);
      }
      parent_pipe_[i] = -1;
    }
    if (child_pipe_[i] >= 0) {
...
void SubProcess::SetProgram(const std::string& file,
  ...
  mutex_lock dataLock(&data_mu_);
  if (running_) {
    ITEX_LOG(FATAL) << "SetProgram called after the process was started.";
    return;
  }
  ...
  if (exec_path_ == nullptr) {
    ITEX_LOG(FATAL) << "SetProgram failed to allocate file string.";
    return;
  }
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if (exec_argv_[i] == nullptr) {
      ITEX_LOG(FATAL) << "SetProgram failed to allocate command argument.";
      return;
    }
  }
...
void SubProcess::SetChannelAction(Channel chan, ChannelAction action) {
  ...
  mutex_lock dataLock(&data_mu_);
  if (running_) {
    ITEX_LOG(FATAL) << "SetChannelAction called after the process was started.";
  } else if (!chan_valid(chan)) {
    ITEX_LOG(FATAL) << "SetChannelAction called with invalid channel: " << chan;
  } else if ((action != ACTION_CLOSE) && (action != ACTION_PIPE) &&
             (action != ACTION_DUPPARENT)) {
    ITEX_LOG(FATAL) << "SetChannelAction called with invalid action: "
                    << action;
  } else {
    action_[chan] = action;
  }
...
bool SubProcess::Start() {
  ...
  mutex_lock dataLock(&data_mu_);
  if (running_) {
    ITEX_LOG(ERROR) << "Start called after the process was started.";
    return false;
  }
  if ((exec_path_ == nullptr) || (exec_argv_ == nullptr)) {
    ITEX_LOG(ERROR) << "Start called without setting a program.";
    return false;
  }
  ...
  for (int i = 0; i < kNFds; i++) {
    if (action_[i] == ACTION_PIPE) {
      int pipe_fds[2];
      if (pipe(pipe_fds) < 0) {
        ITEX_LOG(ERROR) << "Start cannot create pipe: " << strerror(errno);
        ClosePipes();
        return false;
      }
      ...
      if (fcntl(parent_pipe_[i], F_SETFL, O_NONBLOCK) < 0) {
        ITEX_LOG(ERROR) << "Start cannot make pipe non-blocking: "
                        << strerror(errno);
        ClosePipes();
        return false;
      }
      if (fcntl(parent_pipe_[i], F_SETFD, FD_CLOEXEC) < 0) {
        ITEX_LOG(ERROR) << "Start cannot make pipe close-on-exec: "
                        << strerror(errno);
        ClosePipes();
        return false;
      }
  ...
  if (ret != 0) {
    ITEX_LOG(ERROR) << "Start cannot initialize POSIX file actions: "
                    << strerror(ret);
    ClosePipes();
    return false;
  }
  ...
  for (int i = 0; i < kNFds; i++) {
    if (parent_pipe_[i] >= 0) {
      ...
      if (ret != 0) {
        ITEX_LOG(ERROR) << "posix_spawn_file_actions_addclose() failed: "
                        << strerror(ret);
      }
    }
    ...
    switch (action_[i]) {
      ...
      case ACTION_PIPE:
        ...
        if (ret != 0) {
          ITEX_LOG(ERROR) << "posix_spawn_file_actions_adddup2() failed: "
                          << strerror(ret);
        }
        ...
        if (i <= CHAN_STDERR) {
          if (devnull_fd < 0) {
            ret = posix_spawn_file_actions_addopen(&file_actions, i,
                                                   "/dev/null", O_RDWR, 0);
            if (ret != 0) {
              ITEX_LOG(ERROR) << "posix_spawn_file_actions_addopen() failed: "
                              << strerror(ret);
            }
            devnull_fd = i;
          } else {
  ...
  ret = posix_spawnp(&pid_, exec_path_, &file_actions, nullptr, exec_argv_,
                     environ);
  if (ret != 0) {
    ITEX_LOG(INFO) << "Start cannot spawn child process: " << strerror(ret);
    ClosePipes();
    return false;
  }
  ...
  if (ret != 0) {
    ITEX_LOG(WARNING) << "Start cannot destroy POSIX file actions: "
                      << strerror(ret);
  }
...
bool SubProcess::Start() {
  ...
  pid_ = fork();
  if (pid_ < 0) {
    ITEX_LOG(ERROR) << "Start cannot fork() child process: " << strerror(errno);
    ClosePipes();
    return false;
  }
...
int SubProcess::Communicate(const std::string* stdin_input,
  ...
  proc_mu_.unlock();
  if (!running) {
    ITEX_LOG(ERROR) << "Communicate called without a running process.";
    return 1;
  }
  ...
  if (sigaction(SIGPIPE, nullptr, &act) < 0) {
    ITEX_LOG(ERROR) << "Communicate cannot get SIGPIPE handler: "
                    << strerror(errno);
    return 1;
  }
  if (act.sa_handler == SIG_DFL) {
    ...
    if (sigaction(SIGPIPE, &act, nullptr) < 0) {
      ITEX_LOG(ERROR) << "Communicate cannot ignore SIGPIPE: "
                      << strerror(errno);
      return 1;
    }
  }
  ...
  while (fd_remain > 0) {
    ...
    if ((n < 0) && !retry(errno)) {
      ITEX_LOG(ERROR) << "Communicate cannot poll(): " << strerror(errno);
      fd_remain = 0;
    } else if (n == 0) {
      ITEX_LOG(ERROR) << "Communicate cannot poll(): timeout not possible";
      fd_remain = 0;
    } else if (n > 0) {
