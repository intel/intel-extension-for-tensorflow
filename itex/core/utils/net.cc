...
bool IsPortAvailable(int* port, bool is_tcp) {
  ...
  ITEX_CHECK_GE(*port, 0);
  ITEX_CHECK_LE(*port, MAX_EPHEMERAL_PORT);
  if (fd < 0) {
    ITEX_LOG(ERROR) << "socket() failed: " << strerror(errno);
    return false;
  }
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one)) < 0) {
    ITEX_LOG(ERROR) << "setsockopt() failed: " << strerror(errno);
    if (close(fd) < 0) {
  ...
  if (bind(fd, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr)) < 0) {
    ITEX_LOG(WARNING) << "bind(port=" << *port
  ...
  if (getsockname(fd, reinterpret_cast<struct sockaddr*>(&addr), &addr_len) <
      0) {
    ITEX_LOG(WARNING) << "getsockname() failed: " << strerror(errno);
    if (close(fd) < 0) {
      ITEX_LOG(ERROR) << "close() failed: " << strerror(errno);
    }
    return false;
  }
  ITEX_CHECK_LE(addr_len, sizeof(addr));
  actual_port = ntohs(addr.sin_port);
  ITEX_CHECK_GT(actual_port, 0);
  if (*port == 0) {
    *port = actual_port;
  } else {
    ITEX_CHECK_EQ(*port, actual_port);
  }
  if (close(fd) < 0) {
    ITEX_LOG(ERROR) << "close() failed: " << strerror(errno);
  }
...
int PickUnusedPortOrDie() {
  ...
  bool is_tcp = true;
  int trial = 0;
  std::default_random_engine rgen(std::random_device{}());  // NOLINT
  std::uniform_int_distribution<int> rdist(MIN_EPHEMERAL_PORT,
                                           MAX_EPHEMERAL_PORT - 1);
  while (true) {
    int port;
    trial++;
    ITEX_CHECK_LE(trial, kMaximumTrials)
