...
#include "itex/core/utils/posix_file_system.h"
...
mutex name_mutex(itex::LINKER_INITIALIZED);
...
class PThread : public Thread {
 public:
  PThread(const ThreadOptions& thread_options, const std::string& name,
    ...
    // There is no mechanism for the thread creation API to fail, so we
    // ITEX_CHECK.
    ITEX_CHECK_EQ(ret, 0) << "Thread " << name
...
class PosixEnv : public Env {
 public:
  ...
  ~PosixEnv() override {
    ITEX_LOG(FATAL) << "Env::Default() must not be destroyed";
  }
  ...
  //   return itex::internal::LoadDynamicLibrary(library_filename, handle);
  // }

  // Status GetSymbolFromLibrary(void* handle, const char* symbol_name,
  //                             void** symbol) override {
  //   return itex::internal::GetSymbolFromLibrary(handle, symbol_name,
  //                                                     symbol);
  // }

  // string FormatLibraryFileName(const string& name,
  //                              const string& version) override {
  //   return itex::internal::FormatLibraryFileName(name, version);
  // }
