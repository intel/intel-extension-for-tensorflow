...
void TensorShape::CheckDimsEqual(int NDIMS) const {
  ITEX_CHECK_EQ(NDIMS, dims())
      << "Asking for tensor of " << NDIMS << " dimensions"
      << " from a tensor of " << dims() << " dimensions";
}

void TensorShape::CheckDimsAtLeast(int NDIMS) const {
  ITEX_CHECK_GE(NDIMS, dims())
      << "Asking for tensor of at least " << NDIMS
...
void TensorShapeRep::DestructorOutOfLine() {
  ITEX_DCHECK(tag() == REP_OUT_OF_LINE);
  delete as64()->dims_;
}
...
void TensorShapeBase<Shape>::AddDim(int64 size) {
  if (!kIsPartial) ITEX_CHECK_GE(size, 0);
  if (unknown_rank()) return;
  ITEX_CHECK_LT(ndims_byte(), MaxDimensions())
      << "Too many dimensions in tensor";
...
void TensorShapeBase<Shape>::InsertDim(int d, int64 size) {
  ITEX_CHECK_GE(d, 0);
  ITEX_CHECK_LE(d, dims());
  if (!kIsPartial) ITEX_CHECK_GE(size, 0);
  ITEX_CHECK_LT(dims(), MaxDimensions());
  gtl::InlinedVector<int64, 8> vals;
  AppendTo(*this, &vals);
...
void TensorShapeBase<Shape>::set_dim(int d, int64 size) {
  ITEX_CHECK_GE(d, 0);
  ITEX_CHECK_LT(d, dims());
  ITEX_CHECK_GE(size, 0);
...
void TensorShapeBase<Shape>::RemoveDimRange(int begin, int end) {
  ...
  ITEX_CHECK_GE(begin, 0);
  ITEX_CHECK_LE(begin, dims());
  ITEX_CHECK_GE(end, 0);
  ITEX_CHECK_LE(end, dims());
  if (begin >= end) return;
  gtl::InlinedVector<int64, 8> vals;
  AppendTo(*this, &vals);
