...
#ifndef ITEX_CORE_UTILS_MUTEX_H_
#define ITEX_CORE_UTILS_MUTEX_H_

#include <chrono>              // NOLINT
#include <condition_variable>  // NOLINT
#include <mutex>               // NOLINT

#include "itex/core/utils/mutex_data.h"
...
// Mimic a subset of the std::unique_lock<itex::mutex> functionality.
class TF_SCOPED_LOCKABLE mutex_lock {
 public:
  typedef ::itex::mutex mutex_type;
...
// Mimic a subset of the std::shared_lock<itex::mutex> functionality.
...
class TF_SCOPED_LOCKABLE tf_shared_lock {
 public:
  ...
  tf_shared_lock(mutex_type* mu, std::try_to_lock_t)
...
std::cv_status condition_variable::wait_for(
  ...
  return itex::internal::wait_until_system_clock(
      &this->cv_, &lock->mutex()->mu_, std::chrono::system_clock::now() + dur);
}

}  // namespace itex

#endif  // ITEX_CORE_UTILS_MUTEX_H_
