...
#ifndef ITEX_CORE_UTILS_SPARSE_GROUP_ITERATOR_H_
#define ITEX_CORE_UTILS_SPARSE_GROUP_ITERATOR_H_
...
class GroupIterable {
 public:
  ...
  IteratorStep at(int64_t loc) {
    ITEX_CHECK(loc >= 0 && loc <= ix_.dim_size(0))
...
typename TTypes<T>::UnalignedVec Group::values() const {
  auto temp_vals = iter_->vals_.vec<T>()(loc_);
  return typename TTypes<T>::UnalignedVec(&temp_vals, next_loc_ - loc_);
}

}  // namespace sparse
}  // namespace itex

#endif  // ITEX_CORE_UTILS_SPARSE_GROUP_ITERATOR_H_
