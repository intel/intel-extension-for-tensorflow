...
#ifndef ITEX_CORE_UTILS_SPARSE_SPARSE_TENSOR_H_
#define ITEX_CORE_UTILS_SPARSE_SPARSE_TENSOR_H_
...
#include "itex/core/utils/sparse/dim_comparator.h"
#include "itex/core/utils/sparse/group_iterator.h"
...
class SparseTensor {
 public:
  ...
  GroupIterable group(const VarDimArray& group_ix) const {
    ITEX_CHECK_LE(group_ix.size(), dims_);
    for (std::size_t di = 0; di < group_ix.size(); ++di) {
      ITEX_CHECK_GE(group_ix[di], 0) << "Group dimension out of range";
      ITEX_CHECK_LT(group_ix[di], dims_) << "Group dimension out of range";
      ITEX_CHECK_EQ(group_ix[di], order_[di])
...
inline void SparseTensor::Reorder(const VarDimArray& order) {
  ITEX_CHECK_EQ(DataTypeToEnum<T>::v(), dtype())
      << "Reorder requested with the wrong datatype";
  ITEX_CHECK_EQ(order.size(), dims_)
...
inline bool SparseTensor::ValidateAndInitializeToDense(Tensor* out,
  ...
  ITEX_CHECK_EQ(out->shape().dims(), dims_)
      << "Incompatible dimensions between SparseTensor and output";

  ITEX_CHECK_EQ(out->dtype(), DataTypeToEnum<T>::v())
...
inline SparseTensor SparseTensor::Concat(
  ...
  ITEX_CHECK_GE(tensors.size(), size_t{1}) << "Cannot concat 0 SparseTensors";
  const int dims = tensors[0].dims_;
  ITEX_CHECK_GE(dims, 1) << "Cannot concat 0-dimensional SparseTensors";
  ...
  for (const SparseTensor& st : tensors) {
    ITEX_CHECK_EQ(st.dims_, dims)
        << "All SparseTensors must have the same rank.";
    ITEX_CHECK_EQ(DataTypeToEnum<T>::v(), st.dtype())
        << "Concat requested with the wrong data type";
    ITEX_CHECK_GE(st.order()[0], 0) << "SparseTensor must be ordered";
    ITEX_CHECK_EQ(st.order()[0], primary_dim)
    ...
    for (int d = 0; d < dims - 1; ++d) {
      ...
      ITEX_CHECK_EQ(final_shape[cdim], st_shape[cdim])
...
#endif  // ITEX_CORE_UTILS_SPARSE_SPARSE_TENSOR_H_
