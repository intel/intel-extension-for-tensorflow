...
==============================================================================*/
#include "itex/core/utils/sparse/sparse_tensor.h"
...
int UnsafeGetDimsFromIx(const Tensor& ix) {
  ITEX_CHECK(TensorShapeUtils::IsMatrix(ix.shape()));
...
SparseTensor::SparseTensor(Tensor ix, Tensor vals, const VarDimArray shape,
  ...
  ITEX_CHECK_EQ(ix_.dtype(), DT_INT64)
      << "indices must be type int64 but got: " << ix_.dtype();
  ITEX_CHECK(TensorShapeUtils::IsVector(vals_.shape()))
      << "vals must be a vec, but got: " << vals_.shape().DebugString();
  ITEX_CHECK_EQ(ix_.shape().dim_size(0), vals_.shape().dim_size(0))
  ...
  ITEX_CHECK_EQ(shape.size(), dims_) << "Shape rank must be SparseTensor rank.";
}
...
bool SparseTensor::IndicesValidVectorFastPath() const {
  ITEX_CHECK_EQ(shape_.size(), 1);
  ITEX_CHECK_EQ(order_[0], 0);
...
bool SparseTensor::IndicesValidMatrix32BitFastPath() const {
  ...
  ITEX_CHECK_EQ(shape_.size(), 2);
  ITEX_CHECK_EQ(order_[0], 0);
  ITEX_CHECK_EQ(order_[1], 1);
  ITEX_CHECK_LE(shape_ptr[0], std::numeric_limits<int32>::max());
  ITEX_CHECK_LE(shape_ptr[1], std::numeric_limits<int32>::max());
...
Status SparseTensor::IndicesValidHelper() const {
  ...
  for (std::size_t n = 0; n < num_entries(); ++n) {
    ...
    if (ITEX_PREDICT_FALSE(!valid || !increasing || !different)) {
