...
#ifndef ITEX_CORE_UTILS_SPARSE_DIM_COMPARATOR_H_
#define ITEX_CORE_UTILS_SPARSE_DIM_COMPARATOR_H_
...
class DimComparator {
 public:
  ...
  DimComparator(const TTypes<int64_t>::Matrix& ix, const VarDimArray& order,
    ...
    ITEX_CHECK_GT(order.size(), size_t{0})
        << "Must order using at least one index";
    ITEX_CHECK_LE(order.size(), shape.size()) << "Can only sort up to dims";
    for (size_t d = 0; d < order.size(); ++d) {
      ITEX_CHECK_GE(order[d], 0);
      ITEX_CHECK_LT(order[d], shape.size());
    }
  }
...
class FixedDimComparator : DimComparator {
 public:
  FixedDimComparator(const TTypes<int64_t>::Matrix& ix,
    ...
    ITEX_CHECK_EQ(order.size(), ORDER_DIM);
  }
  inline bool operator()(const int64_t i, const int64_t j) const {
    bool value = false;
...
#endif  // ITEX_CORE_UTILS_SPARSE_DIM_COMPARATOR_H_
