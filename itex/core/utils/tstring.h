...
#ifndef ITEX_CORE_UTILS_TSTRING_H_
#define ITEX_CORE_UTILS_TSTRING_H_
...
// itex::tstring is the scalar type for DT_STRING tensors.
//
...
inline tstring& tstring::erase(size_t pos, size_t len) {
  std::copy(data() + pos + len, data() + pos + size(), mdata() + pos);
  resize(size() - len);
...
inline tstring& tstring::insert(size_t pos, const tstring& str, size_t subpos,
  ...
  std::copy_backward(data() + pos, data() + orig_size, mdata() + pos + sublen);
  std::copy_n(str.data() + subpos, sublen, mdata() + pos);
...
inline tstring& tstring::insert(size_t pos, size_t n, char c) {
  ...
  std::copy_backward(data() + pos, data() + size_, mdata() + pos + n);
  std::fill_n(mdata() + pos, n, c);
...
#endif  // ITEX_CORE_UTILS_TSTRING_H_
