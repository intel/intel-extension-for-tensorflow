...
//                          device_type, int id) {
//   ITEX_CHECK(IsJobName(job)) << job;
//   ITEX_CHECK_LE(0, replica);
//   ITEX_CHECK_LE(0, task);
//   ITEX_CHECK(!device_type.empty());
//   ITEX_CHECK_LE(0, id);
...
bool DeviceNameUtils::IsCompleteSpecification(const ParsedName& pattern,
  ...
  ITEX_CHECK(name.has_job && name.has_replica && name.has_task &&
...
std::string DeviceNameUtils::LocalName(StringPiece fullname) {
  ParsedName x;
  ITEX_CHECK(ParseFullName(fullname, &x)) << fullname;
