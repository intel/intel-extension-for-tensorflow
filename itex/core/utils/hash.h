...
#ifndef ITEX_CORE_UTILS_HASH_H_
#define ITEX_CORE_UTILS_HASH_H_
...
//
// In particular, itex::hash is not the identity function for pointers.
...
struct hash<T, typename std::enable_if<std::is_enum<T>::value>::type> {
  size_t operator()(T value) const {
    ...
    // itex::hash<EnumTy>() once we stop building with GCC versions old
...
using StringPieceHasher = ::itex::hash<StringPiece>;
...
// namespace std {
// template <>
// struct hash<itex::tstring> {
//   size_t operator()(const itex::tstring& s) const {
//     return static_cast<size_t>(itex::Hash64(s.data(), s.size()));
//   }
// };
// }  // namespace std

#endif  // ITEX_CORE_UTILS_HASH_H_
