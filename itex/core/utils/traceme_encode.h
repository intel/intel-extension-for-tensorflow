...
#ifndef ITEX_CORE_UTILS_TRACEME_ENCODE_H_
#define ITEX_CORE_UTILS_TRACEME_ENCODE_H_
...
TF_ATTRIBUTE_ALWAYS_INLINE inline char* Append(char* out,
  ...
  ITEX_DCHECK(!absl::StrContains(str, '#'))
      << "'#' is not a valid character in TraceMeEncode";
  const size_t str_size = str.size();
  if (ITEX_PREDICT_TRUE(str_size > 0)) {
    std::copy_n(str.data(), str_size, out);
    out += str_size;
  }
  return out;
}
...
TF_ATTRIBUTE_ALWAYS_INLINE inline std::string AppendArgs(
  ...
  if (ITEX_PREDICT_TRUE(args.size() > 0)) {
    ...
    *(out - 1) = '#';
    ITEX_DCHECK_EQ(out, begin + new_size);
  }
  return name;
}
...
TF_ATTRIBUTE_ALWAYS_INLINE inline void AppendMetadata(
  ...
  if (!ITEX_PREDICT_FALSE(new_metadata.empty())) {
    if (!name->empty() && name->back() == '#') {  // name already has metadata
      name->back() = ',';
      if (ITEX_PREDICT_TRUE(new_metadata.front() == '#')) {
...
#endif  // ITEX_CORE_UTILS_TRACEME_ENCODE_H_
