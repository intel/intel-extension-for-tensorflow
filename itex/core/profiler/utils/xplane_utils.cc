...
#include "itex/core/profiler/utils/xplane_visitor.h"
...
void MergePlanes(const XPlane& src_plane, XPlane* dst_plane) {
  ...
  src.ForEachStat([&](const itex::profiler::XStatVisitor& stat) {
  ...
  src.ForEachLine([&](const itex::profiler::XLineVisitor& line) {
    ...
    line.ForEachEvent([&](const itex::profiler::XEventVisitor& event) {
      ...
      event.ForEachStat([&](const itex::profiler::XStatVisitor& stat) {
